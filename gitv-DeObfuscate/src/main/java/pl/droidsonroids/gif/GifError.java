...
public enum GifError {
    ...
    IMAGE_DEFECT(HistoryInfoHelper.MSG_MERGE, "Image is defective, decoding aborted"),
    ...
    INVALID_BYTE_BUFFER(GalaVideoPlayerParams.PROJ_EVENT_REPORTER, "Invalid and/or indirect byte buffer specified"),
    ...
    static GifError fromCode(int code) {
        for (GifError gifError : values()) {
            if (gifError.errorCode == code) {
                return gifError;
            }
        }
        GifError gifError2 = UNKNOWN;
        gifError2.errorCode = code;
        return gifError2;
    }
