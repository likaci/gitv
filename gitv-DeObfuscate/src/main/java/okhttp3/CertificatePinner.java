...
public final class CertificatePinner {
    ...
    static final class Pin {
        ...
        Pin(String pattern, String pin) {
            ...
            if (pattern.startsWith(WILDCARD)) {
                host = HttpUrl.parse(WebConstants.WEB_SITE_BASE_HTTP + pattern.substring(WILDCARD.length())).host();
            } else {
                host = HttpUrl.parse(WebConstants.WEB_SITE_BASE_HTTP + pattern).host();
            }
        ...
        public int hashCode() {
            return ((((this.pattern.hashCode() + WidgetType.ITEM_SETTING_NETWORK) * 31) + this.hashAlgorithm.hashCode()) * 31) + this.hash.hashCode();
        }
    ...
    public void check(String hostname, List<Certificate> peerCertificates) throws SSLPeerUnverifiedException {
        ...
        if (!pins.isEmpty()) {
            ...
            message.append("\n  Pinned certificates for ").append(hostname).append(SOAP.DELIM);
            pinsSize = pins.size();
            for (p = 0; p < pinsSize; p++) {
