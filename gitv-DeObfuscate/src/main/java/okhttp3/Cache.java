...
public final class Cache implements Closeable, Flushable {
    ...
    class C21461 implements InternalCache {
        C21461() {
        }
    ...
    class C21472 implements Iterator<String> {
        boolean canRemove;
        final Iterator<Snapshot> delegate = Cache.this.cache.snapshots();
        String nextUrl;

        C21472() throws IOException {
        }
    ...
    private static final class Entry {
        ...
        private boolean isHttps() {
            return this.url.startsWith(WebConstants.WEB_SITE_BASE_HTTPS);
        }
        ...
        public Response response(Snapshot snapshot) {
            ...
            String contentLength = this.responseHeaders.get(HTTP.CONTENT_LENGTH);
    ...
    CacheRequest put(Response response) {
        ...
        } else if (!requestMethod.equals(HTTP.GET) || HttpHeaders.hasVaryAll(response)) {
            return null;
        } else {
    ...
    static int readInt(BufferedSource source) throws IOException {
        try {
            ...
            String line = source.readUtf8LineStrict();
            if (result >= 0 && result <= TTL.MAX_VALUE && line.isEmpty()) {
                return (int) result;
            }
