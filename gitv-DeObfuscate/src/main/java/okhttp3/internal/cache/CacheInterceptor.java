...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        try {
            ...
            if (cacheResponse != null) {
                if (networkResponse.code() == TipType.CONCRETE_TYPE_REPLAY_PLAYNEXT) {
    ...
    private static Headers combine(Headers cachedHeaders, Headers networkHeaders) {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (!HTTP.CONTENT_LENGTH.equalsIgnoreCase(fieldName) && isEndToEnd(fieldName)) {
    ...
    static boolean isEndToEnd(String fieldName) {
        if (HTTP.CONNECTION.equalsIgnoreCase(fieldName) || HTTP.KEEP_ALIVE.equalsIgnoreCase(fieldName) || "Proxy-Authenticate".equalsIgnoreCase(fieldName) || "Proxy-Authorization".equalsIgnoreCase(fieldName) || "TE".equalsIgnoreCase(fieldName) || "Trailers".equalsIgnoreCase(fieldName) || HTTP.TRANSFER_ENCODING.equalsIgnoreCase(fieldName) || "Upgrade".equalsIgnoreCase(fieldName)) {
            return false;
        }
        return true;
    }
