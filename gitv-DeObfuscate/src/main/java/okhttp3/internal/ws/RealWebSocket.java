...
public final class RealWebSocket implements WebSocket, FrameCallback {
    ...
    class C21811 implements Runnable {
        C21811() {
        }
    ...
    public RealWebSocket(Request request, WebSocketListener listener, Random random) {
        if (HTTP.GET.equals(request.method())) {
            ...
            this.writerRunnable = new C21811();
            return;
        }
    ...
    public void connect(OkHttpClient client) {
        ...
        final Request request = this.originalRequest.newBuilder().header("Upgrade", "websocket").header(HTTP.CONNECTION, "Upgrade").header("Sec-WebSocket-Key", this.key).header("Sec-WebSocket-Version", "13").build();
