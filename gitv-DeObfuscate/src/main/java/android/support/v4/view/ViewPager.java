...
public class ViewPager extends ViewGroup {
    ...
    private static final Comparator<ItemInfo> COMPARATOR = new C00191();
    ...
    private static final Interpolator sInterpolator = new C00202();
    ...
    private final Runnable mEndScrollRunnable = new C00213();
    ...
    class C00191 implements Comparator<ItemInfo> {
        C00191() {
        }
    ...
    class C00202 implements Interpolator {
        C00202() {
        }
    ...
    public static class SavedState extends BaseSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C00221());
        Parcelable adapterState;
        ClassLoader loader;
        int position;

        class C00221 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C00221() {
            }
    ...
    float distanceInfluenceForSnapDuration(float f) {
        return (float) Math.sin((double) ((float) (((double) (f - ThreeDimensionalParams.TEXT_SCALE_FOR_3D)) * 0.4712389167638204d)));
    }
            ...
            while (curIndex < this.mItems.size()) {
                ...
                if (ii.position >= this.mCurItem) {
                    ...
                    if (curItem != null) {
                        ...
                        leftWidthNeeded = (2.0f - curItem.widthFactor) + (((float) getPaddingLeft()) / ((float) getClientWidth()));
                        pos = this.mCurItem - 1;
                        while (pos >= 0) {
                        ...
                        if (extraWidthRight < 2.0f) {
                            ...
                            rightWidthNeeded = (((float) getPaddingRight()) / ((float) getClientWidth())) + 2.0f;
                            pos = this.mCurItem + 1;
                            while (pos < N) {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp != null && lp.isDecor) {
                    ...
                    int vgrav = lp.gravity & HistoryInfoHelper.MSG_MERGE;
            ...
            case 2:
                ...
                if (activePointerId != -1) {
                    ...
                    if (dx == 0.0f || isGutterDrag(this.mLastMotionX, dx) || !canScroll(this, false, (int) dx, (int) x2, (int) y)) {
                        if (xDiff > ((float) this.mTouchSlop) && ThreeDimensionalParams.TEXT_SCALE_FOR_3D * xDiff > yDiff) {
                            this.mIsBeingDragged = true;
                            setScrollState(1);
                            if (dx > 0.0f) {
    ...
    private int determineTargetPage(int currentPage, float pageOffset, int velocity, int deltaX) {
        ...
        if (Math.abs(deltaX) <= this.mFlingDistance || Math.abs(velocity) <= this.mMinimumVelocity) {
            targetPage = (int) ((((float) currentPage) + pageOffset) + (currentPage >= this.mCurItem ? WindowZoomRatio.WINDOW_ZOOM_RATIO_MIN : 0.6f));
        } else {
        ...
        if (this.mPageMargin > 0 && this.mMarginDrawable != null && this.mItems.size() > 0 && this.mAdapter != null) {
            ...
            while (pos < lastPos) {
                ...
                if (((float) this.mPageMargin) + drawAt > ((float) scrollX)) {
                    this.mMarginDrawable.setBounds((int) drawAt, this.mTopPageBounds, (int) ((((float) this.mPageMargin) + drawAt) + ThreeDimensionalParams.TEXT_SCALE_FOR_3D), this.mBottomPageBounds);
    ...
    public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
        ...
        if ((descendantFocusability == SendFlag.FLAG_KEY_PINGBACK_MID && focusableCount != views.size()) || !isFocusable()) {
            return;
        }
