...
public class UpgradeSettingUtils {
        ...
        TVApi.moduleUpdate.call(new IApiCallback<ApiResultModuleUpdate>() {
                ...
                public void run() {
                    ...
                    if (UpdateManager.getInstance().isShowingDialog()) {
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(UpgradeSettingUtils.TAG, "check Apk upgrade dialog showing, do nothing");
                        }
            ...
            public void onSuccess(ApiResultModuleUpdate result) {
                if (result == null || ListUtils.isEmpty(result.data)) {
                    LogUtils.m1568d(UpgradeSettingUtils.TAG, "check Apk app upgrade, result is null");
                ...
                for (ModuleUpdate module : result.data) {
                    if (UpgradeSettingUtils.MAIN_APK_UPGRADE_KEY.equals(module.key)) {
                        ...
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(UpgradeSettingUtils.TAG, "check upgrade success version : " + version.toString());
                        }
                        UpdateManager.getInstance().setAppVersion(version);
                        UpgradeSettingUtils.mHandler.post(new C10721());
                        return;
                    }
                    LogUtils.m1568d(UpgradeSettingUtils.TAG, "check Apk app upgrade, module key is not equal to MAIN_APK_UPGRADE_KEY");
                    UpgradeSettingUtils.showNoneedUpdateToast(context);
                }
            }
            ...
            public void onException(ApiException exception) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1569d(UpgradeSettingUtils.TAG, "check Apk app upgrade request failed, exception = ", exception);
                }
                UpgradeSettingUtils.showNoneedUpdateToast(context);
            }
        }, "{}");
    ...
    private static void showNoUpdateToast(Context context) {
        if (context != null) {
            QToast.makeTextAndShow(context, context.getString(C0508R.string.not_need_update), 3000);
        }
    }
