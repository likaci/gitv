...
public class ScreenSaverImageProvider {
    ...
    private ScreenSaverImageProvider() {
        ...
        LogUtils.m1568d(TAG, "local parent screensaver image path : " + this.mParentPath);
    }
    ...
    private String initDownloadFile() {
        ...
        if (imgDir.mkdir()) {
            LogUtils.m1577w(TAG, "initDownloadFile, create image directory success, path = " + path);
            return path;
        }
        LogUtils.m1577w(TAG, "initDownloadFile, create image directory failed!!!, path = " + path);
        return "";
    }
            ...
            if (!ListUtils.isEmpty(this.mLocalScreenSaverModelList)) {
                for (ScreenSaverModel model : this.mLocalScreenSaverModelList) {
                    if (!new File(model.getImagePath()).exists()) {
                        ...
                        LogUtils.m1568d(TAG, "prepare, file not exists , path: " + model.getImagePath() + " ,remove this screensaver data");
                    }
                }
            }
        }
    ...
    public void download() {
        LogUtils.m1568d(TAG, "download, download new screen saver data");
    ...
    public void download(List<ChannelLabel> channelLabelList) {
        if (ListUtils.isEmpty((List) channelLabelList)) {
            LogUtils.m1568d(TAG, "download, screen saver label is empty");
            return;
        }
        LogUtils.m1568d(TAG, "download, all raw screen saver label size :" + ListUtils.getCount((List) channelLabelList));
        ...
        if (StringUtils.isEmpty(downloadPath)) {
            LogUtils.m1577w(TAG, "download, current download path empty, do not download images");
            return;
        }
        resetCount();
            ...
            public void run() {
                for (ChannelLabel label : list) {
                    ...
                    if (StringUtils.isEmpty(imageUrl)) {
                        LogUtils.m1568d(ScreenSaverImageProvider.TAG, "current image url is empty, do not need to download");
                    } else if (file.exists()) {
                        LogUtils.m1568d(ScreenSaverImageProvider.TAG, "current image has been downloaded, imageUrl :" + imageUrl);
                        ...
                        LogUtils.m1568d(ScreenSaverImageProvider.TAG, "download, failed, url : " + imageUrl);
                    }
                }
    ...
    private void downloadFinished(List<ScreenSaverModel> tempScreenSaverModels) {
        LogUtils.m1568d(TAG, "downloadFinished, screen saver data size : " + ListUtils.getCount((List) tempScreenSaverModels));
    ...
    public void onEmptyScreenSaverData() {
        LogUtils.m1568d(TAG, "onEmptyScreenSaverData");
    ...
    private void deleteOldImage(List<ScreenSaverModel> oldScreenSaverModels, List<ScreenSaverModel> newScreenSaverModels) {
        if (ListUtils.isEmpty((List) oldScreenSaverModels)) {
            LogUtils.m1568d(TAG, "deleteOldImage, old screensaver images are empty, do not need to delete images");
            return;
        }
            ...
            for (String oldImagePath : oldImagePathList) {
                if (!newImagePathList.contains(oldImagePath)) {
                    ...
                    if (file.exists()) {
                        file.delete();
                        if (!file.exists()) {
                            LogUtils.m1568d(TAG, "deleteOldImage, delete old image success, path :" + oldImagePath);
                        }
                    } else {
                        LogUtils.m1577w(TAG, "deleteOldImage, try to delete a image, but the image file is not exist, path :" + oldImagePath);
                    }
                }
            }
        } catch (Exception e) {
            LogUtils.m1572e(TAG, "deleteOldImage, exception :", e);
        }
    }
    ...
    private synchronized void writeScreenSaverModelListToCache(List<ScreenSaverModel> screenSaverModels) {
        try {
            ...
            LogUtils.m1568d(TAG, "write ScreenSaver data to disk success ");
            setLocalData(screenSaverModels);
            LogUtils.m1568d(TAG, "write ScreenSaver data to memory success");
        } catch (IOException e) {
            LogUtils.m1578w(TAG, "writeScreenSaverModelListToCache, write screen saver data failed", e);
        }
    }
    ...
    private synchronized List<ScreenSaverModel> readLocalScreenSaverModelList() {
        ...
        try {
            ...
            LogUtils.m1572e(TAG, "readLocalScreenSaverModelList, read screen saver data failed", e);
        }
        LogUtils.m1568d(TAG, "readLocalScreenSaverModelList, size = " + (!ListUtils.isEmpty(labelList) ? labelList.size() : 0));
        return labelList;
    }
