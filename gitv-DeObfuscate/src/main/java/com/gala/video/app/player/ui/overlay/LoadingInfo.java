...
public class LoadingInfo {
    ...
    public void setTitle(String title) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setTitle(" + title + ")");
        }
        this.mTitle = title;
    }
    ...
    public void setIsExclusive(boolean exclusive) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setIsExclusive(" + exclusive + ")");
        }
    ...
    public void setIsVip(boolean vip) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setIsVip(" + vip + ")");
        }
        this.mIsVip = vip;
    }
    ...
    public void setIs3d(boolean is3d) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setIs3d(" + is3d + ")");
        }
        this.mIs3d = is3d;
    }
    ...
    public void setLiveTime(long time) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setLiveTime(" + time + ")");
        }
        this.mLiveTime = time;
        if (checkLiveTime() && !StringUtils.isEmpty(this.mTitle)) {
            ...
            this.mTitle = AppRuntimeEnv.get().getApplicationContext().getResources().getString(C1291R.string.live_loding_name, new Object[]{liveTime, this.mTitle});
        }
    }

    public LoadingInfo(String name) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "LoadingInfo(" + name);
        }
