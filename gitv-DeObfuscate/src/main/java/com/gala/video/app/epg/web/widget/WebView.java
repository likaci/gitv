...
public class WebView extends AbsWebView {
    ...
    class C12702 implements IExceptionCallback {
        C12702() {
        }
    ...
    protected void initView() {
        LayoutInflater.from(this.mContext).inflate(C0508R.layout.epg_layout_webview, this, true);
        startLoading();
    }
    ...
    protected WebBaseEvent getBaseEvent() {
        ...
        LogUtils.m1568d(this.TAG, " webview init WebEvent, url: " + this.mWebUrl + ", isAccelerateExclude: " + isAcceler);
        return WebBasicEventFactory.create(isAcceler);
    }

    protected View getLoadingView() {
        View view = findViewById(C0508R.id.epg_webview_loading);
        view.setBackgroundColor(-16777216);
        return view;
    }

    protected LinearLayout getProgressBarItem() {
        ViewStub viewStub = (ViewStub) findViewById(C0508R.id.epg_webview_lading_layout_viewstub);
        if (viewStub == null) {
            return null;
        }
        LinearLayout progressBar = (LinearLayout) viewStub.inflate();
        ((TextView) progressBar.findViewById(C0508R.id.share_progress_tv)).setTextColor(ResourceUtil.getColor(C0508R.color.albumview_focus_color));
        return progressBar;
    }

    private GlobalQRFeedbackPanel getGlobalQRFeedbackPanel() {
        if (this.mQrFeedbackPanel == null) {
            this.mQrFeedbackPanel = (GlobalQRFeedbackPanel) ((ViewStub) findViewById(C0508R.id.epg_webview_qr_panel_layout_viewstub)).inflate();
            this.mQrFeedbackPanel.setQRTextColor(ResourceUtil.getColor(C0508R.color.albumview_focus_color));
            this.mQrFeedbackPanel.post(new C12681());
        }
        return this.mQrFeedbackPanel;
    }

    protected void showErrorView(String error) {
        new NetWorkCheckApi().check(error, new C12702());
    }
