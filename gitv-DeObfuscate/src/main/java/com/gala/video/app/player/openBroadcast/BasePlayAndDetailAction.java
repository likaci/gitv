...
public abstract class BasePlayAndDetailAction extends BaseAction {
    ...
    public void process(Context context, Intent intent, LoadingCallback loadingCallback) {
        try {
            ...
            if (!checkParamsValidity(playDict)) {
                LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "checkParamsValidity is false. ");
                if (loadingCallback != null) {
                    loadingCallback.onFail();
                    LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "loadingCallback.onCancel()...");
                }
            } else if (playDict != null) {
                String videoId = playDict.optString("videoId");
                if (TextUtils.isEmpty(videoId)) {
                    videoId = playDict.optString("vrsAlbumId");
                    LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "===videoId====vrsAlbumId:" + videoId);
                }
                String episodeId = playDict.optString("episodeId");
                if (TextUtils.isEmpty(episodeId)) {
                    episodeId = playDict.optString("vrsTvId");
                    LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "===episodeId====chnIdStr:" + episodeId);
                }
                String history = playDict.optString("history");
                LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "===history====history:" + history);
                ...
                if (!TextUtils.isEmpty(history)) {
                    ...
                    try {
                        ...
                        LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "[INVALID-PARAMTER] [action:BasePlayAndDetailAction][reason:invalid field--history:" + history + "][NumberFormatException:" + e.getMessage() + AlbumEnterFactory.SIGN_STR);
                    }
                ...
                LogUtils.m1568d("openplay/broadcast/BasePlayAndDetailAction", "===actionSuffix====: " + actionSuffix);
                LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "===playTime====playTime:" + playTime);
                if (playTime == HISTORY_VALUE_FOR_ALBUM_ID && actionSuffix.equals("ACTION_DETAIL")) {
                    LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "===fetchFullAlbumInfo====videoId = " + videoId + " ;playTime = " + playTime);
                    fetchFullAlbumInfo(context, videoId, playTime, loadingCallback);
                    return;
                }
                LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "===fetchFullAlbumInfo====episodeId = " + episodeId + " ;playTime = " + playTime);
                fetchFullAlbumInfo(context, episodeId, playTime, loadingCallback);
            }
        } catch (Exception e2) {
            LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "[UNKNOWN-EXCEPTION] [reason:exception occurred when OpenDetailAction process.][Exception:" + e2.getMessage() + AlbumEnterFactory.SIGN_STR);
            e2.printStackTrace();
            if (loadingCallback != null) {
                loadingCallback.onFail();
                LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "loadingCallback.onFail();");
            }
        }
    }
    ...
    public boolean checkParamsValidity(JSONObject playDict) {
        if (super.checkParamsValidity(playDict)) {
            ...
            if (TextUtils.isEmpty(episodeId)) {
                episodeId = playDict.optString("vrsTvId");
                LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "===episodeId====vrsTvId:" + episodeId);
            }
            ...
            LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "[INVALID-PARAMTER] [action:BasePlayAndDetailAction][reason:missing field--videoId|episodeId][playInfo:" + playDict.toString() + AlbumEnterFactory.SIGN_STR);
        }
        return false;
    }
    ...
    protected void startPlay(Context context, Album albumInfo) {
        ...
        LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "DetailIntentUtils.startPlayer... albumInfo= " + albumInfo.toString() + "builder = " + builder.toString());
    ...
    void fetchFullAlbumInfo(final Context context, String id, final int playTime, final LoadingCallback loadingCallback) {
        TVApi.albumInfo.call(new IApiCallback<ApiResultAlbum>() {
            public void onSuccess(ApiResultAlbum apiResultAlbum) {
                if (loadingCallback != null) {
                    LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "dealPlayAndDetailAction()--onSuccess---LoadingActivity.isInterrupted()= " + loadingCallback.iscancel());
                ...
                if (album == null) {
                    LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "album is null. return");
                    if (loadingCallback != null) {
                        loadingCallback.onFail();
                        return;
                    }
                ...
                LogUtils.m1574i("openplay/broadcast/BasePlayAndDetailAction", "dealDetailAcion()--playTime = " + playTime + " ;album.playTime =  " + album.playTime + " ;album.tvQid = " + album.tvQid + " ;album.qpId = " + album.qpId + " ;album.type = " + album.type + " ;album.toString() = " + album.toString());
                if (loadingCallback != null) {
            ...
            public void onException(ApiException arg0) {
                LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "TVApi--IApiCallback--onException = " + arg0.toString());
                if (loadingCallback != null) {
                    if (!NetworkUtils.isNetworkAvaliable()) {
                        LogUtils.m1571e("openplay/broadcast/BasePlayAndDetailAction", "TVApi--IApiCallback--NetworkUtils.isNetworkAvaliable() =false");
