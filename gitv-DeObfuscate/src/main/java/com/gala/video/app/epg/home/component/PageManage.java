...
public class PageManage {
    ...
    private Runnable mRetryRunnable = new C05931();
    ...
    class C05931 implements Runnable {
        ...
        public void run() {
            if (NetworkUtils.isNetworkAvaliable()) {
                ...
                LogUtils.m1568d(PageManage.this.TAG, "retry");
    ...
    public void initLoading() {
        ...
        LogUtils.m1568d(this.TAG, "initLoading");
    }

    public void loadData(TabModel tab) {
        LogUtils.m1568d(this.TAG, "loadData tab info :" + tab + "Engine id:" + this.mEngine.getId());
    ...
    public void bindDataSource(List<CardInfoModel> cards) {
        ...
        LogUtils.m1568d(this.TAG, "bindDataSource Engine id " + this.mEngine.getId());
        this.mEngine.setData(cards);
    }
    ...
    public void onUikitEvent(UikitEvent event) {
        if (this.mEngine != null && event.uikitEngineId == this.mEngine.getId()) {
            LogUtils.m1568d(this.TAG, "receive loader event: " + event + ",page index = " + this.mPageIndex);
            switch (event.eventType) {
                case 0:
                    LogUtils.m1568d(this.TAG, "onUikitEvent LOADER_PAGE_INIT:" + event.layoutChange);
                    if (event.layoutChange == 1) {
                        if (event.cardList != null) {
                            this.mLoadSuccessTime = SystemClock.elapsedRealtime();
                            this.mNoData = false;
                            if (LogUtils.mIsDebug) {
                                LogUtils.m1568d(this.TAG, "[start performance] load page data cost " + (this.mLoadSuccessTime - this.mStartLoadTime) + " ms" + ", pageindex = " + this.mPageIndex);
                            }
                            bindDataSource(event.cardList);
                        } else {
                            LogUtils.m1571e(this.TAG, "initPageAction card list null");
                        }
                    } else if (event.layoutChange == 0) {
                        LogUtils.m1574i(this.TAG, "initPageAction NODATA!!! ,page index = " + this.mPageIndex);
                        this.mNoData = true;
                    ...
                    return;
                case 32:
                    LogUtils.m1568d(this.TAG, "onUikitEvent LOADER_SET_CARDS-" + event.sourceId);
                    bindDataSource(event.cardList);
                    return;
                case 33:
                    LogUtils.m1568d(this.TAG, "onUikitEvent LOADER_ADD_CARDS-" + event.sourceId + "-pageNo-" + event.pageNo);
                    appendPageData(event.cardList);
                    return;
                case 34:
                    LogUtils.m1568d(this.TAG, "onUikitEvent LOADER_UPDATE_CARD card id-" + event.cardInfoModel.getId() + "-pageNo-" + event.pageNo + "-source-" + event.cardInfoModel.mSource);
    ...
    public void createDefault(int index) {
        ...
        bindDataSource(cards);
        LogUtils.m1568d(this.TAG, "createDefault");
    }
    ...
    public void onPageIn() {
        LogUtils.m1568d(this.TAG, "onPageIn mBuilded:" + this.mBuilded + "mStarted:" + this.mStarted);
    ...
    public void onActivityIn() {
        LogUtils.m1568d(this.TAG, "onActivityIn mStarted:" + this.mStarted + "mSelected:" + this.mSelected);
    ...
    public void onActivityOut() {
        LogUtils.m1568d(this.TAG, "onActivityOut " + this.mSelected);
        this.mStarted = false;
        if (this.mSelected) {
            this.mEngine.hide();
    ...
    public void handleTabResourceRetry() {
        if (shouldRetry()) {
            LogUtils.m1568d(this.TAG, "should retry");
            mHandler.removeCallbacksAndMessages(this.mRetryRunnable);
