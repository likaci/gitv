...
public class CountDownView extends TextView implements ICountDownView {
    ...
    private void init(Context context) {
        ...
        setTextColor(-1);
        setBackgroundResource(C1291R.drawable.player_ad_timming_bg);
        this.mAdTimeFullSize = (float) context.getResources().getDimensionPixelSize(C1291R.dimen.dimen_40dp);
        this.mAdWidth = (float) context.getResources().getDimensionPixelSize(C1291R.dimen.dimen_63dp);
    }

    public void show(int countDownTime) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "show(" + countDownTime + ")");
        }
