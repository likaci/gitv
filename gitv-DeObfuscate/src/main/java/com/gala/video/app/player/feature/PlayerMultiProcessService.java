...
public class PlayerMultiProcessService extends Service {
    ...
    private class PlayerMultiProcessBinder extends Stub {
        ...
        public Bundle invoke(Bundle params) throws RemoteException {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(PlayerMultiProcessService.TAG, "invoke, cmd=" + cmd);
            }
            if (PlayerCommand.LOAD_PLUGIN_ASYNC.equals(cmd)) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(PlayerMultiProcessService.TAG, "getPumaLog():\n" + log);
                }
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(PlayerMultiProcessService.TAG, "getModuleVersion():" + versionStr);
                }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onStartCommand start");
        }
    ...
    public IBinder onBind(Intent intent) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onStartCommand onBind");
        }
        initBinder(intent);
        return sBinder;
    }
