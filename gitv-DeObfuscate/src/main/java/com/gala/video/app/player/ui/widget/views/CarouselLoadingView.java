...
public class CarouselLoadingView extends RelativeLayout implements IScreenUISwitcher {
    ...
    private Handler mHandler = new C15461();
    ...
    class C15461 extends Handler {
        C15461() {
        }

        public void handleMessage(Message msg) {
            LogUtils.m1568d(CarouselLoadingView.TAG, "handleMessage()" + msg.what);
            switch (msg.what) {
                case 101:
    ...
    private void initHelpTipData() {
        ...
        if (this.mHasLoadingTip) {
            ...
            LogUtils.m1568d(TAG, "initHelpTipData == " + loadingHelpText);
        }
    }
    ...
    private void initView() {
        ...
        this.mLoadingLayout = (RelativeLayout) LayoutInflater.from(this.mContext).inflate(C1291R.layout.player_carousel_loadingscreen, null);
        initHelpTipData();
        if (this.mHasLoadingTip) {
            this.mHelpTipView = (TextView) this.mLoadingLayout.findViewById(C1291R.id.help_description);
            this.mHelpTipBubble = getResources().getDrawable(C1291R.drawable.player_carousel_help);
        }
        this.mTxtName = (TextView) this.mLoadingLayout.findViewById(C1291R.id.description);
        this.mAnimView = (ImageView) this.mLoadingLayout.findViewById(C1291R.id.loading);
        this.mLoadingContainer = (RelativeLayout) this.mLoadingLayout.findViewById(C1291R.id.loading_container);
        this.mLogo = (ImageView) this.mLoadingLayout.findViewById(C1291R.id.logo);
    ...
    public void showPlaying() {
        LogUtils.m1568d(TAG, "showPlaying() mAlreayPlay=" + this.mAlreayPlay);
        if (!this.mAlreayPlay) {
    ...
    public void show() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showLoading()");
        }
        setVisibility(0);
    }
    ...
    public void hide() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "hideLoading()");
        }
        setVisibility(8);
    ...
    public void switchScreen(boolean isFullScreen, float zoomRatio) {
        LogUtils.m1568d(TAG, "switchScreen: " + isFullScreen);
        ...
        if (this.mHasLoadingTip) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "measureHelpTip=" + this.mIsFullScreen);
            }
            measureHelpTip();
        }
    }
    ...
    public void setCurrentChannel(TVChannelCarousel channelCarousel) {
        if (channelCarousel != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "setCurrentChannel name=" + name);
            }
        }
        if (this.mHasLoadingTip) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "showHelpTip=" + this.mHasLoadingTip);
            }
            measureHelpTip();
            randomHelpTip();
        }
