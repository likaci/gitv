...
import com.gala.video.app.epg.C0508R.array;
...
public class CommonInitTask implements Runnable {
    ...
    private final LoginCallbackRecorderListener mLoginListener = new C07434();

    class C07401 implements IVoiceStartUpPrepareListener {
        C07401() {
        }
    ...
    class C07412 implements ICrashCallback {
        C07412() {
        }
        ...
        public void onCrash(JSONObject jsonObject, int type, String javaMessage) {
            try {
                ...
                params.add(Keys.f2035T, "0").add("ec", "303").add("pfec", "").add(Keys.ERREASON, errorReason).add("activity", activityName).add(Keys.EXCPTNNM, excptnnm).add(Keys.LEFTDATASIZE, leftdatasize + "").add(Keys.CRASHTYPE, crashType).add("e", eventID);
    ...
    class C07423 implements OnNetStateChangedListener {
        C07423() {
        }

        public void onStateChanged(int from, int to) {
            LogUtils.m1568d(CommonInitTask.TAG, "registerNetworkStatusChanged from " + from + " to " + to);
            switch (to) {
                case 1:
                case 2:
    ...
    class C07434 implements LoginCallbackRecorderListener {
        C07434() {
        }
    ...
    public void run() {
        ...
        if ("true".equalsIgnoreCase(BuildCache.getInstance().getString(BuildConstance.APK_SUPPORT_VOICE, "false"))) {
            VoiceStartup.initialize(VoiceAppConfig.getVoiceListenerList(), new C07401());
        }
        ...
        LogUtils.m1574i(TAG, "set ImageProvider config RGB_565");
    ...
    public void initCrashMode(Context context) {
        if (Project.getInstance().getBuild().isInitCrashHandler()) {
            CrashReporter.getInstance().init(context, new CrashReportParamsBuilder().setCallback(new C07412()).enableFullLog(true).setLogSize(500).disableLogcat(true).build());
        }
    }
    ...
    private void registerNetworkStatusChanged() {
        NetWorkManager.getInstance().registerStateChangedListener(new C07423());
    }

    private void initPlugin() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "initPlugin<<()");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "initailizePlugin: hostVersion=" + hostVersion + ", domain=" + domain + ", apkVersion=" + apkVersion + "isMulti =" + Project.getInstance().getBuild().supportPlayerMultiProcess() + "isPlayerProcess =" + GetInterfaceTools.getIInit().isPlayerProcess());
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "initPlugin>>()" + openPluginIOBalance);
        }
    }

    public void initPingback(Context context) {
        LogUtils.m1568d(TAG, "initPingBack");
    ...
    private void handleAdCrash() {
        if (isStartVideoCrash()) {
            ...
            LogUtils.m1571e(TAG, "ad crash " + adCrashTimes);
    ...
    private void deleteDir(File dir) {
        if (dir != null) {
            ...
            LogUtils.m1568d(TAG, "delete child dir= " + dir + ",ret = " + dir.delete());
        }
    }

    public void finishActivity() {
        List<Activity> list = AppRuntimeEnv.get().getActivityList();
        LogUtils.m1571e(TAG, "finishActivity =====  " + list.size());
