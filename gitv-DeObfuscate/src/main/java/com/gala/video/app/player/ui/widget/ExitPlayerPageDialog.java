...
public class ExitPlayerPageDialog extends AlertDialog {
    ...
    private IItemListener<IVideo> mContentItemListener = new C15286();
    private Context mContext;
    private FetchExitAlbumsTask mExitAlbumsTask;
    private OnFocusChangeListener mFocusChangedListener = new C15253();
    private OnKeyListener mKeyEventListener = new C15264();
    private IGalleryUIStyle mPortGalleryUIStyle = PlayerAppConfig.getUIStyle().getExitDialogPortGalleryUIStyle();
    private ITaskResultListener mTaskListener = new C15275();
    ...
    class C15242 implements OnClickListener {
        C15242() {
        }
    ...
    class C15253 implements OnFocusChangeListener {
        C15253() {
        }
    ...
    class C15264 implements OnKeyListener {
        C15264() {
        }

        public boolean onKey(View v, int keyCode, KeyEvent event) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(ExitPlayerPageDialog.TAG, ">> mKeyEventListener.onKey, v=" + v + ", event=" + event);
            }
    ...
    class C15275 implements ITaskResultListener {
        C15275() {
        }
        ...
        public void onFailed(ApiException e) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(ExitPlayerPageDialog.TAG, ">> FetchExitAlbumsTask, onFailed, e=" + e.getMessage());
            }
        }
    }

    class C15286 implements IItemListener<IVideo> {
        C15286() {
        }

        public void onItemClicked(IVideo data, int index) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(ExitPlayerPageDialog.TAG, ">> onItemClicked, index=" + index + ", data=" + data);
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(C1291R.layout.player_layout_play_page_exit_dialog);
        initWindow();
        initViews();
        setupViews();
        initFocus();
    ...
    private void initViews() {
        ...
        this.mContentContainer.addView(this.mContent.getView(), new FrameLayout.LayoutParams(-1, ResourceUtil.getDimen(C1291R.dimen.dimen_370dp)));
        this.mContent.show();
        LinearLayout.LayoutParams bottomLP = new LinearLayout.LayoutParams((ResourceUtil.getDimen(C1291R.dimen.dimen_195dp) + bgPadding.left) + bgPadding.right, (ResourceUtil.getDimen(C1291R.dimen.dimen_55dp) + bgPadding.top) + bgPadding.bottom);
    ...
    private void setupViews() {
        ...
        this.mBottomLeft.setOnClickListener(new C15231());
        this.mBottomRight.setOnClickListener(new C15242());
        ...
        if (album != null) {
            switch (GetInterfaceTools.getAlbumInfoHelper().getJumpType(album)) {
                ...
                case DETAILS:
                    ...
                    return;
                default:
                    LogUtils.m1571e(TAG, "album is " + DataUtils.albumInfoToString(album));
                    return;
            }
        }
    }
    ...
    private void gotoPlayerActivity(Album album) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> gotoPlayerActivity, album=" + album);
        }
    ...
    private void gotoDetailActivity(Album album) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> gotoDetailActivity, album=" + album);
        }
        String from = IntentConfig2.FROM_EXIT_DIALOG;
    ...
    private void sendExitDialogPageShowPingback() {
        if (this.mContext instanceof IPingbackContext) {
            ...
            LogUtils.m1568d(TAG, "sendExitDialogPageShowPingback, mContext is not instance of IPingbackContext!!");
        }
    }
    ...
    private void sendWaitButtonClickedPingback() {
        if (this.mContext instanceof IPingbackContext) {
            IPingbackContext pingbackContext = this.mContext;
            PingbackFactory.instance().createPingback(39).addItem(C0165R.R_TYPE("")).addItem(BLOCK.BLOCK_TYPE("exit")).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE("wait")).addItem(pingbackContext.getItem("rpage")).addItem(C1.C1_TYPE("")).addItem(pingbackContext.getItem("now_c1")).addItem(pingbackContext.getItem("now_qpid")).post();
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "sendWaitButtonClickedPingback, mContext is not instance of IPingbackContext!!");
        }
    }
    ...
    private void sendContentItemClickedPingback(IVideo video, int index) {
        if (this.mContext instanceof IPingbackContext) {
            String str;
            IPingbackContext pingbackContext = this.mContext;
            IPingback addItem = PingbackFactory.instance().createPingback(39).addItem(C0165R.R_TYPE(index == 0 ? String.valueOf("首页") : video.getAlbumId())).addItem(BLOCK.BLOCK_TYPE("exit")).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE(String.valueOf(index + 1))).addItem(pingbackContext.getItem("rpage"));
            if (index == 0) {
                str = "";
            } else {
            ...
            LogUtils.m1568d(TAG, "sendContentItemClickedPingback, mContext is not instance of IPingbackContext!!");
        }
    }
}
