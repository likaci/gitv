...
public class LabelAlbumAdapter extends Adapter<MyViewHolder> {
    ...
    private ColorStateList mColorStateList = ResourceUtil.getColorStateList(C0508R.drawable.epg_label_left_color_selector);
    private Context mContext;
    protected List<Tag> mDataList;
    private int mGreenColor = ResourceUtil.getColor(C0508R.color.gala_green);
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        if (holder == null || holder.itemView == null) {
            ...
            LogUtils.m1571e(TAG, "onBindViewHolder mDataList is null !");
        } else {
            Tag tag = (Tag) this.mDataList.get(position);
            if (tag == null) {
                LogUtils.m1571e(TAG, "onBindViewHolder tag is null !");
                return;
            }
            String name = tag.getName();
            TextView title = (TextView) holder.itemView.findViewById(C0508R.id.epg_label_tv);
            if (title != null) {
                title.setPadding(ResourceUtil.getDimen(C0508R.dimen.dimen_56dp), 0, ResourceUtil.getDimen(C0508R.dimen.dimen_20dp), 0);
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        int layout = C0508R.layout.epg_label_item_view;
        if (viewType == 0 || viewType == 2) {
            layout = C0508R.layout.epg_label_item_view;
        } else if (viewType == 1) {
            layout = C0508R.layout.epg_label_item_view_level1;
        }
