...
public class CtrlButtonPanel implements OnClickListener, OnFocusChangeListener, OnKeyListener {
    ...
    class C13401 implements IImageCallback {
        C13401() {
        }
        ...
        public void onFailure(ImageRequest arg0, Exception e) {
            LogUtils.m1568d(CtrlButtonPanel.TAG, "showEquityImage(): load image failure.");
    ...
    private void initViews() {
        this.mPanelView = this.mRootView.findViewById(C1291R.id.share_detail_ll_btn_panel);
        this.mBtnFav = (Button) this.mRootView.findViewById(C1291R.id.share_detail_btn_album_fav);
        this.mBtnFull = (Button) this.mRootView.findViewById(C1291R.id.share_detail_btn_album_full);
        this.mBtnBuy = (Button) this.mRootView.findViewById(C1291R.id.share_detail_btn_album_vip);
        this.mBtnDesc = (Button) this.mRootView.findViewById(C1291R.id.share_detail_btn_album_desc);
        this.mEvEquity = (EquityView) this.mRootView.findViewById(C1291R.id.share_detail_ev_equity);
        setupFavButton();
        setupFullButton();
        setupBuyButton();
        setupDescButton();
    ...
    private void setupFavButton() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> setupFavButton.");
        }
    ...
    private void setupFullButton() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> setupFullButton.");
        }
        ...
        this.mBtnFull.setText(ResourceUtil.getStr(this.mEnableWindowPlay ? C1291R.string.full_screen : C1291R.string.start_play));
        updateFullButton();
    }

    private void setupBuyButton() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> setupBuyButton.");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "vip tip string is [" + this.mVipString + AlbumEnterFactory.SIGN_STR);
        }
    ...
    private void setupDescButton() {
        ...
        if (this.mEnableWindowPlay) {
            this.mBtnDesc.setNextFocusLeftId(C1291R.id.share_detail_playwindow);
        } else {
    ...
    private void setupEquityImg() {
        ...
        if (this.mEnableWindowPlay) {
            this.mEvEquity.setNextFocusLeftId(C1291R.id.share_detail_playwindow);
        } else {
    ...
    public void onClick(View v) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> onClick, v.id=" + v.getId());
        }
        int i = v.getId();
        if (i == C1291R.id.share_detail_btn_album_fav) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "onClick(fav)");
            }
            notifyFavButtonClicked();
            sendFavButtonClickedPingback();
        } else if (i == C1291R.id.share_detail_btn_album_full) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "onClick(full) mShowBuy=" + this.mShowBuy);
            }
            if (this.mShowBuy) {
                CreateInterfaceTools.createWebRoleFactory().showRoleInVip(this.mContext);
            }
            notifyFullButtonClicked();
        } else if (i == C1291R.id.share_detail_btn_album_vip) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "onClick(VIP)");
            }
            notifyVIPButtonClicked();
            sendBuyButtonClickedPingback();
        } else if (i == C1291R.id.share_detail_btn_album_desc) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "onClick(desc)");
            }
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        ...
        setZoomAnim(v);
        LogUtils.m1568d(TAG, "onFocusChange(): mIsEquityFocus -> " + this.mIsEquityFocus);
    }
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> onKey, v.id=" + v.getId() + "keyCode=" + keyCode + ", event=" + event);
        }
        if (this.mCurScreenMode == ScreenMode.FULLSCREEN && (19 == event.getKeyCode() || 20 == event.getKeyCode() || 21 == event.getKeyCode() || 22 == event.getKeyCode())) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, ">> onKey, now is fullscreen mode, but playPanel not consume key, so we consume this key");
            }
            return true;
        }
        int i = v.getId();
        if (i == C1291R.id.share_detail_btn_album_fav) {
            if (19 == event.getKeyCode() && event.getAction() == 0) {
                notifyFocusChange(DetailButtonKeyFront.UP, C1291R.id.share_detail_btn_album_fav);
            }
        } else if (i == C1291R.id.share_detail_btn_album_full) {
            if (event.getAction() == 0) {
                if (19 == event.getKeyCode()) {
                    notifyFocusChange(DetailButtonKeyFront.UP, C1291R.id.share_detail_btn_album_full);
                } else if (21 == event.getKeyCode()) {
                    notifyFocusChange(DetailButtonKeyFront.LEFT, C1291R.id.share_detail_btn_album_full);
                }
            }
        } else if (i == C1291R.id.share_detail_btn_album_vip) {
            if (19 == event.getKeyCode() && event.getAction() == 0) {
                notifyFocusChange(DetailButtonKeyFront.UP, C1291R.id.share_detail_btn_album_vip);
            }
        } else if (i == C1291R.id.share_detail_btn_album_desc) {
            if (21 == event.getKeyCode()) {
                notifyFocusChange(DetailButtonKeyFront.LEFT, C1291R.id.share_detail_btn_album_desc);
            }
    ...
    private synchronized void setFoucsTextColor(Button btn, boolean hasFocus) {
        int btnTextColorNormal = ResourceUtil.getColor(C1291R.color.detail_text_color_default);
        int btnTextColorFocused = ResourceUtil.getColor(C1291R.color.detail_text_color_focused);
        if (!hasFocus) {
    ...
    private void updateButtonBg(Button btn, boolean hasFocus) {
        Drawable drawable;
        if (hasFocus) {
            drawable = ResourceUtil.getDrawable(C1291R.drawable.share_skin_button_bg_focused);
        } else {
            drawable = ResourceUtil.getDrawable(C1291R.drawable.share_skin_button_bg_unfocused);
        }
    ...
    private void updateDescButton(Button btnDesc, boolean hasFocus) {
        Drawable drawable;
        if (hasFocus) {
            drawable = ResourceUtil.getDrawable(C1291R.drawable.player_detail_skin_button_bg_desc_focused);
        } else {
            drawable = ResourceUtil.getDrawable(C1291R.drawable.player_detail_skin_button_bg_desc_unfocused);
        }
        AppClientUtils.setBackgroundDrawable(btnDesc, drawable);
    }

    private void setButtonIcon(Button button, Drawable draw) {
        draw.setBounds(0, ResourceUtil.getDimen(C1291R.dimen.dimen_01dp), ResourceUtil.getDimen(C1291R.dimen.dimen_22dp), ResourceUtil.getDimen(C1291R.dimen.dimen_20dp));
        button.setCompoundDrawables(draw, null, null, null);
        button.setCompoundDrawablePadding(ResourceUtil.getDimen(C1291R.dimen.dimen_6dp));
    }

    private void loadEquityImage(String url) {
        if (StringUtils.isEmpty((CharSequence) url)) {
            LogUtils.m1568d(TAG, "loadEquityImage(): url is null.");
            hideEquityImage();
            updateFocusPath();
            return;
        }
        LogUtils.m1568d(TAG, "loadEquityImage(): url -> " + url);
        ImageProviderApi.getImageProvider().loadImage(new ImageRequest(url, this.mEvEquity), new C13401());
    }

    private void showEquityImage() {
        this.mEvEquity.setVisibility(0);
        LogUtils.m1568d(TAG, "showEquityImage(): mIsEquityFocus -> " + this.mIsEquityFocus);
    ...
    private void updateFavorButton() {
        LogUtils.m1568d(TAG, ">>updateFavorButton()" + this.mIsFavored);
        this.mBtnFav.setText(ResourceUtil.getStr(this.mIsFavored ? C1291R.string.added_to_favorite : C1291R.string.add_to_favorite));
        Drawable icon = ResourceUtil.getDrawable(getAddToFavIconId(this.mIsFavored, this.mBtnFav.hasFocus()));
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> updateFavorButton, mBtnFav " + this.mBtnFav.hasFocus());
        }
        setButtonIcon(this.mBtnFav, icon);
    }

    private int getAddToFavIconId(boolean isFavorite, boolean hasFocus) {
        return isFavorite ? hasFocus ? C1291R.drawable.player_detail_button_img_faved_focused : C1291R.drawable.player_detail_button_img_faved_normal : hasFocus ? C1291R.drawable.player_detail_button_img_fav_focused : C1291R.drawable.player_detail_button_img_fav_normal;
    }

    private void updateFullButton() {
        LogUtils.m1568d(TAG, ">>updateFullButton(), mEnableWindowPlay=" + this.mEnableWindowPlay);
        Drawable icon = ResourceUtil.getDrawable(getPlayIconId(this.mEnableWindowPlay, this.mBtnFull.hasFocus()));
        this.mBtnFull.setNextFocusLeftId(this.mEnableWindowPlay ? C1291R.id.share_detail_playwindow : C1291R.id.share_detail_btn_album_full);
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> updateFullButton, mBtnFull " + this.mBtnFull.hasFocus());
        }
    ...
    private void updateBuyButton() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> updateBuyButton, mBtnBuy " + this.mBtnBuy.hasFocus() + ", mIsCoupon=" + this.mIsCoupon);
        }
        setButtonIcon(this.mBtnBuy, icon);
    }

    private int getBuyIconId(boolean isCoupon, boolean hasFocus) {
        return isCoupon ? hasFocus ? C1291R.drawable.player_detail_button_img_coupon_focused : C1291R.drawable.player_detail_button_img_coupon_normal : hasFocus ? C1291R.drawable.player_detail_button_img_vip_focused : C1291R.drawable.player_detail_button_img_vip_normal;
    }

    private int getPlayIconId(boolean isEnableWindowPlay, boolean hasFocus) {
        return isEnableWindowPlay ? hasFocus ? C1291R.drawable.player_detail_button_img_full_focused : C1291R.drawable.player_detail_button_img_full_normal : hasFocus ? C1291R.drawable.player_detail_button_img_play_focused : C1291R.drawable.player_detail_button_img_play_normal;
    }
    ...
    private void showBuyBtn() {
        if (this.mBtnBuy != null) {
            ...
            if (this.mAlbumInfo.isAlbumCoupon()) {
                textResId = C1291R.string.btn_coupon;
            } else if (this.mAlbumInfo.isAlbumSinglePay()) {
                textResId = C1291R.string.btn_buy_album;
            } else if (isUserVip) {
                textResId = C1291R.string.share_detail_btn_renewal_vip;
            } else {
                textResId = C1291R.string.share_detail_btn_join_vip;
            }
            this.mBtnBuy.setText(ResourceUtil.getStr(textResId));
            updateBuyButton();
            LogUtils.m1568d(TAG, "showBuyBtn(): mIsUserActed -> " + this.mIsUserActed + ", mIsEquityFocus -> " + this.mIsEquityFocus);
    ...
    private void updateButtonFocusPath() {
        LogUtils.m1568d(TAG, "updateFocusPath");
        ViewGroup btnPanel = (ViewGroup) this.mRootView.findViewById(C1291R.id.share_detail_ll_btn_panel);
    ...
    public void notifyVIPInfoReady() {
        LogUtils.m1568d(TAG, ">> notifyVIPInfoReady, mAlbumInfo=" + this.mAlbumInfo.toStringBrief());
    ...
    public void notifyTvodReady() {
        LogUtils.m1568d(TAG, "notifyTvodReady()");
        if (this.mAlbumInfo.isAlbumSinglePay()) {
            boolean isUserVip = GetInterfaceTools.getIGalaAccountManager().isVip();
            LogUtils.m1568d(TAG, "notifyTvodReady(): pay, isUserVip -> " + isUserVip);
    ...
    public void notifyScreenModeSwitched(ScreenMode mode, boolean isError) {
        LogUtils.m1568d(TAG, "notifyScreenModeSwitched(): >> mode -> " + mode);
        this.mCurScreenMode = mode;
    }

    public void updateFullBtnText() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> updateFullBtnText, mAlbumInfo=" + this.mAlbumInfo);
        }
        ...
        if (albumInfo.isSourceType()) {
            ...
            if (!StringUtils.isEmpty(time)) {
                ...
                LogUtils.m1568d(TAG, "Invalid Variety issue time, info=" + albumInfo);
            }
        } else if (albumInfo.isTvSeries()) {
            int playOrder = albumInfo.getPlayOrder();
            if (playOrder < 1) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1569d(TAG, "Invalid play order, info=" + albumInfo, new Throwable());
                }
            } else if (this.mOldPlayOrder != playOrder) {
                this.mOldPlayOrder = playOrder;
                this.mBtnFull.setText(ResourceUtil.getStr(C1291R.string.play_order, Integer.valueOf(playOrder)));
            } else if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "the same play order, do not need refresh, playOrder=" + playOrder);
            }
        } else if (albumInfo.getKind() == VideoKind.VIDEO_SINGLE) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "updateFullBtnText");
            }
            this.mBtnFull.setText(ResourceUtil.getStr(this.mEnableWindowPlay ? C1291R.string.full_screen : C1291R.string.start_play));
        }
    }
    ...
    public void setAlbumInfo(AlbumInfo albumInfo) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> setAlbumInfo, albumInfo=" + albumInfo);
        }
        if (albumInfo != null) {
    ...
    private void sendFavButtonClickedPingback() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> sendFavButtonClickedPingback");
        }
