...
public class OpenDetailAction extends BasePlayAndDetailAction {
    ...
    String playAndDetail(Context context, Album album) {
        LogUtils.m1574i("openplay/broadcast/OpenDetailAction", "===playAndDetail===");
        String page = "";
        JumpKind jumpType = GetInterfaceTools.getAlbumInfoHelper().getJumpType(album);
        LogUtils.m1574i("openplay/broadcast/OpenDetailAction", "jumpType =  " + jumpType);
        switch (jumpType) {
            case PLAY:
            case PLAY_LIST:
                if (hasPlayPermission()) {
                    page = "playvideo";
                    LogUtils.m1574i("openplay/broadcast/OpenDetailAction", "dealDetailAcion()--hasPlayPermission --startPlay");
                    startPlay(context.getApplicationContext(), album);
                    return page;
                }
                page = "home";
                LogUtils.m1574i("openplay/broadcast/OpenDetailAction", "dealDetailAcion()--no PlayPermission --startHomeActivity");
    ...
    public boolean checkParamsValidity(JSONObject playDict) {
        if (super.checkParamsValidity(playDict)) {
            ...
            if (TextUtils.isEmpty(chnIdStr)) {
                ...
                LogUtils.m1574i("openplay/broadcast/OpenDetailAction", "===vrsChnId====chnIdStr:" + chnIdStr);
            }
            if (TextUtils.isEmpty(chnIdStr)) {
                LogUtils.m1571e("openplay/broadcast/OpenDetailAction", "[INVALID-PARAMTER] [action:ACTION_DETAIL][reason:missing field--chnId][playInfo:" + playDict.toString() + AlbumEnterFactory.SIGN_STR);
            } else {
                LogUtils.m1574i("openplay/broadcast/OpenDetailAction", "===checkParamsValidity===chnIdStr ==" + chnIdStr);
                return true;
            }
        }
        return false;
