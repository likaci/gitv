...
public class TabBarAdapter {
    ...
    private static final int TAB_HOLDER = C0508R.id.epg_tab_host;
    ...
    private class ToastRunnable implements Runnable {
        ...
        public void run() {
            ...
            TabBarAdapter.this.mToast = QToast.makeText(TabBarAdapter.this.mContext, TabBarAdapter.this.mContext.getResources().getString(C0508R.string.tab_click_guide_toast), (int) QToast.LENGTH_4000);
    ...
    public TabBarAdapter(Context context, List<TabData> tabBarItems, ImageView indicator) {
        ...
        this.mNormalIndicator = context.getResources().getDrawable(C0508R.drawable.epg_tab_bar_decorated_line);
        this.mVipIndicator = context.getResources().getDrawable(C0508R.drawable.epg_tab_bar_decorated_vip_line);
        if (mToastCount < 0) {
    ...
    private void initTabNameView(TabNameView tabNameView) {
        int left = (int) this.mContext.getResources().getDimension(C0508R.dimen.dimen_20dp);
        int top = (int) this.mContext.getResources().getDimension(C0508R.dimen.dimen_14dp);
        int right = (int) this.mContext.getResources().getDimension(C0508R.dimen.dimen_20dp);
        int bottom = (int) this.mContext.getResources().getDimension(C0508R.dimen.dimen_14dp);
        tabNameView.setGravity(16);
        tabNameView.setPadding(left, 0, right, 0);
        tabNameView.setHeight((int) this.mContext.getResources().getDimension(C0508R.dimen.dimen_54dp));
        tabNameView.setTextSize(0, (float) ResourceUtil.getDimen(C0508R.dimen.dimen_25dp));
        tabNameView.setTextColor(this.mContext.getResources().getColor(C0508R.color.home_tab_name_text_normal));
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (this.mTabBarItems != null && position < this.mTabBarItems.size()) {
            ...
            if (isVip) {
                tabNameView.setTextShaderColor(this.mContext.getResources().getColor(C0508R.color.home_vip_tab_name_text_focus_shader_start), this.mContext.getResources().getColor(C0508R.color.home_vip_tab_name_text_focus_shader_end));
            } else {
                tabNameView.setTextShaderColor(this.mContext.getResources().getColor(C0508R.color.home_tab_name_text_focus_shader_start), this.mContext.getResources().getColor(C0508R.color.home_tab_name_text_focus_shader_end));
            }
        }
        Object result = tabNameView.getTag(TAB_HOLDER);
        if (result instanceof TabData) {
            if (((TabData) result).isIsVipTab()) {
                setBackground(tabNameView, this.mContext.getResources().getDrawable(C0508R.drawable.epg_tab_bar_special_bg));
            } else {
                setBackground(tabNameView, this.mContext.getResources().getDrawable(C0508R.drawable.epg_tab_bar_normal_bg));
            }
        }
        tabNameView.setText(name);
        MarginLayoutParams layoutParams = (LayoutParams) tabNameView.getLayoutParams();
        if (layoutParams == null) {
            layoutParams = new LayoutParams(-2, this.mContext.getResources().getDimensionPixelSize(C0508R.dimen.dimen_44dp));
        }
        layoutParams.topMargin = ResourceUtil.getDimen(C0508R.dimen.dimen_7dp);
        layoutParams.bottomMargin = ResourceUtil.getDimen(C0508R.dimen.dimen_6dp);
        MarginLayoutParams marginLayoutParams = layoutParams;
        if (position != 0) {
            layoutParams.leftMargin = ResourceUtil.getDimensionPixelSize(C0508R.dimen.dimen_29dp);
        } else {
