...
public class IntertrustDrmPluginProvider {
    ...
    public static String getIntertrustDrmModulePath() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> getIntertrustDrmModulePath");
        }
        if (mAccessToken.isAllowedAccess()) {
            mAccessToken.increaseAccessCount();
            if (PlayerDebugUtils.testIntertrustDrmPluginFail()) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(TAG, "<< getIntertrustDrmModulePath: failed, debug load intertrust drm plugin failed.");
                }
                return "";
            }
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, ">> IntertrustDrmPluginProvider " + mHPluginInfo);
            }
            ...
            if (provider == null) {
                ...
                if (result != null) {
                    ...
                    if (result.getData() != null) {
                        ...
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(TAG, "getIntertrustDrmModulePath: success, libPath=" + sModulePath);
                        }
                    } else if (LogUtils.mIsDebug) {
                        LogUtils.m1571e(TAG, "getIntertrustDrmModulePath: failed, result.getData() is null");
                    }
                } else if (LogUtils.mIsDebug) {
                    LogUtils.m1571e(TAG, "getIntertrustDrmModulePath: failed, result is null");
                }
            } else {
                sModulePath = provider.getPluginFilePath();
            }
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "<< getIntertrustDrmModulePath: return " + sModulePath);
            }
            return sModulePath;
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "<< getIntertrustDrmModulePath: failed, not allowed access.");
        }
        return "";
    }
