...
public class VideoChecker {
    ...
    public static boolean checkHistory(IVideo video) {
        ...
        if (realSize != markSize) {
            LogUtils.m1577w(TAG, "wrong episodes info! real(" + realSize + "), mark(" + markSize + ")");
            sanity = false;
        }
        if (playOrder <= realSize) {
            return sanity;
        }
        LogUtils.m1577w(TAG, "wrong play order! play order(" + playOrder + "), real(" + realSize + ")");
        return false;
    }
    ...
    public static boolean isValidAlbumId(String albumId) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isValidAlbumId(" + albumId + ") return " + valid);
        }
        return valid;
    }
    ...
    public static boolean isValidTvId(String vrsTvId) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isValidTvId(" + vrsTvId + ") return " + valid);
        }
        return valid;
    }
    ...
    public static boolean isValidVid(String vid) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isValidVid(" + vid + ") return " + valid);
        }
        return valid;
    }
    ...
    public static boolean isExistInEpisodeList(List<IVideo> episodeList, int playOrder) {
        ...
        if (!ListUtils.isEmpty((List) episodeList)) {
            for (IVideo e : episodeList) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(TAG, "isExistInEpisodeList: episode order: " + e.getAlbum().order);
                }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isExistInEpisodeList(" + playOrder + "): result=" + ret);
        }
        return ret;
    }
