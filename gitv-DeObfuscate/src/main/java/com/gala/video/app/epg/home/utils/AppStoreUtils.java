...
public class AppStoreUtils {
    ...
    private static DownloadListener mDownloadListener = new C06881();

    static class C06881 implements DownloadListener {
        ...
        public void onStart() {
            AppStoreUtils.refreshToast(this.mContext, ResourceUtil.getStr(C0508R.string.dialog_app_start_download));
        }
        ...
        public void onStop() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(AppStoreUtils.TAG, "download stop");
            }
        }

        public void onComplete(final int code, final String path, final String name) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(AppStoreUtils.TAG, "download complete,  " + code + " , path: " + path);
            }
            AppStoreUtils.runOnUiThread(new Runnable() {
                public void run() {
                    if (code == 0) {
                        C06881.this.refreshDownloadSuccess(code, path, name);
                    } else {
                        C06881.this.refreshDownloadFailed(code, path, name);
                    }
                }
            });
        }
        ...
        private void refreshDownloadSuccess(int code, String path, String name) {
            try {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1572e(AppStoreUtils.TAG, "install app failed,,exception = ", e);
                }
            }
        }
        ...
        private void refreshDownloadFailed(int code, String path, String name) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(AppStoreUtils.TAG, "download failed -> " + code + " , path: " + path);
            }
            if (code != 3) {
                QToast.makeTextAndShow(this.mContext, ResourceUtil.getStr(C0508R.string.dialog_app_download_error), (int) QToast.LENGTH_4000);
            }
        }
    }
    ...
    public static void onAppOnlineClick(Context context, String appPackageName, int appId) {
        try {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1571e(TAG, "onAppOnlineClick() -> exception");
            }
            e.printStackTrace();
        }
    }
    ...
    private static boolean isInstalled(Context context, String appPackageName) {
        if (!StringUtils.isEmpty((CharSequence) appPackageName)) {
            try {
                ...
                LogUtils.m1569d(TAG, "packageName is not found, NameNotFoundException:", e);
                return false;
            }
        } else if (!LogUtils.mIsDebug) {
            return false;
        } else {
            LogUtils.m1568d(TAG, "packageName is null ");
            return false;
        }
    }
    ...
    private static void startDownloadAppStore(Context context, String apkDownloadUrl) {
        ...
        if (StringUtils.isEmpty((CharSequence) apkDownloadUrl)) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "appDownloadUrl is null");
            }
            refreshToast(context, context.getString(C0508R.string.dialog_app_nodata));
            return;
        }
        ...
        if (state == 2) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "init failed");
            }
        } else if (state == 1) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "file exist");
            }
            AppStoreManager.getInstance().install(DownloadManager.getInstance().getApkFilePath(mApkDownloadUrl));
        } else if (state == 3) {
            refreshToast(context, context.getString(C0508R.string.dialog_app_downloading));
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "downloading");
            }
        }
    }
