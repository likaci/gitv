...
public class EntryUtils {
    ...
    private static int parse(String str, int defaultValue) {
        ...
        try {
            ...
            LogUtils.m1578w(TAG, "parse(" + str + ") error!", e);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "parse(" + str + ", " + defaultValue + ") return " + value);
        }
        return value;
    }
    ...
    public static void startSettingActivity(final Context context, String action) {
        LogUtils.m1568d(TAG, "startSettingActivity(" + action + ")");
        configStartFlag(context, GLOBAL_INTENT_FLAG);
        Intent intent = new Intent();
        ...
        try {
            ...
            new Handler(context.getMainLooper()).post(new Runnable() {
                public void run() {
                    QToast.makeText(context, C0508R.string.setting_not_install, 3000).show();
                }
            });
        }
    }
