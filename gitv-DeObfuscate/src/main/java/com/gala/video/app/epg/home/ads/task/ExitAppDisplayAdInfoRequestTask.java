...
public class ExitAppDisplayAdInfoRequestTask {
    ...
    class C05822 implements IImageCallback {
        C05822() {
        }
        ...
        public void onSuccess(ImageRequest imageRequest, Bitmap bitmap) {
            if (ExitAppDisplayAdInfoRequestTask.this.mTimeHandler.hasMessages(113)) {
                ExitAppDisplayAdInfoRequestTask.this.mTimeHandler.removeCallbacksAndMessages(null);
                LogUtils.m1568d(ExitAppDisplayAdInfoRequestTask.TAG, "downloadAdImage success");
            ...
            LogUtils.m1568d(ExitAppDisplayAdInfoRequestTask.TAG, "downloadAdImage success time out ");
        }

        public void onFailure(ImageRequest imageRequest, Exception e) {
            if (ExitAppDisplayAdInfoRequestTask.this.mTimeHandler.hasMessages(113)) {
                ExitAppDisplayAdInfoRequestTask.this.mTimeHandler.removeCallbacksAndMessages(null);
                LogUtils.m1568d(ExitAppDisplayAdInfoRequestTask.TAG, "downloadAdImage failed");
                ExitAppDisplayAdInfoRequestTask.this.isRequestSuccess = true;
                ExitAppDisplayAdInfoRequestTask.this.mTimeHandler.sendEmptyMessage(111);
                return;
            }
            LogUtils.m1568d(ExitAppDisplayAdInfoRequestTask.TAG, "downloadAdImage failed time out ");
        }
    }
    ...
    private class TimeHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 110:
                    LogUtils.m1568d(ExitAppDisplayAdInfoRequestTask.TAG, "TimeHandler, fetch ad data success");
                    ...
                    return;
                case 111:
                    LogUtils.m1568d(ExitAppDisplayAdInfoRequestTask.TAG, "TimeHandler, fetch ad data failed");
                    if (ExitAppDisplayAdInfoRequestTask.this.mOnExitAdRequestListener != null) {
                        ExitAppDisplayAdInfoRequestTask.this.mOnExitAdRequestListener.onFailed();
                    }
                    HomePingbackFactory.instance().createPingback(CommonPingback.AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_exitapk").addItem("st", MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR).addItem("td", String.valueOf(SystemClock.elapsedRealtime() - ExitAppDisplayAdInfoRequestTask.this.startTime)).addItem(Keys.f2035T, "11").addItem("ct", "150619_request").setOthersNull().post();
                    return;
                case 112:
                    LogUtils.m1568d(ExitAppDisplayAdInfoRequestTask.TAG, "TimeHandler, no ad data");
                    if (ExitAppDisplayAdInfoRequestTask.this.mOnExitAdRequestListener != null) {
                        ExitAppDisplayAdInfoRequestTask.this.mOnExitAdRequestListener.onNoAdData();
                    }
                    HomePingbackFactory.instance().createPingback(CommonPingback.AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_exitapk").addItem("st", "0").addItem("td", String.valueOf(SystemClock.elapsedRealtime() - ExitAppDisplayAdInfoRequestTask.this.startTime)).addItem(Keys.f2035T, "11").addItem("ct", "150619_request").setOthersNull().post();
                    return;
                case 113:
                    if (!ExitAppDisplayAdInfoRequestTask.this.isRequestSuccess) {
                        LogUtils.m1568d(ExitAppDisplayAdInfoRequestTask.TAG, "TimeHandler, fetch exit ad time out");
                        if (ExitAppDisplayAdInfoRequestTask.this.mOnExitAdRequestListener != null) {
    ...
    public void execute() {
        LogUtils.m1568d(TAG, "execute exit app ad download task");
        clear();
        this.mTimeHandler.sendEmptyMessageDelayed(113, this.mDownloadLimitTime);
        this.startTime = SystemClock.elapsedRealtime();
        ThreadUtils.execute(new C05811());
    }
    ...
    private boolean requestAndParseAdInfo() {
        ...
        LogUtils.m1568d(TAG, "requestAndParseAdInfo, adJson is empty");
    ...
    private boolean parseAdJson(String adJson, AdsClient adsClient) {
        ...
        try {
            ...
            if (ListUtils.isEmpty((List) slotList)) {
                LogUtils.m1568d(TAG, "parseAdJson, no slot with the type of SLOT_TYPE_PAGE");
                return false;
            }
            for (CupidAdSlot slot : slotList) {
                ...
                if (ListUtils.isEmpty((List) cupidAdList)) {
                    LogUtils.m1568d(TAG, "parseAdJson, the CupidAd with " + mSlotId + " is empty.");
                } else {
                    for (CupidAd cupidAd : cupidAdList) {
                        if (cupidAd == null) {
                            LogUtils.m1568d(TAG, "parseAdJson, CupidAd object is null.");
                            ...
                            LogUtils.m1568d(TAG, "parseAdJson, exit app ad info = " + model);
                            return true;
                        }
                    }
                    continue;
            ...
            LogUtils.m1572e(TAG, "parseAdJson, exception", e2);
        }
    }

    private void downloadAdImage() {
        LogUtils.m1568d(TAG, "downloadAdImage");
        if (this.mCurrentExitAppAdModle != null) {
            this.mImageProvider.loadImage(new ImageRequest(this.mCurrentExitAppAdModle.getAdImageUrl()), new C05822());
        }
    }
}
