...
public class RecommendView extends CloudView {
    private static final int IMAGE_PADDING = ResourceUtil.getDimen(C0508R.dimen.dimen_28dp);
    ...
    private final LiveCornerListener mLiveCornerListener = new C08782();
    ...
    class C08782 implements LiveCornerListener {
        C08782() {
        }

        public void showBefore() {
            RecommendView.this.setRTCornerimage(C0508R.drawable.share_corner_notice);
        }

        public void showPlaying() {
            RecommendView.this.setRTCornerimage(C0508R.drawable.share_corner_living);
        }

        public void showEnd() {
            RecommendView.this.setRTCornerimage(C0508R.drawable.share_corner_end_living);
        }
    }
    ...
    public void setViewParams(int type, int width, int height) {
        ...
        this.mWidth = width;
        this.mHeight = height;
        setBackgroundResource(C0508R.drawable.share_item_rect_selector_recommendview);
        initView();
        initListener();
        initTitleBg();
    }
    ...
    private void initTitleBg() {
        if (this.mPhotoType == 1) {
            ...
            this.mTitleBgView.setHeight(ResourceUtil.getDimen(C0508R.dimen.dimen_60dp));
            this.mTitleView.setGravity(Gravity4CuteText.CENTER_NONE);
            this.mTitleView.setMarginBottom(ResourceUtil.getDimen(C0508R.dimen.dimen_28dp));
            this.mNameView.setMarginBottom(ResourceUtil.getDimen(C0508R.dimen.dimen_6dp));
        } else {
            this.mTitleBgView.setHeight(ResourceUtil.getDimen(C0508R.dimen.dimen_36dp));
            if (LogUtils.mIsDebug) {
                LogUtils.m1571e(TAG, "initTitleBg no mPhotoType");
            }
        }
    }
    ...
    private void initView() {
        ...
        lp.leftMargin = ResourceUtil.getDimen(C0508R.dimen.dimen_24dp);
        lp.topMargin = ResourceUtil.getDimen(C0508R.dimen.dimen_2dp);
        setLayoutParams(lp);
        ...
        this.mCoreImageView.setDrawable(ResourceUtil.getDrawable(C0508R.drawable.share_default_image));
    }
    ...
    private void setCornerLTImage(IData info) {
        ...
        if (info.getCornerStatus(7)) {
            leftResId = C0508R.drawable.share_corner_yongquan;
        } else if (isSingleBuy) {
            leftResId = C0508R.drawable.share_corner_fufeidianbo;
        } else if (isVip) {
            leftResId = C0508R.drawable.share_corner_vip;
        }
    ...
    private void setCornerRTImage(IData info) {
        ...
        if ((info.getData() instanceof ChannelLabel) && !LivePlayingType.DEFAULT.equals(((ChannelLabel) info.getData()).getLivePlayingType())) {
            ...
            resId = C0508R.drawable.share_corner_dujia;
        } else if (isDubo) {
            resId = C0508R.drawable.share_corner_dubo;
        }
        if (resId != 0) {
