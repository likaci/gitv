...
public class DetailPingBackUtils {
    ...
    public static void sendEquityClickPingBack(AlbumInfo albumInfo, IPingbackContext pingbackContext) {
        if (albumInfo == null) {
            ...
            params.add(Keys.f2035T, "20").add("e", e).add("rpage", "detail").add("block", block).add("rseat", rseat).add("c1", c1).add("now_c1", now_c1).add("r", r).add("s2", s2).add("rt", "i").add("rfr", rfr).add("tabid", "").add("now_qpid", now_qpid);
    ...
    public static String getBlock(AlbumInfo albumInfo) {
        ...
        if (albumInfo.isAlbumCoupon()) {
            return block + ResourceUtil.getStr(C1291R.string.share_detail_pingback_coupon) + "_" + ResourceUtil.getStr(C1291R.string.share_detail_pingback_coupon_see);
        }
        if (albumInfo.isAlbumSinglePay()) {
            return block + ResourceUtil.getStr(C1291R.string.share_detail_pingback_tvod) + "_" + ResourceUtil.getStr(C1291R.string.share_detail_pingback_tvod_buy);
        }
        if (albumInfo.isAlbumVip()) {
            if (isUserVip) {
                return block + ResourceUtil.getStr(C1291R.string.share_detail_pingback_vip) + "_" + ResourceUtil.getStr(C1291R.string.share_detail_pingback_vip_renewal);
            }
            return block + ResourceUtil.getStr(C1291R.string.share_detail_pingback_vip) + "_" + ResourceUtil.getStr(C1291R.string.share_detail_pingback_vip_join);
        } else if (isUserVip) {
            return block + ResourceUtil.getStr(C1291R.string.share_detail_pingback_free) + "_" + ResourceUtil.getStr(C1291R.string.share_detail_pingback_vip_renewal);
        } else {
            return block + ResourceUtil.getStr(C1291R.string.share_detail_pingback_free) + "_" + ResourceUtil.getStr(C1291R.string.share_detail_pingback_vip_join);
        }
    }
    ...
    public static String getEquityImageClickRseat(AlbumInfo albumInfo) {
        ...
        String rseat = ResourceUtil.getStr(C1291R.string.share_detail_pingback_equity_image) + "_";
        boolean isUserVip = GetInterfaceTools.getIGalaAccountManager().isVip();
        if (albumInfo.isAlbumCoupon()) {
            if (albumInfo.isVipAuthorized() || !isUserVip) {
                return rseat + ResourceUtil.getStr(C1291R.string.share_detail_pingback_vip_join);
            }
            return rseat + ResourceUtil.getStr(C1291R.string.share_detail_pingback_coupon_see);
            ...
            return rseat + ResourceUtil.getStr(C1291R.string.share_detail_pingback_tvod_buy);
        }
    }
