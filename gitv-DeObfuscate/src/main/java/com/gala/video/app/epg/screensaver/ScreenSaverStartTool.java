...
public class ScreenSaverStartTool {
    ...
    private IScreenSaverBeforeFadeIn mIScreenSaverBeforeFadeInCallBack = new C07872();
    private IScreenSaverAdClick mScreenSaverAdClickListener = new C07861();
    private IScreenSaverClick mScreenSaverClickListener = new C07883();

    class C07861 implements IScreenSaverAdClick {
        C07861() {
        }
    ...
    class C07872 implements IScreenSaverBeforeFadeIn {
        C07872() {
        }

        public boolean onBeforeFadeIn(ScreenSaverModel screenSaverModel, TextView mTvClickTxtView) {
            LogUtils.m1568d(ScreenSaverStartTool.TAG, "IScreenSaverOperate.IScreenSaverBeforeFadeIn, onBeforeFadeIn, screensavermodel " + screenSaverModel);
    ...
    class C07883 implements IScreenSaverClick {
        C07883() {
        }

        public boolean onKeyEvent(KeyEvent event, ScreenSaverModel model, Context context) {
            if (model == null) {
                LogUtils.m1577w(ScreenSaverStartTool.TAG, "on click screen saver image, model is null");
                return true;
            }
            ...
            LogUtils.m1568d(ScreenSaverStartTool.TAG, "on click screen saver image, not support jump, or not support Resource type");
            return true;
        }
    }
    ...
    private void onScreenSaverAdClick(ScreenSaverAdModel screenSaverAdModel, Context context) {
        if (screenSaverAdModel == null) {
            LogUtils.m1568d(TAG, "onScreenSaverAdClick, screen saver ad data is empty");
            return;
        }
        ...
        if (screenSaverAdModel.isEnableJumping()) {
            ...
            LogUtils.m1568d(TAG, "onScreenSaverAdClick, screen saver ad can not jump");
        }
    }
