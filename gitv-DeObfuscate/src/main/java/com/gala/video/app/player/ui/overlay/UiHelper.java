...
public class UiHelper {
    ...
    public static void setButtonContent(Context context, int textcolor, int drawableLeftResId, Button button) {
        ...
        LogUtils.m1568d(TAG, "setButtonContent: textWidth=" + textWidth);
        int buttonWidth = button.getLayoutParams().width;
        if (LogUtils.mIsDebug) {
            LogUtils.m1574i(TAG, "setButtonContent: textSize=" + textSize);
        }
        int drawableSize = PlayerAppConfig.getButtonLeftDrawableSizeForDetailPage(textSize);
        if (LogUtils.mIsDebug) {
            LogUtils.m1574i(TAG, "setButtonContent: drawableSize=" + drawableSize);
        }
        ...
        int contentSpacing = context.getResources().getDimensionPixelSize(C1291R.dimen.dimen_3dp);
    ...
    public static void setButtonContent(Context context, Button button, String text, int drawableLeftResId) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">>setButtonContent" + text + ";drawableLeftResId:" + drawableLeftResId);
        }
        button.setText(text);
