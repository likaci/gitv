...
public class FeedBackFactory extends Wrapper {
    ...
    private String getErrorMessage(ApiExceptionModel model) {
        ...
        if (event == null) {
            LogUtils.m1571e(TAG, "getErrorMessage()---event is null");
            return context.getString(C0508R.string.devcheck_json_exception);
        }
        switch (event) {
            case C_ERROR_MAC:
                return context.getString(C0508R.string.device_cannot_use) + "";
            case C_ERROR_E000054:
                return context.getString(C0508R.string.common_api_error_msg, new Object[]{model.getApiCode()});
            case C_ERROR_E000012:
                return context.getString(C0508R.string.common_data_error_msg, new Object[]{model.getApiCode()});
            ...
            case C_ERROR_JSON:
                return context.getString(C0508R.string.devcheck_json_exception);
            case C_ERROR_HTTP:
                return context.getString(C0508R.string.devcheck_http_exception) + model.getHttpCode();
            case C_ERROR_SERVER:
                return context.getString(C0508R.string.server_error);
            case C_ERROR_INTERNET:
                return context.getString(Project.getInstance().getResProvider().getCannotConnInternet());
            case C_ERROR_NONET:
                return context.getString(C0508R.string.no_network);
            case C_ERROR_DATAISNULL:
                return context.getString(C0508R.string.devcheck_json_exception);
            default:
                LogUtils.m1571e(TAG, "getErrorMessage()---default--");
                return context.getString(C0508R.string.devcheck_json_exception);
        }
    }
}
