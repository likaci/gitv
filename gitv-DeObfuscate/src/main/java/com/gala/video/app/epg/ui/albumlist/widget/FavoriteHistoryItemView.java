...
public class FavoriteHistoryItemView extends AlbumView {
    ...
    protected void focusRecyclerCover(boolean hasFocus) {
        ...
        if (this.mRecycleCoverView != null) {
            this.mRecycleCoverView.setDrawable(ResourceUtil.getDrawable(hasFocus ? C0508R.drawable.epg_ic_recycle_focus_cover : C0508R.drawable.epg_ic_recycle_unfocus_cover));
            getRecycleTitleView();
        ...
        if (visible == 0) {
            if (StringUtils.isEmpty(this.mRecycleTitleText)) {
                if (this.mPageType.equals("history")) {
                    this.mRecycleTitleText = ResourceUtil.getStr(C0508R.string.record_ok_delete);
                } else if (this.mPageType.equals("favorite")) {
                    this.mRecycleTitleText = ResourceUtil.getStr(C0508R.string.favor_ok_delete);
                } else if (this.mPageType.equals(SubPage)) {
                    this.mRecycleTitleText = ResourceUtil.getStr(C0508R.string.subscrible_ok_delete);
                } else {
                    this.mRecycleTitleText = ResourceUtil.getStr(C0508R.string.record_ok_delete);
                }
            }
            if (StringUtils.isEmpty(this.mRecycleTitleText2)) {
                if (this.mPageType.equals(SubPage)) {
                    this.mRecycleTitleText2 = ResourceUtil.getStr(C0508R.string.subscrible_delete_back_quit);
                } else {
                    this.mRecycleTitleText2 = ResourceUtil.getStr(C0508R.string.delete_back_quit);
                }
            }
        }
