...
class FeedBackController extends Wrapper {
    ...
    private OnClickListener mFeedBackBtnListener = new C05372();
    ...
    class C05394 implements IFeedbackResultListener {
        C05394() {
        }
        ...
        public void sendReportSuccess(String feedbackId, String ip) {
            if (LogRecordDebugUtils.testLogRecordExceptionForF00001()) {
                ...
                params.add(Keys.f2035T, "11").add("st", "0").add("ct", "150721_feedback").add("ec", "").add("pfec", "").add("e", LogRecordUtils.getEventID()).add(Keys.FEEDBACK_ID, feedbackId).add(Keys.FBTYPE, InterfaceKey.EPG_FB);
                PingBack.getInstance().postPingBackToLongYuan(params.build());
                FeedBackController.this.feedBackSuccess(feedbackId, ip);
            }
            LogUtils.m1568d("EPG/utils/FeedBackController", ">>>>> send feedback success, start upload net diagnose info");
        ...
        public void sendReportFailed(String e) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1571e("EPG/utils/FeedBackController", "feedBack---onFail---error=" + e);
            }
            PingBackParams params = new PingBackParams();
            params.add(Keys.f2035T, "11").add("st", MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR).add("ct", "150721_feedback").add("ec", "315011").add("pfec", "").add("e", LogRecordUtils.getEventID()).add(Keys.FEEDBACK_ID, "").add(Keys.FBTYPE, InterfaceKey.EPG_FB);
            PingBack.getInstance().postPingBackToLongYuan(params.build());
    ...
    class C05427 implements Runnable {
        C05427() {
        }

        public void run() {
            if (FeedBackController.this.mPrepareListener != null) {
                LogUtils.m1568d("EPG/utils/FeedBackController", "feedBack()----prepare start");
                FeedBackController.this.prepare();
            }
            LogUtils.m1568d("EPG/utils/FeedBackController", "feedBack()----prepare end");
    ...
    public void showQRDialog(FeedBackModel model, OnDismissListener dismissListener) {
        ...
        showQRDialog(feedBackModel, onDismissListener, this.mContext.getString(C0508R.string.popup_dialog_feedback_btn_text), this.mFeedBackBtnListener, this.mContext.getString(C0508R.string.back), this.mCancelBtnListener);
    }
    ...
    public void showQRDialog(FeedBackModel model, OnDismissListener dismissListener, String leftText, OnClickListener leftListener, String rightText, OnClickListener rightListener) {
        if (model == null) {
            LogUtils.m1571e("EPG/utils/FeedBackController", "showQRDialog()----FeedBackModel is null");
        } else if (this.mContext == null) {
            LogUtils.m1571e("EPG/utils/FeedBackController", "feedBack()---mContext=" + this.mContext);
        } else {
            ...
            if (StringUtils.isEmpty((CharSequence) leftText)) {
                leftText = this.mContext.getString(C0508R.string.popup_dialog_feedback_btn_text);
                leftListener = this.mFeedBackBtnListener;
            }
            if (StringUtils.isEmpty((CharSequence) rightText)) {
                rightText = this.mContext.getString(C0508R.string.back);
                rightListener = this.mCancelBtnListener;
            }
            this.mDialog.setParams(this.mContext.getString(C0508R.string.feedback_tip, new Object[]{this.mFeedBackModel.getErrorMsg()}), leftText, leftListener, rightText, rightListener);
            this.mDialog.setGravity(3);
            this.mDialog.show();
            ThreadUtils.execute(new C05361());
        }
    }

    private void dismissDialog() {
        LogUtils.m1568d("EPG/utils/FeedBackController", "dismissDialog, mDialog=" + this.mDialog);
    ...
    public void clearCurrentDialog() {
        LogUtils.m1568d("EPG/utils/FeedBackController", "clearCurrentDialog: mDialog=" + this.mDialog);
        if (this.mDialog != null) {
    ...
    private void startUpload(boolean isErrorType) {
        if (GetInterfaceTools.getILogRecordProvider().isLogRecordFeatureReady()) {
            ...
            LogUtils.m1568d("EPG/utils/FeedBackController", "mFeedBackModel.isNeedLogcat() = " + this.mFeedBackModel.isNeedLogcat());
            ...
            if (isErrorType) {
                ...
                NetDiagnoseUploader.getInstance().doNetDiagnoseToAutoTracker(provider, new INetDiagnoseResultListener() {
                    public void onReslut(String info) {
                        LogUtils.m1568d("EPG/utils/FeedBackController", ">>>>> net diagnose successs, start upload to Tracker");
            ...
            GetInterfaceTools.getILogRecordProvider().getUploadCore().sendRecorder(extraInfo, option, recorder, new C05394());
            return;
        }
    ...
    private void sendAutoTracker(UploadExtraInfo extraInfo, UploadOption option, Recorder errorRecorder, final String finalEventId) {
        GetInterfaceTools.getILogRecordProvider().getUploadCore().sendRecorder(extraInfo, option, errorRecorder, new IFeedbackResultListener() {
            ...
            public void sendReportSuccess(String feedbackId, String ip) {
                LogUtils.m1570d("EPG/utils/FeedBackController", ">>>>> logrecord 'error_type' send pingback, eventid=", finalEventId);
                PingBackParams params = new PingBackParams();
                params.add(Keys.f2035T, "11").add("st", "0").add("ct", "150721_feedback").add("ec", "").add("pfec", "").add("e", finalEventId).add(Keys.FEEDBACK_ID, feedbackId).add(Keys.FBTYPE, MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR);
                PingBack.getInstance().postPingBackToLongYuan(params.build());
    ...
    private void prepare() {
        ...
        ThreadUtils.execute(new C05416());
        try {
    ...
    private void feedBack(boolean needPrepare) {
        ...
        this.mDialog = Project.getInstance().getControl().getGlobalDialog(this.mContext);
        String cancelFeedback = this.mContext.getString(C0508R.string.global_feedback_cancel);
        this.mDialog.setParams(this.mContext.getString(C0508R.string.global_feedback_uploading), cancelFeedback, this.mCancelUploadListener);
        this.mDialog.setOnDismissListener(this.mUploadingDismissListener);
        this.mDialog.show();
        if (needPrepare) {
            ThreadUtils.execute(new C05427());
        } else {
            startUpload(false);
        }
    }
    ...
    private void feedBackFail(final String error) {
        if (this.mDialog == null || !this.mDialog.isShowing()) {
            LogUtils.m1568d("EPG/utils/FeedBackController", "feedBackFail()----the uploading dialog is not show, so break!");
            return;
        }
            ...
            public void run() {
                try {
                    if (((Activity) FeedBackController.this.mContext).isFinishing()) {
                        LogUtils.m1571e("EPG/utils/FeedBackController", "EPG/utils/FeedBackController--->>feedBackFail()----activity is finish");
                        return;
                    }
                } catch (Exception e) {
                }
                FeedBackController.this.dismissDialog();
                Bitmap qRBitmap = BitmapFactory.decodeResource(FeedBackController.this.mContext.getResources(), C0508R.drawable.share_btn_transparent);
                String message = FeedBackController.this.mContext.getString(C0508R.string.logrecordFailed_with_qr);
                String rptBtnText = FeedBackController.this.mContext.getString(C0508R.string.logrecordRetry);
                String cancelBtnText = FeedBackController.this.mContext.getString(C0508R.string.Cancel);
                ...
                ((GlobalQRFeedBackDialog) FeedBackController.this.mDialog).setParams(stringModel, qRBitmap, rptBtnText, FeedBackController.this.onRetryBtnListener, cancelBtnText, new C05431());
                FeedBackController.this.mIsRestryClick = false;
    ...
    private void feedBackSuccess(final String feedbackId, final String ip) {
        if (this.mDialog == null || !this.mDialog.isShowing()) {
            LogUtils.m1568d("EPG/utils/FeedBackController", "feedBackSuccess()----the uploading dialog is not show, so break!");
            return;
        }
        this.mIsUploadCancel = false;
        this.mhandler.post(new Runnable() {
            public void run() {
                try {
                    if (((Activity) FeedBackController.this.mContext).isFinishing()) {
                        LogUtils.m1571e("EPG/utils/FeedBackController", "EPG/utils/FeedBackController--->>feedBackSuccess()----activity is finish");
                        return;
                    }
                } catch (Exception e) {
                }
                ...
                String rightTopMessage = FeedBackController.this.mContext.getString(C0508R.string.logrecordSuccess_with_qr_right_top);
                String rightBottomMessage = FeedBackController.this.mContext.getString(C0508R.string.logrecordSuccess_with_qr_right_bottom, new Object[]{mIp, DeviceUtils.getMacAddr(), LogRecordUtils.getVersionCode(), time});
                String leftBottomMessage = FeedBackController.this.mContext.getString(C0508R.string.logrecordSuccess_with_qr_left_bottom, new Object[]{feedbackId});
                StringModel stringModel = new StringModel();
    ...
    private void sendShowPingBack() {
        PingBackParams params = new PingBackParams();
        params.add("qtcurl", "failfb_dlg").add(Keys.f2035T, "21").add("block", SourceType.feedback.toString()).add("bstp", "1");
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
