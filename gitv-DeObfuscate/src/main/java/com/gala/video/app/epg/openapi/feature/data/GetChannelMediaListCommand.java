...
public class GetChannelMediaListCommand extends MaxCommand<List<Media>> {
    ...
    private class MyListener extends ResultListHolder<Media> implements IApiCallback<ApiResultAlbumList> {
        ...
        public void onException(ApiException exception) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyListener.onException(" + exception + ")");
            }
        ...
        public void onSuccess(ApiResultAlbumList albumList) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyListener.onSuccess(" + albumList + ")");
            }
            ...
            if (albumList != null && albumList.data != null) {
                for (Album info : albumList.data) {
                    ...
                    if (media != null) {
                        ...
                        LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyListener.onSuccess() cannot translate to sdk media!!!");
                    }
                    if (isReachMax()) {
                        LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyListener.onSuccess() reach max size !!!" + this);
                        return;
                    }
                }
            }
    ...
    private class MyQipuListener extends ResultListHolder<Media> implements IVrsCallback<ApiResultPlayListQipu> {
        ...
        public void onException(ApiException exception) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyQipuListener.onException(" + exception + ")");
            }
            setNetworkValid(!OpenApiNetwork.isNetworkInvalid(exception));
            setCode(7);
        }
        ...
        public void onSuccess(ApiResultPlayListQipu albumList) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyQipuListener.onSuccess(" + albumList + ")");
            }
            setNetworkValid(true);
            if (albumList == null) {
                return;
            ...
            if (ListUtils.isEmpty(albumList.getAlbumList())) {
                LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyQipuListener.onSuccess() Didn't get data from server.");
                return;
            }
            for (Album info : albumList.getAlbumList()) {
                ...
                if (media != null) {
                    ...
                    LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyQipuListener.onSuccess() cannot translate to sdk media!!!");
                }
                if (isReachMax()) {
                    LogUtils.m1577w(GetChannelMediaListCommand.TAG, "MyQipuListener.onSuccess() reach max size !!!" + this);
                    return;
                }
            }
        }
    ...
    private Bundle getPlaylistMedia(Bundle params, LocalChannel channel, int channelType, int maxCount) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getPlaylistMedia() maxCount=" + maxCount + ", qipu=" + qipu + ", channelType=" + channelType);
        }
        return result;
    }
    ...
    private Bundle getChannelMedia(Bundle params, LocalChannel channel, int channelType, int maxCount) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getChannelMedia() pageNo=" + pageNo + ", pageSize=" + pageSize + ", maxCount=" + maxCount + ", chnId=" + chnId + ", virtualChnId=" + virtualChnId + ", channelType=" + channelType);
        }
        return result;
    }
}
