    ...
    public Bundle onProcess(Bundle params) {
        ...
        if (activateCode == null || activateCode.equals("")) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "process() OpenActivatePageCommand activateCode is null!!!");
            }
            return OpenApiResultCreater.createResultBundle(6);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "process() OpenActivatePageCommand activateCode is :" + activateCode);
        }
