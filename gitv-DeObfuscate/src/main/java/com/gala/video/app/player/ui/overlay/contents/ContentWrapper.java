...
public class ContentWrapper<T, E> extends FrameLayout implements IContent<T, E> {
    ...
    public T getContentData() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "getContentData, unhandled content type, mContent=" + this.mContent);
        }
    ...
    private void addTitleView() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> addTitleView");
        }
        this.mTitleRoot = LayoutInflater.from(this.mContext).inflate(C1291R.layout.player_layout_detail_title_content, this, true);
        this.mTitleText = (TextView) this.mTitleRoot.findViewById(C1291R.id.detail_title_content_title);
    }

    private void addContentView() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> addContentView");
        }
        if (this.mContent != null) {
            ...
            LogUtils.m1568d(this.TAG, "addContentView, mContent is null !!!");
        }
    }

    private void setupViews() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> setupViews");
        }
    ...
    private void setupContentTitleParams() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> setupContentTitleParams");
        }
        LayoutParams titleTextLP = (LayoutParams) this.mTitleText.getLayoutParams();
        if (titleTextLP == null) {
            titleTextLP = new LayoutParams(-2, -2);
        }
        titleTextLP.leftMargin = ResourceUtil.getDimen(C1291R.dimen.dimen_55dp);
        titleTextLP.topMargin = ResourceUtil.getDimen(C1291R.dimen.dimen_12dp);
        this.mTitleText.setLayoutParams(titleTextLP);
        this.mTitleText.setTextSize(0, (float) ResourceUtil.getDimensionPixelSize(C1291R.dimen.dimen_25dp));
        this.mTitleText.setTextColor(ResourceUtil.getColor(C1291R.color.detail_title_text_color_new));
    }
    ...
    public void setData(T data) {
        ...
        if (this.mContent != null) {
            if (this.mContent instanceof EpisodeListContent) {
                ...
                LogUtils.m1568d(this.TAG, "setData, unhandled content type, content=" + this.mContent);
            }
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setData, mContent is null !!!");
        }
    }

    public void setSelection(E item) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> setSelection, item");
        }
        if (this.mContent != null) {
            if (this.mContent instanceof EpisodeListContent) {
                ...
                LogUtils.m1568d(this.TAG, "setSelection, unhandled content type, content=" + this.mContent);
            }
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setSelection, mContent is null !!!");
        }
    }
    ...
    public void setItemListener(IItemListener<E> listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> setItemListener, listener=" + listener);
        }
        if (this.mContent != null) {
            if (this.mContent instanceof EpisodeListContent) {
                ...
                LogUtils.m1568d(this.TAG, "setItemListener, unhandled content type, content=" + this.mContent);
            }
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setItemListener, mContent is null !!!");
        }
    }

    public void setOnHorizontalScrollListener(OnHorizontalScrollListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> setOnHorizontalScrollListener, listener=" + listener);
        }
    ...
    public void setUpdateInfo(String info) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> setUpdateInfo:" + info);
        }
        this.mInfo = info;
