...
public class TimedSeekBar extends RelativeLayout implements IThreeDimensional, ISeekOverlay {
    ...
    private final Runnable mCheckLayoutRunnable = new C15623();
    private Context mContext;
    private Handler mHandler = new C15601();
    ...
    private final OnSeekBarChangeListener mSeekBarChangeListener = new C15612();
            ...
            switch (msg.what) {
                case 1:
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(TimedSeekBar.TAG, "handleMessage(" + msg + ") mIsSeeking=" + TimedSeekBar.this.mIsSeeking);
                    }
                ...
                case 2:
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(TimedSeekBar.TAG, "handleMessage(" + msg + ") mIsSeeking=" + TimedSeekBar.this.mIsSeeking + ", getProgress= " + TimedSeekBar.this.mSeekBar.getProgress() + ",changeprogess= " + changeprogess);
                    }
                    TimedSeekBar.this.mIsSeeking = false;
                    return;
                default:
    ...
    class C15612 implements OnSeekBarChangeListener {
        boolean mIsFromUser;
        ...
        public void onStopTrackingTouch(ISeekBar seekBar) {
            int seekTo;
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TimedSeekBar.TAG, "onStopTrackingTouch(" + this.mIsFromUser + ")");
            }
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TimedSeekBar.TAG, "onStopTrackingTouch() progress=" + progress + ", seekTo=" + seekTo);
            }
        ...
        public void onStartTrackingTouch(ISeekBar seekBar) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TimedSeekBar.TAG, "onStartTrackingTouch(" + seekBar + ")");
            }
        ...
        public void onProgressChanged(ISeekBar seekBar, int progress, boolean fromUser) {
            int seekTo;
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TimedSeekBar.TAG, "onProgressChanged(" + progress + ", " + fromUser + ")");
            }
            this.mIsFromUser = fromUser;
            TimedSeekBar.this.mIsDragging = true;
            TimedSeekBar.this.mIsTouchMove = true;
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TimedSeekBar.TAG, "onProgressChanged(" + TimedSeekBar.this.mIsDragging + ")");
            }
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TimedSeekBar.TAG, "onProgressChanged(" + this.mIsFromUser + ")");
            }
            progress *= 1000;
            if (progress <= TimedSeekBar.this.mMaxSeekableProgress || progress > TimedSeekBar.this.mMaxProgress) {
                seekTo = TimedSeekBar.this.mMaxProgress > progress ? progress : TimedSeekBar.this.mMaxProgress;
    ...
    class C15623 implements Runnable {
        C15623() {
        }
    ...
    private void initView() {
        ...
        this.mSeekBarLayout = (FrameLayout) findViewById(C1291R.id.layout_seekbar);
        this.mSeekBar = (ISeekBar) findViewById(C1291R.id.play_seekbar);
        this.mSeekBar.setOnSeekBarChangeListener(this.mSeekBarChangeListener);
        this.mTextVideoTime = (EnhancedTextView) findViewById(C1291R.id.play_text_video_time);
        this.mTxtTotalTime = (EnhancedTextView) findViewById(C1291R.id.play_text_total_time);
        this.mHeaderView = (ImageView) findViewById(C1291R.id.view_header);
        this.mTailerView = (ImageView) findViewById(C1291R.id.view_tail);
        this.mPauseButton = (ImageView) findViewById(C1291R.id.img_pause_button);
        this.mTimedLeftMargin = this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.player_seekbar_time_left_margin);
        this.mSeekbarWidth = this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.player_seekbar_width);
        this.mSeekbarLeftMargin = this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.player_seekbar_left_margin);
        this.mPauseWidth = this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.player_button_width);
        setVisibility(8);
    }

    public void startTipMode() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "startTipMode() mIsTipMode=" + this.mIsTipMode);
        }
    ...
    public void stopTipMode() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "stopTipMode() mIsTipMode=" + this.mIsTipMode);
        }
    ...
    public boolean isTipMode() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isTipMode() return " + this.mIsTipMode);
        }
    ...
    protected void setTailerProgress(int tailerProgress) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setTailerProgress(" + tailerProgress + ")");
        }
        this.mTailerProgress = tailerProgress;
        int seekBarWidth = this.mSeekBarLayout.getWidth();
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "seerBarWidth=" + seekBarWidth);
        }
        ...
        float x = ((((float) this.mTailerProgress) / ((float) this.mMaxProgress)) * ((float) seekBarWidth)) - ((float) (ResourceUtil.getDimensionPixelSize(C1291R.dimen.dimen_3dp) / 2));
        params.leftMargin = ((int) x) + 1;
        float maxLeftMargin = (float) (seekBarWidth - ResourceUtil.getDimensionPixelSize(C1291R.dimen.dimen_3dp));
        if (x > maxLeftMargin) {
            f = maxLeftMargin;
        } else {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setTailerProgress() x=" + x + ", maxLeftMargin=" + maxLeftMargin + ", width=" + getWidth() + ", leftMargin=" + params.leftMargin);
        }
    ...
    protected void setHeaderProgress(int headerProgress) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setHeaderProgress(" + headerProgress + ") header width=" + this.mHeaderView.getWidth());
        }
        int seekBarWidth = this.mSeekBarLayout.getWidth();
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "seerBarWidth=" + seekBarWidth);
        ...
        params.leftMargin = (int) (((((float) headerProgress) / ((float) this.mMaxProgress)) * ((float) seekBarWidth)) - ((float) (ResourceUtil.getDimensionPixelSize(C1291R.dimen.dimen_3dp) / 2)));
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setHeaderProgress() leftMargin=" + params.leftMargin);
        }
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onVisibilityChanged = " + visibility);
        }
        if (visibility == 0) {
    ...
    public void setSecondaryProgress(int secondaryProgress) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setSecondaryProgress(" + secondaryProgress + ")");
        }
    ...
    public void hide() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "amazion hide()");
        }
        setVisibility(8);
    ...
    public boolean isDragging() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isDragging() returns " + this.mIsDragging);
        }
    ...
    private void checkHeadAndTailLayout() {
        ...
        if (relayout) {
            LogUtils.m1577w(TAG, "checkHeadAndTailLayout() Why not layout children??????");
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onSizeChanged(" + w + ", " + h + ", " + oldw + ", " + oldh + ")");
        }
        checkHeadAndTailLayout();
    }
    ...
    public void onSeekBegin(View view, int i) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onSeekBegin(" + i + ")");
        }
        this.mTextVideoTime.setTextColor(this.mContext.getResources().getColor(C1291R.color.highlight_time));
        this.mTextVideoTime.setTextSize(0, (float) this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_30dp));
        this.mTxtTotalTime.setTextSize(0, (float) this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_19dp));
    }
    ...
    public void onSeekEnd(View view, int i) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onSeekEnd(" + i + ")");
        }
        this.mTextVideoTime.setTextColor(this.mContext.getResources().getColor(C1291R.color.normal_time));
        this.mTextVideoTime.setTextSize(0, (float) this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_21dp));
        this.mTxtTotalTime.setTextSize(0, (float) this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_21dp));
    }
}
