...
public class NetDiagnoseActivity extends QMultiScreenActivity {
    ...
    private INDDoneListener mCdnListener = new C09292();
    private OnClickListener mClickListener = new C09369();
    private INDDoneListener mCollectListener = new C09303();
    private Context mContext;
    private INetDiagnoseController mController;
    private int mCurrentNetDiagnoseType = 0;
    private INDDoneListener mDnsListener = new C09336();
    private boolean mIsSendedFeedbackSuccess;
    private boolean mIsSendingFeedback;
    private INDDoneListener mNetConnListener = new C09281();
    private INDDoneListener mNsLookUp = new C09347();
        ...
        public void onFocusChange(View v, boolean hasFocus) {
            if (v instanceof LinearLayout) {
                ...
                if (txtView != null && (txtView instanceof TextView)) {
                    if (hasFocus) {
                        ((TextView) txtView).setTextColor(NetDiagnoseActivity.this.getResources().getColor(C0508R.color.gala_write));
                    } else {
                        ((TextView) txtView).setTextColor(NetDiagnoseActivity.this.getResources().getColor(C0508R.color.setting_white));
                    }
                }
            }
        }
    ...
    private INDDoneListener mThirdSpeedListener = new C09314();
    private INDDoneListener mTraceRouteListener = new C09325();
    private INDUploadCallback mUploadcallback = new C09358();
    private NetTestingView mViewTesting;
    private List<INDWrapperOperate> mWrapperList = null;

    class C09281 implements INDDoneListener {
        C09281() {
        }
    ...
    class C09292 implements INDDoneListener {
        C09292() {
        }
        ...
        public void onFinish(Map<String, Object> resultMap) {
            ...
            if (((Boolean) resultMap.get("success")).booleanValue()) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(NetDiagnoseActivity.this.TAG, "onCdnDiagnoseSuccess( AvgSpeed:" + result.getCdnDiagnoseAvgSpeed() + ")");
                }
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDiagnoseActivity.this.TAG, "onCdnDiagnoseFailed(" + result + ")");
            }
            NetDiagnoseActivity.this.updateUI(DiagnoseStatus.CDN_DISCONNECTED);
        }
    }

    class C09303 implements INDDoneListener {
        C09303() {
        }
    ...
    class C09314 implements INDDoneListener {
        C09314() {
        }
    ...
    class C09325 implements INDDoneListener {
        C09325() {
        }
        ...
        public void onFinish(Map<String, Object> resultMap) {
            NetDiagnoseInfo result = (NetDiagnoseInfo) resultMap.get("data");
            NetDiagnoseActivity.this.setReslut(result);
            LogUtils.m1568d(NetDiagnoseActivity.this.TAG, "mTraceRouteListener: " + result.getDnsResult());
            NetDiagnoseActivity.this.updateUI(DiagnoseStatus.TRACEROUT_DONE);
        }
    }

    class C09336 implements INDDoneListener {
        C09336() {
        }
    ...
    class C09347 implements INDDoneListener {
        C09347() {
        }
    ...
    class C09358 implements INDUploadCallback {
        C09358() {
        }

        public void uploadNetDiagnoseDone() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDiagnoseActivity.this.TAG, "uploadNetDiagnoseDone ");
            }
            NetDiagnoseActivity.this.mController.stopCheck();
        }
    }

    class C09369 implements OnClickListener {
        C09369() {
        }

        public void onClick(View v) {
            int i = v.getId();
            if (i == C0508R.id.epg_btn_dianose_cancel) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(NetDiagnoseActivity.this.TAG, "finish");
                }
                NetDiagnoseActivity.this.finish();
            } else if (i == C0508R.id.epg_btn_dianose_retry) {
                NetDiagnoseActivity.this.mIsSendedFeedbackSuccess = false;
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(NetDiagnoseActivity.this.TAG, "btn_dianose_retry ");
                }
                NetDiagnoseActivity.this.mController.stopCheck();
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(NetDiagnoseActivity.this.TAG, "retry");
                }
            ...
            } else if (i == C0508R.id.epg_btn_dianose_report) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(NetDiagnoseActivity.this.TAG, "report");
                }
                if (NetDiagnoseActivity.this.mIsSendingFeedback) {
                    QToast.makeTextAndShow(NetDiagnoseActivity.this.mContext, C0508R.string.feedback_sending, 2000);
                } else if (NetDiagnoseActivity.this.mIsSendedFeedbackSuccess) {
                    QToast.makeTextAndShow(NetDiagnoseActivity.this.mContext, C0508R.string.feedback_retry, 2000);
                } else {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "onCreate ");
        }
        setContentView(C0508R.layout.epg_activity_net_diagnose);
        this.mContext = this;
        initViews();
    }
    ...
    private void initViews() {
        ...
        this.mViewTesting = (NetTestingView) findViewById(C0508R.id.epg_layout_testing_id);
        this.mViewTesting.setVisibility(0);
        this.mViewTesting.setNeedShowNetSpeedResult(this.mCurrentNetDiagnoseType == 1);
        this.mBtnCancel = this.mViewTesting.findViewById(C0508R.id.epg_btn_dianose_cancel);
        ...
        this.mBtnRetry = this.mViewTesting.findViewById(C0508R.id.epg_btn_dianose_retry);
        ...
        this.mBtnReport = this.mViewTesting.findViewById(C0508R.id.epg_btn_dianose_report);
        ...
        this.mTextProgress = (TextView) this.mViewTesting.findViewById(C0508R.id.epg_net_test_progressbar_text);
        this.mProgressBar = (ImageView) this.mViewTesting.findViewById(C0508R.id.epg_net_test_progressbar);
        initAnimation();
    }

    protected void onResume() {
        LogUtils.m1568d(this.TAG, "onResume start");
        super.onResume();
        ...
        LogUtils.m1568d(this.TAG, "onResume end");
    }
    ...
    protected View getBackgroundContainer() {
        return findViewById(C0508R.id.net_diagnose_layout);
    }

    private void switchView(DiagnoseStatus status) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "switchView(" + status + ")");
        }
        switch (status) {
            case TESTING_BEGIN:
                this.mTextProgress.setText(getResources().getText(C0508R.string.net_diagnosis_route));
            ...
            case ROUTER_CONNECTED:
                this.mTextProgress.setText(getResources().getText(C0508R.string.net_diagnosis_net));
                return;
            case ROUTER_DISCONNECTED:
                this.mTextProgress.setText(getResources().getText(C0508R.string.result_no_internet));
            ...
            case INTERNET_CONNECTED:
                this.mTextProgress.setText(getResources().getText(C0508R.string.net_diagnosis_cdn));
                return;
            case INTERNET_DISCONNECTED:
                ...
                return;
            case CDN_CONNECTED:
                this.mTextProgress.setText(getResources().getText(C0508R.string.net_diagnosis_third_speed_test));
                return;
            case CDN_DISCONNECTED:
                this.mTextProgress.setText(getResources().getText(C0508R.string.net_diagnosis_third_speed_test));
                return;
            case SPEED_TEST_DONE:
                this.mTextProgress.setText(getResources().getText(C0508R.string.net_diagnosis_traceroute_test));
                return;
            case TRACEROUT_DONE:
                this.mTextProgress.setText(getResources().getText(C0508R.string.net_diagnosis_dns_test));
                return;
            case DNS_DONE:
                this.mTextProgress.setText(getResources().getText(C0508R.string.net_diagnosis_collection_test));
                return;
    ...
    private void updateUI(final DiagnoseStatus status) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "updateUI, status=" + status);
        }
    ...
    private Animation getProgressBarAnimation() {
        Animation animation = AnimationUtils.loadAnimation(this.mContext, C0508R.anim.epg_litchi_anima_load_item);
    ...
    private void uploadResult() {
        if (this.mResult != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "uploadResult jsonResult size=" + (cdnResult != null ? Integer.valueOf(cdnResult.length()) : "NULL"));
            }
            ...
            LogUtils.m1571e(this.TAG, "uploadResult mResult is null!");
        }
    }
}
