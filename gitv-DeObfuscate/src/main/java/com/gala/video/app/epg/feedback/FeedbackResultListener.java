...
public class FeedbackResultListener extends Wrapper implements IFeedbackResultListener {
    ...
    private OnClickListener onRetryBtnListener = new C05546();
    ...
    class C05546 implements OnClickListener {
        C05546() {
        }
    ...
    private FeedbackData createFeedbackModel(boolean isAdsInfo) {
        ...
        if (isAdsInfo) {
            ...
            LogUtils.m1570d(TAG, ">>>>> Ads LogC content - ", GalaPlayerLogProvider.getCupIdLog());
            LogUtils.m1570d(TAG, ">>>>> Ads LogJ content - ", AdsClient.getFeedbackLog());
    ...
    public void beginsendLog() {
        this.mHandler.post(new C05471());
    }

    public void lastsendNotComplete() {
        this.mHandler.post(new C05482());
    }
    ...
    public void sendReportSuccess(final String feedbackId, final String ip) {
        if (LogRecordDebugUtils.testLogRecordExceptionForF00001()) {
            ...
            if (!this.mIsNormalReport) {
                LogUtils.m1570d(TAG, ">>>>> logrecord pingback --- type = ", this.mRecorderType.toString());
            ...
            LogUtils.m1568d(TAG, ">>>>> logrecord pingback --- mIsNormalReport");
                    ...
                    public void run() {
                        try {
                            if (((Activity) FeedbackResultListener.this.mContext).isFinishing()) {
                                LogUtils.m1571e(FeedbackResultListener.TAG, "FeedbackResultListener--->>feedBackSuccess()----activity is finish");
                                return;
                            }
                        } catch (Exception e) {
                        }
                        Bitmap qRBitmap = BitmapFactory.decodeResource(FeedbackResultListener.this.mContext.getResources(), C0508R.drawable.share_btn_transparent);
                        String mIp = ip;
                        ...
                        String rightTopMessage = FeedbackResultListener.this.mContext.getString(C0508R.string.logrecordSuccess_with_qr_right_top);
                        String rightBottomMessage = FeedbackResultListener.this.mContext.getString(C0508R.string.logrecordSuccess_with_qr_right_bottom, new Object[]{mIp, DeviceUtils.getMacAddr(), LogRecordUtils.getVersionCode(), mTime});
                        String leftBottomMessage = FeedbackResultListener.this.mContext.getString(C0508R.string.logrecordSuccess_with_qr_left_bottom, new Object[]{feedbackId});
                        ...
                        switch (FeedbackResultListener.this.mPageType) {
                            case 0:
                            case 1:
                                LogUtils.m1568d(FeedbackResultListener.TAG, ">>>>> feedback success, no need net diagnose");
                                return;
                            case 3:
                                LogUtils.m1568d(FeedbackResultListener.TAG, ">>>>> feedback success [FEEDBACK_KEY], start net diagnose");
                                NetDiagnoseUploader.getInstance().doTotalNetDiagnose();
                                return;
                            case 4:
                                LogUtils.m1568d(FeedbackResultListener.TAG, ">>>>> feedback success [FEEDBACK_ACTIVITY], start net diagnose");
                                NetDiagnoseUploader.getInstance().doTotalNetDiagnose();
                                return;
                            case 5:
                                LogUtils.m1568d(FeedbackResultListener.TAG, ">>>>> feedback success [GLOABAL_FEEDBACK_PANEL], start net diagnose");
                                ...
                                if (FeedbackResultListener.this.mApiException != null) {
                                    ...
                                    LogUtils.m1568d(FeedbackResultListener.TAG, ">>>>> ApiException is null, apiCode & httpcode & url is null!!!");
                                }
        ...
        if (this.mRecorderType == RecorderType._FEEDBACK) {
            this.mHandler.post(new Runnable() {
                ...
                public void run() {
                    try {
                        if (((Activity) FeedbackResultListener.this.mContext).isFinishing()) {
                            LogUtils.m1571e(FeedbackResultListener.TAG, "FeedbackResultListener--->>feedBackFail()----activity is finish");
                            return;
                        }
                    } catch (Exception e) {
                    }
                    Bitmap qRBitmap = BitmapFactory.decodeResource(FeedbackResultListener.this.mContext.getResources(), C0508R.drawable.share_btn_transparent);
                    String message = FeedbackResultListener.this.mContext.getString(C0508R.string.logrecordFailed_with_qr);
                    String rptBtnText = FeedbackResultListener.this.mContext.getString(C0508R.string.logrecordRetry);
                    String cancelBtnText = FeedbackResultListener.this.mContext.getString(C0508R.string.Cancel);
                    FeedbackResultListener.this.mDialog = EpgAppConfig.getGlobalQRDialog(FeedbackResultListener.this.mContext);
                    ...
                    ((GlobalQRFeedBackDialog) FeedbackResultListener.this.mDialog).setParams(stringModel, qRBitmap, rptBtnText, FeedbackResultListener.this.onRetryBtnListener, cancelBtnText, new C05501());
                    FeedbackResultListener.this.mDialog.show();
                    FeedbackResultListener.this.sendShowPingBack(FeedbackResultListener.this.mSourceType.toString());
                    if (FeedbackResultListener.this.mUploadResultControllerListener != null) {
    ...
    private void sendUploadFeedbackPingback(String st, String ec, String pfec, String e, String feedbackid, String fbtype) {
        ...
        params.add(Keys.f2035T, "11").add("st", st).add("ct", "150721_feedback").add("ec", ec).add("pfec", pfec).add("e", e).add(Keys.FEEDBACK_ID, feedbackid).add(Keys.FBTYPE, fbtype);
    ...
    private String getFailedString(String code) {
        ...
        if (code != null) {
            ...
            if (code.equals("-100")) {
                sb.append(this.mContext.getString(C0508R.string.logrecordFailed_not_connect_server));
            } else if (code.equals(LogRecordUtils.EXCEPTION_F10000)) {
                sb.append(this.mContext.getString(C0508R.string.logrecordfailed_internal_error_code));
            } else if (code.equals(LogRecordUtils.EXCEPTION_F00001)) {
                sb.append(this.mContext.getString(C0508R.string.logrecordfailed_null_request_body_code));
            } else if (code.equals(LogRecordUtils.EXCEPTION_F00002)) {
                sb.append(this.mContext.getString(C0508R.string.logrecordfailed_ungzip_error_code));
            } else if (code.equals(LogRecordUtils.EXCEPTION_F00003)) {
                sb.append(this.mContext.getString(C0508R.string.logrecordfailed_json_invalid_code));
            } else if (code.equals("-300")) {
                sb.append(this.mContext.getString(C0508R.string.logrecordfailed_inter_know));
            } else {
                sb.append(this.mContext.getString(C0508R.string.logrecordfailed_unknow));
            }
            sb.append("\n");
        }
    ...
    private void dismissDialog() {
        LogUtils.m1568d(TAG, "dismissDialog, mDialog=" + this.mDialog);
    ...
    private void sendShowPingBack(String block) {
        ...
        PingBackParams params = new PingBackParams();
        params.add("qtcurl", "failfb_dlg").add(Keys.f2035T, "21").add("block", block).add("bstp", "1");
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
