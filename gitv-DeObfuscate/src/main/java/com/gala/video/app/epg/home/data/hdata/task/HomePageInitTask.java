...
public class HomePageInitTask extends BaseRequestTask {
    ...
    class C06441 implements MyObserver {
        C06441() {
        }
    ...
    public HomePageInitTask() {
        this.mLoadPage = -1;
        this.DeviceResultObserver = new C06441();
    ...
    public void invoke() {
        ...
        if (!UikitDataCacheSecurity.getInstance().isCacheSecurity()) {
            ...
            LogUtils.m1568d(TAG, "HomePageInitTask- read tab info: size " + tabList.size());
            UikitDataCacheSecurity.getInstance().setSecurity(false);
            TabModel targetPageModel = calcPriorityLoadPage(tabList);
            LogUtils.m1568d(TAG, "read and parse page data start target model@" + targetPageModel);
    ...
    public void onOneTaskFinished() {
        LogUtils.m1568d(TAG, "perform HomePageInitTask finished");
