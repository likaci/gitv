...
public class UiUtils {
    ...
    static class C15811 implements Runnable {
        C15811() {
        }
        ...
        public void run() {
            ...
            if (!StringUtils.isEmpty(path)) {
                Bitmap bitmap = UiUtils.createTargetBitmap(path, ResourceUtil.getDimen(C1291R.dimen.dimen_600dp), ResourceUtil.getDimen(C1291R.dimen.dimen_337dp), ScaleType.CENTER_INSIDE, Config.ARGB_8888);
    ...
    public static Rect getBgDrawablePaddings(Context context, int itemBgResId) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getBgDrawablePaddings: " + bgDrawablePaddings);
        }
    ...
    public static PlayerErrorPanelUIConfig getPlayerErrorPanelUIConfigForVIP() {
        ...
        config.setBackgroundScaleType(ImageView.ScaleType.FIT_XY).setMainTextColor(ResourceUtil.getColor(C1291R.color.player_ui_vip_error_tip_main_start), ResourceUtil.getColor(C1291R.color.player_ui_vip_error_tip_main_end)).setMainTextSize(ResourceUtil.getDimensionPixelSize(C1291R.dimen.dimen_24dp)).setMainTextGravity(1).setMainTextTopMargin(ResourceUtil.getDimen(C1291R.dimen.player_error_main_text_topmargin_for_vip));
        return config;
    }

    public static PlayerErrorPanelUIConfig getPlayerErrorPanelUIConfigForCommon() {
        PlayerErrorPanelUIConfig config = new PlayerErrorPanelUIConfig();
        config.setBackgroundScaleType(ImageView.ScaleType.FIT_XY).setMainTextColor(ResourceUtil.getColor(C1291R.color.player_ui_common_error_tip_main), ResourceUtil.getColor(C1291R.color.player_ui_common_error_tip_main)).setMainTextSize(ResourceUtil.getDimensionPixelSize(C1291R.dimen.dimen_20dp)).setCornerTextColor(ResourceUtil.getColor(C1291R.color.player_ui_common_error_tip_corner), ResourceUtil.getColor(C1291R.color.player_ui_common_error_tip_corner)).setCornerTextSize(ResourceUtil.getDimensionPixelSize(C1291R.dimen.dimen_17dp));
        return config;
    }

    public static PlayerErrorPanelUIConfig getPlayerErrorPanelUIConfigForCarousel() {
        PlayerErrorPanelUIConfig config = new PlayerErrorPanelUIConfig();
        config.setBackgroundScaleType(ImageView.ScaleType.FIT_XY);
        return config;
    }

    public static PlayerErrorPanelInfo getPlayerErrorPanelInfoForVIP() {
        PlayerErrorPanelInfo info = new PlayerErrorPanelInfo();
        info.setBackgroundDrawable(getPlayerErrorBGForVIP()).setMainText(ResourceUtil.getStr(C1291R.string.player_vip_error_main_text));
        return info;
    }

    public static PlayerErrorPanelInfo getPlayerErrorPanelInfoForCommon(String errorMessage) {
        PlayerErrorPanelInfo info = new PlayerErrorPanelInfo();
        info.setBackgroundDrawable(LoadingViewAnimManager.getInstance(ResourceUtil.getContext()).getBackground()).setMainText(errorMessage).setCornerText(ResourceUtil.getStr(C1291R.string.player_common_error_tip_text));
        return info;
    }

    public static PlayerErrorPanelInfo getPlayerErrorPanelInfoForCarousel() {
        PlayerErrorPanelInfo info = new PlayerErrorPanelInfo();
        info.setBackgroundDrawable(ResourceUtil.getDrawable(C1291R.drawable.player_bg_fullscreen_live_nosignal));
        return info;
    }
    ...
    private static Drawable getPlayerErrorBGForVIP() {
        ...
        ThreadUtils.execute(new C15811());
        return drawable;
    }
    ...
    private static Bitmap createTargetBitmap(String path, int targetWidth, int targetHeight, ScaleType type, Config config) {
        int i = 1;
        try {
