...
public class StartScreenImageAd implements IStartScreenAd {
    ...
    class C05641 implements IFileCallback {
        ...
        public void onSuccess(FileRequest fileRequest, String s) {
            ...
            LogUtils.m1568d(StartScreenImageAd.TAG, "request advertisement data and image success ,time cost : " + elapse);
            Bitmap bitmap = BitmapFactory.decodeFile(s);
            if (elapse >= 2000 || bitmap == null) {
                LogUtils.m1568d(StartScreenImageAd.TAG, "it is timeout when downloading image.");
    ...
    class C05652 implements Runnable {
        ...
        public void run() {
            if (StartScreenImageAd.this.mCountDownTime > 0) {
                LogUtils.m1568d(StartScreenImageAd.TAG, StartScreenImageAd.this.mCountDownTime + SOAP.DELIM + System.currentTimeMillis());
            ...
            LogUtils.m1568d(StartScreenImageAd.TAG, "start image screen is finished");
    ...
    private void sendPingback(long interval) {
        ...
        HomePingbackFactory.instance().createPingback(CommonPingback.AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_startapk").addItem("st", st).addItem("td", String.valueOf(interval)).addItem("r", DBColumns.PIC).addItem(Keys.f2035T, "11").addItem("ct", "150619_request").setOthersNull().post();
    }
    ...
    public void showAd(ViewGroup container) {
        ...
        ImageView imgView = (ImageView) container.findViewById(C0508R.id.epg_screen_ad);
        TextView textView = (TextView) container.findViewById(C0508R.id.epg_tv_adtime);
        TextView tipsTxtView = (TextView) container.findViewById(C0508R.id.epg_start_ad_jump_tip);
        ...
        TextView adBadgeTxtView = (TextView) container.findViewById(C0508R.id.epg_tv_ad_badge);
    ...
    private StartAdModel parseAdDataToStartAdModel(CupidAd cupidAd) {
        if (cupidAd == null) {
            LogUtils.m1577w(TAG, "parseAdDataToStartAdModel, cupidAd data is null");
            return null;
        }
    ...
    private void loadImage() {
        LogUtils.m1568d(TAG, "loadImage");
        this.mStartTime = SystemClock.elapsedRealtime();
        this.mDownloader.loadFile(new FileRequest(this.mAdImageUrl), new C05641());
    }
    ...
    private void showAd(ImageView imageView, TextView textView) {
        ...
        if (this.mBitmap == null) {
            ...
            LogUtils.m1568d(TAG, "ad image is null");
            return;
        }
        LogUtils.m1568d(TAG, "showAd mAdId=" + this.mAdId);
    ...
    public void onClick() {
        if (this.mStartAdModel == null) {
            LogUtils.m1577w(TAG, "onClick, ad click info is null!");
            return;
        }
    ...
    private String getBlockForAd(StartAdModel startAdModel) {
        ...
        switch (startAdModel.getAdClickType()) {
            ...
            case CAROUSEL_ILLEGAL:
                LogUtils.m1568d(TAG, "getBlockForAd, start image ad click type :" + AdClickType.CAROUSEL_ILLEGAL);
                break;
            case VIDEO_ILLEGAL:
                LogUtils.m1568d(TAG, "getBlockForAd, start image ad click type :" + AdClickType.VIDEO_ILLEGAL);
                break;
            default:
                LogUtils.m1568d(TAG, "getBlockForAd, start image ad data :" + startAdModel);
                break;
        }
        return block;
    }
