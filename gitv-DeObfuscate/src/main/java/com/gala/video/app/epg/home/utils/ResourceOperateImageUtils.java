...
public class ResourceOperateImageUtils {
    ...
    public static void onClick(Context context, ChannelLabel label, ResourceOperatePingbackModel pingbackModel) {
        if (label == null) {
            LogUtils.m1577w(TAG, "onClick, current ChannelLabel is null");
            return;
        }
        ItemDataType itemDataType = DataBuildTool.getItemType(label);
        if (itemDataType == null) {
            LogUtils.m1577w(TAG, "onClick, current ItemDataType is null");
            ...
            switch (itemDataType) {
                ...
                case LIVE_CHANNEL:
                    if (Project.getInstance().getControl().isOpenCarousel()) {
                        ...
                        LogUtils.m1568d(TAG, "LIVE_CHANNEL: id = " + channelCarousel.id + ", tableNo = " + channelCarousel.tableNo + ", name = " + channelCarousel.name);
                    ...
                    LogUtils.m1568d(TAG, "onClick , not support carousel");
                ...
                case TV_TAG:
                    ...
                    if (tvTags == null) {
                        LogUtils.m1577w(TAG, "onClick, itemDataType = TV_TAG, TVTags(ChannelLabel.ItemKvs.getTVTag()) data is null");
                        return;
                ...
                case TV_TAG_ALL:
                    ...
                    if (tvAllTags == null) {
                        LogUtils.m1577w(TAG, "onClick, itemDataType = TV_TAG_ALL, TVTags(ChannelLabel.ItemKvs.getTVTag()) data is null");
                        return;
            ...
            LogUtils.m1577w(TAG, "onClick, net work illegal");
        }
    }
    ...
    public static boolean isSupportResType(ChannelLabel label) {
        ...
        LogUtils.m1577w(TAG, "isSupportResType, not support Resource type :" + itemDataType);
        return false;
    }
    ...
    public static void onClickForNotSupportJump(Context context) {
        QToast.makeTextAndShow(context, C0508R.string.screen_saver_click_error_hint, 2000);
    }
    ...
    public static String getRValue(ChannelLabel label) {
        String r = "";
        if (label == null) {
            LogUtils.m1577w(TAG, "getRValue, channel label is null");
            return r;
        }
        ItemDataType itemDataType = DataBuildTool.getItemType(label);
        if (itemDataType == null) {
            LogUtils.m1577w(TAG, "getRValue, current ItemDataType is null");
            return r;
        }
        switch (itemDataType) {
            case ALBUM:
