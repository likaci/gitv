...
public class NetDiagnoseUploader {
    ...
    private INetDiagnoseResultListener mNetDiagnoseResultListener = new C09495();

    class C09484 implements IFeedbackResultListener {
        C09484() {
        }
        ...
        public void sendReportSuccess(String s, String s1) {
            LogUtils.m1568d(NetDiagnoseUploader.LOG_TAG, ">>>>> send net diagnose info to feedback success! shutdown ThreadPoolExecutor");
        ...
        public void sendReportFailed(String s) {
            LogUtils.m1571e(NetDiagnoseUploader.LOG_TAG, ">>>>> send net diagnose info to feedback failed!  shutdown ThreadPoolExecutor");
            GetInterfaceTools.getILogRecordProvider().getUploadCore().resetFeedbackValue();
            NetDiagnoseUploader.this.mExecutor.shutdown();
        }
    }

    class C09495 implements INetDiagnoseResultListener {
        C09495() {
        }

        public void onReslut(String info) {
            NetDiagnoseUploader.this.mFilterFlag = false;
            LogUtils.m1568d(NetDiagnoseUploader.LOG_TAG, ">>>>> net diagnose finished");
    ...
    public void doPingNs(final PingNslookupProvider provider) {
        if (provider == null) {
            LogUtils.m1568d(LOG_TAG, ">>>>> doPingNs - return, PingNslookupProvider is null");
        } else if (this.mExecutor == null) {
            LogUtils.m1568d(LOG_TAG, ">>>>> doPingNs - return, ExecutorService is null");
        } else if (this.mFilterFlag) {
            LogUtils.m1571e(LOG_TAG, ">>>>> doPingNs - return, last time Net Diagnose is not finish");
        } else {
            provider.setNetDiagnoseResultListener(this.mNetDiagnoseResultListener);
            if (this.mExecutor.isShutdown()) {
                LogUtils.m1568d(LOG_TAG, ">>>>> ExecutorService has already shutdown");
    ...
    private PingNslookupProvider getPingNsProviderByUrl(String url) {
        try {
            ...
            LogUtils.m1570d(LOG_TAG, ">>>>> doPingNs[url] - sdkError null, ping URL(url).getHost(), ", host);
            return new PingNslookupProvider(this.mContext, new String[]{host});
        } catch (MalformedURLException e) {
            LogUtils.m1571e(LOG_TAG, ">>>>> doPingNs[all] - sdkError null, URL(url).getHost() error");
    ...
    private PingNslookupProvider getPingNsProviderByError(String errorcode, String url) {
        if (StringUtils.isEmpty((CharSequence) errorcode) && StringUtils.isEmpty((CharSequence) url)) {
            LogUtils.m1568d(LOG_TAG, ">>>>> doPingNs[all] - sdkError null, errorcode is null, url is null");
            return new PingNslookupProvider(this.mContext);
        } else if (!"-50".equals(errorcode) && !"-100".equals(errorcode) && !"HTTP_ERR_-50".equals(errorcode) && !"HTTP_ERR_-100".equals(errorcode)) {
            LogUtils.m1570d(LOG_TAG, ">>>>> doPingNs[no] - sdkError null, errorcode is ", errorcode);
            return null;
        } else if (!StringUtils.isEmpty((CharSequence) url)) {
            return getPingNsProviderByUrl(url);
        } else {
            LogUtils.m1568d(LOG_TAG, ">>>>> doPingNs[all] - sdkError null, errorcode is -50 / -100, url is null");
            return new PingNslookupProvider(this.mContext);
        }
    }
    ...
    private PingNslookupProvider getPingNsProviderBySdkError(ISdkError sdkError) {
        ...
        LogUtils.m1570d(LOG_TAG, ">>>>> doPingNs - sdkError module is ", Integer.valueOf(module));
        if (module == 101) {
    ...
    public void doTotalNetDiagnose() {
        if (this.mExecutor == null) {
            LogUtils.m1571e(LOG_TAG, ">>>>> doTotalNetDiagnose - return, ExecutorService is null");
        } else if (this.mFilterFlag) {
            LogUtils.m1571e(LOG_TAG, ">>>>> doTotalNetDiagnose - return, last time Net Diagnose is not finish");
        } else {
            LogUtils.m1568d(LOG_TAG, ">>>>> doTotalNetDiagnose");
            final NetDiagnoseProvider provider = new NetDiagnoseProvider(this.mContext);
            provider.setNetDiagnoseResultListener(this.mNetDiagnoseResultListener);
            if (this.mExecutor.isShutdown()) {
                LogUtils.m1568d(LOG_TAG, ">>>>> doTotalNetDiagnose - ExecutorService has already shutdown");
                this.mExecutor = Executors.newSingleThreadExecutor();
            }
            this.mExecutor.execute(new Runnable() {
                public void run() {
    ...
    public void doNetDiagnoseToAutoTracker(final PingNslookupProvider provider, INetDiagnoseResultListener listener) {
        if (provider == null) {
            ...
            LogUtils.m1568d(LOG_TAG, ">>>>> doNetDiagnoseToAutoTracker - return, ExecutorService is null");
        } else if (this.mFilterFlag) {
            LogUtils.m1571e(LOG_TAG, ">>>>> doNetDiagnoseToAutoTracker - return, last time Net Diagnose is not finish");
        } else {
            provider.setNetDiagnoseResultListener(listener);
            if (this.mExecutor.isShutdown()) {
                LogUtils.m1568d(LOG_TAG, ">>>>> doNetDiagnoseToAutoTracker - ExecutorService has already shutdown");
                this.mExecutor = Executors.newSingleThreadExecutor();
            }
            this.mExecutor.execute(new Runnable() {
                public void run() {
