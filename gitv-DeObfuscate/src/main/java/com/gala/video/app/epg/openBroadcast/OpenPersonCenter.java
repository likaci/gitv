...
public class OpenPersonCenter extends BaseAction {
    ...
    public void process(Context context, Intent intent, LoadingCallback loadingCallback) {
        try {
            LogUtils.m1568d("OpenPersonCenter", "process(context,intent)");
            ...
            LogUtils.m1571e("OpenPersonCenter", "checkParamsValidity is false. ");
            if (loadingCallback != null) {
                loadingCallback.onFail();
                LogUtils.m1571e("OpenPersonCenter", "loadingCallback.onCancel()...");
            }
        } catch (Exception e) {
            LogUtils.m1571e("OpenPersonCenter", "[UNKNOWN-EXCEPTION] [reason:exception occurred when OpenPersonCenter process.][Exception:" + e.getMessage() + AlbumEnterFactory.SIGN_STR);
            e.printStackTrace();
            if (loadingCallback != null) {
                loadingCallback.onFail();
                LogUtils.m1571e("OpenPersonCenter", "loadingCallback.onFail();");
            }
        }
    }

    private void dealPersonCenter(Context context) {
        LogUtils.m1568d("OpenPersonCenter", "dealPersonCenter");
