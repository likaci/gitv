...
public class PlayerConfigManager {
    ...
    public static IPlayerConfig getPlayerConfig() {
        ...
        MyLogUtils.m462d(TAG, "getPlayerConfig() " + testCloudJsConfig);
        if (checkConfigAvailable(sRemotePlayerConfig) && testCloudJsConfig == 0) {
            MyLogUtils.m462d(TAG, "getPlayerConfig(), return " + sRemotePlayerConfig);
            return sRemotePlayerConfig;
        } else if (checkConfigAvailable(sCachedPlayerConfig)) {
            MyLogUtils.m462d(TAG, "getPlayerConfig(), return " + sCachedPlayerConfig);
            return sCachedPlayerConfig;
        } else if (checkConfigAvailable(sLocalPlayerConfig)) {
            MyLogUtils.m462d(TAG, "getPlayerConfig(), return " + sLocalPlayerConfig);
            return sLocalPlayerConfig;
        } else {
            MyLogUtils.m462d(TAG, "getPlayerConfig(), return " + sDefaultPlayerConfig);
