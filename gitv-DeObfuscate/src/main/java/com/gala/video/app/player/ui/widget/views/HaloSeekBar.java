...
public class HaloSeekBar extends View implements ISeekBar {
    ...
    static class SavedState extends BaseSavedState {
        public static final Creator<SavedState> CREATOR = new C15481();
        int progress;
        int secondaryProgress;

        static class C15481 implements Creator<SavedState> {
            C15481() {
            }
    ...
    public HaloSeekBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray array = getContext().obtainStyledAttributes(attrs, C1291R.styleable.HaloSeekBar);
        this.mProgress = array.getInteger(C1291R.styleable.HaloSeekBar_progress, 0);
        this.mSecondaryProgress = array.getInteger(C1291R.styleable.HaloSeekBar_secondaryProgress, 0);
        this.mMax = array.getInteger(C1291R.styleable.HaloSeekBar_max, 100);
        this.mSecondaryProgressColor = array.getColor(C1291R.styleable.HaloSeekBar_secondaryProgressColor, 1719828865);
        this.mBackgroundColor = array.getColor(C1291R.styleable.HaloSeekBar_backgroud, 1283621249);
        this.mProgressColor = array.getColor(C1291R.styleable.HaloSeekBar_progressColor, 1528064);
        this.mMiddleColor = Color.parseColor("#10c607");
        this.mThumbColor = Color.parseColor("#96ff00");
        array.recycle();
        this.mThumbLength = context.getResources().getDimensionPixelSize(C1291R.dimen.dimen_200dp);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        this.mHeight = (int) getResources().getDimension(C1291R.dimen.dimen_12dp);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1574i(TAG, "onLayout changed=" + changed + " (" + left + "," + top + "," + right + "," + bottom + ")");
        }
        if (changed) {
            initProgress();
            postInvalidate();
    ...
    public int getProgressHeight() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1574i(TAG, "getProgressHeight = " + this.mHeight);
        }
        return this.mHeight;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1574i(TAG, "onDraw() progress=" + progress);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case 0:
                if (LogUtils.mIsDebug) {
                    LogUtils.m1574i(TAG, "onTouchEvent ACTION_DOWN");
                }
            ...
            case 1:
                if (LogUtils.mIsDebug) {
                    LogUtils.m1574i(TAG, "onTouchEvent ACTION_UP");
                }
                if (this.mIsDragging) {
                    trackTouchEvent(event);
                    onStopTrackingTouch();
            ...
            case 2:
                if (LogUtils.mIsDebug) {
                    LogUtils.m1574i(TAG, "onTouchEvent ACTION_MOVE");
                }
                if (this.mIsDragging) {
                    trackTouchEvent(event);
