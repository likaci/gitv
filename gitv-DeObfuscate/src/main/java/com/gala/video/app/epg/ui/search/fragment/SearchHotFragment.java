...
public class SearchHotFragment extends SearchBaseFragment {
    ...
    private WidgetStatusListener mGridViewListener = new C09996();
    ...
    private WidgetStatusListener mPhotoViewListener = new C09934();
    ...
    class C09934 implements WidgetStatusListener {
        C09934() {
        }
        ...
        public void onItemSelectChange(View v, int position, boolean isSelected) {
            View title = v.findViewById(C0508R.id.epg_searchVip_titleText);
            if (position == 3) {
                if (isSelected) {
                    AppClientUtils.setBackgroundDrawable(title, ResourceUtil.getDrawable(C0508R.drawable.share_bg_focus));
                } else {
                    AppClientUtils.setBackgroundDrawable(title, ResourceUtil.getDrawable(C0508R.drawable.share_bg_unfocus));
                }
            } else if (isSelected) {
                AppClientUtils.setBackgroundDrawable(title, ResourceUtil.getDrawable(C0508R.drawable.share_item_title_focus_bg));
            } else {
                AppClientUtils.setBackgroundDrawable(title, ResourceUtil.getDrawable(C0508R.drawable.share_album_desc_bg));
            }
            ...
            if (SearchEnterUtils.checkNetWork(SearchHotFragment.this.mContext) && SearchHotFragment.this.mAlbumDataList != null && position < ListUtils.getCount(SearchHotFragment.this.mAlbumDataList)) {
                ...
                if (position == 3) {
                    ...
                    if (!hasVipTab || vipTabModel == null) {
                        LogUtils.m1577w(SearchHotFragment.LOG_TAG, "onItemClick, VIP_VIDEO, vip tabModel:" + vipTabModel + ", hasVipTab : " + hasVipTab);
    ...
    class C09965 implements Runnable {
        ...
        class C09941 implements Runnable {
            ...
            public void run() {
                ...
                SearchHotFragment.this.setSearchHotLayoutMarginTop(C0508R.dimen.dimen_48dp);
        ...
        class C09952 implements Runnable {
            ...
            public void run() {
                ...
                SearchHotFragment.this.setSearchHotLayoutMarginTop(C0508R.dimen.dimen_162dp);
        ...
        public void run() {
            ...
            if (SearchHotFragment.this.mHasHistory) {
                SearchHotFragment.this.mHandler.post(new C09941());
            } else {
                SearchHotFragment.this.mHandler.post(new C09952());
            }
        }
    }

    class C09996 implements WidgetStatusListener {
        C09996() {
        }
        ...
        public void onItemSelectChange(View view, int position, boolean hasFocus) {
            ...
            if (hasFocus) {
                ...
                textView.setTextColor(SearchHotFragment.this.getColor(C0508R.color.gala_write));
            ...
            textView.setTextColor(SearchHotFragment.this.getColor(C0508R.color.search_right_text_color));
                ...
                if (tag != null) {
                    ...
                    if (1 == viewTag) {
                        ...
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1570d(SearchHotFragment.LOG_TAG, ">>>>> name: ", key, "  type: ", type, "  qpid: ", qpid);
                        }
                    } else if (viewTag == 0) {
                        clickType = 0;
        ...
        private void insertDB(final String key, final String qpid, final String type) {
            ThreadUtils.execute(new Runnable() {
                public void run() {
                    LogUtils.m1576i("SQL_TEST", "SearchRightDefaultFragment --- click ---key=", key);
    ...
    private static class IApiCallbackImpl implements IApiCallback<ApiResultHotWords> {
        ...
        public void onSuccess(final ApiResultHotWords result) {
            ...
            if (outer != null) {
                if (result == null || result.data == null) {
                    LogUtils.m1571e(SearchHotFragment.LOG_TAG, "requestData --- onSuccess result is null");
                } else {
        ...
        public void onException(final ApiException exception) {
            final SearchHotFragment outer = (SearchHotFragment) this.mOuter.get();
            if (outer != null) {
                LogUtils.m1573e(SearchHotFragment.LOG_TAG, ">>>>>>>>>>>> Api.hotword.call Exception: ", exception.getMessage());
                ...
                NetWorkManager.getInstance().checkNetWork(new StateCallback() {
                    ...
                    public void getStateResult(int arg0) {
                        outer.runOnUiThread(new C10011());
                    }
                });
            }
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        this.mMainView = inflater.inflate(C0508R.layout.epg_fragment_search_right, null);
        init(this.mMainView);
        showLoading();
        initInThread();
    ...
    public void initInThread() {
        ThreadUtils.execute(new C09901());
    }
    ...
    private void doAfterRequestSucc(List<String> hotwords) {
        ...
        this.mHandler.postDelayed(new C09912(), 500);
    }
    ...
    private void doAfterRequestException(ApiException exception) {
        ...
        this.mQrPanel = (GlobalQRFeedbackPanel) this.mMainView.findViewById(C0508R.id.epg_search_default_QR_panel);
        ...
        if (model.isShowQR()) {
            ...
            this.mTextView = (TextView) this.mMainView.findViewById(C0508R.id.epg_search_hot_exception_textview);
            this.mTextView.setTextColor(getColor(C0508R.color.albumview_yellow_color));
            this.mTextView.setText(content.replaceAll("\\n", ""));
            this.mTextView.setVisibility(0);
        }
        setHistoryViewAdapter();
        this.mHandler.postDelayed(new C09923(), 500);
        hideLoading();
    }
    ...
    private void initSearchHotGridView() {
        ...
        LogUtils.m1571e(LOG_TAG, ">>>>>> hot gridview is null");
    }
    ...
    private PhotoGridParams getHotWordsGrideViewParams() {
        ...
        p.horizontalSpace = getDimen(C0508R.dimen.dimen_6dp);
        p.contentHeight = getDimen(C0508R.dimen.dimen_46dp);
        p.contentWidth = getDimen(C0508R.dimen.dimen_262dp);
        p.scaleRate = 1.1f;
        return p;
    }
    ...
    private void initSearchHistoryGridView() {
        if (this.mSearchHistoryGridView != null) {
            LogUtils.m1568d(LOG_TAG, ">>>>>> init history gridview");
    ...
    private PhotoGridParams getPhotoViewParams() {
        int itemWidth = getDimen(C0508R.dimen.dimen_116dp);
        int itemHeight = getDimen(C0508R.dimen.dimen_160dp);
        int itemPadding = getDimen(C0508R.dimen.dimen_20dp);
        PhotoGridParams p = new PhotoGridParams();
        p.columnNum = 4;
    ...
    private void initLayout() {
        this.mSearchHotGridView = (PhotoGridView) this.mMainView.findViewById(C0508R.id.epg_search_hot_gridview);
        this.mSearchHistoryGridView = (PhotoGridView) this.mMainView.findViewById(C0508R.id.epg_search_history_gridview);
        this.mSearchHistoryLayout = (RelativeLayout) this.mMainView.findViewById(C0508R.id.epg_search_history_layout);
        this.mHotTitleView = this.mMainView.findViewById(C0508R.id.epg_search_hot_title_layout);
        this.mHistoryTitleView = this.mMainView.findViewById(C0508R.id.epg_search_history_title_layout);
        this.mSearchVipGridView = (PhotoGridView) this.mMainView.findViewById(C0508R.id.epg_search_vipList);
    }
    ...
    private void setHistoryViewAdapter() {
        if (this.mContext != null && this.mHistoryTitleView != null) {
            ThreadUtils.execute(new C09965());
        }
    }
            ...
            for (int i = 0; i < preNum; i++) {
                ...
                while (j < vipNum) {
                    if (preValue.equals((String) vipList.get(j))) {
                        LogUtils.m1573e(LOG_TAG, ">>>>> filterList --- ", preValue);
                        break;
                    }
                    j++;
                }
    ...
    private void sendClickPingback(int tag, int pos) {
        ...
        if (tag == 0) {
            if (ListUtils.isLegal(this.mHotHordsList, pos)) {
                ...
                params.add(Keys.f2035T, "20").add("rseat", lineIndex + "_" + rowIndex).add("rpage", "search").add("block", "hotword").add("rt", "i").add("r", selectedHotWord);
            ...
            if (ListUtils.isLegal(this.mHistoryBeanList, pos)) {
                ...
                params = new PingBackParams();
                params.add(Keys.f2035T, "20").add("rseat", lineIndex + "_" + rowIndex).add("rpage", "search").add("block", "searchhistory").add("rt", "i").add("r", selectedHistoryWord);
                PingBack.getInstance().postPingBackToLongYuan(params.build());
            }
            ...
            params = new PingBackParams();
            params.add(Keys.f2035T, "20").add("rseat", "1_" + (pos + 1)).add("rpage", "search").add("block", "viphot").add("rt", "i").add("plid", plid).add("r", qpid);
            PingBack.getInstance().postPingBackToLongYuan(params.build());
        }
    }
    ...
    public void requestDefaultFocus() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d("FOCUS_TEST", ">>>>> mSearchEvent.onRequestRightDefaultFocus() --- SearchRightDefault");
        }
