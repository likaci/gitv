...
public class ScreenSaverOperateImageRequestTask extends BaseRequestTask {
    ...
    class C06521 implements IVrsCallback<ApiResultChannelLabels> {
        C06521() {
        }

        public void onSuccess(ApiResultChannelLabels result) {
            if (result == null) {
                LogUtils.m1577w(ScreenSaverOperateImageRequestTask.TAG, "fetchScreenSaverImageData, onSuccess, ApiResultChannelLabels is null");
            ...
            if (labels == null) {
                LogUtils.m1577w(ScreenSaverOperateImageRequestTask.TAG, "fetchScreenSaverImageData, onSuccess, ChannelLabels is null");
                ScreenSaverOperateImageRequestTask.this.clearLimitBehavior();
                ScreenSaverImageProvider.getInstance().onEmptyScreenSaverData();
                return;
            }
            ...
            if (ListUtils.isEmpty((List) labelList)) {
                LogUtils.m1577w(ScreenSaverOperateImageRequestTask.TAG, "fetchScreenSaverImageData, onSuccess, ChannelLabel List is empty");
                ScreenSaverOperateImageRequestTask.this.clearLimitBehavior();
                ScreenSaverImageProvider.getInstance().onEmptyScreenSaverData();
                return;
            }
            LogUtils.m1568d(ScreenSaverOperateImageRequestTask.TAG, "fetchScreenSaverImageData, onSuccess, ChannelLabel size : " + ListUtils.getCount((List) labelList));
            ...
            while (iterator.hasNext()) {
                ...
                if (ResourceType.LIVE == label.getType() && !label.checkLive()) {
                    iterator.remove();
                    LogUtils.m1568d(ScreenSaverOperateImageRequestTask.TAG, "fetchScreenSaverImageData, remove out of date live data, id : " + label.itemId);
                }
            }
        ...
        public void onException(ApiException e) {
            LogUtils.m1578w(ScreenSaverOperateImageRequestTask.TAG, "fetchScreenSaverImageData, onException, ApiException e ", e);
            ScreenSaverOperateImageRequestTask.this.clearLimitBehavior();
        }
    }
    ...
    private void fetchScreenSaverImageData() {
        ...
        if (dynamicResult != null) {
            LogUtils.m1577w(TAG, "fetchScreenSaverImageData, screensaver operation image resource id : " + dynamicResult.getSpecifiedOperateImageResId(OperationImageType.SCREENSAVER));
            VrsHelper.channelLabelsSize.call(new C06521(), id, "0", "5.0", "60");
            return;
        }
        LogUtils.m1577w(TAG, "fetchScreenSaverImageData, get dynamic data, dynamicResult is null");
    }
}
