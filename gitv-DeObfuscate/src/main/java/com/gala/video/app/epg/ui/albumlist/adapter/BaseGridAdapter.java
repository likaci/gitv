...
public abstract class BaseGridAdapter<T> extends Adapter<ViewHolder> {
        ...
        public void onSuccess(ImageRequest imageRequest, Bitmap bitmap) {
            ...
            if (outer != null) {
                if (bitmap != null && imageRequest != null) {
                    ...
                    LogUtils.m1568d(outer.TAG, "loadBitmap >> onSuccess bitmap = null!");
                }
            }
        }
        ...
        public void onFailure(ImageRequest imageRequest, Exception exception) {
            ...
            if (outer != null) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1571e(outer.TAG, "loadBitmap---fail !!! Load,url = " + this.imageUrl + ", consume(time3-time2) = " + (System.currentTimeMillis() - this.time1) + "ms");
                }
    ...
    public AlbumViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view;
        if (viewType == 17) {
            view = LayoutInflater.from(this.mContext).inflate(C0508R.layout.share_albumlist5_loading, parent, false);
        } else {
    ...
    protected void loadBitmap(View convertView, String imageUrl) {
        if (this.mStopLoadtask || convertView == null || StringUtils.isEmpty((CharSequence) imageUrl)) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1573e("loadBitmap() -> convertView == null or mStopLoadTask = " + this.mStopLoadtask + ",convertView:" + convertView + ",imageUrl:" + imageUrl);
            }
        } else if (StringUtils.isEmpty((CharSequence) imageUrl)) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1571e(this.TAG, "loadBitmap() -> imageUrl is null");
            }
    ...
    public void updateData(List<T> datas) {
        if (datas != null) {
            ...
            if (oldDataSize <= 0 || size < oldDataSize) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1571e(this.TAG, "updateData notifyDataSetChanged");
                }
                notifyDataSetChanged();
                return;
            }
            if (LogUtils.mIsDebug) {
                LogUtils.m1571e(this.TAG, "updateData notifyDataSetUpdate");
            }
            notifyDataSetAdd();
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1571e(this.TAG, "updateData datas is null!");
        }
    }
    ...
    public void recycleBitmap(View view) {
        if (view != null) {
            ...
            LogUtils.m1571e(this.TAG, "recycleBitmap view is null");
        }
    }
