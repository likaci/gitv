...
public class FetchBannerAdTask {
    ...
    public void setNowId(String id) {
        LogUtils.m1568d(TAG, "FetchBannerAdTask id = " + id);
        this.mId = id;
    }
    ...
    private String getDetailBannerUrl(String albumId, String tvQid, String channelId) {
        ...
        LogUtils.m1568d(TAG, "getDetailBannerUrl --- pi = " + pi + " pc = " + pc + " isVip = " + isVip);
        ...
        LogUtils.m1568d(TAG, "getDetailBannerUrl ---url is " + url);
        String response = "";
        try {
            HttpUtil hu = new HttpUtil(url);
            LogUtils.m1568d(TAG, "FetchBannerAdTask hu = " + hu);
            response = hu.get();
        } catch (Exception e) {
            LogUtils.m1571e(TAG, "Exception= " + e);
        }
        LogUtils.m1568d(TAG, "FetchBannerAdTask response = " + response);
        return response;
    }
    ...
    public void execute(final String id) {
        new Thread8K(new Runnable() {
            public void run() {
                LogUtils.m1568d(FetchBannerAdTask.TAG, "on run : ");
                if (ListUtils.isEmpty(FetchBannerAdTask.this.mAds)) {
                    ...
                    LogUtils.m1568d(FetchBannerAdTask.TAG, "on run : mAlbumId" + FetchBannerAdTask.this.mAlbumId + "mTvQId" + FetchBannerAdTask.this.mTvQId + "mChannelId" + FetchBannerAdTask.this.mChannelId);
                ...
                for (BannerImageAdModel ad : FetchBannerAdTask.this.mAds) {
                    if (StringUtils.equals(id, ad.getAdZoneId())) {
                        ...
                        LogUtils.m1568d(FetchBannerAdTask.TAG, "success fetch ad zone id is->" + id);
                        LogUtils.m1568d(FetchBannerAdTask.TAG, "success fetch ad id is->" + ad.getAdId());
                        return;
                    }
                }
                FetchBannerAdTask.this.mListener.onSuccess(null);
                LogUtils.m1568d(FetchBannerAdTask.TAG, "success but no ad");
            }
