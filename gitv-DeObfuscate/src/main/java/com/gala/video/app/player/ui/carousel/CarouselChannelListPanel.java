...
public class CarouselChannelListPanel {
    ...
    private PlayerListListener mPlayerListListener = new C14541();
    ...
    class C14541 implements PlayerListListener {
        C14541() {
        }
            ...
            if (holder != null) {
                ...
                if (itemView != null) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(CarouselChannelListPanel.this.TAG, "onItemFocusChanged position = " + position + "/" + isSelected);
                    }
                    ...
                    int itemSpreadBgResId = C1291R.drawable.player_carousel_list_spread;
                    int channelNormalColor = CarouselChannelListPanel.this.mContext.getResources().getColor(C1291R.color.player_ui_text_color_default);
                    int tvNameColor = CarouselChannelListPanel.this.mContext.getResources().getColor(C1291R.color.player_ui_carousel_item_tvname_normal);
                    int spreadColor = CarouselChannelListPanel.this.mContext.getResources().getColor(C1291R.color.player_ui_carousel_item_channel_selected);
                    int focusColor = CarouselChannelListPanel.this.mContext.getResources().getColor(C1291R.color.player_ui_text_color_focused);
                    if (isSelected) {
                        ...
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(CarouselChannelListPanel.this.TAG, "onItemFocusChanged mSpreadPosition=" + CarouselChannelListPanel.this.mSpreadPosition);
                        }
        ...
        public void onItemClick(ViewHolder holder, int tag) {
            if (holder != null) {
                int position = holder.getLayoutPosition();
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(CarouselChannelListPanel.this.TAG, "onItemClick position = " + position);
                }
                if (!ListUtils.isEmpty(CarouselChannelListPanel.this.mChannelList)) {
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(CarouselChannelListPanel.this.TAG, "onItemClick channel = " + channel);
                    }
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(CarouselChannelListPanel.this.TAG, "onItemClick mCurrentCarosel = " + CarouselChannelListPanel.this.mCurrentCarosel);
                    }
                    if (CarouselChannelListPanel.this.mCurrentCarosel == null || channel == null || CarouselChannelListPanel.this.mCurrentCarosel.id != channel.id) {
                        ...
                        LogUtils.m1568d(CarouselChannelListPanel.this.TAG, "onItemClick same channel");
                    }
    ...
    public void show(TVChannelCarouselTag tvtag) {
        if (tvtag != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "show() tvtag=" + tvtag + ", mChannelList=" + (this.mChannelList == null ? -1 : this.mChannelList.size()));
            }
            ...
            LogUtils.m1568d(this.TAG, "show() tag is null");
        }
    }

    private void refreshPlayList(List<TVChannelCarousel> allChannelList, TVChannelCarouselTag tvtag) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "refreshPlayList() size=" + allChannelList.size());
        }
    ...
    private void setPosition() {
        if (this.mCurrentCarosel != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "show playingindex=" + getPlayIndex(this.mCurrentCarosel.id));
            }
        }
    }
    ...
    public void setFocus() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setFocusPosition()");
        }
    ...
    public void setAllChannelList(List<TVChannelCarousel> allChannelList, TVChannelCarouselTag tag) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setAllChannelList() allChannelList=" + ListUtils.getCount((List) allChannelList) + ", tag=" + tag);
        }
        if (this.mCurrentLabel != tag) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "setAllChannelList() labels are not the same");
            }
    ...
    public void setCurrentPlayIndex(long tvId, boolean needFocus) {
        LogUtils.m1568d(this.TAG, "setCurrentPlayIndex" + tvId + ", needFocus=" + needFocus);
        int index = -1;
        if (!ListUtils.isEmpty(this.mChannelList)) {
        ...
        LogUtils.m1568d(this.TAG, "setCurrentPlayIndex index=" + index);
    ...
    private int getPlayIndex(long tvId) {
        ...
        LogUtils.m1568d(this.TAG, "getPlayIndex=" + tvId + ", index=" + index);
        return index;
    }

    public void setChannelProgramName(List<CarouselChannelDetail> list, TVChannelCarouselTag tag) {
        LogUtils.m1568d(this.TAG, "setChannelProgramName list=" + list.size());
    ...
    private void updateProgramList(List<CarouselChannelDetail> list) {
        LogUtils.m1568d(this.TAG, "updateProgramList list=" + list.size());
    ...
    public boolean isEnableShow() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "isEnableShow()" + this.mIsEnableShow);
        }
    ...
    public boolean isLoadingState() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "isLoadingState()" + this.mState);
        }
        if (this.mState == 1) {
            return true;
        }
    ...
    private void updateCurrentIndex(long currentId, int size) {
        for (int i = 0; i < size; i++) {
            if (currentId == ((TVChannelCarousel) this.mChannelList.get(i)).id) {
                ...
                LogUtils.m1568d(this.TAG, "notifyChannelInfoChange for currentIndex=" + this.mCurrentIndex);
                return;
            }
        }
    }
    ...
    public void setCurrentChannel(TVChannelCarousel channelCarousel) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setCurrentChannel channelCarousel=" + channelCarousel + ", mCurrentCarosel=" + this.mCurrentCarosel);
        }
    ...
    public void setChannelChangeByIndex(int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setChannelChangeByIndex=" + index);
        }
        this.mIndex = index;
    }
    ...
    public TVChannelCarousel notifyRequestProgramme() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "notifyRequestProgramme()");
        }
        ...
        if (!(ListUtils.isEmpty(this.mChannelList) || this.mChannelInfoListener == null)) {
            ...
            if (this.mSpreadPosition >= 0) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(this.TAG, "notifyRequestProgramme() channel=" + channel + ",mSpreadPosition=" + this.mSpreadPosition);
                }
    ...
    public void updateSpreadPosition(int position) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "updateSpreadPosition =" + position);
        }
        this.mSpreadPosition = -1;
    }
    ...
    public void notifyUserClickProgramme() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "notifyUserClickProgramme()");
        }
        if (!ListUtils.isEmpty(this.mChannelList) && this.mSpreadPosition >= 0) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "onItemClick mCurrentCarosel.id = " + this.mCurrentCarosel.id + ",channel.id=" + channel.id);
            }
            if (this.mCurrentCarosel == null || channel == null || this.mCurrentCarosel.id != channel.id) {
                ...
                LogUtils.m1568d(this.TAG, "notifyUserClickProgramme same channel");
            }
    ...
    public void setVideo(IVideo video) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setVideo()");
        }
        this.mVideo = video;
    }
    ...
    public void notifyChannelInfoChange(int chnChangeOffset, boolean needRequestAll, boolean realChannelChange) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "notifyChannelInfoChange chnChangeOffset=" + chnChangeOffset + ", needRequestAll=" + needRequestAll + ", realChannelChange=" + realChannelChange);
        }
        if (!ListUtils.isEmpty(this.mChannelList)) {
            if (needRequestAll) {
            }
            ...
            int size = this.mChannelList.size();
            LogUtils.m1568d(this.TAG, "notifyChannelInfoChange currentIndex=" + this.mCurrentIndex);
            ...
            LogUtils.m1568d(this.TAG, "notifyChannelInfoChange ChannelList is null");
        }
    }
    ...
    private void sendShowPingback() {
        ...
        if (this.mPingbackContext != null) {
            ...
            LogUtils.m1568d(this.TAG, "sendShowPingback mPingbackContext is null");
        }
    }

    private void sendClickPingback(TVChannelCarousel channel) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "sendClickPingback() channel=" + channel);
        }
