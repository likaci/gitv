...
public class MainPagePresenter implements IActivityLifeCycle, IExitDialogStatusListener, IStatusListener {
    ...
    private OnPageChangeListener mPageChangeListener = new C06772();
    ...
    private OnFocusChangeListener mTabHostFocusChangeListener = new C06783();
    private List<TabData> mTabListInfo = new ArrayList();
    private int mTargetPage = -1;
    private RelativeLayout mTopLayout;
    private OnTurnPageListener mTurnPageListener = new C06761();
    private Handler mhandle;
    private final UIEvent muiEvent;

    class C06761 implements OnTurnPageListener {
        C06761() {
        }

        public void onTurnPage(int newPage) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MainPagePresenter.TAG, "onTurnPage newPage = " + newPage);
            }
            MainPagePresenter.this.mPager.setCurrentItem(newPage);
        }
    }

    class C06772 extends SimpleOnPageChangeListener {
        C06772() {
        }
        ...
        public void onPageSelected(int position) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MainPagePresenter.TAG, "onPageSelected position = " + position + " PrePageIndex = " + MainPagePresenter.this.mPrePageIndex);
            }
        ...
        public void onPageScrollStateChanged(int state) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MainPagePresenter.TAG, "onPageScrollStateChanged state = " + state + " getCurrentItem = " + MainPagePresenter.this.mPager.getCurrentItem());
            }
            if (state == 0) {
    ...
    class C06783 implements OnFocusChangeListener {
        C06783() {
        }
    ...
    public MainPagePresenter(Context context, View rootView, ActionBarPresenter presenter, UIEvent uiEvent, int targetPage, Presenter promotionPresenter) {
        ...
        this.mPager = (ScrollViewPager) rootView.findViewById(C0508R.id.epg_pager);
        this.mTabBarHost = (TabBarHost) rootView.findViewById(C0508R.id.epg_tab_host);
        this.mTabHostDecoratedView = (ImageView) rootView.findViewById(C0508R.id.epg_tab_bar_decorated);
        this.mTopLayout = (RelativeLayout) rootView.findViewById(C0508R.id.epg_top_layout);
        ...
        this.mSettingView = (TabBarSettingView) rootView.findViewById(C0508R.id.epg_home_tab_setting_view);
    }
    ...
    public void buildTabHost(PageManage[] pages, int max) {
        ...
        if (pages != null) {
            ...
            for (PageManage tab : pages) {
                ...
                LogUtils.m1568d(TAG, "tab host data = " + tab.mTabdata);
                size++;
                if (size >= max) {
                    break;
                }
            }
            LogUtils.m1568d(TAG, "tab host size : " + this.mTabListInfo.size());
        ...
        if (pages != null) {
            ...
            while (index < len && index < pages.length) {
                if (pages[index].mChild != null) {
                    LogUtils.m1568d(TAG, "setFirstRowFocusUpId id = " + this.mTabBarIds[index]);
    ...
    public void buildPages(UIController controller) {
        ...
        if (pageTabs != null) {
            ...
            LogUtils.m1568d(TAG, "default focus tab index = " + this.mDefaultTab + ",current tab host index = " + tabIndex);
    ...
    private void buildPageViews(List<ViewGroup> pageViews, Context context) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "build page views size = " + pageViews.size());
        }
    ...
    public void onActivityPause() {
        LogUtils.m1568d(TAG, "onActivityPause!!!");
    }

    public void onActivityStop() {
        LogUtils.m1568d(TAG, "onActivityStop!!!");
