...
public class ActionBarPresenter {
    ...
    private MyObserver mDynamicRequestFinish = new C06743();
    private final Handler mHandler = new Handler(Looper.myLooper());
    private IMessageNotification mMessageNotification = new C06711();
    private IMessageUpdating mMessageUpdating = new C06722();
    private final VipAnimationView mVipAnimationView;

    class C06711 implements IMessageNotification {
        C06711() {
        }
    ...
    class C06722 implements IMessageUpdating {
        C06722() {
        }
    ...
    class C06743 implements MyObserver {
        C06743() {
        }
    ...
    public ActionBarPresenter(Context context, View root) {
        ...
        this.mActionBar = (ActionBarLayout) root.findViewById(C0508R.id.epg_action_bar);
        this.mActionTip = (TextView) root.findViewById(C0508R.id.epg_actionbar_tip);
    ...
    private void resetActionBar() {
        ...
        LogUtils.m1574i(TAG, "resetActionBar isResetActionBar:" + isResetActionBar);
        if (isResetActionBar) {
    ...
    public void setActionBarResource(RelativeLayout rl) {
        ImageView divider = (ImageView) rl.findViewById(C0508R.id.epg_divider);
        ImageView logoId = (ImageView) rl.findViewById(C0508R.id.epg_logo_id);
        ((TextView) rl.findViewById(C0508R.id.epg_time)).setTextColor(this.mContext.getResources().getColor(C0508R.color.home_state_bar_time));
        AppClientUtils.setBackgroundDrawable(divider, this.mContext.getResources().getDrawable(C0508R.color.home_state_bar_divider));
        logoId.setImageResource(C0508R.drawable.share_gitv);
        for (int i = 0; i < this.mActionBar.getChildCount(); i++) {
            ...
            AppClientUtils.setBackgroundDrawable(itemView, this.mContext.getResources().getDrawable(C0508R.drawable.epg_action_bar_bg_unfocused));
