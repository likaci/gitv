...
public class XWalkViewEvent extends WebBaseEvent {
    ...
    private IXWalkResourceClient mXWalkResourceClient = new C12571();
    private IXWalkUIClient mXWalkUIClient = new C12582();
    private IXWalkView mXWalkView;

    class C12571 implements IXWalkResourceClient {
        C12571() {
        }

        public void onLoadStarted(IXWalkView view, String url) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "onLoadStarted() -> url:" + url);
        }

        public void onLoadFinished(IXWalkView view, String url) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "onLoadFinished() -> url:" + url);
        }
        ...
        public void onReceivedLoadError(IXWalkView view, int errorCode, String description, String failingUrl) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "onReceivedLoadError() -> failingUrl:" + failingUrl + ",errorCode:" + errorCode);
        }

        public void onReceivedSslError(IXWalkView view, ValueCallback<Boolean> callback, SslError error) {
            LogUtils.m1571e(XWalkViewEvent.TAG, "onReceivedSslError:" + error);
            if (WebSDKDataUtils.isSSLProceed(error)) {
                LogUtils.m1568d(XWalkViewEvent.TAG, "onReceivedSslError -> need Proceed");
        ...
        public WebResourceResponse shouldInterceptLoadRequest(IXWalkView view, String url) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "XwalkView shouldInterceptLoadRequest url:" + url);
        ...
        public boolean shouldOverrideUrlLoading(IXWalkView view, String url) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "shouldOverrideUrlLoading url:" + url);
    ...
    class C12582 implements IXWalkUIClient {
        C12582() {
        }

        public boolean shouldOverrideKeyEvent(IXWalkView view, KeyEvent event) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "shouldOverrideKeyEvent");
            return false;
        }

        public void onFullscreenToggled(IXWalkView view, boolean enterFullscreen) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "onFullscreenToggled");
        }

        public boolean onConsoleMessage(IXWalkView view, String message, int lineNumber, String sourceId) {
            LogUtils.m1568d("Subject_HTML_Client", "line " + lineNumber + " of " + sourceId + SOAP.DELIM + message);
            return false;
        }

        public void onPageLoadStarted(IXWalkView view, String url) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "onPageLoadStarted url:" + url);
        ...
        public void onPageLoadStopped(IXWalkView view, String url, String status) {
            LogUtils.m1568d(XWalkViewEvent.TAG, "onPageLoadStopped url:" + url + ",status:" + status);
            if (XWalkViewEvent.this.mOnLoadListener != null) {
                if (StringUtils.isEmpty((CharSequence) status) || WebConstants.STATUS_FAILED.equals(status)) {
                    ...
                    LogUtils.m1568d(XWalkViewEvent.TAG, "STATUS_FINISHED.equals(status) status:" + status);
    ...
    public XWalkViewEvent() {
        LogUtils.m1568d(TAG, "WebEvent Type: XwalkViewEvent");
    }
    ...
    public void loadJsMethod(String jsUrl) {
        LogUtils.m1568d(TAG, "loadJsMethod:" + jsUrl);
        if (isAlready()) {
