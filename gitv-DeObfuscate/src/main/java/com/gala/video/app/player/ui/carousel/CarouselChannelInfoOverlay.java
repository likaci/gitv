...
public class CarouselChannelInfoOverlay {
    ...
    private ChannelDetailCallback mCallBack = new C14531();
    ...
    class C14531 implements ChannelDetailCallback {
        C14531() {
        }

        public void onCacheReady(CarouselChannelDetail detail) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(CarouselChannelInfoOverlay.TAG, "ChannelDetailCallback onCacheReady =" + detail);
            }
        ...
        public void onDataReady(CarouselChannelDetail detail) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(CarouselChannelInfoOverlay.TAG, "ChannelDetailCallback onDataReady =" + detail);
            }
            CarouselChannelInfoOverlay.this.setCurrentChannelDetail(detail);
        }
    ...
    public void initView() {
        this.mChannelInfoContainer = (LinearLayout) this.mRoot.findViewById(C1291R.id.channel_info_container);
        this.mChannelId = (TextView) this.mRoot.findViewById(C1291R.id.channel_id);
        this.mChannelName = (TextView) this.mRoot.findViewById(C1291R.id.channel_name);
        this.mPlayingName = (AlwaysMarqueeTextView) this.mRoot.findViewById(C1291R.id.playing_name);
        this.mPlayingTime = (TextView) this.mRoot.findViewById(C1291R.id.playing_time);
        this.mHelpUp = (TextView) this.mRoot.findViewById(C1291R.id.channel_info_help_up);
        this.mHelpDown = (TextView) this.mRoot.findViewById(C1291R.id.channel_info_help_down);
        this.mImgHelpup = (ImageView) this.mRoot.findViewById(C1291R.id.img_help_up);
        this.mImgHelpdown = (ImageView) this.mRoot.findViewById(C1291R.id.img_help_down);
        showHelpTip();
    }

    private void showHelpTip() {
        int txtUpId = C1291R.string.channel_list;
        int drawableUpId = C1291R.drawable.player_carousel_ok;
        int txtDownId = C1291R.string.change_channel;
        int drawableDownId = C1291R.drawable.player_carousel_updown;
    ...
    public void hide() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "CarouselChannelInfoOverlay hide");
        }
    ...
    public void setCurrentChannelDetail(CarouselChannelDetail currentInfo) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setCurrentChannelDetail channelCarousel=" + currentInfo);
        }
        if (currentInfo != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "setCurrentChannelDetail currentName=" + currentName + ", nextName=" + nextName + ", currentTime=" + currentTime);
            }
        }
    }
    ...
    public void setCurrentChannel(TVChannelCarousel channelCarousel) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setCurrentChannel channelCarousel=" + channelCarousel);
        }
        if (channelCarousel != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "setCurrentChannel name=" + name + ", id=" + id);
            }
        }
    }
    ...
    public void updateChannel(TVChannelCarousel channelCarousel, boolean needRequest, CarouselChannelDetail currentCarouselDetail) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "show() updateChannel=" + channelCarousel);
        }
        if (channelCarousel != null) {
            String name = channelCarousel.name;
            CharSequence id = String.valueOf(channelCarousel.sid);
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "updateChannel name=" + name + ", id=" + id);
            }
        }
    }
    ...
    private void show(TVChannelCarousel channelCarousel, boolean needRequest, CarouselChannelDetail currentCarouselDetail) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showUnchangeTip() mChannelCarousel=" + channelCarousel + ", currentCarouselDetail=" + currentCarouselDetail);
        }
    ...
    private void sendShowPingback() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "sendShowPingback()");
        }
        String C1 = "101221";
        String C2 = "";
        ...
        if (this.mPingbackContext != null) {
            ...
            LogUtils.m1568d(TAG, "sendShowPingback() mPingbackContext is null");
        }
    }

    public void setVideo(IVideo video) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setVideo() c2=" + video.getAlbum().live_channelId);
        }
        this.mVideo = video;
    }
}
