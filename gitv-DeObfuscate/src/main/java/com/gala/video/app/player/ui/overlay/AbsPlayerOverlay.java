...
public abstract class AbsPlayerOverlay implements ISuperPlayerOverlay {
    ...
    protected IAdDataProvider mAdProdiver = new C14736();
    protected OnUserBitStreamChangeListener mBitStreamListener = new C14692();
    protected OnUserChangeVideoRatioListener mChangeVideoRatioListener = new C14703();
    ...
    protected OnAdStateListener mOnAdStateListener = new C14725();
    ...
    protected OnUserSkipHeadTailChangeListener mSkipHeadTailListener = new C14714();
    protected int mState;
    protected TipWrapper mTip;
    private TipUIHelper mTipHelper;
    protected IVideo mVideo;
    protected OnUserVideoChangeListener mVideoChangeListener = new C14681();

    class C14681 implements OnUserVideoChangeListener {
        C14681() {
        }
    ...
    class C14692 implements OnUserBitStreamChangeListener {
        C14692() {
        }
    ...
    class C14703 implements OnUserChangeVideoRatioListener {
        C14703() {
        }
    ...
    class C14714 implements OnUserSkipHeadTailChangeListener {
        C14714() {
        }
    ...
    class C14725 implements OnAdStateListener {
        C14725() {
        }
        ...
        public void onShow(int i, Object object) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(AbsPlayerOverlay.TAG, "onShow():" + i);
            }
        ...
        public void onRequest(int type) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(AbsPlayerOverlay.TAG, "onRequest()");
            }
    ...
    class C14736 implements IAdDataProvider {
        C14736() {
        }

        public void setAdData(int type, BaseAdData baseAdData) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(AbsPlayerOverlay.TAG, "setAdData()");
            }
    ...
    public void setCurrentVideo(IVideo video) {
        if (video != null) {
            ...
            LogUtils.m1568d(TAG, "setMovie, movie is null, return");
        }
    }
    ...
    public boolean onDlnaKeyEvent(DlnaKeyEvent event, KeyKind key) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onDLNAKeyEvent: {" + event + "}, {" + key + "}");
        }
    ...
    public void clearError() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "clearError()");
        }
    ...
    public void onNetworkChange(int netState) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onNetworkChange: network connected=" + this.mIsNetworkConnected);
        }
    ...
    public void setOnUserVideoChangeListener(OnUserVideoChangeListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setOnUserVideoChangeListener(" + listener + ")");
        }
    ...
    protected void notifyVideoChange(View view, IVideo movie) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyVideoChange(" + movie + ")");
        }
    ...
    public void setOnUserBitStreamChangeListener(OnUserBitStreamChangeListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setOnUserDefinitionChangeListener(" + listener + ")");
        }
    ...
    protected void notifyBitStreamSelected(View view, BitStream bitStream) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyBitStreamSelected(" + view + ", " + bitStream + ")");
        }
    ...
    protected void notifyHDRToggleChanged(View view, boolean isOpen) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyHDRToggleChanged(" + view + ", " + isOpen + ")");
        }
        if (this.mOuterBitStreamListener != null) {
    ...
    public void setOnUserSkipHeadTailChangeListener(OnUserSkipHeadTailChangeListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setOnUserSkipHeadTailChangeListener(" + listener + ")");
        }
    ...
    protected void notifySkipHeadAndTail(View view, boolean skip) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifySkipHeadAndTail(" + view + ", " + skip + ")");
        }
    ...
    public void setOnUserChangeVideoRatioListener(OnUserChangeVideoRatioListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setOnUserSetDisplayModeListner(" + listener + ")");
        }
    ...
    protected void notifyVideoRatioSelected(int ratio) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyVideoRatioSelected(" + ratio + ")");
        }
    ...
    public void updateSkipHeadAndTail(boolean isSkipTail) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "updateSkipHeadAndTail(" + isSkipTail + ")");
        }
    ...
    public void updateBitStream(List<BitStream> list, BitStream bitStream) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "updateBitStream(" + bitStream + ", list " + list + ")");
        }
            ...
            if (this.mVideo != null) {
                ...
                if (!StringUtils.isEmpty(rblock)) {
                    ...
                    if (!StringUtils.isEmpty(rseat)) {
                        ...
                        PingbackFactory.instance().createPingback(27).addItem(C0165R.R_TYPE(resouceId)).addItem(BLOCK.BLOCK_TYPE(rblock)).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE(rseat)).addItem(RPAGE.RPAGE_ID(rpage)).addItem(C1.C1_TYPE(getC1())).addItem(C2.C2_TYPE(getC2())).addItem(NOW_C1.NOW_C1_TYPE(getC1())).addItem(NOW_QPID.NOW_QPID_TYPE(qpid)).addItem(NOW_C2.NOW_C2_TYPE(getC2())).addItem(STATE.STATE_TYPE(getState())).post();
                    } else if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(TAG, "<<onKeyEvent rseat is invalid");
                    }
                } else if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(TAG, "<<onKeyEvent invalid block");
                }
            } else if (LogUtils.mIsDebug) {
                LogUtils.m1571e(TAG, "sendKeyEventPingback mCurrentVideo is null");
            }
        }
    }
    ...
    public void showTip(ITip origintip) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showTip:" + (this.mTip == null ? null : this.mTip.toString()));
        }
    }
    ...
    public boolean handleJsKeyEvent(KeyEvent event) {
        ...
        switch (event.getKeyCode()) {
            ...
            case 66:
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(TAG, "handleJsKeyEvent: handled");
                }
                return true;
            default:
                return false;
    ...
    public int getClickThroughAdType() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getClickThroughAdType:" + type);
        }
        return type;
    }
    ...
    public void setOnUserPlayPauseListener(OnUserPlayPauseListener onUserPlayPauseListener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setOnUserPlayPauseListener()" + onUserPlayPauseListener);
        }
