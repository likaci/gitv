...
public class OpenPlayListener extends VoiceListener {
    ...
    private class MyEpisodeListener implements IApiCallback<ApiResultEpisodeList> {
        ...
        public void onException(ApiException e) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1569d(OpenPlayListener.TAG, "onException()", e);
            }
            this.mResult = null;
        }
        ...
        public void onSuccess(ApiResultEpisodeList result) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(OpenPlayListener.TAG, "onSuccess() result=" + result);
            }
            this.mResult = result;
        }
        ...
        public Album getVideo(int episodeIndex) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(OpenPlayListener.TAG, "getVideo() return=" + video);
            }
            return video;
        }
    }
    ...
    protected List<AbsVoiceAction> doOpenAction() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "OpenPlayHelper/getSupportedVoices()/TYPE_PLAY");
        }
        ...
        try {
            actions.add(new AbsVoiceAction(VoiceEventFactory.createVoiceEvent(16, "")) {
                protected boolean dispatchVoiceEvent(VoiceEvent event) {
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(OpenPlayListener.TAG, "OpenPlayHelper/dispatchVoiceEvent()/TYPE_PLAY");
                    }
                    return OpenPlayListener.this.dispatchVoiceEvent(event);
                }
            });
        } catch (Exception e) {
            LogUtils.m1571e(TAG, "OpenPlayHelper/getSupportedVoices()/TYPE_PLAY Exception e = " + e.getMessage());
            e.printStackTrace();
        }
        return actions;
    }
    ...
    private boolean dispatchVoiceEvent(final VoiceEvent event) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "dispatchVoiceEvent(return=" + handled + ")");
        }
        return handled;
    }
    ...
    private boolean playNomal(String keyword) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "playNomal(return=" + handled + ")");
        }
        return handled;
    }
    ...
    private boolean playByEpisodeIndex(String keyword, int episodeIndex) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "playByEpisodeIndex(return=" + handled + ")");
        }
        return handled;
    }
    ...
    private boolean playByChannelName(String keyword, String channelName) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "playByChannelName(return=" + handled + ")");
        }
        return handled;
    }
    ...
    private boolean playByChannelAndEpisode(String keyword, String channelName, int episodeIndex) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "playByChannelAndEpisode(return=" + handled + ")");
        }
        return handled;
    }
    ...
    private Album fetchEpisodeItem(Album episode, int episodeIndex) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "fetchEpisodeItem(episode=" + episode + ", index=" + episodeIndex + ")");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "fetchEpisodeItem() return " + episode);
        }
        return episodeItem;
    }
}
