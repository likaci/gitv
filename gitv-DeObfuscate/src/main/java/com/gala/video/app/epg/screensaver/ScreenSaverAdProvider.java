...
class ScreenSaverAdProvider {
    ...
    class C07771 implements Runnable {
        ...
        public void run() {
            ...
            LogUtils.m1568d(ScreenSaverAdProvider.TAG, "fetchAdData, all raw screensaver ads count :" + ListUtils.getCount(ScreenSaverAdProvider.this.mRawAdInfoList));
            LogUtils.m1568d(ScreenSaverAdProvider.TAG, "fetchAdData, ad image downloaded success, the actual count of ads that can be shown :" + ListUtils.getCount(ScreenSaverAdProvider.this.mAdInfoListWithImageDownLoaded));
            LogUtils.m1568d(ScreenSaverAdProvider.TAG, "fetchAdData, download screensaver ad image cost : " + (SystemClock.elapsedRealtime() - startTime) + " ms");
        }
    }
    ...
    private void initDownloadDir() {
        ...
        LogUtils.m1568d(TAG, "initDownloadDir, path saving screensaver ad images :" + imgDir);
    }

    public boolean isAdPrepared() {
        LogUtils.m1568d(TAG, "isAdPrepared, screensaver ad is prepared: " + this.mIsAdPrepared);
    ...
    public void fetchAdData() {
        ThreadUtils.execute(new C07771());
    }

    private String requestAdData() {
        AdResult result = new AdResult();
        LogUtils.m1568d(TAG, "requestAdData, start requesting screen saver ad");
        result.ad = GetInterfaceTools.getIAdApi().getScreenSaverAds(AdsClient.getSDKVersion());
        if (result.isEmpty()) {
            LogUtils.m1577w(TAG, "requestAdData, the result of request ad is empty");
        }
        return result.ad;
    }

    private void parseScreenSaverAdJson(String adJson, AdsClient adsClient) {
        LogUtils.m1574i(TAG, "parseScreenSaverAdJson,");
        ...
        try {
            ...
            if (ListUtils.isEmpty((List) slotList)) {
                LogUtils.m1577w(TAG, "parseScreenSaverAdJson, no CupidAdSlot's list with the type of SLOT_TYPE_PAGE");
                return;
            }
            for (CupidAdSlot slot : slotList) {
                int slotId = slot.getSlotId();
                LogUtils.m1568d(TAG, "parseScreenSaverAdJson, CupidAdSlot id: " + slotId);
                List<CupidAd> cupidAdList = adsClient.getAdSchedules(slotId);
                if (ListUtils.isEmpty((List) cupidAdList)) {
                    LogUtils.m1577w(TAG, "parseScreenSaverAdJson, the CupidAd's list with " + slotId + " is empty");
                } else {
                    for (CupidAd cupidAd : cupidAdList) {
                        if (cupidAd == null) {
                            LogUtils.m1577w(TAG, "parseScreenSaverAdJson, the current CupidAd object is null.");
                        } else if ("screensaver".equals(cupidAd.getCreativeType())) {
                            Map screenAds = cupidAd.getCreativeObject();
                            if (ListUtils.isEmpty(screenAds)) {
                                LogUtils.m1577w(TAG, "parseScreenSaverAdJson, the current CupidAd@" + cupidAd.hashCode() + " Map is empty");
                            } else {
                                ...
                                LogUtils.m1568d(TAG, "screen saver advertisement info = " + screenSaverAdModel);
                            }
                        }
                    }
                }
            }
            LogUtils.m1568d(TAG, "parseScreenSaverAdJson, raw ad data size:" + ListUtils.getCount((List) this.mRawAdInfoList));
        } catch (JSONException e) {
            LogUtils.m1572e(TAG, "parseScreenSaverAdJson, JSONException : ", e);
        } catch (Exception e2) {
            LogUtils.m1572e(TAG, "parseScreenSaverAdJson, exception : ", e2);
        }
    }

    private void downloadAdImages() {
        if (ListUtils.isEmpty(this.mRawAdInfoList)) {
            LogUtils.m1577w(TAG, "downloadAdImages, the current raw ad data is empty");
            return;
        }
        ...
        while (it.hasNext()) {
            ...
            if (this.mImageDownLoader.downloadImage(model.getAdImageUrl(), filename)) {
                ...
                LogUtils.m1568d(TAG, "downloadAdImages, downloading one of the screen ad image success, file path = " + filename);
                this.mAdInfoListWithImageDownLoaded.add(model);
            } else {
                LogUtils.m1571e(TAG, "downloadAdImages, downloading one of the screen ad image failed, url = " + imageUrl);
            }
        }
        int count = ListUtils.getCount(this.mAdInfoListWithImageDownLoaded);
        LogUtils.m1568d(TAG, "downloadAdImages, downloading a round screen ad images finished, result size = " + count);
