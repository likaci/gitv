...
public class OpenPurchaseAction extends BaseAction {
    ...
    public void process(Context context, Intent intent, LoadingCallback loadingCallback) {
        try {
            LogUtils.m1568d("OpenPurchaseAction", "process(context,intent)");
            JSONObject playDict = OpenPlayIntentUtils.parsePlayInfo(intent.getExtras());
            if (!checkParamsValidity(playDict)) {
                LogUtils.m1571e("OpenPurchaseAction", "checkParamsValidity is false. ");
                if (loadingCallback != null) {
                    loadingCallback.onFail();
                    LogUtils.m1571e("OpenPurchaseAction", "loadingCallback.onCancel()...");
                }
            ...
            LogUtils.m1571e("OpenPurchaseAction", "process---exception = " + e.getMessage());
            e.printStackTrace();
            if (loadingCallback != null) {
                loadingCallback.onFail();
                LogUtils.m1571e("OpenPurchaseAction", "loadingCallback.onFail();");
            }
        }
    }

    void startPurchase(Context context) {
        LogUtils.m1574i("OpenPurchaseAction", "startPurchase");
    ...
    void checkAuthIdAndApiKey(final Context context, final LoadingCallback loadingCallback) {
        if (TVApiBase.getTVApiProperty().checkAuthIdAndApiKeyAvailable()) {
            LogUtils.m1568d("OpenPurchaseAction", "checkAuthIdAndApiKeyAvailable = true");
            startPurchase(context);
            return;
        }
        LogUtils.m1568d("OpenPurchaseAction", "checkAuthIdAndApiKeyAvailable = false, start DeviceCheck...");
        TVApi.deviceCheckP.callSync(new IApiCallback<ApiResultDeviceCheck>() {
            public void onSuccess(ApiResultDeviceCheck apiResult) {
                LogUtils.m1568d("OpenPurchaseAction", "onDeviceCheck-onSuccess");
                OpenPurchaseAction.this.startPurchase(context);
            }

            public void onException(ApiException e) {
                LogUtils.m1571e("OpenPurchaseAction", "onDeviceCheck-onException e=" + e);
                e.printStackTrace();
                if (!NetworkUtils.isNetworkAvaliable()) {
                    LogUtils.m1571e("OpenPurchaseAction", "TVApi--IApiCallback--NetworkUtils.isNetworkAvaliable() =false");
