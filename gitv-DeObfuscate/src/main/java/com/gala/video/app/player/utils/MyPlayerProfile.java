...
public class MyPlayerProfile implements IPlayerProfile {
    ...
    class C15761 implements StateCallback {
        C15761() {
        }
    ...
    class C15772 extends HashMap<String, String> {
        C15772() {
    ...
    public boolean isNetworkAvaliable() {
        if (!Project.getInstance().getBuild().supportPlayerMultiProcess() || sNetworkManagerReturned) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "isNetworkAvaliable() returns " + available + ", supportPlayerMultiProcess=" + Project.getInstance().getBuild().supportPlayerMultiProcess() + ", sNetworkManagerReturned=" + sNetworkManagerReturned + ", state=" + state);
            }
            return available;
        }
        NetWorkManager.getInstance().checkNetWork(new C15761());
        if (!LogUtils.mIsDebug) {
            return true;
        }
        LogUtils.m1568d(TAG, "isNetworkAvaliable() returns true, (multi-process and networkmanager not returned)");
        return true;
    }
    ...
    public boolean shouldResetSurface() {
        boolean ret = false;
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "shouldResetSurface: model=" + Build.MODEL);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "shouldResetSurface: return " + ret);
        }
        return ret;
    }
    ...
    public boolean isPayBeforePreview() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isPayBeforePreview():provider.getDynamicQDataModel=" + provider.getDynamicQDataModel().getPayBeforePreview());
        }
    ...
    public boolean isPayAfterPreview() {
        IDynamicQDataProvider provider = GetInterfaceTools.getIDynamicQDataProvider();
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isPayAfterPreview():provider.getDynamicQDataModel=" + provider.getDynamicQDataModel().getPayAfterPreview());
        }
        if (provider.getDynamicQDataModel() != null) {
    ...
    public boolean isDisableSafeMode() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isDisableSafeMode() return " + isDisableSafeMode);
        }
    ...
    public boolean isDisableAdvanceMode() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isDisableAdvanceMode() return " + isDisableAdvanceMode);
        }
    ...
    public boolean canSeekBeforeStart() {
        ...
        MyLogUtils.m462d(TAG, "canSeekBeforeStart from DebugOptions!");
        return false;
    }
    ...
    public int getSurfaceFormat() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getSurfaceFormat(), shouldChangeSurfaceFormat from package script:" + shouldChangeSurfaceFormat);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getSurfaceFormat(), return " + surfaceFormat);
        }
    ...
    public void setUserTypeJson(String s) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> setUserTypeJson(" + s + ")");
        }
    ...
    public String getPassportDeviceId() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getPassportDeviceId(), deviceId=" + TVApiBase.getTVApiProperty().getPassportDeviceId());
        }
    ...
    public int getRetryTimesInPlaying() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getRetryTimesInPlaying: retry_times_after_started=" + result.getRetryTimesAfterStarted());
        }
    ...
    public int getRetryTimesInPreparing() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getRetryTimesInPreparing: retry_times_before_started=" + result.getRetryTimesBeforeStarted());
        }
    ...
    public boolean isDisableP2PUpload() {
        ...
        LogUtils.m1568d(TAG, "isDisableP2PUpload=" + isDisableP2PUpload);
    ...
    public boolean isEnableH211() {
        if (DebugOptionsCache.isEnableH211()) {
            MyLogUtils.m462d(TAG, "isEnableH211 from DebugOptions");
            return true;
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "dynamicResultSupportH211 = " + isDynamicSupportH211 + ", deviceSupportH211=" + isDeviceSupportH211);
        }
    ...
    public boolean isOpen4kStream() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isOpen4kStream ret=" + ret + ", flag=" + flag);
        }
        return ret;
    }
    ...
    public boolean isPreferNativePlayerSafeModeFor4K() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isPreferSystemFor4K, return " + isPreferNativePlayerSafeModeFor4K);
        }
    ...
    public boolean showEpisodeAsGallery(int chnId) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isEpisodesWithImageChannelIds: ret=" + ret);
        }
        return ret;
    }
    ...
    public boolean isCollectNetDoctorInfoOnError() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isCollectNetDoctorInfoOnError(), configEnable:" + configEnable + "dynamicSwitcherEnable:" + dynamicSwitcherEnable);
        }
        if (configEnable) {
    ...
    public boolean isEnableDolby() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "profile isEnableDolby =  return " + isEnableDolby);
        }
    ...
    public boolean isEnableHCDNPreDeploy() {
        ...
        if (Project.getInstance().getBuild().isEnableHCDNPreDeploy()) {
            ...
            LogUtils.m1568d(TAG, "isEnableHCDNPreDeploy(), glay scale switcher return " + enable);
            return enable;
        }
        LogUtils.m1568d(TAG, "isEnableHCDNPreDeploy(), config return false");
        return false;
    }
    ...
    public void onLoginSuccess(Map<String, String> info) {
        if (info != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "onLoginSuccess() cookie:" + cookie + ", uid=" + uid + ", account=" + account + ", nickName=" + ", vipDate=" + vipDate + ", userTypeH5=" + userTypeH5 + ", isLitchiH5=" + isLitchiH5);
            }
    ...
    public void onPurchaseSuccess(Map<String, String> info) {
        if (info != null) {
            ...
            boolean isLitchiH5 = StringUtils.equals((CharSequence) info.get("isLitchiH5"), "true");
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "onPurchaseSuccess() cookie:" + cookie + ", uid=" + uid + ", account=" + account + ", nickName=" + ", vipDate=" + vipDate + ", userTypeH5=" + userTypeH5 + ", isLitchiH5=" + isLitchiH5);
            }
            LoginParam4H5 param = new LoginParam4H5();
            param.cookie = cookie;
            param.uid = uid;
    ...
    public boolean isOpenDrmRootCheck() {
        ...
        LogUtils.m1568d(TAG, "isOpenDrmRootCheck=" + isOpenDrmRootCheck);
    ...
    public boolean isOpenAdVipGuide() {
        ...
        LogUtils.m1568d(TAG, "isOpenAdVipGuide=" + isOpenAdVipGuide);
    ...
    public String getAdVipGuideTipText() {
        ...
        LogUtils.m1568d(TAG, "getAdGuideTipText=" + adGuideTipText);
        return adGuideTipText;
    }

    public boolean isDisableADCache() {
        if (DebugOptionsCache.isCloseADCache()) {
            MyLogUtils.m462d(TAG, "isDisableADCache from DebugOption");
            return true;
        }
        ...
        LogUtils.m1568d(TAG, "isDisableADCache() isDynamicDisableADCache=" + isDynamicDisableADCache + ", isDeviceDisableADCache=" + isDeviceDisableADCache);
    ...
    public boolean isEnableLocalServerF4v2Hls() {
        ...
        if (DebugOptionsCache.isEnableADLocalServer()) {
            MyLogUtils.m462d(TAG, "isEnableLocalServerF4v2Hls from DebugOption");
        } else {
            ...
            MyLogUtils.m462d(TAG, "isEnableLocalServer(), isPlayerConfigOpen:" + isPlayerConfigOpen + ", isGrayScaleOpen:" + isGrayScaleOpen + ", return " + isEnableLocalServer);
        }
    ...
    public boolean isDeviceSupportDolbyVision() {
        ...
        MyLogUtils.m462d(TAG, "isDeviceSupportDolbyVision=" + isDeviceSupportDolbyVision);
    ...
    public boolean isDeviceSupportHDR10() {
        ...
        MyLogUtils.m462d(TAG, "isDeviceSupportHDR10=" + isDeviceSupportHDR10);
    ...
    public boolean isEnableHcdnMultiProcess() {
        ...
        MyLogUtils.m462d(TAG, "isEnableHcdnMultiProcess(), isGrayScaleEnable:" + isGrayScaleEnable + ", isPlayerConfigEnable:" + isPlayerConfigEnable + ", return " + isEnableMultiProcess);
    ...
    public String getPlayerTipCollections() {
        ...
        MyLogUtils.m462d(TAG, "getPlayerTipCollections():" + str);
        return str;
    }
}
