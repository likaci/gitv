...
public class StartOperateImageProvider {
    ...
    public void download(List<ChannelLabel> channelLabels) {
        ...
        LogUtils.m1568d(TAG, "download, all raw start operate label size :" + labelSize);
        ...
        DownloaderAPI.getDownloader().loadFiles(requestList, new IFileCallback() {
            ...
            public void onFailure(FileRequest fileRequest, Exception e) {
                ...
                LogUtils.m1568d(StartOperateImageProvider.TAG, "download, IFileCallback---OnFailure,url ," + (fileRequest != null ? fileRequest.getUrl() : "FileRequest is null"));
            }
        });
    }

    private void downloadFinished(List<StartOperateImageModel> tempStartOperateImageModels) {
        LogUtils.m1568d(TAG, "downloadFinished, start operate data size : " + ListUtils.getCount((List) tempStartOperateImageModels));
    ...
    public void onEmptyStartOperateData() {
        LogUtils.m1568d(TAG, "onEmptyStartOperateData");
    ...
    public StartOperateImageModel getStartOperateImageModel() {
        ...
        LogUtils.m1568d(TAG, "getStartOperateImageModel, index = " + index);
    ...
    private synchronized int getLocalTestIndex() {
        ...
        try {
            ...
            LogUtils.m1578w(TAG, "getLocalTestIndex, read test index failed", e);
        }
        return index;
    }
    ...
    private synchronized void writeTestIndexToLocal() {
        try {
            ...
            LogUtils.m1578w(TAG, "writeTestIndexToLocal, write test index failed", e);
        }
    }

    private synchronized void writeStartOperateImageModelListToCache(List<StartOperateImageModel> StartOperateImageModels) {
        LogUtils.m1568d(TAG, "writeStartOperateImageModelListToCache...");
        try {
            ...
            LogUtils.m1578w(TAG, "writeStartOperateImageModelListToCache, write start operate data failed", e);
        }
    }
    ...
    private synchronized List<StartOperateImageModel> getLocalStartOperateModelList() {
        ...
        try {
            ...
            LogUtils.m1572e(TAG, "getLocalStartOperateModelList, read start operate data failed", e);
        }
        LogUtils.m1568d(TAG, "getLocalStartOperateModelList, size = " + (!ListUtils.isEmpty(labelList) ? labelList.size() : 0));
        return labelList;
    }
