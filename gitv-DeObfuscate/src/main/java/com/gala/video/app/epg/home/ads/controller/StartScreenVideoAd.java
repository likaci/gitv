...
public class StartScreenVideoAd implements IStartScreenAd {
    ...
    private MyObserver mObserver = new C05715();
    private Runnable mOnlyLoadPluginRunnable = new C05704();
    private String mPingbackBlock = "";
    private String mPlayUrl = "";
    private IGalaVideoPlayer mPlayer;
    private OnAdSpecialEventListener mSpecialEventListener = new C05671();
    private long mStartPlayTime = 0;
    private long mStartRequestTime = 0;
    private Handler mTimeHandler;
    private Runnable mTimeRunnable = new C05693();
    private OnPlayerStateChangedListener mVideoStateListener = new C05682();

    class C05671 implements OnAdSpecialEventListener {
        C05671() {
        }

        public void onAdSpecialEvent(AdSpecialEvent event) {
            LogUtils.m1568d(StartScreenVideoAd.TAG, "onAdSpecialEvent event=" + event);
    ...
    class C05682 implements OnPlayerStateChangedListener {
        C05682() {
        }

        public void onVideoStarted(IVideo iVideo) {
            LogUtils.m1568d(StartScreenVideoAd.TAG, "onVideoStarted");
        }
        ...
        public void onPlaybackFinished() {
            LogUtils.m1568d(StartScreenVideoAd.TAG, "onPlaybackFinished");
        ...
        public boolean onError(IVideo iVideo, ISdkError iSdkError) {
            LogUtils.m1568d(StartScreenVideoAd.TAG, "onError ");
            HomeController.sUIEvent.post(UIEventType.BUILD_UI_RESUME, null);
            HomePingbackFactory.instance().createPingback(CommonPingback.DATA_ERROR_PINGBACK).addItem("ec", "ad_start_vid").addItem("pfec", "wrong").addItem(Keys.f2035T, "0").setOthersNull().post();
            return false;
        }

        public void onAdStarted() {
            LogUtils.m1568d(StartScreenVideoAd.TAG, "onAdStarted");
        }

        public void onAdEnd() {
            LogUtils.m1568d(StartScreenVideoAd.TAG, "on ad play end play cost : " + (System.currentTimeMillis() - StartScreenVideoAd.this.mStartPlayTime) + " ms");
            if (StartScreenVideoAd.this.mCallBack != null) {
                StartScreenVideoAd.this.mCallBack.onFinished();
            }
        ...
        public void onPrepared() {
            ...
            LogUtils.m1568d(StartScreenVideoAd.TAG, "[StartScreenVideoAd-Performance]MediaPlayer.onPrepared timeCost=" + (currentTime - StartScreenVideoAd.this.mCreatePlayerReadyTime));
            LogUtils.m1568d(StartScreenVideoAd.TAG, "onPrepared,prepared time cost : " + totalCost + " ms");
            HomeController.sUIEvent.post(UIEventType.BUILD_UI_RESUME, null);
    ...
    class C05693 implements Runnable {
        C05693() {
        }
        ...
        public void run() {
            ...
            if (StartScreenVideoAd.this.mCallBack != null && !StartScreenVideoAd.this.mIsAdPrepared) {
                LogUtils.m1577w(StartScreenVideoAd.TAG, "[StartScreenVideoAd-Performance]on ad play time out");
                StartScreenVideoAd.this.mCallBack.onTimeOut();
                HomeController.sUIEvent.post(UIEventType.BUILD_UI_RESUME, null);
                HomePingbackFactory.instance().createPingback(CommonPingback.DATA_ERROR_PINGBACK).addItem("ec", "ad_start_vid").addItem("pfec", PingbackConstants.ACT_MIXER_TIMEOUT).addItem(Keys.f2035T, "0").setOthersNull().post();
    ...
    class C05704 implements Runnable {
        C05704() {
        }

        public void run() {
            LogUtils.m1568d(StartScreenVideoAd.TAG, "load player plugin");
    ...
    class C05715 implements MyObserver {
        C05715() {
        }

        public void update(String event) {
            LogUtils.m1568d(StartScreenVideoAd.TAG, "receive message " + event);
    ...
    private class LoadPlayerFeatureRunnable implements Runnable {
        ...
        public void run() {
            ...
            GetInterfaceTools.getPlayerFeatureProxy().getPlayerFeatureOnlyInitJava();
            LogUtils.m1568d(StartScreenVideoAd.TAG, "getPlayerFeatureOnlyInitJava cost " + (SystemClock.elapsedRealtime() - start));
    ...
    public void loadData(long requestInterval) {
        LogUtils.m1568d(TAG, "load video ad data");
        ...
        LogUtils.m1568d(TAG, ">>has video ad playUrl: " + this.mPlayUrl + ",adid = " + this.mAdId);
        ...
        LogUtils.m1568d(TAG, "[StartScreenVideoAd-Performance]loadData timeCost=" + (SystemClock.elapsedRealtime() - startTime));
        LogUtils.m1568d(TAG, "<<has video ad : " + hasAd);
    ...
    private void sendPingback(long elapse, boolean hasAd) {
        HomePingbackFactory.instance().createPingback(CommonPingback.AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_startapk").addItem("st", hasAd ? "vid_local" : "vid_download").addItem("td", String.valueOf(elapse)).addItem("r", "vid").addItem(Keys.f2035T, "11").addItem("ct", "150619_request").setOthersNull().post();
    }
    ...
    public boolean dispatchKeyEvent(KeyEvent keyEvent) {
        ...
        LogUtils.m1568d(TAG, "dispatch keyevent to player,return : " + handled);
        return handled;
    }
    ...
    private void sendPrepareCostPingback(long cost) {
        HomePingbackFactory.instance().createPingback(CommonPingback.LOAD_FINISHED_PINGBACK).addItem("td", String.valueOf(cost)).addItem(Keys.LDTYPE, "ad_startapk_vid_content").addItem(Keys.f2035T, "11").addItem("ct", "160602_load").setOthersNull().post();
    }

    private void sendLoadPluginCostPingback(long cost) {
        HomePingbackFactory.instance().createPingback(CommonPingback.LOAD_FINISHED_PINGBACK).addItem("td", String.valueOf(cost)).addItem(Keys.LDTYPE, "ad_startapk_vid_plugin").addItem(Keys.f2035T, "11").addItem("ct", "160602_load").setOthersNull().post();
    }

    private void prepare(ViewGroup container) {
        LogUtils.m1568d(TAG, "prepare");
        if (this.mHasAd) {
    ...
    private void parseClickInfo(CupidAd cupidAd) {
        ...
        if (cupidAd != null) {
            ...
            LogUtils.m1568d(TAG, "video click through type = " + clickThroughType);
    ...
    private void startPlayer(ViewGroup container) {
        if (!this.mIsTimeOut) {
            ...
            LogUtils.m1568d(TAG, "container@" + container);
            container.setVisibility(0);
            LogUtils.m1568d(TAG, "start ad play play url: " + this.mPlayUrl);
            ...
            LogUtils.m1568d(TAG, "[StartScreenVideoAd-Performance]PlayerFeature.createPlayer timeCost=" + (this.mCreatePlayerReadyTime - this.mStartPlayTime));
        }
    }
}
