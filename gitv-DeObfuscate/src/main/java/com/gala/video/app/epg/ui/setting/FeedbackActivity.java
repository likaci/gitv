...
public class FeedbackActivity extends QMultiScreenActivity {
    ...
    class C10474 implements Runnable {
        C10474() {
        }

        public void run() {
            FeedbackActivity.this.mQRColorBgView.setBackgroundColor(ResourceUtil.getColor(C0508R.color.transparent));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(C0508R.layout.epg_activity_feedback);
        initView();
        loadData();
    }
    ...
    private void loadData() {
        ...
        ThreadUtils.execute(new C10441());
    }

    private void setDocumentContent() {
        CharSequence messageInfo = fetchMessageInfo();
        LogUtils.m1576i(LOG_TAG, "setDocumentContent --- messageInfo = ", messageInfo);
    ...
    private String fetchMessageInfo() {
        if (this.mDynamicResult != null) {
            ...
            if (!StringUtils.isEmpty(document)) {
                LogUtils.m1576i(LOG_TAG, "fetchMessageInfo --- mDynamicResult.document = ", document);
                return document.replace("\\n", "<br />");
            }
        }
        LogUtils.m1571e(LOG_TAG, "fetchMessageInfo --- mDynamicResult or mDynamicResult.document is empty");
        return null;
    }

    private void setText(final TextView textView, final String s) {
        if (textView == null || StringUtils.isEmpty((CharSequence) s)) {
            LogUtils.m1573e(LOG_TAG, "setText --- textView = ", textView, " s = ", s);
            return;
        }
    ...
    private StringBuilder assembleUrl(StringBuilder url) {
        if (StringUtils.isEmpty((CharSequence) url)) {
            LogUtils.m1571e(LOG_TAG, "assembleUrl --- url is empty");
            return null;
        }
        ...
        LogUtils.m1576i(LOG_TAG, "after assembleUrl the url is ", url);
        return url;
    }
    ...
    public void assembleFirstKeyValuePair(StringBuilder url, String key, String value) {
        if (StringUtils.isEmpty((CharSequence) url) || StringUtils.isEmpty((CharSequence) key)) {
            LogUtils.m1571e(LOG_TAG, "assembleFirstKeyValuePair --- url or key is empty");
        } else {
    ...
    public static void assembleOtherKeyValuePair(StringBuilder url, String key, String value) {
        if (StringUtils.isEmpty((CharSequence) url) || StringUtils.isEmpty((CharSequence) key)) {
            LogUtils.m1571e(LOG_TAG, "assembleOtherKeyValuePair --- url or key is empty");
        } else {
            url.append("&").append(key).append(SearchCriteria.EQ).append(value);
        }
    }
    ...
    private void setQRViewBitmap() {
        final Bitmap bitmap = QRUtils.createQRImage(fetchFeedbackUrl(), ResourceUtil.getDimen(C0508R.dimen.dimen_343dp), ResourceUtil.getDimen(C0508R.dimen.dimen_343dp));
        if (bitmap == null) {
            LogUtils.m1571e(LOG_TAG, "setQRViewBitmap --- bitmap is null");
            setDefaultQRBitmap();
            return;
        }
        ...
        runOnUiThread(new Runnable() {
            public void run() {
                ...
                FeedbackActivity.this.mQRColorBgView.setBackgroundColor(ResourceUtil.getColor(C0508R.color.gala_write));
            }
        });
    }

    private void initView() {
        this.mProgressBarLayout = findViewById(C0508R.id.epg_progressbar_layout);
        this.mProgressBar = findViewById(C0508R.id.epg_fb_progressbar);
        this.mProgressBarTextView = (TextView) findViewById(C0508R.id.epg_fb_progressbar_textview);
        this.mQRView = (ImageView) findViewById(C0508R.id.epg_fb_qr);
        this.mQRColorBgView = findViewById(C0508R.id.epg_fb_qr_color_bg);
        this.mMessageInfoTextView = (TextView) findViewById(C0508R.id.epg_fb_message_info);
        this.mQRColorBgView.setBackgroundColor(ResourceUtil.getColor(C0508R.color.transparent));
    }

    private void setDefaultQRBitmap() {
        LogUtils.m1568d(LOG_TAG, "setDefaultQRBitmap");
        runOnUiThread(new C10474());
    }

    protected View getBackgroundContainer() {
        return findViewById(C0508R.id.epg_fb_main_layout);
    }
}
