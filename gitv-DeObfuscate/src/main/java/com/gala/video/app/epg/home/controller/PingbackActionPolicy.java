...
public class PingbackActionPolicy extends ActionPolicy {
    ...
    private MyObserver mStartPreviewObserver = new C06161();

    class C06161 implements MyObserver {
        C06161() {
        }
        ...
        if (!isItemViewVisiable(blocksView, position, false)) {
            ...
            for (Entry<String, Integer> entry : UikitSourceDataCache.getInvocationBannerMap(defaultTabResourceGroupId).entrySet()) {
                ...
                if (((Integer) entry.getValue()).intValue() == line) {
                    ...
                    LogUtils.m1568d(str, append.append(adZoneId).append(", ad resultId = ").append(resultId).toString());
                }
            }
        }
    }
            ...
            if (baseActionModel != null) {
                if (ItemDataType.BANNER_IMAGE_AD.equals(baseActionModel.getItemType())) {
                    ...
                    if (!fullyVisible) {
                        ...
                        LogUtils.m1568d(this.TAG, "checkAndSendBannerAdShowPingback, banner ad (id = " + bannerAdId + ")is not visible now");
                        ...
                        if (HomeConstants.mIsStartPreViewFinished) {
                            ...
                            LogUtils.m1568d(this.TAG, "checkAndSendBannerAdShowPingback, send banner ad show pingback, item line : " + item.getLine() + ", card line : " + card.getLine() + ", onAdStarted:" + bannerAdId);
                            return;
                        }
                        LogUtils.m1568d(this.TAG, "checkAndSendBannerAdShowPingback, start preview page not finished, not send banner show pingback");
                    }
                }
            }
        }
                        ...
                        if (card == null) {
                            ...
                            if (((isScroll && !isItemViewVisiable(blocksView, i, true)) || !isScroll) && this.mShowList.indexOfKey(key) >= 0) {
                                if (System.currentTimeMillis() - ((Long) this.mShowList.get(key)).longValue() > 500) {
                                    ...
                                    if (dataList.isEmpty()) {
                                        LogUtils.m1577w(this.TAG, "onSendCardShowPingback. CardInfoModel List isEmpty ; dataIndex = -1");
                                        resourceIndex = -1;
                                    } else {
                                    ...
                                    if (cardList.isEmpty()) {
                                        LogUtils.m1577w(this.TAG, "onSendCardShowPingback. cardList List isEmpty ; cardIndex = -1");
                                    ...
                                    if (cardInfoModel.getCardType() == (short) 102) {
                                        ...
                                        if (TextUtils.equals("banner", cardInfoModel.mSource)) {
                                            block = "通栏广告";
                                            qpid = "通栏广告";
                                            LogUtils.m1568d(this.TAG, "send banner ad show ping back");
                                        }
                                    }
        ...
        try {
            ...
            if (item != null) {
                ...
                if (card != null && (card instanceof CoverFlowCard) && isItemViewVisiable(cast((ViewGroup) parent), first, false)) {
                    LogUtils.m1571e(this.TAG, "onCoverFlowAdShow,first=" + first + ",card=" + card);
                    ((CoverFlowCard) card).sendAdPingback();
                }
            }
        } catch (Exception e) {
            LogUtils.m1571e(this.TAG, "onCoverFlowAdShow.Exception e.getMessage()= " + e.getMessage());
            e.printStackTrace();
        }
    }
}
