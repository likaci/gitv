...
public class StarListAdapter extends Adapter<DetailViewHolder> {
    private static final int CIRCLE_SIZE = ResourceUtil.getDimen(C1291R.dimen.dimen_153dp);
    ...
    class C15341 implements IImageCallback {
        C15341() {
        }
            ...
            if (bitmap != null) {
                ...
                if (cookie != null) {
                    ...
                    if (url == null || url.equals(rightUrl)) {
                        ...
                        LogUtils.m1568d(StarListAdapter.TAG, "--return---current.url=" + url + "---right.url=" + rightUrl);
                    }
                } else if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(StarListAdapter.TAG, "loadBitmap >> onSuccess-------  cookie = null !! ");
                }
            } else if (LogUtils.mIsDebug) {
                LogUtils.m1568d(StarListAdapter.TAG, "loadBitmap >> onSuccess-------  netBitmap = null !! ");
            }
        }
    ...
    public DetailViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> onCreateViewHolder");
        }
    ...
    private DetailViewHolder createCircleView() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> createCircleView");
        }
    ...
    public void onBindViewHolder(DetailViewHolder detailViewHolder, int i) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> onBindViewHolder");
        }
    ...
    private void setViewContent(int index, CirclePersonView view) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> setViewContent, index=" + index + ", view=" + view);
        }
        if (ListUtils.isEmpty(this.mDataList) || index >= getCount()) {
            showDefaultImage(view);
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "setViewContent, invalid index=" + index);
                return;
            }
            return;
        }
    ...
    private void updateMainImage(IStarData data, CirclePersonView view) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> updateMainImage");
        }
    ...
    private void loadImage(CirclePersonView view, String url) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> loadImage, view=" + view + ", url=" + url);
        }
        if (view == null) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "loadImage, view is null!");
            }
        } else if (StringUtils.isEmpty((CharSequence) url)) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "loadImage, invalid url=" + url);
            }
            showDefaultImage(view);
        } else {
            ...
            if (!this.mCanceledTask) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(TAG, "loadImage, sizedURL=" + sizedURL);
                }
                this.mImageProvider.loadImage(imageRequest, new C15341());
            } else if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "loadImage, mCanceledTask is true, return");
            }
        }
    }
    ...
    public void updateDataSet(List<IStarData> list) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> updateDataSet, list.size=" + list.size());
        }
    ...
    public void changeDataSet(List<IStarData> list) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> changeDataSet, list.size=" + list.size());
        }
        this.mDataList.clear();
        this.mDataList.addAll(list);
    ...
    public void onCancelAllTasks() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onCancelAllTasks");
        }
    ...
    public void onReloadTasks(List<View> list) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onReloadTasks");
        }
        reloadBitmap(list);
    }

    private void reloadBitmap(List<View> list) {
        LogUtils.m1571e(TAG, "reloadBitmap " + list);
        for (View view : list) {
