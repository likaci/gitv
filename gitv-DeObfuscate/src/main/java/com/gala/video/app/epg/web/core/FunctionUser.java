...
public class FunctionUser implements IFunUser {
    ...
    public void onLoginSuccess(String userInfo) {
        LogUtils.m1568d(TAG, "H5 onLoginSuccess");
    ...
    public void setActivityResult(String result, int resultCode) {
        LogUtils.m1568d(TAG, "H5 setActivityResult resultCode:" + resultCode);
        IWebBaseClickType type = new BuyActivityResult(this.mContext);
        WebBaseTypeParams typeParams = new WebBaseTypeParams();
        typeParams.setJsonString(result);
        type.onClick(typeParams);
        if (this.mContext instanceof Activity) {
            LogUtils.m1568d(TAG, "H5 setActivityResult setResult:" + resultCode);
    ...
    public void onPushMsg(String msg) {
        if (StringUtils.isEmpty((CharSequence) msg)) {
            LogUtils.m1571e(TAG, "onPushMsg msg is null");
            return;
        }
        ...
        try {
            ...
            LogUtils.m1572e(TAG, "parse message content occur exception :", e);
        }
        if (msgContent == null) {
            LogUtils.m1571e(TAG, "onPushMsg msgContent is null");
            return;
        }
        int i;
