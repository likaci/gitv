...
public class AppListActivity extends QMultiScreenActivity implements Observer, LoadAppCallback, OnClickListener, OnFocusChangeListener {
    ...
    private OnItemClickListener mOnItemClickListener = new C08843();
    private OnItemFocusChangedListener mOnItemFocusChangedListener = new C08832();
    protected int mSelectedPosition = 0;
    private int mTopCount = 0;

    class C08821 implements OnGlobalLayoutListener {
        C08821() {
        }
    ...
    class C08832 implements OnItemFocusChangedListener {
        C08832() {
        }
    ...
    class C08843 implements OnItemClickListener {
        C08843() {
        }
    ...
    protected void setAdapterData() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(LOG_TAG, "setAdapterData() -> setTotalDataSize -> mInfoList.count=" + count);
        }
    ...
    protected void setCountView() {
        this.mAllNumView.setText(this.mInfoList.size() + getString(C0508R.string.app_item_count));
    }

    protected void prepareView() {
        CharSequence name = getIntent().getStringExtra("channelName");
        this.mAllTitleName = (TextView) findViewById(C0508R.id.epg_app_title_text);
        if (!StringUtils.isEmpty(name)) {
            this.mAllTitleName.setText(name);
        }
        this.mAllNumView = (TextView) findViewById(C0508R.id.epg_app_sum_text);
        this.mMenuDesView = findViewById(C0508R.id.epg_app_page_menu_desc);
        setDescViewProperty(this.mMenuDesView);
        this.mAppMenu = (RelativeLayout) findViewById(C0508R.id.epg_app_manage_dialog);
        this.mAppNotice = (TextView) findViewById(C0508R.id.epg_app_name);
        this.mMenuTitle = (TextView) findViewById(C0508R.id.epg_text_menu_des);
        this.mBringToTopItem = (TextView) findViewById(C0508R.id.epg_top_btn);
        this.mBringUninstallItem = (TextView) findViewById(C0508R.id.epg_uninstall_btn);
        this.mBringInstallItem = (TextView) findViewById(C0508R.id.epg_install_btn);
    ...
    protected void setContentView() {
        setContentView(C0508R.layout.epg_activity_app_page);
    }

    protected void prepareListData() {
        LogUtils.m1568d(LOG_TAG, "prepareListData()");
    ...
    private void initGridView() {
        this.mGridView = (VerticalGridView) findViewById(C0508R.id.epg_app_gridview_layout);
        ...
        this.mGridView.setPadding(ResourceUtil.getDimen(C0508R.dimen.dimen_22dp), 0, ResourceUtil.getDimen(C0508R.dimen.dimen_4dp), 0);
        int ninePatchBorder = CloudUtils.calcNinePatchBorder(ResourceUtil.getDrawable(C0508R.drawable.share_item_rect_btn_selector));
        this.mGridView.setVerticalMargin(ResourceUtil.getDimen(C0508R.dimen.dimen_16dp) - (ninePatchBorder * 2));
        this.mGridView.setHorizontalMargin(ResourceUtil.getDimen(C0508R.dimen.dimen_16dp) - (ninePatchBorder * 2));
        LogUtils.m1576i(LOG_TAG, "ninePatchBorder = ", Integer.valueOf(ninePatchBorder));
        this.mGridView.setContentWidth(ResourceUtil.getDimen(C0508R.dimen.dimen_188dp) + (ninePatchBorder * 2));
        this.mGridView.setContentHeight(ResourceUtil.getDimen(C0508R.dimen.dimen_188dp) + (ninePatchBorder * 2));
        ...
        this.mGridView.getViewTreeObserver().addOnGlobalLayoutListener(new C08821());
    }

    protected View getBackgroundContainer() {
        return findViewById(C0508R.id.epg_app_container);
    }

    protected void onDestroy() {
        LogUtils.m1574i(LOG_TAG, "onDestroy()");
        super.onDestroy();
        try {
            if (this.mAppManager != null) {
                this.mAppManager.deleteObserver(this);
                LogUtils.m1574i(LOG_TAG, "onDestroy() -> mAppManager.deleteObservers()");
            }
        } catch (Exception e) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1572e(LOG_TAG, "onDestroy() e:", e);
            }
        }
    }

    public void update(Observable observable, Object data) {
        LogUtils.m1568d(LOG_TAG, "update()");
        if (data instanceof AppOperation) {
            ...
            if (LogUtils.mIsDebug) {
                ...
                if (app != null) {
                    LogUtils.m1568d(LOG_TAG, "update() -> app packageName=" + app.getAppPackageName() + "app name=" + app.getAppName());
                }
            }
            switch (type) {
                case 0:
    ...
    private void showAppMenu(AppInfo appInfo, int index) {
        ...
        if (this.mSelectedPosition < this.mTopCount) {
            this.mBringToTopItem.setBackgroundResource(C0508R.drawable.epg_applist_unuse_btn_selector);
        } else {
            this.mBringToTopItem.setBackgroundResource(C0508R.drawable.epg_applist_focus_btn_selector);
        }
        ...
        if (this.mIsUpdateSystem) {
            this.mBringUninstallItem.setBackgroundResource(C0508R.drawable.epg_applist_unuse_btn_selector);
        } else {
            this.mBringUninstallItem.setBackgroundResource(this.mIsSystemApp ? C0508R.drawable.epg_applist_unuse_btn_selector : C0508R.drawable.epg_applist_focus_btn_selector);
        }
    }
    ...
    public void onLoadDone(List<AppInfo> appInfos) {
        LogUtils.m1568d(LOG_TAG, "--onLoadDone() -> ");
        if (ListUtils.isEmpty((List) appInfos)) {
            LogUtils.m1571e(LOG_TAG, "onLoadDone() -> app list is empty !");
            return;
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(LOG_TAG, "onLoadDone() -> mAppInfos.size()  =" + appInfos.size());
        }
    ...
    public void onClick(View v) {
        ...
        } else if (i == C0508R.id.epg_uninstall_btn) {
            if (this.mSelectedPosition == -1) {
                return;
            }
        ...
        } else if (i == C0508R.id.epg_install_btn && this.mSelectedPosition != -1) {
    ...
    public void onFocusChange(View view, boolean hasFocus) {
        ...
        if (i == C0508R.id.epg_top_btn) {
            if (hasFocus) {
                this.mBringToTopItem.setTextColor(ResourceUtil.getColor(C0508R.color.card_title_color));
                if (!Project.getInstance().getBuild().isHomeVersion() || this.mTopCount <= 0 || this.mSelectedPosition >= this.mTopCount) {
                    this.mAppNotice.setText(getResources().getString(C0508R.string.app_dialog_text));
                } else {
                    this.mAppNotice.setText(getResources().getString(C0508R.string.app_lock_tip, new Object[]{this.mTopCount + ""}));
                }
                view.bringToFront();
                ((RelativeLayout) view.getParent()).invalidate();
            } else {
                this.mAppNotice.setText("");
                this.mBringToTopItem.setTextColor(ResourceUtil.getColor(C0508R.color.card_grey_color));
            }
        } else if (i == C0508R.id.epg_uninstall_btn) {
            if (hasFocus) {
                this.mBringUninstallItem.setTextColor(ResourceUtil.getColor(C0508R.color.card_title_color));
                view.bringToFront();
                ((RelativeLayout) view.getParent()).invalidate();
                if (this.mIsSystemApp) {
                    String str;
                    String head = getResources().getString(C0508R.string.app_menu_recovery_tip_head);
                    String tail = getResources().getString(C0508R.string.app_menu_recovery_tip_tail);
                    if (this.mIsUpdateSystem) {
                        str = "<font color= '#bebebe'>" + head + "</font>" + "<font color='#ffb400'>" + this.mAppName + "</font>" + "<font color= '#bebebe'>" + tail + "</font>";
                    } else {
                        str = "<font color='#ffb400'>" + this.mAppName + "</font>" + "<font color= '#bebebe'>" + getResources().getString(C0508R.string.app_menu_system_tip) + "</font>";
                    }
                    this.mAppNotice.setText(Html.fromHtml(str));
                } else {
                    this.mAppNotice.setText(Html.fromHtml("<font color= '#bebebe'>" + getResources().getString(C0508R.string.app_menu_uninstall_tip) + this.mAppName + "</font>"));
                }
            } else {
                this.mAppNotice.setText("");
                this.mBringUninstallItem.setTextColor(ResourceUtil.getColor(C0508R.color.card_grey_color));
            }
        } else if (i == C0508R.id.epg_install_btn && hasFocus) {
            view.bringToFront();
            ((RelativeLayout) view.getParent()).invalidate();
        }
    ...
    public List<AbsVoiceAction> getSupportedVoices() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(LOG_TAG, "AppListActivity/List<AbsVoiceAction> getSupportedVoices()");
        }
