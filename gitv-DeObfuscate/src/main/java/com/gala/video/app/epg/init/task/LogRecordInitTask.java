    ...
    class C07471 implements Runnable {
        ...
        public void run() {
            if (LogRecordPreference.getLogrecordOpen(AppRuntimeEnv.get().getApplicationContext())) {
                ...
                LogUtils.m1568d(LogRecordInitTask.TAG, "LogRecordFeatureProvider execute");
                return;
            }
            LogUtils.m1568d(LogRecordInitTask.TAG, "LogRecordPreference.getLogrecordOpen is false");
        }
    }

    public void run() {
        LogUtils.m1568d(TAG, "LogRecordInitTask execute");
        new Thread8K(new C07471(), TAG).start();
    }
