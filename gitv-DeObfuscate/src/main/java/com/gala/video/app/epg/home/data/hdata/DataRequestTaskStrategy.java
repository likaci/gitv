...
public final class DataRequestTaskStrategy {
    ...
    private onExecuteListener mCallBack = new C06221();
    ...
    IStatusListener mScreenSaverListener = new C06232();
    ...
    class C06221 implements onExecuteListener {
        C06221() {
        }

        public void onFinished() {
            DataRequestTaskStrategy.this.mTaskCount.decrementAndGet();
            LogUtils.m1568d(DataRequestTaskStrategy.TAG, "execute task action onFinished task count reserved = " + DataRequestTaskStrategy.this.mTaskCount.get());
        }
    }

    class C06232 implements IStatusListener {
        C06232() {
        }

        public void onStart() {
            LogUtils.m1568d(DataRequestTaskStrategy.TAG, "screen saver start");
        }

        public void onStop() {
            LogUtils.m1568d(DataRequestTaskStrategy.TAG, "screen saver stop");
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    e.printStackTrace();
                    LogUtils.m1571e(DataRequestTaskStrategy.TAG, "execute task exception : " + e);
                }
            }
        }
    }
    ...
    private class RefreshHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            LogUtils.m1568d(DataRequestTaskStrategy.TAG, "handle message action = " + action + " level = " + action.getPeriodismLevel());
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 100:
                    ...
                    LogUtils.m1568d(DataRequestTaskStrategy.TAG, "time is up to fall in in active state,do not refresh data");
                    return;
                default:
                    return;
            }
    ...
    public void notifyHomeKeyEvent() {
        LogUtils.m1568d(TAG, "notifyHomeKeyEvent");
    ...
    public void notifyHomeResumed() {
        LogUtils.m1568d(TAG, "notify home fragment resumed");
    ...
    public void notifyHomeStopped() {
        LogUtils.m1568d(TAG, "notify home fragment stopped");
        this.mIsInForeground = false;
    }

    private void doPendingTask() {
        if (!this.mPendingTaskActionsQueue.isEmpty()) {
            LogUtils.m1568d(TAG, "execute pending task ,task queue size = " + this.mPendingTaskActionsQueue.size());
            if (!this.mPendingTaskActionsQueue.isEmpty()) {
    ...
    private void doScheduleTaskForTimer(TaskAction action) {
        LogUtils.m1568d(TAG, "scheduleTask ,pendding queue size = " + this.mPendingTaskActionsQueue.size() + " shouldScheduleOnlyForEvent = " + shouldScheduleOnlyForEvent() + " isActive : " + this.mIsHomeActive);
        if ((this.mIsInForeground && this.mIsHomeActive && !shouldScheduleOnlyForEvent()) || -2 == action.getPeriodismLevel()) {
            LogUtils.m1568d(TAG, "task real executed action= " + action);
            this.mRunningTaskActionsQueue.add(action);
            this.mTaskCount.incrementAndGet();
            return;
        }
        ...
        LogUtils.m1568d(TAG, "added task to pending queue ,queue size = " + this.mPendingTaskActionsQueue.size());
    }
    ...
    private void scheduleNextTask(TaskAction action) {
        ...
        LogUtils.m1568d(TAG, "schedule next task ,level = " + level + " interval: " + interval + " action = " + action);
    }
    ...
    private boolean shouldSchedule() {
        ...
        LogUtils.m1568d(TAG, "should schedule : " + result);
        return result;
    }
    ...
    private boolean shouldScheduleOnlyForEvent() {
        ...
        LogUtils.m1568d(TAG, "should schedule only for event: " + result);
        return result;
    }

    public void start() {
        LogUtils.m1568d(TAG, "start task dispatcher");
    ...
    private void taskOneAndExecute() throws InterruptedException {
        ...
        LogUtils.m1568d(TAG, "execute task action = " + action + " running task size : " + this.mRunningTaskActionsQueue.size());
    ...
    private TaskAction findInFlightTaskAction(TaskAction action) {
        ...
        LogUtils.m1568d(TAG, "find in flight action@" + result);
        return result;
    }

    public boolean isAllTaskFinished() {
        LogUtils.m1568d(TAG, "isAllTaskFinished task reserved : " + this.mTaskCount.get());
