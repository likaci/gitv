...
public class InteractView extends FrameLayout {
    ...
    private Handler mMainHandler = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(InteractView.TAG, "handleMessage<<(msg=" + msg + ")");
            }
            switch (msg.what) {
                case 0:
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(InteractView.TAG, "handleMessage animationEnabled=" + InteractView.this.mPraiseText.animationEnabled());
                    }
    ...
    class C15531 implements OnClickListener {
        ...
        public void onClick(View v) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(InteractView.TAG, "onClick() mLastInteractItemView=" + InteractView.this.mLastInteractItemView);
            }
            if (InteractView.this.mPraiseText == null) {
                InteractView.this.mPraiseText = new MyTextView(InteractView.this.getContext());
                InteractView.this.mPraiseText.setTextSize(0, (float) InteractView.this.getResources().getDimensionPixelOffset(C1291R.dimen.live_praise_text_min_size));
                InteractView.this.mPraiseText.setGravity(17);
                InteractView.this.mPraiseText.setTextColor(InteractView.this.getResources().getColor(C1291R.color.praise_text_color));
                InteractView.this.mPraiseText.setVisibility(8);
            ...
            if (InteractView.this.mLastInteractItemView == null || !InteractView.this.mLastInteractItemView.handledClick()) {
                ...
                newItem.setDrawableResId(C1291R.drawable.player_ic_heart);
    ...
    private class MyTextView extends TextView {
        ...
        class C15562 implements AnimatorListener {
            C15562() {
            }
        ...
        public void setAnimationEnabled(boolean enabled) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(InteractView.TAG, "setAnimationEnabled=" + enabled);
            }
        ...
        public boolean animationEnabled() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(InteractView.TAG, "animationEnabled=" + this.mAnimationEnabled.get());
            }
        ...
        public void startPraiseAnimation() {
            ...
            scaleAnimation.addListener(new C15562());
    ...
    private void initView() {
        this.mInteractLayout = LayoutInflater.from(getContext()).inflate(C1291R.layout.player_layout_live_interact_view, null);
        addView(this.mInteractLayout, computeInteractLayoutParams());
        this.mInteractBtn = this.mInteractLayout.findViewById(C1291R.id.interact_btn);
        this.mInteractBtn.requestFocus();
        this.mInteractBtn.setOnClickListener(new C15531());
    }

    private LayoutParams computeInteractLayoutParams() {
        LayoutParams layoutParams = new LayoutParams(-2, -2, 85);
        layoutParams.rightMargin = getResources().getDimensionPixelOffset(C1291R.dimen.live_praise_margin_right);
        layoutParams.bottomMargin = getResources().getDimensionPixelOffset(C1291R.dimen.live_praise_margin_bottom);
        return layoutParams;
    }
    ...
    private LayoutParams computeInteractTextLayoutParams() {
        ...
        layoutParams.rightMargin = (this.mInteractLayout.getWidth() + params.rightMargin) + getResources().getDimensionPixelOffset(C1291R.dimen.live_praise_text_margin_right);
    ...
    private void notifyOnTranslate(InteractItemView view) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyOnTranslate<<(view=" + view + ")");
        }
        if (!(this.mOnTranslateListener == null || view == null)) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "notifyOnTranslate...[translateListener=" + this.mOnTranslateListener + ", clickCount=" + view.getClickCount() + AlbumEnterFactory.SIGN_STR);
            }
            this.mOnTranslateListener.onStartTranslate(view.getClickCount());
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyOnTranslate>>()");
        }
    }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        boolean handled;
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "dispatchKeyEvent<<(event=" + event + ")");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "dispatchKeyEvent>>() return " + handled);
        }
        return handled;
    }
}
