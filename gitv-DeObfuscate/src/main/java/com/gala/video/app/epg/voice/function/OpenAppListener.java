...
public class OpenAppListener extends VoiceListener {
    ...
    private final BroadcastReceiver mReceiver = new C12451();

    class C12451 extends BroadcastReceiver {

        class C12441 implements Runnable {
            C12441() {
            }
        ...
        public void onReceive(Context context, Intent intent) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(OpenAppListener.TAG, "receive intent = " + intent);
            }
            new Thread8K(new C12441()).start();
        }
    }
        ...
        try {
            ...
            synchronized (this.mMap) {
                ...
                for (int i = 0; i < packages.size(); i++) {
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(TAG, "prepare() appName=" + appName + ", packageName=" + packageName);
                    }
                }
            }
        } catch (Exception ex) {
            ...
            for (final String finalAppName : this.mMap.keySet()) {
                actions.add(new AbsVoiceAction(VoiceEventFactory.createVoiceEvent(4, finalAppName)) {
                    protected boolean dispatchVoiceEvent(VoiceEvent event) {
                        ...
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(OpenAppListener.TAG, "OpenAppHelper/dispatchVoiceEvent()");
                        }
