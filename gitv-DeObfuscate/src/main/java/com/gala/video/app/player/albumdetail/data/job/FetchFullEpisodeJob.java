...
public class FetchFullEpisodeJob extends AlbumJob {
    ...
    public void onRun(final JobController controller) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onRun tvCount = " + albumInfo.getTvCount());
        }
        ThreadUtils.execute(new Runnable() {
            public void run() {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(FetchFullEpisodeJob.TAG, "total = " + total + "ListUtils.getCount(video.getEpisodeVideos())=" + episodeSize + "maxOrder=" + maxOrder);
                }
    ...
    public void modifyVideoData(List<Episode> episodes, int total) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyJobSuccess(), episodes size=" + episodes.size() + "maxOrder" + maxOrder);
        }
