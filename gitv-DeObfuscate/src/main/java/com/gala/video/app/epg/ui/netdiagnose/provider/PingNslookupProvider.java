...
public class PingNslookupProvider extends NDBaseProvider {
    ...
    class C09501 implements INDDoneListener {
        C09501() {
        }

        public void onFinish(Map<String, Object> resultMap) {
            PingNslookupProvider.this.setReslut((NetDiagnoseInfo) resultMap.get("data"));
            LogUtils.m1574i(PingNslookupProvider.TAG, "onFinish mDnsListener success: " + ((Boolean) resultMap.get("success")).booleanValue());
        }
    }

    class C09512 implements INDDoneListener {
        C09512() {
        }

        public void onFinish(Map<String, Object> resultMap) {
            PingNslookupProvider.this.setReslut((NetDiagnoseInfo) resultMap.get("data"));
            LogUtils.m1574i(PingNslookupProvider.TAG, "onFinish mNsLookUp success: " + ((Boolean) resultMap.get("success")).booleanValue());
    ...
    public PingNslookupProvider(Context context) {
        super(context);
        this.mPingCallBack = new C09501();
        this.mNsLookUp = new C09512();
    }
