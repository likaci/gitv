...
public class AlbumDetailActivity extends QMultiScreenActivity implements IPingbackContext {
    ...
    private IStatusListener mScreenSaverStatusListener = new C12932();
    private IAlbumDetailUiConfig mUiConfig = new AlbumDetailUiConfig();
    private boolean mWindowHasFocus = false;

    class C12921 implements OnHomePressedListener {
        C12921() {
        }
    ...
    class C12932 implements IStatusListener {
        C12932() {
        }
    ...
    public boolean handleKeyEvent(KeyEvent event) {
        if (!this.mWindowHasFocus) {
            ...
            try {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(this.TAG, "handleKeyEvent, unhandled key event=" + event);
                }
                return super.handleKeyEvent(event);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private synchronized void registerHomeKeyForLauncher() {
        if (!this.mIsRegisterHomeMonitor) {
            this.mHomeMonitorHelper = new HomeMonitorHelper(new C12921(), this);
    ...
    private void initPingBack(Intent passedIntent) {
        ...
        PingbackItem e = C0164E.E_ID(passedIntent.getStringExtra("eventId"));
    ...
    private void sendCustomerPageExitPingback() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> sendCustomerPageExitPingback");
        }
    ...
    private void sendCustomerPageLoadedPingback() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> sendCustomerPageLoadedPingback");
        }
        if (!this.mPageLoadedAlreadySend) {
            ...
            LogUtils.m1568d(this.TAG, "sendCustomerPageLoadedPingback, send already.");
        }
    }
