...
public class VerticalScrollLayout extends LinearLayout {
    ...
    private static int sBottomScrollOffset = ResourceUtil.getDimen(C1291R.dimen.dimen_18dp);
    private static int sTopScrollOffset = ResourceUtil.getDimen(C1291R.dimen.dimen_4dp);
    private static int sTopTitleHeight = ResourceUtil.getDimen(C1291R.dimen.detail_top_title_height);
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        switch (event.getKeyCode()) {
            ...
            default:
                ...
                LogUtils.m1568d(TAG, "unhandled key event=" + event);
                return false;
        }
    }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (currentFocused != this) {
            ...
            if (nextFocused == null) {
                ...
                LogUtils.m1568d(TAG, "arrowScroll, return false, next focusable view is null.");
                return false;
                ...
                if (!up && getChildAt(getChildCount() - 1).getBottom() <= (getScrollY() + getHeight()) - getPaddingBottom()) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(TAG, "arrowScroll, return true, don't need scroll on down direction");
                    }
                    return true;
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(TAG, "arrowScroll, return true, don't need scroll on up direction");
                    }
                    return true;
                }
                ...
                LogUtils.m1568d(TAG, "arrowScroll, return false, request next focus failed.");
                return false;
            }
        } else if (!LogUtils.mIsDebug) {
            return false;
        } else {
            LogUtils.m1568d(TAG, "arrowScroll, return false, focus on this VerticalScrollView.");
            return false;
        }
    }
    ...
    private void doScrollY(int delta) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> doScrollY, delta=" + delta);
        }
        if (delta != 0) {
