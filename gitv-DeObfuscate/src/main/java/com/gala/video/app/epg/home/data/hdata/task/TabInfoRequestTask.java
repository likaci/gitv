...
public class TabInfoRequestTask extends BaseRequestTask {
    ...
    class C06551 implements IApiCallback<ApiResultTabInfo> {
        C06551() {
        }
        ...
        public void onException(ApiException e) {
            String str;
            LogUtils.m1571e(TabInfoRequestTask.TAG, "Get tab info failed");
    ...
    public void invoke() {
        LogUtils.m1568d(TAG, "invoke TabInfoRequestTask");
    ...
    private void buildTabInfo() {
        TVApi.tabInfo.callSync(new C06551(), new String[0]);
    }
                        ...
                        if (cont.type1 == 7) {
                            if (Project.getInstance().getControl().isOpenCarousel() || cont.chnId != 1000005) {
                                if ("轮播".equals(cont.name)) {
                                    LogUtils.m1568d(TAG, "buildTabPageInfoList, get tab info, carousel tab channel id: " + cont.chnId);
                                }
                                ...
                                LogUtils.m1568d(TAG, "buildTabPageInfoList, not support carousel , remove carousel tab");
                            }
                        }
                    }
                            ...
                            if (cont2.type1 == 7) {
                                if (Project.getInstance().getControl().isOpenCarousel() || cont2.chnId != 1000005) {
                                    if ("轮播".equals(cont2.name)) {
                                        LogUtils.m1568d(TAG, "buildTabPageInfoList, get hide tab info, carousel tab channel id: " + cont2.chnId);
                                    }
                                    tabInfo = new TabModel();
                                    ...
                                    LogUtils.m1568d(TAG, "buildTabPageInfoList, get hide tab info, not support carousel , remove carousel tab");
                                }
                            }
                        }
                    }
    ...
    private void checkTabInfoChange(List<TabModel> pageInfoListCache, List<TabModel> pageInfoListNew) {
        ...
        LogUtils.m1568d(TAG, "new tab info list size=" + newLength);
        ...
        LogUtils.m1568d(TAG, "cache tab info list size=" + cacheLength);
        ...
        for (int i = 0; i < newLength; i++) {
                ...
                if (isSameTabs(cacheTab, newTab)) {
                    ...
                    if (i != j) {
                        ...
                        if (newTab.isVipTab() != cacheTab.isVipTab()) {
                            ...
                            LogUtils.m1568d(TAG, "tab layout change: is same tab but i!=j");
                        } else {
                    ...
                    if (isLayoutChanged(cacheTab, newTab)) {
                        ...
                        if (this.mStatus != WidgetChangeStatus.TabLayoutChange) {
                            this.mStatus = WidgetChangeStatus.TabLayoutChange;
                            LogUtils.m1568d(TAG, "tab layout change: is layout change");
                        }
                        if (!checkFlags.contains(cacheTab.getResourceGroupId())) {
                            checkFlags.add(cacheTab.getResourceGroupId());
                        }
                    ...
                    if (isDataChanged(cacheTab, newTab)) {
                        ...
                        if (newTab.isVipTab() != cacheTab.isVipTab()) {
                            this.mStatus = WidgetChangeStatus.TabLayoutChange;
                            newTab.setWidgetChangeStatus(WidgetChangeStatus.TabLayoutChange);
                            LogUtils.m1568d(TAG, "tab layout change: is data change but vip is changed");
                        } else {
                            newTab.setWidgetChangeStatus(WidgetChangeStatus.TabDataChange);
                            if (this.mStatus == WidgetChangeStatus.NoChange) {
                                this.mStatus = WidgetChangeStatus.TabDataChange;
            ...
            if (isNew) {
                ...
                if (this.mStatus != WidgetChangeStatus.TabLayoutChange) {
                    this.mStatus = WidgetChangeStatus.TabLayoutChange;
                    LogUtils.m1568d(TAG, "tab layout change: new tab");
                }
            }
        }
        if (checkFlags.size() < cacheLength) {
            this.mStatus = WidgetChangeStatus.TabLayoutChange;
            LogUtils.m1568d(TAG, "tab layout change: delete tab");
        }
    }
    ...
    public void onOneTaskFinished() {
        LogUtils.m1568d(TAG, "send tab info change event,event = " + this.mStatus);
