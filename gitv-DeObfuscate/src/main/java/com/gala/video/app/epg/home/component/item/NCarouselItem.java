...
public class NCarouselItem extends Item implements Presenter, IKeyDispatcher, OnClickListener, IActivityLifeCycle {
    ...
    class C05951 implements onWindowModeSwitchListener {
        C05951() {
        }

        public void onWindowModeSwitched(int current) {
            NCarouselItem.this.mCurrentScreenMode = current;
            LogUtils.m1568d(NCarouselItem.this.TAG, "onWindowModeSwitched current screen mode = " + NCarouselItem.this.mCurrentScreenMode);
        }
    }
    ...
    private class CarouseObserver implements MyObserver {
        ...
        public void update(String event) {
            ...
            if (!ThreadUtils.isUIThread()) {
                NCarouselItem.this.mPlayHandler.post(new C05961());
    ...
    private class MyPluginStateListener implements OnStateChangedListener {
        ...
        public void onSuccess() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NCarouselItem.this.TAG, "onSuccess: player plugin loaded success.");
            }
            if (NCarouselItem.this.onBindSuccess() && NCarouselItem.this.mView.isCoverAttached() && NCarouselItem.this.isPlayItemVisible()) {
                LogUtils.m1568d(NCarouselItem.this.TAG, "view is attached to window");
                NCarouselItem.this.mPlayHandler.sendEmptyMessageDelayed(102, 1200);
                return;
            }
            LogUtils.m1568d(NCarouselItem.this.TAG, "view is detached from window");
            NCarouselItem.this.mPlayHandler.removeCallbacksAndMessages(null);
        }

        public void onFailed() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NCarouselItem.this.TAG, "player plugin loaded failed.");
            }
        }
    ...
    private class PlayHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            boolean z = true;
            LogUtils.m1568d(NCarouselItem.this.TAG, "on recieve message  = " + msg.what);
            switch (msg.what) {
                case 100:
    ...
    public void onUnBind() {
        LogUtils.m1568d(this.TAG, "onUnBind");
        stopAfterUnBind();
    }

    private void stopAfterUnBind() {
        LogUtils.m1568d(this.TAG, "stopAfterUnBind");
        if (onBindSuccess()) {
            AppRuntimeEnv.get().setIsPlayInHome(false);
            if (this.mPlayHandler.hasMessages(103)) {
                LogUtils.m1568d(this.TAG, "handler has stop message,ignore current messsage!");
            } else {
    ...
    private boolean isPlayItemVisible() {
        ...
        LogUtils.m1568d(this.TAG, "is visible = " + isVisible);
    ...
    public void onPlayItemVisible() {
        LogUtils.m1574i(this.TAG, "onPlayItemVisible");
    ...
    public void carouseItemClick() {
        LogUtils.m1574i(this.TAG, "carouseItemClick: ");
        if (this.mPlayer != null) {
            Message msg = Message.obtain(this.mPlayHandler);
            msg.what = 106;
            msg.arg1 = 1;
    ...
    private void start() {
        if (isPlayItemVisible() && !this.mIsPaused) {
            ...
            this.mPlayViewParent = (FrameLayout) ((Activity) this.mContext).findViewById(C0508R.id.epg_carousel_window);
            LogUtils.m1568d(this.TAG, "cover view width = " + this.mView.getCoverWidth());
            if (this.mView.getCoverWidth() != 0 && this.mView.getCoverHeight() != 0) {
                ...
                this.mPlayer.setOnWindowChangeListener(new C05951());
                this.mView.setCoverImage(null);
                LogUtils.m1568d(this.TAG, "start current mode = " + this.mCurrentScreenMode);
    ...
    private ChannelCarousel getPlayInfo() {
        ...
        LogUtils.m1568d(this.TAG, "carousel playInfo : " + PlayInfo);
        return PlayInfo;
    }
    ...
    private LayoutParams createPlayLayout() {
        ...
        LogUtils.m1568d(this.TAG, "video player layout params (" + videoLayoutParams.width + "," + videoLayoutParams.height + "," + videoLayoutParams.leftMargin + "," + videoLayoutParams.topMargin + ")" + ",horizontal padding = " + 0 + ",vertical padding = " + 0);
    ...
    private void updateIncomeSrcForPingback() {
        ...
        LogUtils.m1568d(this.TAG, "updateIncomeSrcForPingback, incomesrc = " + PingBackCollectionFieldUtils.getIncomeSrc());
    }
    ...
    private Drawable getCoverDrawable() {
        ...
        return this.mContext.getResources().getDrawable(C0508R.drawable.epg_home_carousel_cover_image);
    }
    ...
    public void sendStopMsg() {
        LogUtils.m1568d(this.TAG, "receive stop cmd is paused = " + this.mIsPaused);
