...
public class MyTabHost extends TabHost implements OnTabChangeListener, TabFocusListener, TabWidgetMeasureListener {
    ...
    private void initViews(Context context) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "initViews: child count=" + getChildCount());
        }
    ...
    protected void onFinishInflate() {
        ...
        LogUtils.m1568d(TAG, "onFinishInflate: child count=" + getChildCount());
    ...
    public void addViewToTabContent(View view) {
        if (view != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "addViewToTabContent: view=" + view + ", parent=" + parent + ", mContentLayout=" + this.mContentLayout);
            }
    ...
    public void onTabWidgetMeasured(int childWidth, int childHeight) {
        LogUtils.m1568d(TAG, "wigetW: childWidth=" + childWidth + ",childHeight=" + childHeight);
        ...
        for (int i = 0; i < size; i++) {
            ...
            LinearLayout indicatorLayout = (LinearLayout) view.findViewById(C1291R.id.ll_indicator);
            LogUtils.m1568d(TAG, "wigetW: indicatorLayout=" + indicatorLayout);
            if (indicatorLayout != null) {
                LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) indicatorLayout.getLayoutParams();
                LogUtils.m1568d(TAG, "wigetW: lp =" + lp);
                if (lp != null) {
                    lp.width = this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_210dp);
                    LogUtils.m1568d(TAG, "view.getPaddingLeft()=" + view.getPaddingLeft() + ",view.getPaddingRight()=" + view.getPaddingRight());
    ...
    public void setTabWidgetWidth(int widthPx) {
        LogUtils.m1568d(TAG, "setTabWidgetWidth:" + widthPx);
    ...
    public void setTabWidgetHeight(int heightPx) {
        LogUtils.m1568d(TAG, "setTabWidgetHeight:" + heightPx);
        if (this.mOuterLayout != null) {
            View tabWidget = this.mOuterLayout.findViewById(16908307);
            LayoutParams params = tabWidget.getLayoutParams();
            if (params != null) {
    ...
    public void addNewTab(String tag, View indicatorView, int tabContentResId) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "addNewTab: tag=" + tag + ", indicator view={" + indicatorView + "}, content id=" + tabContentResId);
        }
    ...
    public void addNewTab(String tag, View indicatorView, final View tabContent) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "addNewTab: tag=" + tag + ", indicatorView={" + indicatorView + "}, tabContent=" + tabContent);
        }
        indicatorView.setTag(tag);
    ...
    public void onTabChanged(String tag) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onTabChanged: " + tag);
        }
        if (this.mTabChangeListener != null) {
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        ...
        LogUtils.m1571e(TAG, "onVisibilityChanged visibility=" + visibility);
