...
public class GalaVideoPlayerGenerator extends Wrapper {
    ...
    private static OnHistoryRecorderListener mHistoryRecorderListener = new C13791();
    private static INetDiagnoseProvider mNetDiagnoseProvider = new C13843();
    private static OnCarouselProgramClickListener mOnCarouselProgramClickListener = new C13865();
    private static OnErrorFinishedListener mOnErrorFinishedListener = new C13854();
    private static OnReleaseListener mReleaseListener = new C13802();

    static class C13791 implements OnHistoryRecorderListener {
        C13791() {
        }
        ...
        public void onWatchTrackAddPlayRecord(Album album, boolean isWeekend) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GalaVideoPlayerGenerator.TAG, "addPlayRecord(), putAndSend(historyInfo = " + historyInfo + ")");
            }
        ...
        public void onAddLocalCarouselPlayRecord(String channelId, String channelName, String channelTableNo, long startTime, long endTime) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GalaVideoPlayerGenerator.TAG, "addLocalCarouselPlayRecordchannelId = " + channelId + "channelName=" + channelName + "channelTableNo=" + channelTableNo + "startTime=" + startTime + "endTime=" + endTime);
            }
            CarouselHistoryInfo info = new CarouselHistoryInfo(channelId, channelTableNo, channelName);
            info.setStartTime(startTime);
            info.setEndTime(endTime);
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GalaVideoPlayerGenerator.TAG, InterfaceKey.EPG_CHCM + info);
            }
            GetInterfaceTools.getICarouselHistoryCacheManager().put(info);
        }
    }

    static class C13802 implements OnReleaseListener {
        C13802() {
        }
    ...
    static class C13843 implements INetDiagnoseProvider {
        private static final String TAG_L = "Player/NetDiagnoseProvider";
        private INetDiagnoseCallback mCallback;
        private INDDoneListener mCdnListener = new C13811();
        private INDDoneListener mCollectListener = new C13822();
        private INDUploadCallback mUploadcallback = new C13833();

        class C13811 implements INDDoneListener {
            C13811() {
            }

            public void onFinish(Map<String, Object> resultMap) {
                NetDiagnoseInfo result = (NetDiagnoseInfo) resultMap.get("data");
                C13843.this.addToLogRecord(C13843.TAG_L, "mCdnListener response = " + result.getDnsResult());
            }
        }

        class C13822 implements INDDoneListener {
            C13822() {
            }

            public void onFinish(Map<String, Object> resultMap) {
                NetDiagnoseInfo result = (NetDiagnoseInfo) resultMap.get("data");
                C13843.this.addToLogRecord(C13843.TAG_L, "mCollectListener response = " + result.getCollectionResult());
                if (C13843.this.mCallback != null) {
                    C13843.this.mCallback.onDiagnoseResult(result.getCollectionResult());
                }
            }
        }

        class C13833 implements INDUploadCallback {
            C13833() {
            }

            public void uploadNetDiagnoseDone() {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(GalaVideoPlayerGenerator.TAG, "uploadNetDiagnoseDone");
                }
            }
        }
        ...
        public void getDiagnoseInfoAsync(INetDiagnoseCallback callback, IVideo video, Context context, int playerType, IPlayerProfile profile) {
            if (callback != null && video != null && context != null && profile != null) {
                ...
                LogUtils.m1568d(TAG_L, "getDiagnoseInfoAsync: callback=" + callback + ", video=" + video + ", context=" + context + ", profile=" + profile);
            }
        }
    ...
    static class C13854 implements OnErrorFinishedListener {
        C13854() {
        }
    ...
    static class C13865 implements OnCarouselProgramClickListener {
        C13865() {
        }
    ...
    private static class MyPlayCoreRedirectOutPageListener implements OnRedirectOutPageListener {
        ...
        public void redirectToBuyPage(int enterType, String buySource, Album albumInfo) {
            if (this.mContext instanceof Activity) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(GalaVideoPlayerGenerator.TAG, "onStrategy eventId=" + eventId + ", from=" + from + ", state=" + state + ", enterType=" + enterType + ", incomesrc=" + params.incomesrc);
                }
        ...
        public void redirectToLoginPage(int type, BitStream bs) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GalaVideoPlayerGenerator.TAG, "redirectToLoginPage type is " + type + " /" + bs);
            }
            if (bs != null) {
                String mBitStremText = PlayerDataUtils.getBitStreamString(this.mContext, bs);
                QToast.makeTextAndShow(this.mContext, String.format(this.mContext.getResources().getString(C1291R.string.player_login_tip_toast), new Object[]{mBitStremText}), 5000);
            }
    ...
    private static class MyShowHintListener implements OnShowHintListener {
        ...
        public void onShowHint(HintType type) {
            switch (type) {
                case NET_CHECK:
                    PlayerToastHelper.showNetDiagnoseToast(this.mContext, this.mContext.getResources().getString(C1291R.string.player_toast_check_net), TOAST_CHECK_NET_DURATION);
                    return;
                case BUFFER_LAG:
                    PlayerToastHelper.showToast(this.mContext, this.mContext.getResources().getString(C1291R.string.player_toast_lag), (int) QToast.LENGTH_LONG);
                    return;
                default:
                    return;
            }
    ...
    private static IGalaVideoPlayer createGalaVideoPlayer(Context context, ViewGroup container, Bundle bundle, OnPlayerStateChangedListener videoStateListener, ScreenMode initialMode, LayoutParams initialParams, WindowZoomRatio windowZoomRatio, IMultiEventHelper multiEventHelper, OnAdSpecialEventListener onSpecialEventListener) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createGalaVideoPlayer, incomesrc=" + incomeSrc);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "[PERF-LOADING]tm_player.prefinit");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "[PERF-LOADING]tm_player-ui.init");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createVideoPlayer() playerSceneProvider=" + playerSceneProvider);
        }
    ...
    private static ScreenMode checkInitialScreenModes(ScreenMode initialMode) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "checkInitialScreenModes: initial mode=" + initialMode);
        }
    ...
    private static LayoutParams checkInitialLayoutParams(LayoutParams initialParams) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "checkInitialLayoutParams: initial params=" + initialParams);
        }
