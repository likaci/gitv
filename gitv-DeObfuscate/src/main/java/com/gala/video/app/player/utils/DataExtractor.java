...
public class DataExtractor {
            ...
            for (IVideo episodeItem : episodes) {
                if (episodeItem == null) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1571e(TAG, "getTrailersList: episodeItem is null");
                    }
                    ...
                    if (episodeItem.getAlbum().order > 0) {
                        ...
                        LogUtils.m1571e(TAG, "getTrailersList: invalid video play order:" + episodeItem);
                    }
                }
            }
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "getTrailerIndicesList, result list=" + trailers);
            }
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1571e(TAG, "getTrailersList: episodes is null or empty");
        }
        return trailers;
    }
    ...
    public static HashMap<Integer, PayMarkType> getVipIndicesList(List<IVideo> episodes) {
        ...
        if (!ListUtils.isEmpty((List) episodes)) {
            for (IVideo episodeItem : episodes) {
                if (episodeItem == null) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1571e(TAG, "getVipIndicesList: episodeItem is null");
                    }
                    ...
                    if (episodeItem.getAlbum().order > 0) {
                        ...
                        LogUtils.m1571e(TAG, "getVipIndicesList: invalid video play order:" + episodeItem);
                    }
                }
            }
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "getVipIndicesList, result list=" + vips);
            }
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1571e(TAG, "getVipIndicesList: episodes is null or empty");
        }
        return vips;
    }
    ...
    public static HashMap<Integer, String> getOneWordList(List<IVideo> episodes) {
        if (episodes == null || episodes.isEmpty()) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1571e(TAG, "getOneWordList episodes is null or empty");
            }
            return null;
        }
        ...
        while (it.hasNext()) {
            ...
            if (episodeItem == null) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1571e(TAG, "getOneWordList, episodeItem is null");
                }
                ...
                LogUtils.m1571e(TAG, "getOneWordList: invalid video play order:" + episodeItem);
            }
        }
        return oneWordList;
    }
    ...
    public static List<Integer> getEpisodeNoPlayList(List<IVideo> episodes, int episodeCount) {
        LogUtils.m1568d(TAG, "getEpisodeNoPlayList episodeCount=" + episodeCount);
        if (ListUtils.isEmpty((List) episodes) || episodeCount < 0) {
            LogUtils.m1568d(TAG, "getEpisodeNoPlayList return");
            return null;
        }
        ...
        LogUtils.m1568d(TAG, "getEpisodeNoPlayList actualEpisodes.size = " + actualEpisodes.size());
        ...
        LogUtils.m1568d(TAG, "getEpisodeNoPlayList realEpisodes.size = " + realEpisodes.size());
        TreeSet<Integer> acturalSet = new TreeSet(actualEpisodes);
        TreeSet<Integer> realSet = new TreeSet(realEpisodes);
        LogUtils.m1568d(TAG, "getEpisodeNoPlayList before CollectionUtils.symDifference");
        TreeSet<Integer> treeSet = symDifference(acturalSet, realSet);
        LogUtils.m1568d(TAG, "getEpisodeNoPlayList after CollectionUtils.symDifference treeSet.size" + treeSet.size());
