...
public class StarsPingbackUtil {
    ...
    public static void sendPageItemClick(IData data, String bolck, int line, int rseat, String s1, String e) {
        if (data != null) {
            ...
            params.add("r", r).add("block", bolck).add("rt", "i").add("rseat", String.valueOf(rseat)).add("rpage", "3").add("c1", c1).add("plid", plid).add("qtcurl", QTCURL.QTCURL_VALUE_STAR).add("s1", s1).add(Keys.f2035T, "20").add("line", String.valueOf(line)).add("e", e);
    ...
    public static void sendPageDetailClick(StarsInfoModel infoModel) {
        PingBackParams params = new PingBackParams();
        params.add("block", BLOCK.BLOCK_VALUE_INITRE).add("rt", "i").add("r", "").add("rseat", BLOCK.BLOCK_VALUE_INITRE).add("rpage", "3").add(Keys.f2035T, "20").add("qtcurl", QTCURL.QTCURL_VALUE_STAR).add("s1", infoModel != null ? infoModel.getBuySource() : "").add("e", infoModel != null ? infoModel.getE() : "");
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }

    public static void sendPageShow(PageShowModel pageShowModel) {
        if (pageShowModel == null) {
            LogUtils.m1571e(TAG, "sendPageShow --- pageShowModel == null");
            return;
        }
        ...
        if (infoModel != null) {
            ...
            e = infoModel.getE();
            LogUtils.m1571e(TAG, "sendPageShow --- e" + e);
        }
