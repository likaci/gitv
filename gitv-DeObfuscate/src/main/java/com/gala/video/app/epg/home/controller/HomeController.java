...
public class HomeController implements View {
    ...
    public Handler mHandler = new C06104();
    ...
    private IHomeDataObserver mTabInfoChangeListener = new C06071();
    ...
    class C06071 implements IHomeDataObserver {
        C06071() {
        }

        public void update(WidgetChangeStatus status, HomeModel data) {
            LogUtils.m1568d(HomeController.TAG, "TAB_INFOChange status:" + status + data);
            switch (status) {
                case InitChange:
    ...
    class C06082 implements Runnable {
        ...
        public void run() {
            LogUtils.m1568d(HomeController.TAG, "showTabManualDialog");
    ...
    class C06104 extends Handler {
        C06104() {
        }

        public void handleMessage(Message msg) {
            LogUtils.m1568d(HomeController.TAG, "home controller message = " + msg.what + ",arg1 = " + msg.arg1 + ",arg2 = " + msg.arg2);
            switch (msg.what) {
                ...
                case 95:
                    ...
                    if (HomeController.this.mIsDefault) {
                        ...
                        LogUtils.m1568d(HomeController.TAG, "BUILD_DEFAULT_TAB_COMPLETED111111");
                        HomeController.this.mMainPagePresenter.buildPages(HomeController.this.mUIController);
                        LogUtils.m1568d(HomeController.TAG, "BUILD_DEFAULT_TAB_COMPLETED222222");
                        break;
                    }
                    break;
                case 96:
                    ...
                    LogUtils.m1571e(HomeController.TAG, "updateTab name fail tabList is null");
                    return;
                case 97:
                ...
                case 98:
                    ...
                    LogUtils.m1568d(HomeController.TAG, "BUILD_TAB_COMPLETED1111");
                    HomeController.this.mMainPagePresenter.buildTabHost(HomeController.this.mUIController.mPageTabs, HomeController.this.mUIController.mTotalTabCount);
                    LogUtils.m1568d(HomeController.TAG, "BUILD_TAB_COMPLETED22222");
                    HomeController.this.mMainPagePresenter.buildPages(HomeController.this.mUIController);
                ...
                case 100:
                    ...
                    if (msg.arg1 == HomeController.this.mMainPagePresenter.getDefaultTab()) {
                        ...
                        LogUtils.m1570d(HomeController.TAG, "[start performance] loadPlugin cost " + StartUpInfo.get().getPluginLoadInterval() + " ms,", "show home page completed cost " + (SystemClock.elapsedRealtime() - StartUpInfo.get().getStartTime()) + " ms");
                        HomeController.this.requestDefaultFocus();
    ...
    class C06115 implements Runnable {
        C06115() {
        }
    ...
    class C06126 implements Runnable {
        C06126() {
        }
    ...
    class C06137 implements OnFocusChangeListener {
        C06137() {
        }
        ...
        public void onFocusChange(android.view.View v, boolean hasFocus) {
            if (hasFocus) {
                ...
                HomeController.this.promotionView.setTextColor(HomeController.this.mContext.getResources().getColor(C0508R.color.action_bar_text_focus));
            } else {
                HomeController.this.promotionView.setTextColor(HomeController.this.mContext.getResources().getColor(C0508R.color.action_bar_text_normal));
            }
    ...
    class C06148 implements OnKeyListener {
        C06148() {
        }
    ...
    class C06159 implements OnClickListener {
        C06159() {
        }
    ...
    public void init(Context context, android.view.View rootView) {
        ...
        this.mTabHosts = (TabBarHost) this.mRootView.findViewById(C0508R.id.epg_tab_host);
        this.mTopLayout = (RelativeLayout) this.mRootView.findViewById(C0508R.id.epg_top_layout);
        this.mViewPager = (ScrollViewPager) this.mRootView.findViewById(C0508R.id.epg_pager);
        this.mViewPager.setTabBarHost(this.mTabHosts);
        this.mLeftShaderImg = (ImageView) this.mRootView.findViewById(C0508R.id.epg_tab_host_leftimage);
        this.mRightShaderImg = (ImageView) this.mRootView.findViewById(C0508R.id.epg_tab_host_rightimage);
        ...
        this.promotionView = (ActionBarItemView) rootView.findViewById(C0508R.id.tv_third_part_app);
    ...
    private void onInitTabMessage() {
        ...
        if (this.mIsDefault) {
            ...
            LogUtils.m1568d(TAG, "onInitTabMessage build default completed : " + ret);
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "[start performance] tab create cost " + (SystemClock.elapsedRealtime() - start) + " ms");
        }
    }
    ...
    private void showTabManualDialog() {
        ...
        this.mHandler.post(new C06082());
    }

    private void cancelTabManualDialog() {
        if (!ThreadUtils.isUIThread()) {
            this.mHandler.post(new C06093());
    ...
    public void requestDefaultFocus() {
        if (this.mMainPagePresenter != null) {
            this.mViewPager.post(new C06115());
        }
    }
    ...
    private void lock(int pageIndex) {
        try {
            ...
            LogUtils.m1569d(TAG, "exception :", e);
        }
    }
    ...
    public boolean checkIfBuildAllCompleted() {
        ...
        if (ret && !mIsBuildUICompleted) {
            ...
            LogUtils.m1574i(TAG, "build ui: all page build complete!!!, build count: " + this.mUIController.mTotalTabCount);
        }
        return ret;
    }
    ...
    private void reloadNoDataPage() {
        for (int i = 0; i < this.mUIController.mTotalTabCount; i++) {
            PageManage page = this.mUIController.mPageTabs[i];
            if (page.mNoData) {
                LogUtils.m1574i(TAG, "reloadNoDataPage index" + i);
    ...
    private void initPromotion() {
        this.promotionView.setOnFocusChangeListener(new C06137());
        this.promotionView.setOnKeyListener(new C06148());
        this.promotionView.setOnClickListener(new C06159());
        this.promotionView.setTextColor(this.mContext.getResources().getColor(C0508R.color.action_bar_text_normal));
        this.promotionView.setIconDrawableWidth(C0508R.dimen.dimen_23dp);
    ...
    public void setRecommendFocusEvent(CheckInBuildEvent event) {
        LogUtils.m1574i(TAG, "setRecommendFocusEvent: " + this.promotionView.getVisibility());
    ...
    private void showAppRecommendIcon(String icon) {
        ...
        LogUtils.m1574i(TAG, "icon=" + icon);
        ImageRequest imageRequest = new ImageRequest(icon);
        imageRequest.setTargetWidth((int) this.mContext.getResources().getDimension(C0508R.dimen.dimen_21dp));
        imageRequest.setTargetHeight((int) this.mContext.getResources().getDimension(C0508R.dimen.dimen_21dp));
