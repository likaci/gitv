...
public class SearchVipAdapter extends BaseAdapter {
    ...
    private final LiveCornerListener mLiveCornerListener = new C09762();
    ...
    class C09751 implements IImageCallback {
        C09751() {
        }
    ...
    class C09762 implements LiveCornerListener {
        C09762() {
        }

        public void showBefore() {
            SearchVipAdapter.this.topRightImg.setImageResource(C0508R.drawable.share_corner_notice);
        }

        public void showPlaying() {
            SearchVipAdapter.this.topRightImg.setImageResource(C0508R.drawable.share_corner_living);
        }

        public void showEnd() {
            SearchVipAdapter.this.topRightImg.setImageResource(C0508R.drawable.share_corner_end_living);
        }
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView != null) {
            ...
            view = this.mInflater.inflate(C0508R.layout.epg_search_vip, null);
        }
        this.image = (ImageView) view.findViewById(C0508R.id.epg_searchVip_img);
        this.topLeftImg = (ImageView) view.findViewById(C0508R.id.epg_searchVip_topLeftImg);
        this.topRightImg = (ImageView) view.findViewById(C0508R.id.epg_searchVip_topRightImg);
        this.title = (TextView) view.findViewById(C0508R.id.epg_searchVip_titleText);
        this.rankImg = (ImageView) view.findViewById(C0508R.id.epg_searchVip_rank);
        if (ListUtils.getCount(this.mAlbumDataList) > position) {
            if (position == 3) {
                this.image.setImageResource(C0508R.drawable.epg_search_vip_entrance);
                this.title.setVisibility(8);
            } else {
                this.image.setImageResource(C0508R.drawable.share_default_image);
                ImageProviderApi.getImageProvider().loadImage(new ImageRequest(getImgUrl(position), this.image), new C09751());
                setTopCorner(position);
                switch (position) {
                    case 0:
                        this.rankImg.setBackgroundResource(C0508R.drawable.share_corner_rank_1);
                        break;
                    case 1:
                        this.rankImg.setBackgroundResource(C0508R.drawable.share_corner_rank_2);
                        break;
                    case 2:
                        this.rankImg.setBackgroundResource(C0508R.drawable.share_corner_rank_3);
                        break;
                }
    ...
    private void setTopCorner(int position) {
        ...
        if (provider.getCornerInfo(channelLabel, 7)) {
            this.topLeftImg.setImageResource(C0508R.drawable.share_corner_yongquan);
        } else if (provider.getCornerInfo(channelLabel, 1)) {
            this.topLeftImg.setImageResource(C0508R.drawable.share_corner_fufeidianbo);
        } else if (provider.getCornerInfo(channelLabel, 0)) {
            this.topLeftImg.setImageResource(C0508R.drawable.share_corner_vip);
        }
        clearLiveCorner();
        if (!LivePlayingType.DEFAULT.equals(channelLabel.getLivePlayingType())) {
            ...
            this.topRightImg.setImageResource(C0508R.drawable.share_corner_dubo);
        } else if (provider.getCornerInfo(channelLabel, 6)) {
            this.topRightImg.setImageResource(C0508R.drawable.share_corner_zhuanti);
        }
    }
