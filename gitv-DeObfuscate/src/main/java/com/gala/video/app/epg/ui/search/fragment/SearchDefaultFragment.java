...
public class SearchDefaultFragment extends SearchBaseFragment {
    ...
    private WidgetStatusListener mGridViewListener = new C09855();
    private Handler mHandler = new Handler(Looper.getMainLooper());
    private SearchHotAdapter mHotAdapter;
    private List<String> mHotHordsList;
    private View mMainView;
    private WidgetStatusListener mPhotoViewListener = new C09866();
    ...
    class C09771 implements Runnable {
        C09771() {
        }
    ...
    class C09812 implements IApiCallback<ApiResultHotWords> {
        C09812() {
        }
            ...
            SearchDefaultFragment.this.mHandler.post(new Runnable() {
                public void run() {
                    if (result == null || result.data == null) {
                        LogUtils.m1571e(SearchDefaultFragment.LOG_TAG, "requestData --- onSuccess result is null");
                    } else {
        ...
        public void onException(final ApiException exception) {
            LogUtils.m1573e(SearchDefaultFragment.LOG_TAG, ">>>>>>>>>>>> Api.hotword.call Exception: ", exception.getMessage());
            ...
            NetWorkManager.getInstance().checkNetWork(new StateCallback() {
                ...
                public void getStateResult(int arg0) {
                    SearchDefaultFragment.this.runOnUiThread(new C09791());
                }
            });
        }
    }
    ...
    class C09855 implements WidgetStatusListener {
        C09855() {
        }
        ...
        public void onItemSelectChange(View view, int position, boolean hasFocus) {
            ...
            if (hasFocus) {
                ...
                textView.setTextColor(SearchDefaultFragment.this.getColor(C0508R.color.gala_write));
                AnimationUtil.scaleAnimation(view, 1.0f, 1.07f, 200);
                return;
            }
            textView.setEllipsize(TruncateAt.END);
            textView.setTextColor(SearchDefaultFragment.this.getColor(C0508R.color.search_right_text_color));
    ...
    class C09866 implements WidgetStatusListener {
        C09866() {
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        this.mMainView = inflater.inflate(C0508R.layout.epg_fragment_search_right_no_result, null);
        init(this.mMainView);
        showLoading();
        initInThread();
    ...
    public void initInThread() {
        ThreadUtils.execute(new C09771());
    }

    private void init(View view) {
        this.mSearchNoResultGridTitle = (RelativeLayout) this.mMainView.findViewById(C0508R.id.epg_search_hot_title_layout);
        this.mSearchNoResultGridView = (PhotoGridView) this.mMainView.findViewById(C0508R.id.epg_search_no_result_gridview);
        this.mSearchVipGridView = (PhotoGridView) this.mMainView.findViewById(C0508R.id.epg_search_no_result_vipList);
    ...
    private void doAfterRequestSucc(List<String> hotwords) {
        ...
        showVipView();
        setHotAdapter();
        this.mHandler.postDelayed(new C09823(), 500);
    }

    private void doAfterRequestException(ApiException exception) {
        GlobalQRFeedbackPanel qrPanel = (GlobalQRFeedbackPanel) this.mMainView.findViewById(C0508R.id.epg_search_noresult_QR_panel);
        ...
        this.mHandler.postDelayed(new C09834(), 500);
        hideLoading();
    }
    ...
    private void sendClickPingback(int tag, int pos) {
        if (tag == 3) {
            ...
            params.add(Keys.f2035T, "20").add("rseat", "1_" + (pos + 1)).add("rpage", "search").add("block", "viphot").add("rt", "i").add("plid", plid);
