...
public class CarouselCountdownView extends LinearLayout {
    ...
    private Handler mHandler = new C15451();
    ...
    private void initView() {
        LayoutInflater.from(this.mContext).inflate(C1291R.layout.player_carousel_countdown_time, this);
        this.mTxtTip = (TextView) findViewById(C1291R.id.carousel_tip);
        this.mTxtHour = (TextView) findViewById(C1291R.id.hour);
        this.mTxtMin = (TextView) findViewById(C1291R.id.min);
        this.mTxtSecond = (TextView) findViewById(C1291R.id.second);
    ...
    public void show() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "CarouselCountdownView show()=" + this.mStartTime);
        }
    ...
    public void setStartTime(long time) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setStartTime:" + time);
        }
    ...
    private void updateTime() {
        if (this.mStartTime > 0) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "updateTime() countDownTime=" + countDownTime);
            }
            if (countDownTime > 0) {
    ...
    private void formatTime(long countDownTime) {
        if (countDownTime < MAX_COUNT_DOWN_TIME && countDownTime >= 0) {
            ...
            setVisibility(0);
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "formatTime strHour:" + strHour + ", strMinute=" + strMinute + ", strSecond=" + strSecond);
            }
