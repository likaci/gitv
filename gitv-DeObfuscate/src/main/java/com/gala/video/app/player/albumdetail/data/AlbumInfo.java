...
public class AlbumInfo {
    ...
    public int getEpisodeMaxOrder() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "getEpisodeMaxOrder " + this.mEpisodeMaxOrder);
        }
    ...
    public void setEpisodeMaxOrder(int order) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "maxOrder is " + this.mEpisodeMaxOrder + "order = " + order);
        }
    ...
    public void setEpisodes(List<Episode> episodes, int totalCount) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setEpisodes: episodes.size= " + episodes.size() + " mAlbumDetailPic " + this.mAlbumDetailPic + "  )" + ListUtils.getCount((List) episodes) + " mEpisodeVideos " + ListUtils.getCount(this.mEpisodeVideos) + " totalCount " + totalCount);
        }
        if (ListUtils.getCount(this.mEpisodeVideos) <= ListUtils.getCount((List) episodes)) {
            ...
            LogUtils.m1568d(this.TAG, "setEpisodes not opreate, maybe cache to current");
        }
    }
    ...
    public int getPlayOrder() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "getPlayOrder() return " + playOrder + ", series=" + isTvSeries());
        }
        return playOrder;
    }
    ...
    public VideoKind getKind(Album album) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "getVideoAlbumKind() isSeries" + album.isSeries() + "album = " + getAlbumType() + ", album.type=" + album.type + "album.isSourceType()" + album.isSourceType());
        }
        if (album.type == AlbumType.VIDEO.getValue()) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "getVideoAlbumKind  unhanlded  albumType" + getAlbumType());
            }
            return VideoKind.VIDEO_SINGLE;
