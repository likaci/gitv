...
public class TipView extends FrameLayout implements IThreeDimensional, ThreeDimensionalParams, IScreenUISwitcher {
    ...
    private AnimationListener mAnimationListener = new C15631();
    ...
    private Runnable mRunnable = new C15674();
    ...
    class C15631 implements AnimationListener {
        ...
        public void onAnimationEnd(Animation animation) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TipView.TAG, "hide() onAnimationEnd");
            }
            TipView.this.hideHalo();
            TipView.this.hideTxtTip();
            TipView.this.mIsAnimProcessing = false;
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TipView.TAG, "onAnimationEnd mIsAnimProcessing=" + TipView.this.mIsAnimProcessing);
            }
        }
        ...
        public void onAnimationStart(Animation animation) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TipView.TAG, "onAnimationStart mIsAnimProcessing=" + TipView.this.mIsAnimProcessing);
            }
        }
    }
    ...
    class C15653 implements AnimationListener {
        C15653() {
        }
    ...
    class C15674 implements Runnable {
        C15674() {
        }
    ...
    class C15685 extends AnimatorListenerAdapter {
        C15685() {
        }
    ...
    private void initView() {
        ...
        ((LayoutInflater) this.mContext.getSystemService("layout_inflater")).inflate(C1291R.layout.player_layout_tipmessage, this);
        this.mTipContent = (FrameLayout) findViewById(C1291R.id.fl_tipcontent);
        this.mFrontPanel = (LinearLayout) findViewById(C1291R.id.front_panel);
        this.mFrontIcon = (ImageView) findViewById(C1291R.id.front_icon);
        this.mTextTipFront = (TextView) findViewById(C1291R.id.tv_tipmessage_front);
        this.mBackPanel = (LinearLayout) findViewById(C1291R.id.back_panel);
        this.mBackIcon = (ImageView) findViewById(C1291R.id.back_icon);
        this.mTextTipBack = (TextView) findViewById(C1291R.id.tv_tipmessage_back);
        this.mImageIndication = (ImageView) findViewById(C1291R.id.tip_indication);
        this.mHalo = (ImageView) findViewById(C1291R.id.tip_halo);
        this.mTextSizeFullScreen = (float) this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_27dp);
    ...
    public void showTip(TipWrapper tip) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showTip tip(" + tip + ")");
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showTip mFirstTip=" + this.mFirstTip + ", mSecondTip=" + this.mSecondTip);
        }
        if (tip != null) {
            ...
            switch (tip.getTipType().getConcreteTipType()) {
                ...
                default:
                    ...
                    if (this.mOldTip != null) {
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(TAG, "showTip olddrawable:" + this.mOldTip.getDrawable() + ", drawable:" + this.mTip.getDrawable());
                        }
    ...
    private void fadeInAnimation() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "fadeInAnimation()");
        }
        Animation animation = new AlphaAnimation(0.0f, 1.0f);
        animation.setDuration(400);
        this.mImageIndication.startAnimation(animation);
        animation.setAnimationListener(new C15642());
    }

    private void fadeOutAnimation() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "fadeOutAnimation()");
        }
        Animation animation = new AlphaAnimation(1.0f, 0.0f);
        this.mHalo.setVisibility(8);
        animation.setDuration(400);
        this.mImageIndication.startAnimation(animation);
        animation.setAnimationListener(new C15653());
    }
    ...
    public void showTipPanel() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showTipPanel()");
        }
        setVisibility(0);
    ...
    private void showFrontTip(TipWrapper tip) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showFrontTip: " + tip.getContent());
        }
    ...
    private void showBackTip(TipWrapper tip) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showBackTip: " + tip.getContent());
        }
        if (tip.getTipType().getTipShowType() == 203) {
    ...
    private void setHalo(TipWrapper tip) {
        if (tip.getDrawable() != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "setHalo mIs3D=" + this.mIs3D + "width=" + width + ", height=" + height);
            }
            if (this.mIs3D) {
                LayoutParams haloparams = (LayoutParams) this.mHalo.getLayoutParams();
                haloparams.width = this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_300dp);
    ...
    public void setThreeDimensional(boolean enable) {
        if (!(this.mFrontIcon == null || this.mBackIcon == null)) {
            ...
            if (enable) {
                frontParams.width = (int) (this.mContext.getResources().getDimension(C1291R.dimen.dimen_35dp) * ThreeDimensionalParams.TEXT_SCALE_FOR_3D);
                backParams.width = (int) (this.mContext.getResources().getDimension(C1291R.dimen.dimen_35dp) * ThreeDimensionalParams.TEXT_SCALE_FOR_3D);
            } else {
                frontParams.width = (int) this.mContext.getResources().getDimension(C1291R.dimen.dimen_35dp);
                backParams.width = (int) this.mContext.getResources().getDimension(C1291R.dimen.dimen_35dp);
            }
    ...
    public void hide(boolean isNeedAnim) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "hide() mFirstTip=" + this.mFirstTip + ", mSecondTip=" + this.mSecondTip + ", isShown=" + getVisibility() + ", mIsFullScreen=" + this.mIsFullScreen);
        }
        if (getVisibility() == 8) {
            resetTipPosition();
            return;
        ...
        if (isNeedAnim) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "hide mIsAnimProcessing=" + this.mIsAnimProcessing);
            }
    ...
    private void resetTipPosition() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "resetTipPosition() mFirstTip=" + this.mFirstTip + ", mSecondTip=" + this.mSecondTip);
        }
        if (!this.mFirstTip && this.mSecondTip) {
            ...
            animStart.addListener(new C15685());
        }
        this.mTip = null;
        this.mFirstTip = false;
        this.mSecondTip = false;
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "mRunnable=" + this.mClickRunnable);
        }
        ...
        if (this.mTip.getTipType().isSupportLogin() && this.mClickRunnable != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "dispatchKeyEvent isSupportLogin");
            }
            return true;
            ...
            LogUtils.m1568d(TAG, "dispatchKeyEvent isVisible:" + getVisibility());
            return false;
    ...
    private void showAd(RelativeLayout view) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showAd()");
        }
        if (!this.mIs3D) {
            if (this.mAdView == null) {
                this.mAdView = view;
                LayoutParams params = new LayoutParams(this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_180dp), this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_60dp));
                params.gravity = 80;
                params.bottomMargin = this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_64dp);
                params.leftMargin = this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_23dp);
    ...
    private void notifyAdShow(int id) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyAdShow()" + this.mOnAdStateListener);
        }
    ...
    public void setOnAdStateListener(OnAdStateListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setOnPageAdvertiseStateChangeListener()" + listener);
        }
    ...
    private void sendTipShowPingback() {
        if (this.mIsPrimary && this.mSender != null && this.mPingbackContext != null && this.mVideo != null && this.mTip != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "sendTipShowPingback() type:" + type);
            }
            switch (type) {
    ...
    private void sendTipClickPingback() {
        if (this.mIsPrimary && this.mSender != null && this.mPingbackContext != null && this.mVideo != null && this.mTip != null) {
            int type = this.mTip.getTipType().getConcreteTipType();
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "sendTipClickPingback() type:" + type);
            }
            switch (type) {
                case TipType.CONCRETE_TYPE_LOGIN /*325*/:
    ...
    private void saveCount(String name) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "saveCount() mIsPrimary:" + this.mIsPrimary);
        }
        if (this.mIsPrimary) {
    ...
    public void setPrimary(boolean b) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setPrimary():" + b);
        }
        this.mIsPrimary = b;
    }
}
