...
public class VerticalTabHost extends MyTabHost {
    ...
    public void onTabWidgetMeasured(int childWidth, int childHeight) {
        LogUtils.m1568d(TAG, "onTabWidgetMeasured: childHeight=" + childHeight);
        int heightW = (childHeight - ((int) (((float) childHeight) * (AnimationUtils.getDefaultZoomRatio() - 1.0f)))) + (this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.dimen_1dp) * 2);
        LogUtils.m1568d(TAG, "heightW: childHeight=" + childHeight);
        ...
        for (int i = 0; i < size; i++) {
            ...
            RelativeLayout indicatorLayout = (RelativeLayout) view.findViewById(C1291R.id.ll_indicator);
            ...
            if (indicatorLayout != null) {
                ...
                LogUtils.m1568d(TAG, "childHeight: lp =" + lp);
                if (lp != null) {
                    lp.height = heightW;
                    int width = lp.width;
                    lp.width = width - ((int) (((float) width) * (AnimationUtils.getDefaultZoomRatio() - 1.0f)));
                    LogUtils.m1568d(TAG, "heightW: heightW=" + heightW + ", view.getPaddingLeft()=" + view.getPaddingLeft() + ",view.getPaddingRight()=" + view.getPaddingRight());
