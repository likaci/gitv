...
public abstract class AbsTabContent<DataType, ItemType> implements IContent<DataType, ItemType> {
    ...
    protected void setupMyRadioGroupCommon(MyRadioGroup radioGroup) {
        radioGroup.setTextSize(ResourceUtil.getDimen(C1291R.dimen.dimen_20dp));
        radioGroup.setTextColors(ResourceUtil.getColor(C1291R.color.player_ui_text_color_default), ResourceUtil.getColor(C1291R.color.player_ui_text_color_selected), ResourceUtil.getColor(C1291R.color.player_ui_text_color_focused), ResourceUtil.getColor(C1291R.color.player_ui_rg_text_color_disabled));
        radioGroup.setItemBackground(C1291R.drawable.player_episode_item_bg);
        radioGroup.setDimens(new int[]{ResourceUtil.getDimen(C1291R.dimen.dimen_120dp), ResourceUtil.getDimen(C1291R.dimen.dimen_60dp)});
        radioGroup.setZoomEnabled(IS_ZOOM_ENABLED);
        radioGroup.setChildAutoAlignTop(false);
        radioGroup.setContentSpacing(ResourceUtil.getDimen(C1291R.dimen.dimen_8dp));
        radioGroup.setDividerPadding(this.mContext.getResources().getDimensionPixelSize(C1291R.dimen.player_definition_widget_divider_padding));
        radioGroup.setShowDivider(((0 | 1) | 2) | 4);
        radioGroup.setDividerDrawable(C1291R.drawable.player_radio_item_divider_transparent);
    }
    ...
    protected void setupContentLayout4HorizontalScrollView(MyRadioGroup radioGroup, LinearLayout ll, HorizontalScrollView scrollView) {
        int itemWidth = ResourceUtil.getDimen(C1291R.dimen.dimen_120dp);
        ...
        int itemBgResId = C1291R.drawable.player_episode_item_bg;
        ...
        params.topMargin = UiUtils.getDimensionInPx(this.mContext, C1291R.dimen.dimen_35dp);
        groupParams.rightMargin = zoomWidth;
        radioGroup.setLayoutParams(groupParams);
        ll.setLayoutParams(params);
        scrollView.setLayoutParams(scrollViewParams);
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setupHorizontalScrollView: params.height=" + params.height + ", params.topMargin=" + params.topMargin);
        }
    }
}
