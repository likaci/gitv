...
public class BufferingView extends RelativeLayout implements IThreeDimensional, IScreenUISwitcher {
    ...
    public void setBufferPercent(int percent) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setBufferProgress(" + percent + ")");
        }
        if (percent > 0) {
    ...
    public void setNetSpeed(long bytePerSecond) {
        ...
        LogUtils.m1568d("ProgressBarCenter", "net speed=" + speed);
    ...
    private void init(Context context) {
        ...
        this.mProgressBarImage = (EnhancedImageView) progressbarView.findViewById(C1291R.id.progressbar_item_center_image_id);
        this.mBuffer = (EnhancedTextView) progressbarView.findViewById(C1291R.id.progress_or_speed);
        this.mBufferFullSize = context.getResources().getDimensionPixelSize(C1291R.dimen.dimen_24dp);
        this.mAnimWidthFullSize = context.getResources().getDimensionPixelSize(C1291R.dimen.dimen_67dp);
        this.mAnimHeightFullSize = context.getResources().getDimensionPixelSize(C1291R.dimen.dimen_56dp);
    }
    ...
    private void startAnim() {
        if (this.mProgressBarImage != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "startAnim animation=" + animation);
            }
            if (animation != null) {
                animation.start();
                return;
            ...
            if (PlayerConfigManager.getPlayerConfig().isSupportAnimation()) {
                anim = (AnimationDrawable) this.mContext.getResources().getDrawable(C1291R.drawable.share_new_anim_load_center);
            } else {
                anim = (AnimationDrawable) this.mContext.getResources().getDrawable(C1291R.drawable.player_new_anim_load_center_single);
            }
            if (anim != null) {
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "onVisibilityChanged visibility=" + visibility);
        }
        if (visibility == 0) {
            startAnim();
