        ...
        if (label.pltRegionCtrls != null) {
            ...
            if (!Precondition.isEmpty(ipLoc) && ipLoc.length >= 5) {
                    ...
                    for (RegionCtrls regionCtrl : regionCtrls) {
                        switch (checkIpLoc(regionCtrl.status, regionCtrl.getCityIds(), ipLoc[4])) {
                            case FALSE:
                                ...
                                LogUtils.m1568d("checkRegionAvailable", "name = " + label.itemName + ", id = " + label.itemId + ", reason = [ status = " + regionCtrl.status + ", cityids = " + Arrays.toString(regionCtrl.getCityIds()) + ", iploc[4]= " + ipLoc[4]);
                                return false;
                            case TRUE:
                                return true;
                            default:
                    ...
                    for (RegionCtrls regionCtrl2 : regionCtrls) {
                        switch (checkIpLoc(regionCtrl2.status, regionCtrl2.getProvinceIds(), ipLoc[3])) {
                            case FALSE:
                                ...
                                LogUtils.m1568d("checkRegionAvailable", "name = " + label.name + ", id = " + label.itemId + ", reason = [ status = " + regionCtrl2.status + ", getProvinceIds = " + Arrays.toString(regionCtrl2.getProvinceIds()) + ", iploc[3]= " + ipLoc[3]);
                                return false;
                            case TRUE:
                                return true;
                            default:
                    ...
                    for (RegionCtrls regionCtrl22 : regionCtrls) {
                        switch (checkIpLoc(regionCtrl22.status, regionCtrl22.getCountryIds(), ipLoc[1])) {
                            case FALSE:
                                ...
                                LogUtils.m1568d("checkRegionAvailable", "name = " + label.name + ", id = " + label.itemId + ", reason = [ status = " + regionCtrl22.status + ", getCountryIds = " + Arrays.toString(regionCtrl22.getCountryIds()) + ", iploc[1]= " + ipLoc[1]);
                                return false;
                            case TRUE:
                                return true;
                            default:
                    ...
                    for (RegionCtrls regionCtrl222 : regionCtrls) {
                        switch (checkIpLoc(regionCtrl222.status, regionCtrl222.getAreaIds(), ipLoc[2])) {
                            case FALSE:
                                ...
                                LogUtils.m1568d("checkRegionAvailable", "name = " + label.name + ", id = " + label.itemId + ", reason = [ status = " + regionCtrl222.status + ", getAreaIds = " + Arrays.toString(regionCtrl222.getAreaIds()) + ", iploc[1]= " + ipLoc[2]);
                                return false;
                            case TRUE:
                                return true;
                            default:
                ...
                if (LogUtils.mIsDebug) {
                    ...
                    if (!defaultStatus) {
                        LogUtils.m1568d("checkRegionAvailable", "name = " + label.name + ", id = " + label.itemId + ", reason = [ defaultStatus = " + pltRegionCtrls.defaultStatus);
                    }
                }
