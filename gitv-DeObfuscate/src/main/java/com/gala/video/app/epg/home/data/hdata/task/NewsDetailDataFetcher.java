...
public class NewsDetailDataFetcher {
    ...
    class C06481 implements IVrsCallback<ApiResultDailyLabels> {
        C06481() {
        }

        public void onSuccess(ApiResultDailyLabels result) {
            if (result == null || ListUtils.isEmpty(result.labels)) {
                LogUtils.m1571e(NewsDetailDataFetcher.TAG, "fetchTabList success null == result");
                return;
            }
            List<DailyLabel> labels = result.labels;
            if (LogUtils.mIsDebug) {
                LogUtils.m1571e(NewsDetailDataFetcher.TAG, "fetchTabList success size = " + labels.size() + ", labels = " + labels);
            }
    ...
    private class FetchDataTask extends AsyncTask<Object, Object, Object> {
        ...
        protected Object doInBackground(Object... params) {
            int end = 6;
            LogUtils.m1568d(NewsDetailDataFetcher.TAG, ">>doInBackground");
        ...
        protected void onPostExecute(Object result) {
            LogUtils.m1568d(NewsDetailDataFetcher.TAG, "onPostExecute mAlbums=" + NewsDetailDataFetcher.this.mTabLabelList);
    ...
    public List<TabDataItem> startLoadByTabListSync(List<DailyTabInfo> infos) {
        LogUtils.m1568d(TAG, ">>startLoadByTabListSync");
        this.mTabLabelList.clear();
        if (ListUtils.isEmpty((List) infos)) {
            LogUtils.m1568d(TAG, "<<startLoadByTabListSync ListUtils.isEmpty(infos)");
            return null;
        }
        int i;
        for (i = 0; i < 6; i++) {
        ...
        LogUtils.m1571e(TAG, "fetch daily news info is success");
    ...
    public List<TabDataItem> startLoadSync() {
        int end = 6;
        LogUtils.m1568d(TAG, ">>startLoadSync");
        this.mTabLabelList.clear();
        fetchTabList();
        ...
        LogUtils.m1568d(TAG, "<<startLoadSync mTabLabelList=" + this.mTabLabelList);
        return new ArrayList(this.mTabLabelList);
    }

    private void fetchTabList() {
        LogUtils.m1568d(TAG, "fetchTabList");
        VrsHelper.dailyLabels.callSync(new C06481(), new String[0]);
    }

    private void fetchEveryTabList(final int index) {
        LogUtils.m1568d(TAG, ">>fetchePlayList index = " + index);
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "fetchePlayList channelId = " + channelId + ", tagSet = " + tagSet);
        }
        ...
        if (Project.getInstance().getBuild().isLitchi()) {
            VrsHelper.channelLabelsSize.callSync(new IVrsCallback<ApiResultChannelLabels>() {
                ...
                public void onSuccess(ApiResultChannelLabels result) {
                    if (result == null) {
                        LogUtils.m1568d(NewsDetailDataFetcher.TAG, "fetch channelLabels  onSuccess, but is null");
                        return;
                    }
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(NewsDetailDataFetcher.TAG, "fetchPlayList onSuccess list size = {" + result.data.getChannelLabelList().size() + "}");
                    }
            ...
            LogUtils.m1571e(TAG, "invalid channelId or tagset");
        } else {
            VrsHelper.dailyInfo.callSync(new IVrsCallback<ApiResultRecommendListQipu>() {
                public void onSuccess(ApiResultRecommendListQipu result) {
                    if (result == null || result.getAlbumList() == null) {
                        LogUtils.m1568d(NewsDetailDataFetcher.TAG, "fetchPlayList onSuccess, but is null");
                        return;
                    }
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(NewsDetailDataFetcher.TAG, "fetchPlayList onSuccess list = {" + result.getAlbumList() + "}");
                    }
