...
public class ExitOperateImagePresenter {
    ...
    private Handler mHandler = new Handler(Looper.getMainLooper()) {
        public void handleMessage(Message msg) {
            ...
            ExitOperateImagePresenter.this.mContentView.setBackgroundResource(C0508R.color.transparent);
            ExitOperateImagePresenter.this.mOperateImageView.setImageBitmap(ExitOperateImagePresenter.this.mOperateBitmap);
            ExitOperateImagePresenter.this.mContentView.setBackgroundResource(C0508R.drawable.share_item_rect_selector);
            ...
            LogUtils.m1568d(ExitOperateImagePresenter.TAG, "show, exit apk operate image show");
        }
    };
    private OnClickListener mImageClickListener = new C05773();
    private OnFocusChangeListener mImageFocusChangeListener = new C05784();
    ...
    class C05784 implements OnFocusChangeListener {
        C05784() {
        }
    ...
    private void onClickImage() {
        if (this.mExitOperateImageModel == null) {
            LogUtils.m1568d(TAG, "onClick, exit operate image data is empty");
            ...
            LogUtils.m1568d(TAG, "on click exit operate image, not support jump");
            ...
            LogUtils.m1577w(TAG, "on click exit operate image, not support Resource type :" + DataBuildTool.getItemType(this.mExitOperateImageModel.getChannelLabel()));
            ResourceOperateImageUtils.onClickForNotSupportJump(this.mContext);
        }
    }
    ...
    public void show() {
        ...
        if (model == null) {
            LogUtils.m1577w(TAG, "show, get exit operate image model is null");
        } else {
            new Thread8K(new Runnable() {
                public void run() {
                    ...
                    if (bitmap == null) {
                        LogUtils.m1568d(ExitOperateImagePresenter.TAG, "show, decodeFile, operate image bitmap is null, path = " + path);
                        return;
                    }
                    ExitOperateImagePresenter.this.mOperateBitmap = bitmap;
                    LogUtils.m1568d(ExitOperateImagePresenter.TAG, "show, decode bitmap, cost time :" + (SystemClock.elapsedRealtime() - startDecodeTime));
