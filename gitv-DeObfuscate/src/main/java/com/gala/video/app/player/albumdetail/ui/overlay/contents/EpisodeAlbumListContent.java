...
public class EpisodeAlbumListContent implements IContent<List<AlbumInfo>, AlbumInfo> {
    ...
    private OnEpisodeClickListener mEpisodeClickListener = new C13372();
    private OnEpisodeFocusChangeListener mEpisodeFocusChangedListener = new C13383();
    ...
    class C13372 implements OnEpisodeClickListener {
        C13372() {
        }
        ...
        public void onEpisodeClick(View v, int index) {
            ...
            if (VideoChecker.isExistInEpisodeListAlbum(EpisodeAlbumListContent.this.mEpisodes, playOrder)) {
                ...
                PlayerToastHelper.showToast(EpisodeAlbumListContent.this.mContext, C1291R.string.prepare_album_episode, 2000);
            } else {
                PlayerToastHelper.showToast(EpisodeAlbumListContent.this.mContext, C1291R.string.no_album_episode, 2000);
            }
        }
    }

    class C13383 implements OnEpisodeFocusChangeListener {
        C13383() {
        }

        public void onEpisodeFocus(int i) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(EpisodeAlbumListContent.this.TAG, ">> mEpisodeFocusChangedListener.onEpisodeFocus, position=" + i);
            }
        }
    }
    ...
    private void handleSelectionRefreshed(AlbumInfo albumInfo) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> handleSelectionRefreshed, new albumInfo=" + albumInfo);
        }
        if (albumInfo != null) {
    ...
    private LayoutParams getContentViewFLP() {
        ...
        if (lp == null) {
            lp = new LayoutParams(-1, ResourceUtil.getDimen(C1291R.dimen.dimen_165dp));
        }
    ...
    private void adjustEpisodelistView() {
        ...
        if (this.mAlbumInfo == null) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "adjustEpisodelistView, mAlbumInfo does not set!!!");
            }
            ...
            if (playOrder > 0) {
                ...
                LogUtils.m1568d(this.TAG, "adjustEpisodelistView, invalid play order, albumInfo=" + this.mAlbumInfo);
            }
        } else if (LogUtils.mIsDebug) {
    ...
    public void setSelection(AlbumInfo item) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> setSelection, oldAlbuminfo=" + this.mAlbumInfo);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> setSelection, newAlbuminfo=" + item);
        }
        if (Looper.getMainLooper() == Looper.myLooper()) {
    ...
    private void refreshEpisodeSelection() {
        ...
        if (this.mAlbumInfo == null) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "refreshEpisodeSelection, mAlbumInfo is null!");
            }
