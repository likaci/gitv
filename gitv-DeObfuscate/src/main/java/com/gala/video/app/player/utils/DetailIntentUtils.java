...
public class DetailIntentUtils {
    ...
    private static class MyPluginStateListener implements OnStateChangedListener {
        ...
        public void onSuccess() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(DetailIntentUtils.TAG, "[PERF-LOADING]tm_plugin.load");
            }
        ...
        public void onFailed() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(DetailIntentUtils.TAG, "[PERF-LOADING] playerplugin.Load onFailed");
            }
        }

        public void onCanceled() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(DetailIntentUtils.TAG, "onCanceled");
            }
            if (this.mIsOut) {
                ((Activity) this.mContext).finish();
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(DetailIntentUtils.TAG, "finish");
                }
            }
        }
        ...
        public void onLoading() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(DetailIntentUtils.TAG, "onLoading");
            }
        }
    }
    ...
    public static void startDetailForLoop(Context context, int channelId, int loopPosition, String from, String channelName) {
        if (context != null && channelId >= 0) {
            LogUtils.m1571e(TAG, ">>startDetailForLoop4 context=" + context + ", channelId = " + channelId + ", loopPosition" + loopPosition + ",channelName" + channelName + ", from=" + from);
            Intent intent = new Intent();
