...
public class UCenterPresenter {
    ...
    private Runnable f1955r = new C11782();
    ...
    class C11782 implements Runnable {
        C11782() {
        }
    ...
    static class ApiCallback implements IApiCallback<ApiResultAlbumList> {
        ...
        public void onSuccess(ApiResultAlbumList result) {
            ...
            if (outer != null) {
                Album album;
                LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> TVApiHelper.get().vipChannelData --- succ");
                ...
                for (int i = 0; i < albums.size(); i++) {
                    ...
                    LogUtils.m1570d("TEST", ">>>>> name,url ---", album2.name, ", ", album2.pic);
        ...
        public void onException(ApiException e) {
            if (((UCenterPresenter) this.mOuter.get()) != null) {
                LogUtils.m1571e("EPG/MyAccount/UCenterPresenter", "Exception --- TVApiHelper.get().vipChannelData.call");
            }
        }
    }
    ...
    static class ApiTinyurlCallback implements IApiCallback<ApiResultTinyurl> {
        ...
        public void onSuccess(ApiResultTinyurl result) {
            ...
            if (outer != null) {
                final Bitmap qrBitmap = QRUtils.createQRImage(result.data.url, ResourceUtil.getDimen(C0508R.dimen.dimen_404dp), ResourceUtil.getDimen(C0508R.dimen.dimen_404dp));
        ...
        public void onException(ApiException exception) {
            final UCenterPresenter outer = (UCenterPresenter) this.mOuter.get();
            if (outer != null) {
                LogUtils.m1571e("EPG/MyAccount/UCenterPresenter", "onException --- TVApi.tinyurl.call");
    ...
    static class ImageCallback implements IImageCallback {
        ...
        public void onSuccess(ImageRequest imageRequest, Bitmap bitmap) {
            UCenterPresenter outer = (UCenterPresenter) this.mOuter.get();
            if (outer != null) {
                LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> mImageProvider.loadImage --- succ");
                BitmapAlbum bitmapAlbum = new BitmapAlbum();
                bitmapAlbum.setBitmap(bitmap);
                bitmapAlbum.setAlbum((Album) imageRequest.getCookie());
                LogUtils.m1570d("TEST2", ">>>>> name, url----", bitmapAlbum.getAlbum().name, ", ", imageRequest.getUrl());
        ...
        public void onFailure(ImageRequest imageRequest, Exception e) {
            if (((UCenterPresenter) this.mOuter.get()) != null) {
                LogUtils.m1573e("EPG/MyAccount/UCenterPresenter", "onFailure --- mImageProvider.loadImage ---", imageRequest.getUrl());
            }
        }
    }
    ...
    static class WorkRunnable implements Runnable {
        ...
        private void checkUserInfo(final UCenterPresenter outer) {
            ...
            if (respBean == null || respBean.getRespResult()) {
                ...
                if (this.mWorkInfo.isShowPingback) {
                    LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> show pingback in 'My-Page'");
                    String s1 = outer.mPingbackS1;
                    String tabsrc = PingBackUtils.getTabSrc();
                    PingBackParams params = new PingBackParams();
                    params.add("bstp", "1").add("s1", s1).add("tabsrc", tabsrc).add("qtcurl", "mine_loggedin").add("block", "mine").add("e", outer.mPingbackE).add(Keys.f2035T, "21");
            ...
            if (exception == null) {
                LogUtils.m1571e("test", ">>>>> checkUserInfo - exception == null");
            } else if ("-50".equals(exception.getHttpCode())) {
                LogUtils.m1571e("test", ">>>>> checkUserInfo - exception - NET_ERROR_CODE");
            } else {
                CharSequence errorCode = exception.getCode();
                LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", "PassportTVHelper.userInfo.call exception code is : " + errorCode);
                if ("-100".equals(errorCode)) {
                    LogUtils.m1571e("test", ">>>>> checkUserInfo - exception - JSON_ERROR_CODE");
                } else if (StringUtils.isEmpty(errorCode)) {
                    LogUtils.m1571e("test", ">>>>> checkUserInfo - exception - errorCode is empty");
                } else {
                    CharSequence warning = AccountInfoTipHelper.getAccountWarningStr(errorCode);
                    if (StringUtils.isEmpty(warning)) {
                        LogUtils.m1571e("test", ">>>>> checkUserInfo - exception - warning tip is empty");
                        LogUtils.m1571e("EPG/MyAccount/UCenterPresenter", ">>>>> warning tip is empty");
                        return;
                    }
        ...
        private void loadVipChannel(UCenterPresenter outer) {
            ...
            LogUtils.m1571e("EPG/MyAccount/UCenterPresenter", ">>>>> loadAlbumPicture error, dynamicQDataModel is null");
        }
    }
    ...
    public void clearUserInfo() {
        ...
        LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>>showAccountExceptionDialog --- OnClickListener");
    }
    ...
    public void logoutDialogDismiss() {
        if (!this.flag) {
            LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> GlobalVipDialog --- OnDismissListener");
    ...
    private void sendClickPingBack(String rseat) {
        ...
        PingBackParams params = new PingBackParams();
        params.add("rpage", "logout").add("block", "rec").add("rseat", rseat).add(Keys.f2035T, "20").add("c1", "").add("r", "").add("s1", this.mPingbackS1).add("e", this.mPingbackE).add("rt", "i");
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
    ...
    public void startSecurityCenter() {
        ...
        ThreadUtils.execute(new C11771());
    }
    ...
    private void callSecurityQRBitmap() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1570d("EPG/MyAccount/UCenterPresenter", ">>>>>change password url:", qrContent);
        }
    ...
    public void checkUserInfo(boolean isShowPingback) {
        if (GetInterfaceTools.getIGalaAccountManager().isLogin(this.mContext)) {
            LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> checkUserInfo - user is login, check userinfo");
            ThreadUtils.execute(new WorkRunnable(this, new WorkInfo(2, isShowPingback)));
            return;
        }
        LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> checkUserInfo - user is no login");
        if (isShowPingback) {
            LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> show pingback in 'My-Page'");
            String s1 = this.mPingbackS1;
            String tabsrc = PingBackUtils.getTabSrc();
            PingBackParams params = new PingBackParams();
            params.add("bstp", "1").add("s1", s1).add("tabsrc", tabsrc).add("qtcurl", "mine_guest").add("block", "mine").add("e", this.mPingbackE).add(Keys.f2035T, "21");
            PingBack.getInstance().postPingBackToLongYuan(params.build());
        }
    }
    ...
    private String getStatus() {
        ...
        if (userType == null) {
            return ResourceUtil.getStr(C0508R.string.str_my_novip);
        }
        ...
        if (userType.isPlatinum()) {
            if (userType.isExpire()) {
                return ResourceUtil.getStr(C0508R.string.vip_expire_tip);
            }
            return getVipDeadLine();
        } else if (userType.isLitchiOverdue()) {
            return ResourceUtil.getStr(C0508R.string.my_overdue_vip);
        } else {
            return ResourceUtil.getStr(C0508R.string.str_my_novip);
        }
    }
    ...
    public void loadAlbumPicture() {
        if (!GetInterfaceTools.getIGalaAccountManager().isLogin(this.mContext)) {
            LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> loadAlbumPicture return, no login");
        } else if (ListUtils.isEmpty(this.mBitmapAlbumList) || this.mBitmapAlbumList.size() != 3) {
            ThreadUtils.execute(new WorkRunnable(this, 1));
        } else {
            LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>> loadAlbumPicture return, bitmapList has values already");
        }
    }
    ...
    public void showLoading() {
        ...
        this.mHandler.postDelayed(this.f1955r, 1500);
    }

    public void stopLoading() {
        this.mLoadingFlag = false;
        if (this.f1955r != null) {
            this.mHandler.removeCallbacks(this.f1955r);
        }
    ...
    public void onPause() {
        if (this.f1955r != null) {
            LogUtils.m1568d("EPG/MyAccount/UCenterPresenter", ">>>>>UCenterPresenter.onPause");
            this.mHandler.removeCallbacks(this.f1955r);
            this.mMyView.hideLoadingUI();
        }
    }
