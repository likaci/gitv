...
public class TopTitlePanel {
    ...
    private void initViews() {
        ...
        this.mTopTitleView = ((ViewStub) this.mRootView.findViewById(C1291R.id.stub_detail_top_title)).inflate();
        this.mTopTitleView.findViewById(C1291R.id.detail_top_view).setBackgroundColor(this.mContext.getResources().getColor(C1291R.color.detail_top_title_background_color));
        this.mTitleText = (TextView) this.mTopTitleView.findViewById(C1291R.id.detail_top_title_text);
        this.mTitleText.setTextColor(this.mContext.getResources().getColor(C1291R.color.detail_top_title_text_color));
        setBackgroud();
    }

    private void setBackgroud() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> setBackgroud");
        }
    ...
    private Drawable createBackGroundDrawable(Drawable drawable) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> createBackGroundDrawable, drawable=" + drawable);
        }
        int w = drawable.getIntrinsicWidth();
        int h = drawable.getIntrinsicHeight();
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createBackGroundDrawable, drawable h=" + h + ", w=" + w);
        }
        int[] deviceSize = DeviceUtils.getDeviceSize(this.mContext);
        int deviceWidth = deviceSize[0];
        int deviceHeight = deviceSize[1];
        int bitmapHeight = ResourceUtil.getDimen(C1291R.dimen.dimen_74dp);
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createBackGroundDrawable, bitmap bitmapHeight=" + bitmapHeight + ", deviceWidth=" + deviceWidth);
        }
    ...
    public void show(String title) {
        ...
        if (!this.mIsPanelShown) {
            ...
            LogUtils.m1568d(TAG, "show, panel is shown.");
        }
    }
    ...
    public void hide() {
        ...
        if (this.mIsPanelShown) {
            ...
            LogUtils.m1568d(TAG, "hide, panel is hidden.");
        }
    }
