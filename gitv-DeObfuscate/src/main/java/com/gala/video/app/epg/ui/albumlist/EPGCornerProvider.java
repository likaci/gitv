...
public class EPGCornerProvider extends Wrapper {
    ...
    public String getDescRB(Album album, QLayoutKind layout) {
        ...
        if (albumType == AlbumKind.SIGLE_VIDEO) {
            ...
            return ResourceUtil.getStr(C0508R.string.offline_album_play_order, Integer.valueOf(order));
        } else if (albumType == AlbumKind.SIGLE_SERIES) {
            if (StringUtils.isEmpty(getDateShort(album))) {
                return str;
            }
            return ResourceUtil.getStr(C0508R.string.album_item_update, format);
        } else if (albumType == AlbumKind.SERIES_ALBUM) {
            if (album.tvsets != album.tvCount && album.tvCount != 0) {
                return ResourceUtil.getStr(C0508R.string.album_item_tvcount, Integer.valueOf(album.tvCount));
            } else if (album.tvsets != album.tvCount || album.tvsets == 0) {
                return str;
            } else {
                return ResourceUtil.getStr(C0508R.string.album_item_tvset, Integer.valueOf(album.tvsets));
            }
            ...
            return ResourceUtil.getStr(C0508R.string.super_album_item_time, format);
        }
    }
    ...
    public Album getRealAlbum(ChannelLabel label) {
        ...
        if (ResourceType.LIVE.equals(label.getType())) {
            ...
            if (ListUtils.isEmpty(liveList)) {
                LogUtils.m1571e(TAG, "getRealAlbum ---ResourceType.LIVE--- liveList = null");
            } else {
                album = (Album) liveList.get(0);
            }
            if (album == null) {
                LogUtils.m1571e(TAG, "getRealAlbum ---ResourceType.LIVE--- album = null");
            }
        } else {
