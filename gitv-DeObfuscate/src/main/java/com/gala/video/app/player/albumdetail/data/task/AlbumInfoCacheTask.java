    ...
    public void getAlbum(final IAlbumInfoCacheTaskListener listener) {
        synchronized (mLock) {
            ...
            if (album != null) {
                ...
                LogUtils.m1568d(TAG, "album is null,now requrest network");
                TVApi.albumInfo.callSync(new IApiCallback<ApiResultAlbum>() {
                    public void onSuccess(ApiResultAlbum result) {
                        ...
                        LogUtils.m1568d(AlbumInfoCacheTask.TAG, "onSuccess: fetched info=" + DataUtils.albumInfoToString(album));
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(AlbumInfoCacheTask.TAG, "end -onRun: album = " + album.isVipForAccount() + "||" + album.isSinglePay() + "||" + album.isCoupon());
                        }
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(AlbumInfoCacheTask.TAG, "end -onRun: local album=" + album);
                        }
                        listener.onSuccess(album);
                    }

                    public void onException(ApiException e) {
                        LogUtils.m1568d(AlbumInfoCacheTask.TAG, "onException: code=" + e.getCode() + ", msg=" + e.getMessage());
                        listener.onFailed(e);
                    }
                }, this.mAlbumId);
            }
