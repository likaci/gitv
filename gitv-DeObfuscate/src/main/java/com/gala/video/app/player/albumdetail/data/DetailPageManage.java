...
public class DetailPageManage {
    ...
    public void loadData(String resId, boolean isSwitchSourceRefresh, AlbumInfo albumInfo, JobController controller) {
        ...
        LogUtils.m1568d(TAG, "resId:" + resId + " ,isSwitchSourceRefresh = " + isSwitchSourceRefresh + ", + albumInfo = " + albumInfo);
        this.mAlbumInfo = albumInfo;
    ...
    public void updateDetailDate(String resId) {
        if (DetailDataCacheManager.instance().isGroupDetailOverTime(resId)) {
            ...
            LogUtils.m1568d(TAG, "updateDetailDate-" + System.currentTimeMillis());
    ...
    private void loadDetailGroupInfo() {
        LogUtils.m1568d(TAG, "loadDetailGroupInfo-" + System.currentTimeMillis());
        UikitEvent event = new UikitEvent();
        event.uikitEngineId = this.mEngine.getId();
        event.eventType = 64;
    ...
    public void onUikitEvent(UikitEvent uikitEvent) {
        if (uikitEvent.uikitEngineId == this.mEngine.getId()) {
            LogUtils.m1568d(TAG, "uikitEvent.eventType = " + uikitEvent.eventType);
            int i;
            CardInfoModel model;
            ...
            switch (uikitEvent.eventType) {
                case 64:
                    LogUtils.m1568d(TAG, "onUikitEvent LOADER_GROUP_DEAIL ");
                    if (uikitEvent.layoutChange == 0) {
                        LogUtils.m1574i(TAG, "initPageAction NODATA!!!  hideLoading");
                        this.mEngine.getPage().hideLoading();
                        return;
                    }
                    return;
                case 69:
                    LogUtils.m1568d(TAG, "mIsSwitchSourceRefresh = " + this.mIsSwitchSourceRefresh);
                    if (uikitEvent.cardList == null) {
                        LogUtils.m1571e(TAG, "uikitEvent.cardList null");
                        return;
                    }
                    LogUtils.m1568d(TAG, "onUikitEvent LOADER_DETAIL_CARDS- uikitEvent.cardList = " + uikitEvent.cardList);
                    ...
                    fetchBannerAd();
                    LogUtils.m1568d(TAG, ">>>> fetchBannerAd ");
                    clearOtherCards();
                    ...
                    return;
                case 71:
                    LogUtils.m1568d(TAG, "onUikitEvent LOADER_MORE_DETAIL_CARDS-" + uikitEvent.sourceId + "-pageNo-" + uikitEvent.pageNo);
                    if (uikitEvent.cardList == null) {
                        LogUtils.m1571e(TAG, "uikitEvent.cardList null");
                        this.mEngine.getPage().hideLoading();
                        return;
                    }
                    LogUtils.m1568d(TAG, "onUikitEvent LOADER_MORE_DETAIL_CARDS- uikitEvent.cardList = " + uikitEvent.cardList);
                    for (i = 0; i < ListUtils.getCount(uikitEvent.cardList); i++) {
                        LogRecordUtils.logd(TAG, "listCard -> " + i + ", data -> " + ((CardInfoModel) uikitEvent.cardList.get(i)).toString());
                    }
                    notifyCard = new CopyOnWriteArrayList();
                    ...
                    return;
                case 73:
                    LogUtils.m1568d(TAG, "onUikitEvent LOADER_DETAIL_SAVE_DATA ");
        ...
        LogUtils.m1568d(TAG, "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx:" + uikitEvent.uikitEngineId + "  " + this.mEngine.getId());
    }
    ...
    public void createDefault(int index) {
        ...
        LogUtils.m1568d(TAG, "createDefault");
    }
    ...
    public void onActivityIn() {
        LogUtils.m1568d(TAG, "onActivityIn");
    ...
    public void onActivityOut() {
        LogUtils.m1568d(TAG, "onActivityOut");
        this.mEngine.stop();
        this.mstarted = false;
