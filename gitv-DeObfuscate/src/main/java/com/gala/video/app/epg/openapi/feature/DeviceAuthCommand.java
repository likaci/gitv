...
public class DeviceAuthCommand extends ServerCommand<Void> {
    ...
    private class MyListener implements IApiCallback<ApiResultDeviceCheck> {
        ...
        public void onSuccess(ApiResultDeviceCheck apiResult) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(DeviceAuthCommand.TAG, "authDevice.onSuccess() success!");
            }
            ...
            if (devCheck != null) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(DeviceAuthCommand.TAG, "authDevice() pass ip=" + devCheck.ip);
                }
                AppRuntimeEnv.get().setDeviceIp(devCheck.ip);
                PingBackParams params = new PingBackParams();
                params.add(Keys.f2035T, Values.value16).add("r", Values.value00001).add("rt", "13").add("st", "0");
                ...
                LogUtils.m1568d(DeviceAuthCommand.TAG, "authDevice() faill for devChek is empty!");
            }
        }

        public void onException(ApiException e) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1569d(DeviceAuthCommand.TAG, "authDevice.onException() faill! ", e);
            }
            if (e != null) {
                LogUtils.m1568d(DeviceAuthCommand.TAG, "authDevice.onException() faii:[" + e.getCode() + "], [" + e.getHttpCode() + "], " + e.getMessage());
                this.exception = e;
            }
    ...
    public synchronized Bundle onProcess(Bundle params) {
        ...
        synchronized (this) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "process() device auth " + this.mDevice.isDevCheckPass() + ", increase=" + listener.increaseAccessCount + ", apiException = " + listener.exception + ", httpCode = " + httpCode);
            }
            int code = 2;
            if (this.mDevice.isDevCheckPass()) {
                code = 0;
    ...
    private boolean isServerError(String serverCode) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isServerError(" + serverCode + ") return " + serverError);
        }
        return serverError;
    }
    ...
    protected boolean isAllowedAccess() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isAllowedAccess() allowed=" + allowed + ", special=" + specialAllowed);
        }
