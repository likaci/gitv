...
public class DataRequestRefreshTask extends BaseRequestTask {
    ...
    class C06331 implements IApiCallback<ApiResultRefreshTime> {
        C06331() {
        }
        ...
        public void onSuccess(ApiResultRefreshTime result) {
            if (result != null && result.getData() != null) {
                for (RefreshTime time : result.getData()) {
                    if (time != null) {
                        ...
                        if (time.ext != null) {
                            ...
                            LogUtils.m1568d(DataRequestRefreshTask.TAG, "home data refresh time rules= " + time.rules + ",group id = " + time.ext.resgid + ", level = " + time.level);
                        }
                    }
                }
                ...
                LogUtils.m1568d(DataRequestRefreshTask.TAG, "fastGroupRefreshTime = " + fastGroupRefreshTime);
        ...
        public void onException(ApiException e) {
            String str;
            LogUtils.m1572e(DataRequestRefreshTask.TAG, "fetch refresh time exception", e);
    ...
    public void invoke() {
        LogUtils.m1568d(TAG, "invoke task refresh time request Task");
        TVApi.refreshTime.callSync(new C06331(), Project.getInstance().getBuild().getVersionString());
    }

    public void onOneTaskFinished() {
        LogUtils.m1568d(TAG, "get current server time : " + DeviceUtils.getServerTimeMillis() + " current day : " + DeviceUtils.getServerCurrentDate());
    }
}
