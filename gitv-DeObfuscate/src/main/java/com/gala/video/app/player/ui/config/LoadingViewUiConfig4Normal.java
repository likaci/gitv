...
public class LoadingViewUiConfig4Normal implements ILoadingViewUiConfig {
    ...
    private TreeMap<String, String> getLoadingFrameAnimPath() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getLoadingFrameAnimPath: playNewUrlList=" + playNewUrlList);
        }
        if (!ListUtils.isEmpty(playNewUrlList)) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "getLoadingFrameAnimPath: play new url not NULL");
            }
            ...
            if (!ListUtils.isEmpty((List) pathList)) {
                ...
                try {
                    if (!StringUtils.isEmpty(imagePath) && imagePath.contains("-")) {
                        ...
                        try {
                            ...
                            if (LogUtils.mIsDebug) {
                                LogUtils.m1568d(TAG, "getLoadingFrameAnimPath listsize=" + pathList.size());
                            }
                            treeMap = treeMap2;
                        } catch (Exception e2) {
                            e = e2;
                            treeMap = treeMap2;
                            if (LogUtils.mIsDebug) {
                                LogUtils.m1572e(TAG, "getLoadingFrameAnimPath: exception happened", e);
                            }
                            return treeMap;
                        }
                    }
                ...
                LogUtils.m1568d(TAG, "getLoadingFrameAnimPath: pathList is null");
                return null;
            }
        }
        return treeMap;
    ...
    private Bitmap decodeFile(String cachePath) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "decodeFile loadingImage=" + loadingImage);
        }
        return loadingImage;
    }
    ...
    public void checkLoadingFrameAnim() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getLoadingFrameAnim() treeMap=" + treeMap);
        }
    }
    ...
    private String getLoadingLogoPath() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getLoadingLogoPath path=" + path);
        }
        return path;
    }
    ...
    private String getLoadingBgPath() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getLoadingBgPath path=" + path);
        }
        return path;
    }
    ...
    public int getLogoWidth() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_logo_width);
    }

    public int getLogoHeight() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_logo_height);
    }

    public int getLogoMarginLeft() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_logo_marginleft);
    }

    public int getSpeedTxtSize() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_speed_txt_size);
    }

    public int getSpeedTxtMargiTop() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_speed_txt_margintop);
    }

    public int getTxtNameMarginLeftAndRight() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_txt_name_margin_left_and_right);
    }
    ...
    public int getTxtNameSize() {
        return getPixelFromDimensIdSafe(C1291R.dimen.video_play_text_size);
    }

    public int getDeriveWidth() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_derive_width);
    }

    public int getDeriveHeight() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_derive_height);
    }

    public int getDeriveMarginTop() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_derive_margin_top);
    }
    ...
    public int getAnimMarginTop() {
        return getPixelFromDimensIdSafe(C1291R.dimen.loading_anim_margintop);
    }

    public int getHelpTipSizeFull() {
        return getPixelFromDimensIdSafe(C1291R.dimen.dimen_30dp);
    }

    public int getHelpTipMarginTopFull() {
        return getPixelFromDimensIdSafe(C1291R.dimen.dimen_40dp);
    }

    public int getHelpTipTxtSizeFull() {
        return getPixelFromDimensIdSafe(C1291R.dimen.dimen_20dp);
    }
}
