    ...
    public static void updateImageView(final ImageView view, String url, final Handler uiHandler) {
        if (view == null) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1577w(TAG, "updateImageView: ImageView is null");
            }
        } else if (TextUtils.isEmpty(url)) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1577w(TAG, "updateImageView: no new logo url, return");
            }
            ...
            ImageProviderApi.getImageProvider().loadImage(new ImageRequest(url), new IImageCallback() {
                public void onSuccess(ImageRequest request, final Bitmap bitmap) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(ImageViewUtils.TAG, "updateImageView onSuccess");
                    }
                    uiHandler.post(new Runnable() {
                        public void run() {
                            if (LogUtils.mIsDebug) {
                                LogUtils.m1568d(ImageViewUtils.TAG, "setImageBitmap" + bitmap);
                            }
                ...
                public void onFailure(ImageRequest request, Exception exception) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1569d(ImageViewUtils.TAG, "updateImageView onFailure", exception);
                    }
                }
            });
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1577w(TAG, "updateImageView null == uiHandler");
        }
    }
            ...
            ImageProviderApi.getImageProvider().loadImage(new ImageRequest(url), new IImageCallback() {
                public void onSuccess(ImageRequest request, Bitmap bitmap) {
                    ...
                    uiHandler.post(new Runnable() {
                        public void run() {
                            if (LogUtils.mIsDebug) {
                                LogUtils.m1568d(ImageViewUtils.TAG, "setImageBitmap" + mScaleBitmap);
                            }
