...
public class MultiEventHelper extends Wrapper {
    ...
    private OnMultiScreenStateChangeListener mMultiScreenStateChangeListener = new C14387();
    ...
    class C14343 implements Runnable {
        C14343() {
        }
    ...
    class C14387 implements OnMultiScreenStateChangeListener {
        C14387() {
        }

        public void onSeekEnd() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onSeekEnd");
            }
        ...
        public void onScreenModeSwitched(ScreenMode screenMode) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onScreenModeSwitched(" + screenMode + ")");
            }
        ...
        public void onMovieSwitched(IVideo video) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onMovieSwitched(" + video.toStringBrief() + ")");
            }
        ...
        public void onMovieStop() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onMovieStop()");
            }
        ...
        public void onMovieStart() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onMovieStart()");
            }
        ...
        public void onMoviePause() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onMoviePause()");
            }
            MultiEventHelper.this.sendMessage(2);
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "notify onSeekFinish in onMoviePause");
            }
            MultiScreen.get().onSeekFinish();
        }
        ...
        public void onMovieComplete() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onMovieComplete()");
            }
            MultiEventHelper.this.sendMessage(3);
            MultiEventHelper.this.resetPlaybackState();
        }
        ...
        public void onError(IVideo video, ISdkError error) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onError(" + (video != null ? video.toStringBrief() : "NULL") + "," + error + ")");
            }
            if (error == null) {
                MultiEventHelper.this.sendMessage(3);
        ...
        public void onBitStreamListReady(List<BitStream> bitStreams) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onBitStreamListReady(" + bitStreams + ")");
            }
        ...
        public void onBitStreamChange(BitStream bitStream) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onBitStreamChange(" + bitStream + ")");
            }
        ...
        public void onAdStart() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(MultiEventHelper.this.TAG, "onAdStart()");
            }
            MultiEventHelper.this.sendMessage(1);
    ...
    public void onGetSceneAction(KeyValue keyValue) {
        String string;
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "onGetSceneAction(" + keyValue + ")");
        }
        keyValue.setAppendSeek(true);
        keyValue.setSceneId(SceneId.PLAY_PAGE);
        if (this.mContext != null) {
            string = this.mContext.getResources().getString(C1291R.string.vc_exit_player);
        } else {
            string = "";
        }
        keyValue.addExactMatch(string, new C14321());
        for (ISceneActionProvider provider : this.mProviderList) {
            List<ISceneActionData> dataList = provider.onGetSceneActionData();
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "onGetSceneAction: provider=" + provider + ", scene data=" + dataList + ")");
            }
            if (dataList != null) {
    ...
    public List<AbsVoiceAction> getSupportedVoices(List<AbsVoiceAction> actions) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "onGetSceneAction(" + actions + ")");
        }
        actions.add(CreateInterfaceTools.createVoiceCommon().createAbsVoiceAction(this.mContext != null ? this.mContext.getResources().getString(C1291R.string.vc_exit_player) : "", new C14332(), KeyWordType.DEFAULT));
        for (ISceneActionProvider provider : this.mProviderList) {
            List<ISceneActionData> dataList = provider.onGetSceneActionData();
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "onGetSceneAction: provider=" + provider + ", scene data=" + dataList + ")");
    ...
    public List<AbsVoiceAction> getSupportedVoicesWithoutPreAndNext(List<AbsVoiceAction> actions) {
        ...
        actions.add(CreateInterfaceTools.createVoiceCommon().createAbsVoiceAction(this.mContext != null ? this.mContext.getResources().getString(C1291R.string.vc_exit_player) : "", new C14343(), KeyWordType.DEFAULT));
        for (ISceneActionProvider provider : this.mProviderList) {
            List<ISceneActionData> dataList = provider.onGetSceneActionData();
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "onGetSceneAction: provider=" + provider + ", scene data=" + dataList + ")");
            ...
            if (dataList != null) {
                ...
                while (i < dataList.size()) {
                    if (((ISceneActionData) dataList.get(i)).getType() == SceneType.PRE_VIDEO || ((ISceneActionData) dataList.get(i)).getType() == SceneType.NEXT_VIDEO) {
                        LogUtils.m1568d(this.TAG, "getSupportedVoicesWithoutPreAndNext: provider=" + provider + ", scene data=" + dataList + ")");
                    } else {
                        actions = getSupportedCommonKeyWordTypeVoices((ISceneActionData) dataList.get(i), actions);
                    }
                    i++;
    ...
    private List<AbsVoiceAction> getSupportedCommonKeyWordTypeVoices(ISceneActionData data, List<AbsVoiceAction> actions) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "handleSceneData>dataList.get(i) = " + type.toString());
        }
        if (!(type == null || this.mContext == null)) {
            ...
            switch (type) {
                case PRE_VIDEO:
                    actions.add(provider.createAbsVoiceAction(res.getString(C1291R.string.vc_prev_episode), data.getActionRunnable(), KeyWordType.DEFAULT));
                    actions.add(provider.createAbsVoiceAction(15, "-1", data.getActionRunnable(), KeyWordType.DEFAULT));
                    break;
                case NEXT_VIDEO:
                    actions.add(provider.createAbsVoiceAction(res.getString(C1291R.string.vc_next_episode), data.getActionRunnable(), KeyWordType.DEFAULT));
                    actions.add(provider.createAbsVoiceAction(15, "1", data.getActionRunnable(), KeyWordType.DEFAULT));
                    break;
                case LAST_VIDEO:
                    actions.add(provider.createAbsVoiceAction(res.getString(C1291R.string.vc_last_episode), data.getActionRunnable(), KeyWordType.DEFAULT));
                    break;
                case OFF_SKIP_TAIL:
                    actions.add(provider.createAbsVoiceAction(res.getString(C1291R.string.vc_disable_skip_ht_6), data.getActionRunnable(), KeyWordType.DEFAULT));
                    break;
                case ON_SKIP_TAIL:
                    actions.add(provider.createAbsVoiceAction(res.getString(C1291R.string.vc_enable_skip_ht_4), data.getActionRunnable(), KeyWordType.DEFAULT));
                    break;
                case SELECT_EPISODE:
                    int episodeSelect = ((Integer) data.getKey().mParams[0]).intValue();
                    String nthEpisodeStr = res.getString(C1291R.string.vc_nth_episode, new Object[]{Integer.valueOf(episodeSelect)});
                ...
                case CHANGE_BITSTREAM:
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(this.TAG, "onGetSceneAction: provider=" + PlayerDataUtils.getBitStreamString(this.mContext, def));
                    }
    ...
    private void handleSceneData(ISceneActionData data, KeyValue keyValue) {
        ...
        if (type != null && this.mContext != null) {
            ...
            switch (type) {
                case PRE_VIDEO:
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_prev_episode), data.getActionRunnable());
                    return;
                case NEXT_VIDEO:
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_next_episode), data.getActionRunnable());
                    return;
                case LAST_VIDEO:
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_last_episode), data.getActionRunnable());
                    return;
                case OFF_SKIP_TAIL:
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_disable_skip_ht_1), data.getActionRunnable());
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_disable_skip_ht_2), data.getActionRunnable());
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_disable_skip_ht_3), data.getActionRunnable());
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_disable_skip_ht_4), data.getActionRunnable());
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_disable_skip_ht_5), data.getActionRunnable());
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_disable_skip_ht_6), data.getActionRunnable());
                    return;
                case ON_SKIP_TAIL:
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_enable_skip_ht_1), data.getActionRunnable());
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_enable_skip_ht_2), data.getActionRunnable());
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_enable_skip_ht_3), data.getActionRunnable());
                    keyValue.addExactMatch(res.getString(C1291R.string.vc_enable_skip_ht_4), data.getActionRunnable());
                    return;
                case SELECT_EPISODE:
                    int episodeSelect = ((Integer) data.getKey().mParams[0]).intValue();
                    keyValue.addReservedMatch(res.getString(C1291R.string.vc_nth_episode, new Object[]{Integer.valueOf(episodeSelect)}), data.getActionRunnable());
                    return;
                case CHANGE_BITSTREAM:
                    BitStream def = data.getKey().mParams[0];
                    if (LogUtils.mIsDebug) {
                ...
                case RECOMMEND_LIST:
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(this.TAG, "onGetSceneAction: provider=" + video + ", videoSelect =" + videoSelect + ")");
                    }
                    keyValue.addFuzzyMatch(video.getAlbumName(), data.getActionRunnable());
                    keyValue.addReservedMatch(res.getString(C1291R.string.vc_nth_video, new Object[]{Integer.valueOf(videoSelect)}), data.getActionRunnable());
                    return;
                default:
                    return;
            }
    ...
    public boolean onDlnaKeyEvent(DlnaKeyEvent event, KeyKind key) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "onDlnaKeyEvent() event=" + event + ", key=" + key);
        }
    ...
    private boolean updownToVolume(int keycode) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "updownToVolume, keycode = " + keycode);
        }
        boolean ret = false;
        int targetKeyCode = -1;
        if (19 == keycode) {
        ...
        if (ret && interval >= 100) {
            ...
            ThreadUtils.execute(new Runnable() {
                public void run() {
                    try {
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(MultiEventHelper.this.TAG, "sendKeyDownUpSync: " + keyCode);
                        }
                        MultiEventHelper.this.mInst.sendKeyDownUpSync(keyCode);
                    } catch (Exception e) {
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1572e(MultiEventHelper.this.TAG, "mInstrument.sendKeyDownUpSync exception", e);
                        }
                    }
                }
            });
    ...
    public boolean onKeyChanged(int keycode) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "onKeyChanged() keycode=" + keycode);
        }
    ...
    public Notify onPhoneSync() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> onPhoneSync()");
        }
        ...
        synchronized (this.mFetchMultiEventLock) {
            ...
            try {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1569d(this.TAG, "onPhoneSync(), mFetchMultiEventLock.wait() throw exception ", e);
                }
            }
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "<< onPhoneSync() ret = " + JSON.toJSONString(notify));
        }
        return notify;
    }
    ...
    private void updateMultiEvent(Notify notify) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> updateMultiEvent()");
        }
        synchronized (this.mFetchMultiEventLock) {
            try {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(this.TAG, "<< updateMultiEvent(), mMultiEvent = " + JSON.toJSONString(notify));
                }
    ...
    private int getPlayState(IGalaVideoPlayer player) {
        ...
        if (this.mLastState != 6) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "getPlayState return " + i);
            }
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "getPlayState return STATE_PURCHASE");
        }
        return i;
    }
    ...
    private void sendMessage(int playState) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "sendMessage, notify.play_state=" + notify.play_state + ", notify.vip_purchase=" + notify.vip_purchase);
        }
    }
    ...
    public boolean onResolutionChanged(String newRes) {
        ...
        if (bs.getDefinition() != 0) {
            ...
            LogUtils.m1568d(this.TAG, "onResolutionChanged(" + newRes + "), invalid resolution! " + bs);
        }
        return false;
    }
    ...
    public boolean onSeekChanged(long newPosition) {
        if (this.mKeyEventHelper != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "onSeekChanged() newPosition=" + newPosition + ", currentPosition=" + currentPosition);
            }
    ...
    public boolean isPushVideoByTvPlatform() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "isPushVideoByTvPlatform return " + useTvPlatform);
        }
    ...
    private boolean isUserCannotPreviewCode(ISdkError error) {
        ...
        if (error != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "isUserCannotPreviewCode(" + errorCode + ") returns " + ret);
            }
        }
        return ret;
    }
    ...
    private boolean isVideoPreview(IVideo video) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "isVideoPreview(" + video + ") returns " + isPreview);
        }
        return isPreview;
    }
}
