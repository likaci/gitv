...
public class FetchDetailHistoryJob extends VideoJob {
    ...
    public void onRun(JobController controller) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> onRun: qpId=" + video.getAlbumId() + "tvid=" + video.getTvId() + this.hasHistory);
        }
        ...
        if (StringUtils.equals("carousel_rec", this.mFrom)) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "<< onRun from carousel_rec");
            }
            ...
            if (historyInfo == null || historyInfo.getAlbum().getContentType() != ContentType.FEATURE_FILM) {
                video.getAlbum().time = "";
                if (LogUtils.mIsDebug) {
                    LogUtils.m1574i(TAG, " onRun -end, historyInfo != ContentType.FEATURE_FILM");
                }
            } else {
            ...
            notifyJobSuccess(controller);
            if (LogUtils.mIsDebug) {
                LogUtils.m1574i(TAG, " onRun -end, video=" + video.toStringBrief());
            }
        } else {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "onRun hasHistory = false.");
            }
            notifyJobSuccess(controller);
        }
    }
