...
public class ExitOperateImageProvider {
    ...
    public void download() {
        LogUtils.m1568d(TAG, "download new exit operate data");
    ...
    public void download(List<ChannelLabel> channelLabels) {
        ...
        if (ListUtils.isEmpty((List) channelLabels)) {
            LogUtils.m1568d(TAG, "download, exit operate label is empty");
            return;
        }
        ...
        LogUtils.m1568d(TAG, "download, all raw exit operate label size :" + labelSize);
        ...
        DownloaderAPI.getDownloader().loadFiles(requestList, new IFileCallback() {
            ...
            public void onFailure(FileRequest fileRequest, Exception e) {
                ...
                LogUtils.m1568d(ExitOperateImageProvider.TAG, "download, IFileCallback---OnFailure,url ," + (fileRequest != null ? fileRequest.getUrl() : "FileRequest is null"));
            }
        });
    }

    private void downloadFinished(List<ExitOperateImageModel> tempExitOperateImageModels) {
        LogUtils.m1568d(TAG, "downloadFinished, exit operate data size : " + ListUtils.getCount((List) tempExitOperateImageModels));
    ...
    public void onEmptyExitOperateData() {
        LogUtils.m1568d(TAG, "onEmptyExitOperateData");
    ...
    public ExitOperateImageModel getExitOperateImageModel() {
        ...
        LogUtils.m1568d(TAG, "getExitOperateImageModel  , index = " + index);
    ...
    private synchronized void writeExitOperateImageModelListToCache(List<ExitOperateImageModel> ExitOperateImageModels) {
        LogUtils.m1568d(TAG, "writeExitOperateImageModelListToCache...");
        try {
            SerializableUtils.write(ExitOperateImageModels, HomeDataConfig.HOME_EXIT_OPERATE_DIR);
            setLocalData(ExitOperateImageModels);
        } catch (IOException e) {
            LogUtils.m1578w(TAG, "writeExitOperateImageModelListToCache, write exit operate data failed", e);
        }
    }
    ...
    private synchronized List<ExitOperateImageModel> getLocalExitOperateModelList() {
        ...
        try {
            ...
            LogUtils.m1572e(TAG, "getLocalExitOperateModelList, read exit operate data failed", e);
        }
        LogUtils.m1568d(TAG, "getLocalExitOperateModelList, size = " + (!ListUtils.isEmpty(labelList) ? labelList.size() : 0));
        return labelList;
    }
