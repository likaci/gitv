...
public class FeedBackActivationStateTimer extends Timer {
    ...
    private final OnNetStateChangedListener mNetStateChangedListener = new C05331();
    private int minutes = 0;
    private int retryTimes = 7;

    class C05331 implements OnNetStateChangedListener {
        C05331() {
        }

        public void onStateChanged(int oldState, int newState) {
            LogUtils.m1571e(FeedBackActivationStateTimer.LOG_TAG, "network state changed net state = " + newState);
            switch (newState) {
                case 1:
                case 2:
    ...
    class FeedBackTimerTask extends TimerTask {
        ...
        class C05351 implements IApiCallback<ApiResultCode> {

            class C05341 implements StateCallback {
                C05341() {
                }

                public void getStateResult(int state) {
                    LogUtils.m1571e(FeedBackActivationStateTimer.LOG_TAG, "onNetworkState---" + state);
                    switch (state) {
                        case 0:
                        case 3:
                        case 4:
            ...
            public void onSuccess(ApiResultCode apiResult) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1571e(FeedBackActivationStateTimer.LOG_TAG, "feedbackState onSuccess result=" + apiResult.getCode());
                }
            ...
            public void onException(ApiException e) {
                if (LogUtils.mIsDebug) {
                    LogUtils.m1571e(FeedBackActivationStateTimer.LOG_TAG, "feedbackState onException e=" + e);
                }
                NetWorkManager.getInstance().checkNetWork(new C05341());
            }
        }
        ...
        public void run() {
            ...
            LogUtils.m1568d(FeedBackActivationStateTimer.LOG_TAG, "timerTask-->minutes= " + FeedBackActivationStateTimer.this.minutes + " count=" + FeedBackActivationStateTimer.this.count);
            if (FeedBackActivationStateTimer.this.count < FeedBackActivationStateTimer.this.retryTimes && Arrays.asList(FeedBackActivationStateTimer.TIMEINTERVAL).contains(String.valueOf(FeedBackActivationStateTimer.this.minutes))) {
                ...
                TVApi.feedbackState.call(new C05351(), GetInterfaceTools.getIGalaVipManager().getActivationAccount(), String.valueOf(FeedBackActivationStateTimer.this.count));
