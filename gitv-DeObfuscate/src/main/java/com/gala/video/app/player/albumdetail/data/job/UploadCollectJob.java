    ...
    public void onRun(final JobController controller) {
        ...
        if (albumInfo != null && albumInfo.getAlbum() != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, ">> onRun: subtype=" + DataHelper.favSubType + ", mSubKey=" + DataHelper.favSubKey + ", chnId=" + albumInfo.getChannelId());
            }
            if (GetInterfaceTools.getIGalaAccountManager().isLogin(context)) {
                ...
                UserHelper.uploadCollect.call(new IVrsCallback<ApiResultCode>() {
                    public void onSuccess(ApiResultCode result) {
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(UploadCollectJob.TAG, "collectUploadAsync login onSuccess");
                        }
                    ...
                    public void onException(ApiException e) {
                        if (LogUtils.mIsDebug) {
                            LogUtils.m1568d(UploadCollectJob.TAG, "collectUploadAsync login onException:" + e.getCode());
                        }
                        if (!(ApiCode.USER_INFO_CHANGED.equals(e.getCode()) || ApiCode.ERROR_USER_IP.equals(e.getCode()))) {
                            QToast.makeTextAndShow(context, C1291R.string.fav_failed, 2000);
                        }
            ...
            UserHelper.uploadCollectForAnonymity.call(new IVrsCallback<ApiResultCode>() {
                public void onSuccess(ApiResultCode arg0) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(UploadCollectJob.TAG, "collectUploadAsync Anonymity onSuccess");
                    }
                    albumInfo.setFavored(true);
                    UploadCollectJob.this.notifyJobSuccess(controller);
                }
                ...
                public void onException(ApiException e) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(UploadCollectJob.TAG, "collectUploadAsync Anonymity onException:" + e.getCode());
                    }
            ...
            LogUtils.m1571e(TAG, "onRun: invalid data!!");
        }
    }
}
