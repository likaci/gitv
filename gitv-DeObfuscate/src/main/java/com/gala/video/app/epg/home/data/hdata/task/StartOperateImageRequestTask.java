...
public class StartOperateImageRequestTask extends BaseRequestTask {
    ...
    class C06541 implements IVrsCallback<ApiResultChannelLabels> {
        C06541() {
        }

        public void onSuccess(ApiResultChannelLabels result) {
            if (result == null) {
                LogUtils.m1577w(StartOperateImageRequestTask.TAG, "fetchStartImageData, onSuccess, ApiResultChannelLabels is null");
            ...
            if (labels == null) {
                LogUtils.m1577w(StartOperateImageRequestTask.TAG, "fetchStartImageData, onSuccess, ChannelLabels is null");
                StartOperateImageRequestTask.this.clearLimitBehavior();
                StartOperateImageProvider.getInstance().onEmptyStartOperateData();
                return;
            }
            ...
            if (ListUtils.isEmpty((List) labelList)) {
                LogUtils.m1577w(StartOperateImageRequestTask.TAG, "fetchStartImageData, onSuccess, ChannelLabel List is empty");
                StartOperateImageRequestTask.this.clearLimitBehavior();
                StartOperateImageProvider.getInstance().onEmptyStartOperateData();
                return;
            }
            LogUtils.m1577w(StartOperateImageRequestTask.TAG, "fetchStartImageData, onSuccess, ChannelLabel size:" + ListUtils.getCount((List) labelList));
            ...
            while (iterator.hasNext()) {
                ...
                if (ResourceType.LIVE == label.getType() && !label.checkLive()) {
                    iterator.remove();
                    LogUtils.m1568d(StartOperateImageRequestTask.TAG, "fetchStartImageData, remove out of date live data, id : " + label.itemId);
                }
            }
    ...
    private void fetchStartImageData() {
        ...
        if (dynamicResult != null) {
            LogUtils.m1577w(TAG, "fetchStartImageData, start operation image resource id : " + dynamicResult.getSpecifiedOperateImageResId(OperationImageType.START));
            VrsHelper.channelLabelsSize.call(new C06541(), id, "0", "5.0", "60");
            return;
        }
        LogUtils.m1577w(TAG, "fetchStartImageData, get dynamic data, dynamicResult is null");
    }
}
