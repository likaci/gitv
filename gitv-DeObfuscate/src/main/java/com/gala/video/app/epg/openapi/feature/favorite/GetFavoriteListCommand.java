...
public class GetFavoriteListCommand extends MaxCommand<List<Media>> {
    ...
    private class AnonymousListener extends ResultListHolder<Media> implements IVrsCallback<ApiResultCollectList> {
        ...
        public void onSuccess(ApiResultCollectList result) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GetFavoriteListCommand.TAG, "AnonymousListener.onSuccess(" + result + ")");
            }
            ...
            for (Album info : result.getAlbumList()) {
                ...
                if (media != null) {
                    ...
                    LogUtils.m1577w(GetFavoriteListCommand.TAG, "onSuccess() cannot translate to sdk media!!!");
                }
                if (isReachMax()) {
                    LogUtils.m1577w(GetFavoriteListCommand.TAG, "onSuccess() reach max size !!!" + this);
                    return;
                }
            }
        }
        ...
        public void onException(ApiException exception) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GetFavoriteListCommand.TAG, "AnonymousListener.onException(" + exception + ")");
            }
    ...
    private class UserListener extends ResultListHolder<Media> implements IVrsCallback<ApiResultCollectList> {
        ...
        public void onSuccess(ApiResultCollectList result) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GetFavoriteListCommand.TAG, "UserListener.onSuccess(" + result + ")");
            }
            setNetworkValid(true);
            for (Album info : result.getAlbumList()) {
                Media media = OpenApiUtils.createSdkMedia(info);
        ...
        public void onException(ApiException exception) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GetFavoriteListCommand.TAG, "UserListener.onException(" + exception + ")");
            }
            setNetworkValid(!OpenApiNetwork.isNetworkInvalid(exception));
            setCode(7);
        }
    ...
    protected Bundle onProcess(Bundle params) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "process() pageNo=" + pageNo + ", pageSize=" + pageSize + ", maxCount=" + maxCount);
        }
