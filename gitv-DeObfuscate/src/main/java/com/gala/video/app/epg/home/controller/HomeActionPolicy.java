...
public class HomeActionPolicy extends ActionPolicy {
    ...
    private class ToastRunnable implements Runnable {
        ...
        public void run() {
            HomeActionPolicy.this.mToast = QToast.makeText(HomeActionPolicy.this.mHomeController.getContext(), ResourceUtil.getStr(C0508R.string.back_key_toast), (int) QToast.LENGTH_4000);
    ...
    public void onFirstLayout(ViewGroup parent) {
        LogUtils.m1568d(TAG, "onFirstLayout pageindex " + this.mPageindex + ",is started = " + this.mBindPage.mStarted + ",builded = " + this.mBindPage.mBuilded + ",is Selected = " + this.mBindPage.mSelected + "is Loading = " + this.mBindPage.mIsLoading + " mIsDefault = " + this.mBindPage.mIsDefault + "Engine id:" + this.mBindPage.getUIkitEngine().getId() + " parent = " + parent);
        ...
        if (this.mBindPage.mBuilded && this.mBindPage.mSelected && LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "[start performance] first layout cost " + (SystemClock.elapsedRealtime() - this.mBindPage.getLoadedTime()) + " ms" + " ,pageindex = " + this.mPageindex);
        }
    ...
    private int getLine(int position) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (card == null) {
                LogUtils.m1577w(TAG, "getLine. card==null.");
