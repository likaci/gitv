...
public abstract class AbsMenuPanel extends RelativeLayout {
    ...
    class C15031 implements OnHorizontalScrollListener {
        C15031() {
        }

        public void onScrollStopped(List<Integer> curItems) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(AbsMenuPanel.this.TAG, "onScrollStopped:" + curItems);
            }
    ...
    public class MyContentItemListener<T> implements IItemListener<T> {
        ...
        public void onItemClicked(T data, int index) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG_M, ">> onItemClicked, mType=" + this.mType + ", data=" + data + ", index=" + index);
            }
            switch (this.mType) {
                case 1:
        ...
        public void onItemSelected(T data, int index) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG_M, ">> onItemSelected, mType=" + this.mType + ", data=" + data + ", index=" + index);
            }
            switch (this.mType) {
                case 8:
                    if (data != null) {
        ...
        public void onItemFilled() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG_M, ">> onItemSelected, mType=" + this.mType);
            }
            switch (this.mType) {
                case 1:
                case 2:
    ...
    static class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(AbsMenuPanel.TAG_S, "HideHandler.handleMessage(" + msg + " )");
            }
    ...
    public AbsMenuPanel(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mOnHorizontalScrollListener = new C15031();
    ...
    public ViewParent invalidateChildInParent(int[] location, Rect dirty) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> invalidateChildInParent, campatibleMode=" + this.campatibleMode + ", clipChildren=" + this.clipChildren);
        }
        if (this.campatibleMode && !this.clipChildren) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "invalidateChildInParent, location=[" + location[0] + ", " + location[1] + "], dirty=" + dirty);
            }
    ...
    protected final void notifyRecommendShow(int start, int end) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> notifyRecommendShow() :" + start + "/" + end);
        }
        if (this.mCurrentVideo != null) {
            ...
            if (!ListUtils.isEmpty(associatives)) {
                ...
                LogUtils.m1577w(this.TAG, "notifyRecommendShow: list is empty!");
            }
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1571e(this.TAG, "notifyRecommendShow: current video is null!");
        }
    }
    ...
    private void sendRecommendClickPingback(IVideo clickVideo) {
        if (clickVideo != null && this.mCurrentVideo != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "sendRecommendClickPingback rseat=" + (index + 1));
            }
            PingbackFactory.instance().createPingback(7).addItem(C0165R.R_TYPE(clickVideo.getTvId())).addItem(BLOCK.BLOCK_TYPE(getProgramBlock())).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE(String.valueOf(index + 1))).addItem(RPAGE.RPAGE_ID(getRpage())).addItem(C1.C1_TYPE(String.valueOf(clickedAlbum.chnId))).addItem(NOW_C1.NOW_C1_TYPE(getC1())).addItem(NOW_QPID.NOW_QPID_TYPE(getNowQpid())).addItem(NOW_C2.NOW_C2_TYPE(getC2())).addItem(C2.C2_TYPE(getC2())).post();
        }
    }
    ...
    private void sendGuessYouLikeClickedPingback(IVideo clickVideo) {
        if (clickVideo == null) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1571e(this.TAG, "sendGuessYouLikeClickedPingback clickvideo is null");
            }
            ...
            if (clickedAlbum != null) {
                ...
                LogUtils.m1571e(this.TAG, "sendGuessYouLikeClickedPingback: null == clickedAlbum!");
            }
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1571e(this.TAG, "sendGuessYouLikeClickedPingback: current video is null!");
        }
    }

    private int findIndexByTvIdFromVideoList(String tvId, List<IVideo> list) {
        LogUtils.m1568d(this.TAG, "findIndexByTvIdFromVideoList() tvId:" + tvId);
        ...
        LogUtils.m1568d(this.TAG, "<<findIndexByTvId ret=" + index);
        return index;
    }
    ...
    protected void notifyVideoChanged(IVideo video) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, ">> notifyEpisodeClick, video=" + video.toStringBrief());
        }
    ...
    protected void notifyVideoRatioSelected(int ratio) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "notifyVideoRatioSelected(" + ratio + ", listener =" + this.mChangeVideoRatioListener + ")");
        }
    ...
    public void setVideo(IVideo video) {
        if (video != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "setVideo: oldVideo=" + oldVideo + ", new video=" + this.mCurrentVideo);
            }
            ...
            LogUtils.m1568d(this.TAG, "setVideo, video is null, return");
        }
    }
    ...
    public void enableShow(boolean enable) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1574i(this.TAG, "enableShow(" + enable + ")");
        }
        if (!enable) {
            hide();
        }
    ...
    public final void show(int key) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1574i(this.TAG, ">> show: enabled=" + this.mEnableShow + ", inited=" + this.mInited + ", videoChanged=" + this.mVideoChanged.get());
        }
        if (this.mEnableShow) {
            this.mKey = key;
    ...
    public void hide() {
        ...
        if (getVisibility() != 8) {
            ...
            doHide();
            if (LogUtils.mIsDebug) {
                LogUtils.m1574i(this.TAG, "hide() mNeedUpdate=" + this.mNeedUpdate);
            }
    ...
    public void setIsShowAssociatives(boolean isShow) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setIsShowAssociatives(" + isShow + ")");
        }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1571e(TAG_S, "AbsMenupanel dispatchKeyEvent(" + event + " )");
        }
        if (event.getAction() == 0) {
            switch (event.getKeyCode()) {
                ...
                case 164:
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1571e(TAG_S, "volume is invalid");
                        break;
                    }
                    break;
                default:
    ...
    protected void notifyUserInteractionBegin(UserInteractionType type) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "notifyUserInteractionBegin(" + type + ")");
        }
        this.mHandler.removeMessages(20000);
    ...
    private void handleVideoClicked(IVideo video, int index, int contentType) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "handleVideoClicked, index=" + index + ", video=" + video);
        }
        ...
        switch (contentType) {
            ...
            default:
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(this.TAG, "handleVideoClicked, unhandled content type=" + contentType);
                    break;
                }
                break;
        }
        if (!video.getTvId().equals(this.mCurrentVideo.getTvId())) {
            ...
            LogUtils.m1568d(this.TAG, "click the same episode from menu panel.");
        }
    }

    private void handleBitstreamChanged(BitStream bitStream, int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "handleBitstreamChanged, index=" + index + ", bitStream=" + bitStream);
        }
    ...
    private void sendClickBitStreamPingback(BitStream bitStream) {
        if (this.mCurrentVideo != null) {
            PingbackFactory.instance().createPingback(31).addItem(C0165R.R_TYPE(getTvqID())).addItem(BLOCK.BLOCK_TYPE(this.mHDRBlock)).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE("ra_" + String.valueOf(BitStream.getBid(bitStream)))).addItem(RPAGE.RPAGE_ID(getRpage())).addItem(C1.C1_TYPE(getC1())).addItem(C2.C2_TYPE(getC2())).addItem(NOW_C1.NOW_C1_TYPE(getC1())).addItem(NOW_QPID.NOW_QPID_TYPE(getNowQpid())).addItem(NOW_C2.NOW_C2_TYPE(getC2())).post();
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1571e(this.TAG, "mCurrentVideo is null");
        }
    }
    ...
    private void handleHDRToggleChanged(int index) {
        if (index <= 1) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "handleHDRToggleChanged, index=" + index);
            }
    ...
    private void handlePause(int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "handlePause, index=" + index);
        }
        switch (index) {
            case 4:
    ...
    private void handleSkipHeaderChanged(Boolean skip, int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "handleSkipHeaderChanged, index=" + index + ", skip=" + skip);
        }
    ...
    private void handleScreenRatioChanged(int ratio, int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "handleScreenRatioChanged, index=" + index + ", ratio=" + ratio);
        }
    ...
    private void handleDimensionChanged(Boolean is3D, int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "handleDimensionChanged, index=" + index + ", is3D=" + is3D);
        }
    }

    private void sendVideoRatioClickPingback(int ratio) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "sendVideoRatioChangePingback()");
        }
        String rseat = "";
        if (ratio == 1) {
            rseat = SEAT_ORIGINAL;
    ...
    private void sendEpisodeClickPingback(List<IVideo> list, IVideo clickVideo, int index, int contentType) {
        ...
        if (clickVideo != null && this.mCurrentVideo != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(this.TAG, "sendEpisodeClickPingback, contentType=" + contentType + ", rseat=" + rseat + ", isPrevue=" + isPrevue + ", clickedc2=" + clickedc2);
            }
            PingbackFactory.instance().createPingback(3).addItem(C0165R.R_TYPE(clickVideo.getTvId())).addItem(BLOCK.BLOCK_TYPE(getProgramBlock())).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE(String.valueOf(rseat))).addItem(RPAGE.RPAGE_ID(getRpage())).addItem(ISPREVUE.ISPREVUE_TYPE(isPrevue)).addItem(C1.C1_TYPE(String.valueOf(clickVideo.getChannelId()))).addItem(C2.C2_TYPE(getC2())).addItem(NOW_C1.NOW_C1_TYPE(String.valueOf(this.mCurrentVideo.getChannelId()))).addItem(NOW_QPID.NOW_QPID_TYPE(getNowQpid())).addItem(NOW_C2.NOW_C2_TYPE(getC2())).addItem(NOW_EPISODE.ITEM(curPlayIndex)).post();
        } else if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "sendEpisodeClickPingback video is null");
        }
    }
    ...
    private void sendSkipHeadTailPingback(boolean skip) {
        if (this.mCurrentVideo != null) {
            ...
            PingbackFactory.instance().createPingback(20).addItem(C0165R.R_TYPE(getTvqID())).addItem(BLOCK.BLOCK_TYPE("skip")).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE(rseat)).addItem(RPAGE.RPAGE_ID(getRpage())).addItem(C1.C1_TYPE(getC1())).addItem(NOW_C1.NOW_C1_TYPE(getC1())).addItem(NOW_QPID.NOW_QPID_TYPE(getNowQpid())).addItem(NOW_C2.NOW_C2_TYPE(getC2())).addItem(C2.C2_TYPE(getC2())).post();
        }
    }
    ...
    public void setAdData(BaseAdData data) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setAdData() data=" + data);
        }
        if (data == null) {
            return;
        }
    ...
    private void setPrecisionAdData(BaseAdData data) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "setPrecisionAdData()");
        }
        if (this.mAssociativeContentHolder != null) {
