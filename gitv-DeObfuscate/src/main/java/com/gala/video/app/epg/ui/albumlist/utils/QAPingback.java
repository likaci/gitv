...
public class QAPingback {
    ...
    public static void multiMenuAfterLoad(Context context, AlbumInfoModel infoModel, BaseDataApi dataApi, View view, long startLoadingTime) {
        if (infoModel != null && dataApi != null && view != null && infoModel.isMultiHasData() && ListUtils.getCount(dataApi.getOriginalDataList()) > 0) {
            ...
            if (dataApi.getCurPage() <= 1) {
                ...
                LogUtils.m1568d(LOG_TAG, ">>>>>ptype = 0-0");
                params.add(Keys.f2035T, "5").add("a", "0").add("s1", "1").add("s2", "3").add("rt", Values.value18).add("e", album.eventId).add("c1", String.valueOf(infoModel.getChannelId())).add(Keys.PTYPE, "0-0").add(Keys.POS, "").add("bkt", "").add(Keys.QISOST, "").add(Keys.TARGET, "").add("site", "gala");
    ...
    public static void channelItemClick(Album album, AlbumInfoModel infoModel, int mIndexOfCurPage) {
        if (album != null) {
            ...
            if (infoModel.isMultiHasData()) {
                PingBackParams params = new PingBackParams();
                LogUtils.m1568d(LOG_TAG, ">>>>>ptype = 1-1");
                params.add(Keys.f2035T, "5").add("s1", "1").add("s2", "3").add("rt", Values.value18).add("e", album.eventId).add("c1", String.valueOf(c1)).add("bkt", album.bkt).add(Keys.TARGET, album.doc_id).add(Keys.QISOST, album.qisost + plid).add(Keys.PTYPE, "1-1").add("site", "gala").add(Keys.POS, String.valueOf(mIndexOfCurPage)).add("a", "0");
                PingBack.getInstance().postPingBackToLongYuan(params.build());
            }
        }
    }
    ...
    public static void clickChannelLabelItem(AlbumInfoModel infoModel, ChannelLabel label) {
        ...
        if (album != null && infoModel != null) {
            ...
            sendMyPageClick(r, ResourceUtil.getStr(C0508R.string.label_recommend), "i", infoModel.getRseat(), getRpageOrQcturl(infoModel), null, String.valueOf(c1), plid, getTvsrchsource(infoModel), getKeyword(infoModel), "");
            if (ResourceType.DIY.equals(label.getType())) {
                sendMyPageClick("H5page", ResourceUtil.getStr(C0508R.string.label_recommend), "i", infoModel.getRseat(), infoModel.getChannelName(), null, "", plid, getTvsrchsource(infoModel), getKeyword(infoModel), "");
            }
        }
    }
    ...
    public static void searchRequestPingback(Context context, List<?> list, long time, int pageNum, int itemCount, int clickType, String keyword) {
        ...
        if (ListUtils.isEmpty((List) list)) {
            ...
            params1.add(Keys.f2035T, "9").add("s1", "1").add("s2", "3").add("rt", "3").add("source", source).add("r", keyword).add("keyword", keyword).add("first_docs", "0").add("second_docs", "0").add("time", String.valueOf(time)).add(Keys.PAGE, String.valueOf(pageNum));
            PingBack.getInstance().postPingBackToLongYuan(params1.build());
            ...
            params1 = new PingBackParams();
            params1.add(Keys.f2035T, "9").add("s1", "1").add("s2", "3").add("rt", "3").add("source", source).add(Keys.DOCIDS, docIds.toString()).add("c1", c1).add("e", e).add("bkt", album.bkt).add(Keys.SEARCH_TIME, String.valueOf(album.searchtime)).add("url", url).add("r", keyword).add("keyword", keyword).add("first_docs", "0").add("second_docs", "0").add("time", String.valueOf(time)).add(Keys.PAGE, String.valueOf(pageNum));
            PingBack.getInstance().postPingBackToLongYuan(params1.build());
        }
    }
    ...
    public static void searchItemClickPingback(Context context, Album album, int position, int clickType, String keyword, int curPageNum, boolean isShowingCard) {
        if (album != null) {
            ...
            if (clickType == 0) {
                params1 = new PingBackParams();
                params1.add(Keys.f2035T, "5").add("s1", "1").add("s2", "3").add("rt", "3").add("a", "0").add(Keys.PTYPE, "1-1").add("c1", String.valueOf(album.c1)).add("e", album.eventId).add("bkt", album.bkt).add("site", album.site).add(Keys.TARGET, album.doc_id).add("r", keyword).add("keyword", keyword).add(Keys.POS, m_n).add(Keys.IS_CARD, isShowingCard ? "1" : "0").add(Keys.PAGE, String.valueOf(curPageNum));
                PingBack.getInstance().postPingBackToLongYuan(params1.build());
            } else if (clickType == 1) {
                params1 = new PingBackParams();
                params1.add(Keys.f2035T, "5").add("s1", "1").add("s2", "3").add("rt", "3").add("a", "0").add(Keys.PTYPE, "1-2").add("c1", String.valueOf(album.c1)).add("e", album.eventId).add("bkt", album.bkt).add("site", album.site).add(Keys.TARGET, album.doc_id).add("r", keyword).add("keyword", keyword).add(Keys.POS, m_n).add(Keys.IS_CARD, isShowingCard ? "1" : "0").add(Keys.PAGE, String.valueOf(curPageNum));
                PingBack.getInstance().postPingBackToLongYuan(params1.build());
            } else if (clickType == 2) {
                params1 = new PingBackParams();
                params1.add(Keys.f2035T, "5").add("s1", "1").add("s2", "3").add("rt", "3").add("a", "0").add(Keys.PTYPE, "").add("c1", String.valueOf(album.c1)).add("e", album.eventId).add("bkt", album.bkt).add("site", album.site).add(Keys.TARGET, album.doc_id).add("r", keyword).add("keyword", keyword).add(Keys.POS, m_n).add(Keys.IS_CARD, isShowingCard ? "1" : "0").add(Keys.PAGE, String.valueOf(curPageNum));
                PingBack.getInstance().postPingBackToLongYuan(params1.build());
            }
        }
    }
    ...
    public static void searchShowPingback() {
        PingBackParams params = new PingBackParams();
        params.add(Keys.f2035T, "21").add("bstp", "1").add("qtcurl", "search").add("block", "search").add("s2", null);
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }

    public static void searchBannerAdShowPingback() {
        PingBackParams params = new PingBackParams();
        params.add(Keys.f2035T, "21").add("qtcurl", "search").add("block", "通栏广告").add("c1", "").add("qpid", "通栏广告").add("bstp", "1");
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
    ...
    public static void error(String lOG_TAG, final String channelId, final String dataTagName, final ApiException e) {
        ThreadUtils.execute(new Runnable() {
            public void run() {
                ...
                PingBackParams params = new PingBackParams();
                params.add(Keys.f2035T, "0").add("e", eventId).add("ec", "315008").add("pfec", pfec).add(Keys.APINAME, apiname);
                PingBack.getInstance().postPingBackToLongYuan(params.build());
    ...
    public static void sendMyPageClick(String r, String block, String rt, String rseat, String rpage, System nullValueA, String c1, String plid, String tvsrchsource, String keyword, String s3, System nullValueB, String qtcurl, String allitem, String dftitem, String sawitem, String line, System nullValueC, String s1) {
        PingBackParams params = new PingBackParams();
        params.add(Keys.f2035T, "20").add("r", r).add("block", block).add("rt", rt).add("rseat", rseat).add("rpage", rpage).add("c1", c1).add("plid", plid).add("tvsrchsource", tvsrchsource).add("keyword", keyword).add(Keys.S3, s3).add("qtcurl", qtcurl).add("allitem", allitem).add("dftitem", dftitem).add("sawitem", sawitem).add("line", line).add("s1", s1);
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
    ...
    public static void sendMyPageShow(String c1, String qtcurl, String qpId, String rfr, System nullValueA, String td, String block, String plid, String r, String s2, System nullValueB, String tvsrchsource, String card, String e, String s1) {
        PingBackParams params = new PingBackParams();
        params.add(Keys.f2035T, "21").add("bstp", "1").add("tabsrc", PingBackUtils.getTabSrc()).add("c1", c1).add("qtcurl", qtcurl).add("qpid", qpId).add("rfr", rfr).add("td", td).add("block", block).add("tvsrchsource", tvsrchsource).add("plid", plid).add("r", r).add("s2", s2).add("card", card).add("e", e).add("s1", s1);
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
}
