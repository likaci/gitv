...
public class VipRightsActivateActivity extends QMultiScreenActivity implements OnClickListener, OnFocusChangeListener {
    ...
    private IKeyboardListener mAccountKeyboardInterface = new C11524();
    private String mActivationCode;
    private Button mBtnLogin;
    private String mCookice;
    private ImageView mIconErrorBottom;
    private IKeyboardListener mPasswordKeyboardInterface = new C11535();
    ...
    class C11492 implements Runnable {
        C11492() {
        }
    ...
    class C11513 implements IActivationCallback {
        ...
        public void onException(ApiException exception) {
            LogUtils.m1573e("EPG/myaccount/VipRightsActivateActivity", ">>>>> Exception -- BOSSHelper.buyProductByActivationCodeOTT.call(), errorCode: ", exception.getCode());
            ...
            if (errorModel != null) {
                ...
                LogUtils.m1571e("EPG/myaccount/VipRightsActivateActivity", ">>>>> ErrorCodeModel -- ErrorCodeModel is null !!");
            }
            ...
            if (StringUtils.isEmpty((CharSequence) warning)) {
                warning = VipRightsActivateActivity.this.getString(C0508R.string.update_network_error);
            }
            vipRightsActivateActivity.promptTextDialog(warning);
            if (VipRightsActivateActivity.this.mVerifClickable) {
                VipRightsActivateActivity.this.runOnUiThread(new C11501());
            }
        }
    }

    class C11524 implements IKeyboardListener {
        C11524() {
        }
    ...
    class C11535 implements IKeyboardListener {
        C11535() {
        }
    ...
    class C11546 implements Runnable {
        C11546() {
        }

        public void run() {
            QToast.makeTextAndShow(VipRightsActivateActivity.this, C0508R.string.login_msg_vip_rights_activate_success, 2000);
        }
    }

    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(C0508R.layout.epg_vip_rights_activate_layout);
        preHandleBigPic();
        Intent intent = getIntent();
        if (intent != null) {
            this.mS2 = intent.getStringExtra(LoginConstant.ACTIVATE_S2);
            if (LogUtils.mIsDebug) {
                LogUtils.m1570d("EPG/myaccount/VipRightsActivateActivity", ">>>>> intent.getStringExtra --- s2=", this.mS2);
            }
        }
        initLayout();
        ...
        ThreadUtils.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        if (VipRightsActivateActivity.this.mVerificationBitmap != null && !VipRightsActivateActivity.this.mVerificationBitmap.isRecycled()) {
                            LogUtils.m1570d("EPG/myaccount/VipRightsActivateActivity", " --- VerificationTask--- runOnUiThread , mVerificationBitmap = ", VipRightsActivateActivity.this.mVerificationBitmap);
                            ...
                            if (!(tempBitmap == null || tempBitmap.isRecycled())) {
                                LogUtils.m1570d("EPG/myaccount/VipRightsActivateActivity", " --- VerificationTask--- recycle , tempBitmap = ", tempBitmap);
                                tempBitmap.recycle();
                            }
            ...
            public void run() {
                try {
                    ...
                    if (bitmap != null) {
                        LogUtils.m1576i("EPG/myaccount/VipRightsActivateActivity", "onSuccess --- mImageProvider： ", url);
                        VipRightsActivateActivity.this.mVerificationBitmap = bitmap;
                        VipRightsActivateActivity.this.runOnUiThread(new C11461());
                        return;
                    }
                    LogUtils.m1573e("EPG/myaccount/VipRightsActivateActivity", "onFailure --- mImageProvider： ", url);
                    VipRightsActivateActivity.this.runOnUiThread(new C11472());
                } catch (IOException e1) {
                    LogUtils.m1573e("EPG/myaccount/VipRightsActivateActivity", "IOException --- ", e1.getMessage());
                    e1.printStackTrace();
                }
            }
        });
    ...
    private void initTxtAccount() {
        ...
        this.mTxtAccount.post(new C11492());
    }
    ...
    private void sendActivateRequest(String activationCode, String verificationCode) {
        GetInterfaceTools.getIGalaAccountManager().buyProductByActivationCodeOTT(activationCode, verificationCode, new C11513());
    }
    ...
    private boolean checkAccountIsValid() {
        if (StringUtils.isEmpty(getAccountTxt())) {
            showTipText(C0508R.string.vip_rights_input_account);
            switchCursor(this.mTxtAccount, false);
            return false;
    ...
    private boolean checkPasswordIsValid() {
        if (StringUtils.isEmpty(getPasswordTxt())) {
            showTipText(C0508R.string.vip_rights_input_password);
    ...
    private void switchKeyboard(TextView textView, boolean isCommitPress) {
        ...
        if (viewID == C0508R.id.epg_input_login_name) {
            ...
            this.mAccountKeyboard.setConfirmTextAndDrawable(C0508R.string.keyboard_get, 0);
    ...
    private void showTipText(int strId) {
        ...
        if (strId > 0) {
            ...
            if (strId == C0508R.string.vip_rights_input_account) {
                text = this.mTipTop;
                this.nIconErrorTop.setVisibility(0);
            } else if (strId == C0508R.string.vip_rights_input_password) {
    ...
    private void toastActivationSuccess() {
        new Handler(Looper.getMainLooper()).post(new C11546());
    }
