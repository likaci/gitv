    ...
    public static void printItemData(ItemData itemData, String className) {
        if (itemData != null && HomeDebug.DEBUG_LOG) {
            try {
                ...
                LogUtils.m1576i(TAG, "className = ", className);
                LogUtils.m1576i(TAG, "itemData = ", itemData);
                LogUtils.m1576i(TAG, "channelLabel = ", channelLabel);
                LogUtils.m1576i(TAG, "album = ", album);
                int channelId = StringUtils.parse(info.getField(2), 0);
                LogUtils.m1576i(TAG, "isSingleType(album) = ", Boolean.valueOf(GetInterfaceTools.getAlbumInfoHelper().isSingleType(album)), " getAlbumType(album) = ", GetInterfaceTools.getAlbumInfoHelper().getAlbumType(album), " channelId = ", Integer.valueOf(channelId), " isVerticalType(channelId) = ", Boolean.valueOf(GetInterfaceTools.getCornerProvider().isSpecialChannel(channelId)), " desL1RBString = ", info.getText(8), " desL3String = ", info.getText(4), " info.getText(IDataConstant.TEXT_TITLE)", info.getText(3));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
