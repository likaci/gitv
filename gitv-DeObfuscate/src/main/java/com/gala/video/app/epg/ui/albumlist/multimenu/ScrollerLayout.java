...
public class ScrollerLayout extends LinearLayout {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        switch (event.getKeyCode()) {
            case 21:
                if (this.mCurViewPos <= 0) {
                    LogUtils.m1568d("EPG/weight/ScrollerLayout", ">>>>> left first");
                    return true;
                }
                computeLeftScroll();
            ...
            case 22:
                if (this.mCurViewPos + 1 >= getChildCount() - 1) {
                    LogUtils.m1568d("EPG/weight/ScrollerLayout", ">>>>> right last");
                    return true;
                }
                computeRightScroll();
                if (this.mCurView == null) {
    ...
    private void computeLeftScroll() {
        ...
        if (getScrollX() > 0 && getLocationX(this.mCurView) + this.mCurView.getWidth() < this.mLayoutCenterX) {
            ...
            LogUtils.m1568d("EPG/weight/ScrollerLayout", ">>>>> dispatchKeyEvent Left--- distance: " + distance);
            this.mScroller.startScroll(0, 0, distance, 0);
            invalidate();
        }
    }
