...
public class MsgPingbackSender {
    public static void sendMsgClickPingback(IMsgContent msgContent, int labelPos, int msgPos) {
        if (msgContent != null) {
            ...
            params.add(Keys.f2035T, "20").add("rt", "i").add("r", r).add("block", block).add("rt", rt).add("rseat", rseat).add("rpage", rpage).add("rlink", rlink).add(Keys.JUMP_TYPE, jumptype).add(Keys.ISREAD, isread);
    ...
    public static void sendMsgsShowPingback(int labelPos, long td) {
        ...
        LogUtils.m1574i("pageshow", "sendMsgsShowPingback --- qtcurl = " + qtcurl + " block = " + block);
        PingBackParams params = new PingBackParams();
        params.add(Keys.f2035T, "21").add("bstp", "1").add("s2", "msg").add("qtcurl", qtcurl).add("block", block).add("td", String.valueOf(td)).add("tabsrc", tabsrc);
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
    ...
    public static void sendMsgTopClickPingback(String rseat, String copy) {
        String r = rseat;
        PingBackParams params = new PingBackParams();
        params.add(Keys.f2035T, "20").add("rt", "i").add("r", r).add("block", "top").add("rt", "i").add("rseat", rseat).add("rpage", "消息中心").add("copy", copy);
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
}
