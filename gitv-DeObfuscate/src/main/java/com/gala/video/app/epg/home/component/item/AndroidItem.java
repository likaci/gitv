...
public abstract class AndroidItem extends Item {
    ...
    private class ImageCallback implements IImageLoadCallback, IGifLoadCallback {
        ...
        public void onSuccess(GifDrawable drawable) {
            if (HomeDebug.DEBUG_LOG) {
                LogUtils.m1568d(AndroidItem.this.TAG, "loadgif onSuccess ,gifdrawable = " + drawable);
            }
            if (drawable == null) {
        ...
        public void onSuccess(Bitmap bitmap) {
            if (HomeDebug.DEBUG_LOG) {
                LogUtils.m1568d(AndroidItem.this.TAG, "load image onSuccess bitmap = " + bitmap);
            }
    ...
    protected void initOnClickListener() {
        this.mView.setOnClickListener(new C05941());
    }
    ...
    private void postEvent(int event, Object result) {
        switch (event) {
            case 3:
                ...
                if (isPageInFront) {
                    if (HomeDebug.DEBUG_LOG) {
                        LogUtils.m1568d(this.TAG, "page in front,load image : " + this.mImageUrl);
                    }
                    this.gifStart = true;
                    loadImage();
                    return;
            ...
            case 261:
                if (!((Boolean) result).booleanValue()) {
                    if (HomeDebug.DEBUG_LOG) {
                        LogUtils.m1568d(this.TAG, "page is inVisible");
                    }
                    recycleAndShowDefaultImage();
                    return;
                    ...
                    loadImage();
                    if (HomeDebug.DEBUG_LOG) {
                        LogUtils.m1568d(this.TAG, "page is visible to user");
                        return;
                    }
                    return;
                } else if (HomeDebug.DEBUG_LOG) {
                    LogUtils.m1571e(this.TAG, "page is visible but is not visible to user");
                    return;
                } else {
                    return;
                }
            case 517:
                ...
                if (HomeDebug.DEBUG_LOG) {
                    LogUtils.m1568d(this.TAG, "card visibility changed = " + cardVisible + " isVisible = " + isVisibleToUser());
                }
            ...
            case 773:
                ...
                if (HomeDebug.DEBUG_LOG) {
                    LogUtils.m1568d(this.TAG, "item visibility changed = " + isVisible);
                    return;
                }
                return;
            case 774:
    ...
    public void loadImage() {
        ...
        if (HomeDebug.DEBUG_LOG && this.mItemData != null) {
            LogUtils.m1568d(this.TAG, "load image url : = " + url + ", gif = " + this.mItemData.mGif + ",name = " + this.mItemData.getTitle());
            LogUtils.m1568d(this.TAG, "load image  gifAvailable = " + this.gifAvailable + " isUseGif() = " + isUseGif());
        }
    ...
    private ImageCropModel createImageModel() {
        ...
        LogUtils.m1568d(this.TAG, "image loader resize factor = " + factor + " ,raw size(" + actualImageWidth + " , " + actualImageHeight + "), item size(" + actualItemWidth + " , " + actualItemHeight + ")");
