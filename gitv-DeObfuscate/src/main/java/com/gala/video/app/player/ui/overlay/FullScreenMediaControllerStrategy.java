...
public class FullScreenMediaControllerStrategy extends AbsMediaControllerStrategy {
    ...
    public void showPaused() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showPaused()" + dumpState());
        }
        this.mState = 13;
        ...
        checkSysTime();
        this.mPause.setImageDrawable(this.mContext.getResources().getDrawable(C1291R.drawable.player_play_button));
    ...
    protected void showLogo() {
        ...
        showWaterLogo();
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showLogo()" + dumpState());
        }
    ...
    private void showWaterLogo() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "showWaterLogo()");
        }
        boolean hideLogo = Project.getInstance().getBuild().isNoLogoUI();
        LogUtils.m1568d(TAG, "showWaterLogo hideLogo = " + hideLogo);
        if (hideLogo) {
    ...
    protected void doHide() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "doHide()" + dumpState());
        }
