...
public class ChannelRequestTask extends BaseRequestTask {
    ...
    class C06301 implements IApiCallback<ApiResultChannelList> {
        C06301() {
        }

        public void onSuccess(ApiResultChannelList result) {
            ChannelRequestTask.this.saveChannels(result.data);
            LogUtils.m1571e(ChannelRequestTask.TAG, "request channel list success");
        }
        ...
        public void onException(ApiException e) {
            ...
            addItem.addItem(str2, str).addItem(Keys.ERRURL, "").addItem(Keys.APINAME, "channelList").addItem("activity", "HomeActivity").addItem(Keys.f2035T, "0").setOthersNull().post();
            if (e != null) {
                LogUtils.m1571e(ChannelRequestTask.TAG, "request channel list failed code = " + e.getCode());
            }
        }
    }
    ...
    public void invoke() {
        LogUtils.m1568d(TAG, "invoke channel task input : " + this.mInput);
        String version = Project.getInstance().getBuild().getVersionString();
        TVApi.channelList.callSync(new C06301(), version, "1", PAGE_SIZE);
    }

    public synchronized void saveChannels(List<Channel> list) {
        if (list == null) {
            LogUtils.m1571e(TAG, "saveChannelInfo()---list=null");
        } else {
    ...
    public void onOneTaskFinished() {
        LogUtils.m1568d(TAG, "channel task request finished");
