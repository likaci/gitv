...
public class SimpleTabHostAdapter implements ISimpleTabHostAdapter {
    ...
    public void updateData(List<IContent<?, ?>> list) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> updateData");
        }
        if (list != null) {
            this.mDatas.clear();
            this.mDatas.addAll(list);
    ...
    public void notifyDataChanged() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> notifyDataChanged");
        }
    ...
    public View getView(int position) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "Error in getView, invalid position=" + position);
        }
        return null;
    }
    ...
    public String getTitle(int position) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "Error in getTitle, invalid position=" + position);
        }
        return null;
    }
