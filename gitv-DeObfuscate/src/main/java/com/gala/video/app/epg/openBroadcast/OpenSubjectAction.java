...
public class OpenSubjectAction extends BaseAction {
    ...
    public void process(Context context, Intent intent, LoadingCallback loadingCallback) {
        try {
            LogUtils.m1568d("openplay/broadcast/OpenSubjectAction", "process(context,intent)");
            JSONObject playDict = OpenPlayIntentUtils.parsePlayInfo(intent.getExtras());
            if (!checkParamsValidity(playDict)) {
                LogUtils.m1571e("openplay/broadcast/OpenSubjectAction", "checkParamsValidity is false. ");
                if (loadingCallback != null) {
                    loadingCallback.onFail();
                    LogUtils.m1571e("openplay/broadcast/OpenSubjectAction", "loadingCallback.onCancel()...");
                }
            ...
            LogUtils.m1571e("openplay/broadcast/OpenSubjectAction", "[UNKNOWN-EXCEPTION] [reason:exception occurred when OpenSubjectAction process.][Exception:" + e.getMessage() + AlbumEnterFactory.SIGN_STR);
            e.printStackTrace();
            if (loadingCallback != null) {
                loadingCallback.onFail();
                LogUtils.m1571e("openplay/broadcast/OpenSubjectAction", "loadingCallback.onFail();");
            }
        }
    }
    ...
    public boolean checkParamsValidity(JSONObject playDict) {
        if (super.checkParamsValidity(playDict)) {
            ...
            LogUtils.m1574i("openplay/broadcast/OpenSubjectAction", "subjectId = " + subjectId);
            if (!TextUtils.isEmpty(subjectId)) {
                return true;
            }
            LogUtils.m1571e("openplay/broadcast/OpenSubjectAction", "[INVALID-PARAMTER] [action:ACTION_SUBJECT][reason:missing field--subjectId][playInfo:" + playDict.toString() + AlbumEnterFactory.SIGN_STR);
        }
        return false;
    }

    void dealSubject(Context context, JSONObject dict) {
        LogUtils.m1574i("openplay/broadcast/OpenSubjectAction", "dealSubjecttest");
    ...
    void checkAuthIdAndApiKey(final Context context, final JSONObject dict, final LoadingCallback loadingCallback) {
        if (TVApiBase.getTVApiProperty().checkAuthIdAndApiKeyAvailable()) {
            LogUtils.m1568d("openplay/broadcast/OpenSubjectAction", "checkAuthIdAndApiKeyAvailable = true");
            dealSubject(context, dict);
            return;
        }
        LogUtils.m1568d("openplay/broadcast/OpenSubjectAction", "checkAuthIdAndApiKeyAvailable = false, start DeviceCheck...");
        TVApi.deviceCheckP.callSync(new IApiCallback<ApiResultDeviceCheck>() {
            public void onSuccess(ApiResultDeviceCheck apiResult) {
                LogUtils.m1568d("openplay/broadcast/OpenSubjectAction", "onDeviceCheck-onSuccess");
                OpenSubjectAction.this.dealSubject(context, dict);
            }

            public void onException(ApiException e) {
                LogUtils.m1571e("openplay/broadcast/OpenSubjectAction", "onDeviceCheck-onException e=" + e);
                e.printStackTrace();
                if (!NetworkUtils.isNetworkAvaliable()) {
                    LogUtils.m1571e("openplay/broadcast/OpenSubjectAction", "TVApi--IApiCallback--NetworkUtils.isNetworkAvaliable() =false");
