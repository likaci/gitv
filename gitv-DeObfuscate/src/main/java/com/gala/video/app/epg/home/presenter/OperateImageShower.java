...
public class OperateImageShower {
    ...
    public void showOperateImage(Context context, View adContainer, Bitmap drawable, StartOperateImageModel model) {
        if (model == null) {
            LogUtils.m1577w(TAG, "showOperateImage, start operate image data is null");
            return;
        }
        this.mContext = context;
        this.mStartOperateImageModel = model;
        ((ImageView) adContainer.findViewById(C0508R.id.epg_screen_ad)).setImageBitmap(drawable);
        adContainer.bringToFront();
        final View adBadge = adContainer.findViewById(C0508R.id.epg_tv_ad_badge);
        adBadge.setVisibility(4);
        TextView jumpTipText = (TextView) adContainer.findViewById(C0508R.id.epg_start_ad_jump_tip);
        ...
        if (DeviceCheckModel.getInstance().isDevCheckPass() && ResourceOperateImageUtils.isSupportJump(model.getChannelLabel()) && ResourceOperateImageUtils.isSupportResType(model.getChannelLabel())) {
            ...
            if (text == null) {
                text = "按\nOK\n键\n了\n解\n详\n情";
                LogUtils.m1568d(TAG, " showOperateImage , read click tips text is null from  local Resource String");
            }
            ...
            if (!ResourceOperateImageUtils.isSupportJump(model.getChannelLabel())) {
                LogUtils.m1568d(TAG, "showOperateImage, not support jump , ");
            }
            if (!DeviceCheckModel.getInstance().isDevCheckPass()) {
                LogUtils.m1568d(TAG, "showOperateImage, device check fail");
            }
            if (!ResourceOperateImageUtils.isSupportResType(model.getChannelLabel())) {
                LogUtils.m1568d(TAG, "showOperateImage, not support type :" + DataBuildTool.getItemType(model.getChannelLabel()));
            }
            this.mEnableJump = false;
            jumpTipText.setVisibility(4);
        }
        final TextView mTimer = (TextView) adContainer.findViewById(C0508R.id.epg_tv_adtime);
        ...
        this.mHandler.postAtFrontOfQueue(new Runnable() {
            ...
            public void run() {
                if (this.mCountDown > 0) {
                    LogUtils.m1568d(OperateImageShower.TAG, "showOperateImage, showing count down: " + this.mCountDown + ", current time millis:" + System.currentTimeMillis());
    ...
    private void onClick() {
        if (this.mStartOperateImageModel == null) {
            LogUtils.m1577w(TAG, "onClick ,start operate image data is null is null");
            return;
        }
