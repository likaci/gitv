...
public class HomePingback extends Wrapper {
    ...
    public final void post() {
        ...
        if (ListUtils.isEmpty(paramsMap)) {
            LogUtils.m1577w(this.TAG, "send, PingbackLog: pingbackType=" + getType() + "send pingback failed, because pingback parameters are empty");
            return;
        }
    ...
    private void print(final StringBuilder builder) {
        if (this.mThreadPoolExecutor == null) {
            LogUtils.m1577w(this.TAG, "print, current ThreadPoolExecutor is null");
            return;
        }
        this.mThreadPoolExecutor.execute(new Runnable() {
            public void run() {
                if (HomePingbackDebug.DEBUG_LOG) {
                    LogUtils.m1568d(HomePingback.this.TAG, "PingbackLog: pingbackType=" + HomePingback.this.getType() + ", send " + "(" + builder.toString() + ")");
                }
            }
        });
    }
