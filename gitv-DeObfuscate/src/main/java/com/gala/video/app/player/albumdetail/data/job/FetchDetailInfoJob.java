...
public class FetchDetailInfoJob extends AlbumJob {
    ...
    AlbumInfo albumInfo;
    IAlbumInfoCacheTaskListener albumInfoCacheTaskListener = new C13011();
    JobController mController;
    int mPlayOrder;
    String mTvQid;

    class C13011 implements IAlbumInfoCacheTaskListener {
        C13011() {
        }

        public void onSuccess(Album album) {
            LogUtils.m1568d(FetchDetailInfoJob.TAG, " onRun onSuccess");
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(FetchDetailInfoJob.TAG, "task.getAlbum() = " + FetchDetailInfoJob.this.albumInfo);
            }
        ...
        public void onFailed(ApiException e) {
            LogUtils.m1568d(FetchDetailInfoJob.TAG, "onException: code=" + e.getCode() + ", msg=" + e.getMessage());
    ...
    public void onRun(JobController controller) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> onRun, albumId=" + this.albumInfo.getAlbumId());
        }
        AlbumInfoCacheTask task = AlbumInfoCacheTask.getInstance(this.albumInfo.getAlbumId());
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, ">> task =" + task);
        }
