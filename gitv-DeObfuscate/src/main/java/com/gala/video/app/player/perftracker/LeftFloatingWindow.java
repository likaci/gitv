...
public class LeftFloatingWindow {
    private final String TAG = ("Player/Perf/LeftFloatingWindow@" + Integer.toHexString(super.hashCode()));
    private View mContent;
    private Context mContext;
    private WindowManager mWMS;

    public LeftFloatingWindow(Context context, View content) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "<init>: context=" + context + ", content=" + content);
        }
    ...
    public void showWindow() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "showWindow");
        }
    ...
    public void hideWindow() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(this.TAG, "hideWindow " + this.mContent);
        }
        try {
            this.mWMS.removeViewImmediate(this.mContent);
        } catch (Exception e) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1572e(this.TAG, "hideWindow: exception happened", e);
            }
        }
    }
