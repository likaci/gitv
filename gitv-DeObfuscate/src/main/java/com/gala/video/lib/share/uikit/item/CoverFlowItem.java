...
public class CoverFlowItem extends Item implements Presenter {
    ...
    public void updateAds(List<Item> adItemList) {
        ...
        if (isHasAdItem(curItemList)) {
            LogUtils.m1585d(TAG, "CoverFlowItem@" + hashCode() + ",updateAds, !isHasAdItem(curItemList) = false ,current coverflowcard item size : " + ListUtils.getCount(curItemList));
            ...
            if (adItemIndexList.size() > 0 && adItemIndexList.contains(Integer.valueOf(0))) {
                ...
                LogUtils.m1585d(TAG, "CoverFlowCard@" + hashCode() + ", hasAdsBeFiltered = true");
            }
            LogUtils.m1585d(TAG, "CoverFlowCard@" + hashCode());
            for (Integer position : adItemIndexList) {
                removeAdItem(position.intValue());
            }
            addAdItem(adItemList, hasAdsBeFiltered);
        } else {
            LogUtils.m1585d(TAG, "CoverFlowItem@" + hashCode() + ", updateAds, (curItemList) = true");
