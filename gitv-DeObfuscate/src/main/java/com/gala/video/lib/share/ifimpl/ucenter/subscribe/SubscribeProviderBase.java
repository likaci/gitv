...
abstract class SubscribeProviderBase extends Wrapper {
    ...
    LoginCallbackRecorderListener mLoginListener = new C17521();
    NetWorkManager netWorkManager;
    String userCookie = "";

    class C17521 implements LoginCallbackRecorderListener {
        ...
        public void onLogin(String uid) {
            ...
            if (SubscribeProviderBase.this.userCookie.length() == 0) {
                ...
                ThreadUtils.execute(new C17501());
            }
        }
    ...
    class C17565 implements IVrsCallback<ApiResultSubscribeState> {
        C17565() {
        }
    ...
    private void getSubscribeStateInner(List<String> uncached) {
        if (isNetworkConnected()) {
            ...
            sb.deleteCharAt(sb.length() - 1);
            UserHelper.subscribeState.callSync(new C17565(), this.userCookie, sb.toString());
        }
    }
    ...
    void getSubscribeList(IVrsCallback<ApiResultSubScribeList> callback, int page, int size, int type, int order) {
        ...
        ThreadUtils.execute(new Runnable() {
            ...
            class C17571 implements IVrsCallback<ApiResultSubScribeList> {
                C17571() {
                }
            ...
            public void run() {
                UserHelper.subscribeList.callSync(new C17571(), SubscribeProviderBase.this.userCookie, "" + i, "" + i2, "" + i3, sorder);
            }
        });
    }
    ...
    private void updateSubscribeInCache(final String qpid, boolean syncToServer) {
        ...
        if (syncToServer) {
            ThreadUtils.execute(new Runnable() {
                ...
                class C17591 implements IVrsCallback<ApiResultCode> {
                    C17591() {
                    }
                ...
                public void run() {
                    UserHelper.subscribe.callSync(new C17591(), cookie, qpid);
                }
            });
        }
    }
    ...
    private void updateCancelSubscribeInCache(final String qpid, boolean syncToServer) {
        ...
        if (syncToServer) {
            ThreadUtils.execute(new Runnable() {
                ...
                class C17611 implements IVrsCallback<ApiResultCode> {
                    C17611() {
                    }
                ...
                public void run() {
                    UserHelper.unsubscribe.callSync(new C17611(), cookie, qpid);
                }
            });
        }
    }
