...
public class DataRefreshPeriodism {
    ...
    public void setRules(int level, String rules) {
        if (rules != null) {
            ...
            LogUtils.m1568d(TAG, "summary periodism size = " + summaryPeriodism.length);
            for (String item : summaryPeriodism) {
                LogUtils.m1568d(TAG, "summary periodism " + item);
                ...
                if (detailPeriodism != null && detailPeriodism.length == 3) {
                    ...
                    models.add(model);
                    LogUtils.m1568d(TAG, "refresh level = " + level + " model = " + model);
                }
            }
            synchronized (this) {
    ...
    public synchronized void setCategory(int level, String resId) {
        if (TextUtils.isEmpty(resId)) {
            LogUtils.m1577w(TAG, "group id is empty,level = " + level);
        } else {
    ...
    public synchronized int getRefreshLevel(String resId) {
        ...
        LogUtils.m1568d(TAG, "getRefreshLevel (" + resId + ", " + level + ")");
        if (level < 1 || level > 3) {
            level = 2;
        }
        return level;
    ...
    public int getRefreshInterval(int level) {
        ...
        if (serverTime == -1) {
            LogUtils.m1571e(TAG, "get Refresh Interval error");
            return 3600000;
        }
        List rules = null;
        synchronized (this) {
        ...
        LogUtils.m1568d(TAG, "current server time = " + serverTime + " level = " + level + " interval = " + interval);
        return interval;
    }

    private synchronized void saveDataToLocalCache(HashMap<Integer, List<Model>> refreshRules) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "save data refresh periodism : " + refreshRules);
        }
        try {
            SerializableUtils.write(refreshRules, "home/v6.0/home_refresh_periodism.dem");
        } catch (Exception e) {
            LogUtils.m1572e(TAG, "save data request periodism exception :", e);
        }
    }
    ...
    private synchronized HashMap<Integer, List<Model>> readFromNativeCache() {
        ...
        try {
            ...
            LogUtils.m1568d(TAG, "read data request periodism from cache : " + result);
        } catch (Exception e) {
            LogUtils.m1571e(TAG, "read data request periodism exception : " + e.getMessage());
        }
        return result;
    }
}
