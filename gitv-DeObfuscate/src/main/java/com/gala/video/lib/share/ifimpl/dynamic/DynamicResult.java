...
class DynamicResult implements IDynamicResult, Serializable {
    ...
    public String getSpecifiedOperateImageResId(OperationImageType type) {
        if (type == null) {
            LogUtils.m1577w(TAG, "current operation image type is null");
            return "";
        }
    ...
    private Map<Integer, List<String>> initImagePaths() {
        ...
        if (imagePathsObj == null) {
            ...
            LogUtils.m1577w(TAG, "local image path is not the type : Map<Integer, List<String>>");
            return imagePaths;
        }
    }
    ...
    private List<String> checkAndGetUrl(int key) {
        ...
        LogUtils.m1568d(TAG, "get " + key + " image local path =  " + imagePaths.get(Integer.valueOf(key)));
    ...
    public List<String> getScreenWechatInteractiveImagePath() {
        ...
        LogUtils.m1568d(TAG, "getScreenWechatInteractiveImagePath, screen wechat interactive image local path = " + list);
    ...
    void removeImagePath(int tag) {
        ...
        LogUtils.m1568d(TAG, "removeImagePath, tag = " + tag + "imagePath = " + imagePath);
        if (imagePath != null) {
    ...
    List<String> getImagePaths(int tag) {
        ...
        if (ListUtils.isEmpty(imagePaths)) {
            LogUtils.m1568d(TAG, "getImagePaths, total image paths are empty");
        } else if (imagePaths.get(Integer.valueOf(tag)) != null) {
            paths.addAll((Collection) imagePaths.get(Integer.valueOf(tag)));
        } else {
            LogUtils.m1568d(TAG, "getImagePaths, imagePaths.get " + tag + " = null");
        }
        return paths;
    }
    ...
    synchronized void saveDataToLocal() {
        ...
        LogUtils.m1568d(TAG, "saveDataToLocal");
        if (entries == null) {
            LogUtils.m1568d(TAG, "DynamicCache, current cache is empty");
        } else {
            for (Entry<String, Object> entry : entries) {
                LogUtils.m1568d(TAG, "DynamicCache," + ((String) entry.getKey()) + SOAP.DELIM + entry.getValue());
            }
        }
        try {
            SerializableUtils.write(DynamicCache.get(), ApiConstants.DynamicQ_DATA_FILENAME);
        } catch (Exception e) {
            LogUtils.m1571e(TAG, "saveDataToLocal, e = " + e);
        }
        LogUtils.m1568d(TAG, "saveDataToLocal finished");
    }
    ...
    synchronized void getSerializableData() {
        try {
            ...
            LogUtils.m1571e(TAG, "getSerializableData, e = " + e);
        }
    }
}
