...
public class OpenApiUtils {
    ...
    public static com.qiyi.tv.client.data.Channel createSdkChannel(Channel channel) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createSdkChannel() from " + DataUtils.channelToString(channel));
        }
            ...
            for (TwoLevelTag twoLevelTag : channel.tags) {
                if (!(twoLevelTag == null || twoLevelTag.tags == null)) {
                    for (ThreeLevelTag threeLevelTag : twoLevelTag.tags) {
                        tags.add(threeLevelTag.f1015n);
                        tags.add(threeLevelTag.f1016v);
                    }
                }
            }
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createSdkChannel() return " + channel4Sdk);
        }
        return channel4Sdk;
    }
    ...
    public static Album createAlbum(Media media) {
        ...
        int i2 = 0;
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createAlbum(" + media + ")");
        }
        Album album = null;
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createAlbum() return " + album);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createAlbum() return " + DataUtils.albumInfoToString(album));
        }
        return album;
    }
    ...
    private static Media createSdkMedia(Album info, String tvPic, String pic) {
        boolean z = true;
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createSdkMedia(" + info + ", " + -1 + ")" + DataUtils.albumInfoToString(info));
        }
        Media media = null;
        if (info != null) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createSdkMedia() return " + media);
        }
        return media;
    }
    ...
    public static ChannelLabel createChannelLabel(Media media) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createChannelLabel(" + media + ")");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createChannelLabel() return " + label);
        }
        return label;
    }
    ...
    public static Media createSdkMedia(ChannelLabel info) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "createSdkMedia(" + info + ")" + DataUtils.channelLabelToString(info));
        }
        Media media = null;
        ...
        if (media != null) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "createSdkMedia() prompt " + info.itemPrompt);
            }
    ...
    public static boolean isVipUser(Context context) {
        ...
        if (userType != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "isVipUser() return " + vip);
            }
        }
        return vip;
    }
    ...
    public static void fillUserFilterTags(List<String> userFilters, List<String> filterTags, List<String> finalNames, List<String> finalValues) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "fillFilterTags(" + userFilters + ", " + filterTags + ")");
        }
        if (userFilters != null && filterTags != null) {
            if (finalNames != null || finalValues != null) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(TAG, "fillFilterTags() finalNames=" + finalNames + ", finalValues=" + finalValues);
                }
            }
        }
    }
    ...
    public static String getUserFilterValues(List<String> userFilters, List<String> filterTags) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getUserFilterValues(" + userFilters + ", " + filterTags + ")");
        }
        String tagValues = "";
        if (userFilters == null || filterTags == null) {
            tagValues = null;
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "getUserFilterValues() return " + tagValues);
        }
        return tagValues;
    }
    ...
    public static int parseErrorCode(String code) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "parseErrorCode(" + code + ")");
        }
        int errorCode = 7;
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "parseErrorCode() return " + errorCode);
        }
        return errorCode;
    }
}
