...
class DynaimicImageDownLoadTask {
    ...
    class C16581 implements IFileCallback {
        C16581() {
        }

        public void onSuccess(FileRequest request, String path) {
            LogUtils.m1568d(DynaimicImageDownLoadTask.TAG, "load image count = " + DynaimicImageDownLoadTask.this.mTotalCount);
            LogUtils.m1568d(DynaimicImageDownLoadTask.TAG, "load image path = " + path);
            LogUtils.m1568d(DynaimicImageDownLoadTask.TAG, "load image cookie = " + ((Integer) request.getCookie()));
    ...
    public void download() {
        if (this.mRequests != null) {
            for (FileRequest request : this.mRequests) {
                mDownloader.loadFile(request, new C16581());
            }
        }
    }
}
