...
public class OpenPlayIntentUtils {
    public static JSONObject parsePlayInfo(Bundle bundle) {
        if (!(bundle == null || bundle.isEmpty())) {
            ...
            if (info != null) {
                LogUtils.m1568d(OpenPlayIntentUtils.class.getName(), "parsePlayInfo: " + info);
                try {
                    return new JSONObject(info);
                } catch (JSONException e) {
                    LogUtils.m1571e(OpenPlayIntentUtils.class.getName(), "[UNKNOWN-EXCEPTION] [reason: can not parse the playInfo field to json.][playInfo:]" + info.toString() + "[JSONException:" + e.getMessage() + AlbumEnterFactory.SIGN_STR);
                    e.printStackTrace();
                    return null;
                }
            }
        }
        LogUtils.m1571e(OpenPlayIntentUtils.class.getName(), "[INVALID-PARAMTER] [reason:missing field--playInfo] ");
        return null;
    }
