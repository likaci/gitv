...
public class ScreenSaverAdModel extends CupidAdModel {
    ...
    public Bitmap getQrBitmap() {
        LogUtils.m1568d(TAG, "getQrBitmap, is should show QR : " + shouldShowQr());
        if (!shouldShowQr() || StringUtils.isEmpty(this.mQrUrl)) {
            return null;
        }
        int dimension = ResourceUtil.getDimensionPixelSize(C1632R.dimen.dimen_124dp);
        LogUtils.m1568d(TAG, "getQrBitmap, dimension = " + dimension);
    ...
    public int getAdDuration() {
        ...
        LogUtils.m1568d(TAG, "getAdDuration, duration = " + duration);
        try {
            showDuration = Integer.parseInt(duration) * 1000;
        } catch (NumberFormatException e) {
            LogUtils.m1571e(TAG, "getAdDurationByPos e = " + e);
        }
