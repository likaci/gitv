...
public class Page extends ComponentGroup implements Presenter {
    ...
    private boolean doUpdateCaredModel(CardInfoModel cardModel) {
        ...
        if (card == null) {
            LOG.m871e("Card not exist, id = " + cardModel.getId());
            return false;
        }
    ...
    private void dochangeCardModel(CardInfoModel model) {
        ...
        if (card == null) {
            LOG.m871e("Card not exist, id = " + model.getId());
            return;
        }
    ...
    public void onShow() {
        Log.d(TAG, "onShow");
        LogUtils.m1593i(TAG, "onShow isStart() = ", Boolean.valueOf(isStart()));
        LogUtils.m1593i(TAG, "onShow mDataSetChanged = " + this.mDataSetChanged, " AppRuntimeEnv.get().isPlayInHome() = ", Boolean.valueOf(AppRuntimeEnv.get().isPlayInHome()));
        if (isStart()) {
            showAllItems();
