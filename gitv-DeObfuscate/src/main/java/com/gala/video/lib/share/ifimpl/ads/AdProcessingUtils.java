...
public class AdProcessingUtils extends Wrapper implements IAdProcessingUtils {
    ...
    class C16551 implements IGifCallback {
        C16551() {
        }

        public void onSuccess(FileRequest fileRequest, GifDrawable gifDrawable) {
            LogUtils.m1568d(AdProcessingUtils.TAG, "downloadImage, download ad  GIF image(show in the second page after clicking Ad) success");
        }

        public void onFailure(FileRequest fileRequest, Exception e) {
            LogUtils.m1577w(AdProcessingUtils.TAG, "downloadImage, download ad GIF image(show in the second page after clicking Ad) failed,  url = " + fileRequest.getUrl());
        }
    }

    class C16562 implements IFileCallback {
        C16562() {
        }

        public void onSuccess(FileRequest fileRequest, String s) {
            LogUtils.m1568d(AdProcessingUtils.TAG, "downloadImage, download ad image(show in the second page after clicking Ad) success");
        }

        public void onFailure(FileRequest fileRequest, Exception e) {
            LogUtils.m1577w(AdProcessingUtils.TAG, "downloadImage, download ad image(show in the second page after clicking Ad) failed,  url = " + fileRequest.getUrl());
        }
    }

    public void onClickAd(Context context, CupidAdModel cupidAdModel, HomeAdPingbackModel pingbackModel) {
        if (cupidAdModel == null) {
            LogUtils.m1571e(TAG, "onClickAd, cupid ad model is null!!!");
            return;
        }
        if (pingbackModel == null) {
            LogUtils.m1577w(TAG, "onClickAd, home ad pingback model is null");
        }
        AdClickType adClickType = cupidAdModel.getAdClickType();
        if (adClickType == null) {
            LogUtils.m1571e(TAG, "onClickAd, ad click type is null!!!");
        ...
        try {
            ...
            switch (adClickType) {
                ...
                case CAROUSEL:
                    ...
                    LogUtils.m1568d(TAG, "onClickAd, channelCarousel: id = " + channelCarousel.id + ", tableNo = " + channelCarousel.tableNo + ", name = " + channelCarousel.name);
                    ...
                    return;
                case VIDEO_ILLEGAL:
                    LogUtils.m1577w(TAG, "onClickAd, video illegal");
                    onClickForNotOpenAdData(context);
                    return;
                case CAROUSEL_ILLEGAL:
                    LogUtils.m1577w(TAG, "onClickAd, carousel illegal");
                    onClickForNotOpenAdData(context);
                    return;
                default:
                    LogUtils.m1577w(TAG, "onClickAd, not support ClickThroughType, ClickThroughType = " + cupidAdModel.getClickThroughType() + " AdClickType = " + cupidAdModel.getAdClickType() + " ClickThroughInfo  = " + cupidAdModel.getClickThroughInfo());
                    onClickForNotOpenAdData(context);
                    return;
            }
        } catch (NumberFormatException e) {
            LogUtils.m1571e(TAG, "onClickAd, NumberFormatException: " + e);
        } catch (Exception e2) {
            LogUtils.m1571e(TAG, "onClickAd, exception: " + e2);
        }
    }

    public void parseAdRawData(CupidAd cupidAdRaw, CupidAdModel cupidAdModel) {
        if (cupidAdRaw == null) {
            LogUtils.m1577w(TAG, "parseAdRawData, raw ad data is null");
        } else if (cupidAdModel == null) {
            LogUtils.m1577w(TAG, "parseAdRawData, CupidAdModel is null");
        } else {
            ...
            if (clickThroughType == null) {
                LogUtils.m1577w(TAG, "parseAdRawData, ad click type is null");
            } else if (StringUtils.isEmpty(clickInfo)) {
                LogUtils.m1577w(TAG, "parseAdRawData, ad click info is null");
            } else {
                ...
                switch (clickThroughType) {
                    ...
                    case VIDEO:
                        ...
                        if (isFindPlid) {
                            ...
                            LogUtils.m1577w(TAG, "clickThroughType is " + ClickThroughType.VIDEO + "(play a video in the APP)" + " but the jumping info format is illegal, " + "info : " + clickInfo);
                    ...
                    case CAROUSEL_STATION:
                        if (Project.getInstance().getControl().isOpenCarousel()) {
                            ...
                            LogUtils.m1577w(TAG, "clickThroughType is " + ClickThroughType.CAROUSEL_STATION + " but the jumping info format is illegal, info : " + clickInfo);
    ...
    public void downloadImage(String imageUrl) {
        if (!StringUtils.isEmpty((CharSequence) imageUrl)) {
            ...
            LogUtils.m1577w(TAG, "downloadImage, local image path, " + localImagePath);
            if (StringUtils.isEmpty(localImagePath)) {
                String suffixName = "";
                ...
                if (".gif".equals(suffixName)) {
                    DownloaderAPI.getDownloader().loadGif(fileRequest, new C16551());
                } else {
                    DownloaderAPI.getDownloader().loadFile(fileRequest, new C16562());
                }
            }
        }
    }
