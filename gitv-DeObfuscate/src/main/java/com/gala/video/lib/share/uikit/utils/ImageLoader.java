...
public class ImageLoader {
    ...
    private IGifCallback mGifCallback = new C18302();
    ...
    private IImageCallback mLoadImageCallback = new C18271();
    ...
    class C18271 implements IImageCallback {
        C18271() {
        }
    ...
    class C18302 implements IGifCallback {
        C18302() {
        }
        ...
        public void onSuccess(final FileRequest fileRequest, final GifDrawable gifDrawable) {
            if (ImageLoader.this.mGifListener == null || fileRequest == null) {
                LogUtils.m1585d("mGifCallback", "loadImage onSuccess return mGifListener==" + ImageLoader.this.mGifListener + "," + "fileRequest=" + fileRequest);
            } else if (!TextUtils.equals(ImageLoader.this.mLastRequestUrl, fileRequest.getUrl())) {
                LogUtils.m1585d("mGifCallback", "loadImage onSuccess return mLastRequestUrl=" + ImageLoader.this.mLastRequestUrl + ",url=" + fileRequest.getUrl());
                ...
                ImageLoader.mMainThreadHandler.post(new Runnable() {
                    public void run() {
                        C18302.this.onSuccess(fileRequest, gifDrawable);
                    }
                });
            }
        }
        ...
        public void onFailure(final FileRequest fileRequest, final Exception e) {
            if (ImageLoader.this.mGifListener != null && fileRequest != null && TextUtils.equals(ImageLoader.this.mLastRequestUrl, fileRequest.getUrl())) {
                if (ThreadUtils.isUIThread()) {
                    ...
                    ImageLoader.mMainThreadHandler.post(new Runnable() {
                        public void run() {
                            C18302.this.onFailure(fileRequest, e);
                        }
                    });
                }
            }
