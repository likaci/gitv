...
class AccessToken {
    ...
    public synchronized boolean isAllowedAccess() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isAllowedAccess() now = " + now + ", mMaxCount = " + this.mMaxCount);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isAllowedAccess() count=" + this.mAccessCount + ", time=" + this.mAccessTime + ", return " + allowed);
        }
        return allowed;
    }
    ...
    public synchronized void increaseAccessCount() {
        ...
        this.mAccessCount++;
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "increaseAccessCount() count=" + this.mAccessCount);
        }
    }
