...
class BackgroundManager extends Wrapper {
    ...
    public Drawable getBackgroundDrawable() {
        if (MemoryLevelInfo.isLowMemoryDevice()) {
            return new ColorDrawable(this.mAppContext.getResources().getColor(C1632R.color.app_background));
        }
    ...
    private Drawable getBackgroundDrawable(String drawableName) {
        LogUtils.m1568d(TAG, "getBackgroundDrawable drawableName  = " + drawableName);
        try {
            if (drawableName.equals(SystemConfigPreference.SETTING_BACKGROUND_NIGHT_DEFAULT)) {
                ...
                LogUtils.m1568d(TAG, "getBackgroundDrawable FilePath  = " + FilePath);
                if (drawableFile.exists() && drawableFile.isFile()) {
                    ...
                    LogUtils.m1571e(TAG, "getBackgroundDrawable FilePath is not exist");
                    this.mCurBGDrawable = getDefaultBackgroundDrawable();
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            LogUtils.m1571e(TAG, "getBackgroundDrawable Exception  e = " + e.getMessage());
            this.mCurBGDrawable = getDefaultBackgroundDrawable();
        }
    ...
    private Drawable getDefaultBackgroundDrawable() {
        return ResourceUtil.getDrawable(C1632R.drawable.share_default_background);
    }

    public void setBackgroundDrawable(String drawableName) {
        LogUtils.m1568d(TAG, "setBackgroundDrawable path  = " + drawableName);
        ...
        if (Project.getInstance().getBuild().isHomeVersion()) {
            ...
            try {
                if (drawableName.equals(SystemConfigPreference.SETTING_BACKGROUND_NIGHT_DEFAULT)) {
                    ...
                    new Canvas(bmp).drawColor(this.mAppContext.getResources().getColor(C1632R.color.setting_night_bg));
                    wpm.setBitmap(bmp);
                    return;
                }
                ...
                LogUtils.m1571e(TAG, "setBackgroundDrawable set launcher backgroud fail---e" + e);
            }
        }
    }
    ...
    private String getSettingBgDrawablePath(String fileName) {
        ...
        LogUtils.m1568d(TAG, "getSettingBgDrawablePath fileName  = " + fileName + " ;folderPath = " + folderPath);
