...
public class GlobalDialog extends AlertDialog implements OnFocusChangeListener {
    ...
    protected final OnDismissListener mInnerDismissListener = new C16493();
    protected final OnShowListener mInnerShownListener = new C16482();
    protected LayoutInflater mLayoutInflater;
    protected OnKeyListener mOnKeyListener = new C16471();
    ...
    class C16471 implements OnKeyListener {
        C16471() {
        }
    ...
    class C16482 implements OnShowListener {
        C16482() {
        }

        public void onShow(DialogInterface dialog) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GlobalDialog.TAG, "mInnerShownListener.onShow(" + dialog + ")");
            }
    ...
    class C16493 implements OnDismissListener {
        C16493() {
        }

        public void onDismiss(DialogInterface dialog) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(GlobalDialog.TAG, "mInnerDismissListener.onDismiss(" + dialog + ")");
            }
    ...
    public GlobalDialog(Context context) {
        super(context, C1632R.style.alert_dialog);
        init(context);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.mContentLayout = (FrameLayout) findViewById(C1632R.id.share_dialog_content_layout);
        this.mBtnLine = this.mContainerView.findViewById(C1632R.id.share_dialog_btn_line);
        this.mLayoutInflater.inflate(this.mContentResId, this.mContentLayout, true);
        initLayout();
        this.mButtonOK = findViewById(C1632R.id.share_dialog_btn1);
        this.mButtonCancel = findViewById(C1632R.id.share_dialog_btn2);
        this.mTextOK = (TextView) findViewById(C1632R.id.share_txt_btn1);
        this.mTextCancel = (TextView) findViewById(C1632R.id.share_txt_btn2);
        this.mButtonLayout = findViewById(C1632R.id.share_dialog_btn_layout);
        this.mDialogLine = findViewById(C1632R.id.share_dialog_line);
        this.mDialogTopView = findViewById(C1632R.id.share_dialog_top_line);
    ...
    protected void initLayout() {
        this.mContentTextView = (TextView) this.mContentLayout.findViewById(C1632R.id.share_dialog_text);
    ...
    protected void initParams() {
        this.mDialogLayoutResId = C1632R.layout.share_global_dialog_layout;
        this.mContentResId = C1632R.layout.share_global_dialog_text_view;
        this.mDialogWidth = getDimen(C1632R.dimen.dimen_570dp);
        this.mOnlyOneBtnHeight = getDimen(C1632R.dimen.dimen_73dp);
        this.mOnlyOneBtnWidth = getDimen(C1632R.dimen.dimen_546dp);
        this.mFirstBtnWidth = getDimen(C1632R.dimen.dimen_273dp);
        this.mFirstBtnHeight = this.mOnlyOneBtnHeight;
        this.mSecondBtnWidth = getDimen(C1632R.dimen.dimen_273dp);
    ...
    public void setOnShowListener(OnShowListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setOnShowListener(" + listener + ")");
        }
    ...
    public void setOnDismissListener(OnDismissListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "setOnDismissListener(" + listener + ")");
        }
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        if (v != null) {
            ...
            if (v.getId() == C1632R.id.share_dialog_btn1) {
                textView = this.mTextOK;
            } else if (v.getId() == C1632R.id.share_dialog_btn2) {
                textView = this.mTextCancel;
            }
            if (!(textView == null || getContext() == null)) {
                if (hasFocus) {
                    textView.setTextColor(getColor(Project.getInstance().getBuild().isLitchi() ? C1632R.color.dialog_text_color_sel : C1632R.color.dialog_text_color_sel));
                } else {
                    textView.setTextColor(getColor(C1632R.color.dialog_text_color_unsel));
                }
            }
            if (hasFocus) {
                v.bringToFront();
