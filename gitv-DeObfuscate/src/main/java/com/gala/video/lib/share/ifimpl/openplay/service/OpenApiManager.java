...
public class OpenApiManager {
    ...
    public synchronized void addAllCommands(List<ServerCommand<?>> commands) {
        ...
        if (LogUtils.mIsDebug) {
            ...
            for (int i = 0; i < size; i++) {
                LogUtils.m1568d(TAG, "OpenApiBinder() add[" + i + "]=" + commands.get(i));
            }
        }
    }
    ...
    public synchronized boolean addCommand(ServerCommand<?> command) {
        boolean z;
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "addCommand(" + command + ")");
        }
    ...
    public synchronized boolean removeCommand(ServerCommand<?> command) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "removeCommand(" + command + ")");
        }
    ...
    private synchronized void replaceWatcherProcess(int pid) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "replaceWatcherProcess(" + pid + ")");
        }
        for (ServerCommand<?> command : this.mCommands) {
    ...
    void clearAuth() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "clearAuth() mExistAuthSuccess=" + this.mExistAuthSuccess);
        }
    ...
    public synchronized boolean isAuthSuccess() {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isAuthSuccess() return " + this.mExistAuthSuccess);
        }
    ...
    synchronized boolean isAllowedClient() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isAllowedClient() pid=" + pid + ", holder=" + holder + " return " + allowed);
        }
        return allowed;
    }
    ...
    synchronized boolean authLocal(Bundle params) {
        ...
        int authCode;
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "authLocal() begin.");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "clientCustomerName=" + clientCustomerName);
        }
        ...
        for (String localPackageName : localPackageNames) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "authLocal() localPackageName=" + localPackageName);
            }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "auhtLocal() find old process " + removed);
        }
        ...
        if (authCode != 0) {
            LogUtils.m1571e(TAG, "OpenAPI auth fail: auth code = " + authCode + ", server version = " + Version.VERSION_CODE);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "authLocal() localCustomerName=" + localCustomerName + ", realLocalName=" + realLocalName + ", pid=" + pid + ", holder=" + holder + " return " + authCode);
        }
        if (authCode == 0) {
            z = true;
        } else {
    ...
    private boolean isCompatible(int serverVersion, int clientVersion) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "isCompatible(" + serverVersion + ", " + clientVersion + ") return " + compatibale);
        }
        return true;
    }
