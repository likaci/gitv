...
class MSHttpServer extends NanoHTTPD {
    ...
    public Response serve(String uri, Method method, Map<String, String> map, Map<String, String> parms, Map<String, String> map2) {
        LogUtils.m1574i(TAG, "ms http server serve uri:" + uri);
    ...
    public static void startServer(Context context, IGalaMSExpand callback) {
        ...
        try {
            ...
            LogUtils.m1574i(MSHttpServer.class.getName(), "ms http server start");
            mMsHttpServer.start();
            mHasStarted = true;
        } catch (Exception e) {
            LogUtils.m1574i(MSHttpServer.class.getName(), "ms http server start error:" + e);
        }
    }
}
