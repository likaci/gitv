...
public class LoginCallbackRecorder {
    ...
    public void addListener(LoginCallbackRecorderListener listener) {
        if (listener != null && !this.mListeners.contains(listener)) {
            ...
            LogUtils.m1573e("addListener fail!!! LoginCallbackRecorderListener param can't be null !!!");
        } else {
            LogUtils.m1573e("addListener fail!!! LoginCallbackRecorderListener param already exist !!!");
        }
    }
    ...
    public void removeListener(LoginCallbackRecorderListener listener) {
        ...
        LogUtils.m1573e("removeListener fail!!! listener already be removed or listener is null !!!");
    }

    void notifyLogin(String uid) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyLogin(" + uid + ") mIsLogin= true");
        }
    ...
    void notifyLogout(String uid) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(TAG, "notifyLogout(" + uid + ") mIsLogin=false");
        }
        for (LoginCallbackRecorderListener listener : this.mListeners) {
