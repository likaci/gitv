...
public class HistoryDataBuilder extends HistoryBase {
    ...
    private class DeleteListener implements IVrsCallback<ApiResultCode> {
        ...
        public void onSuccess(ApiResultCode result) {
            LogUtils.m1571e(HistoryInfoHelper.TAG, "---deleteCallback---onSuccess--");
        }

        public void onException(ApiException e) {
            LogUtils.m1571e(HistoryInfoHelper.TAG, "---deleteCallback---onException()--code=" + (e != null ? e.getCode() : ""));
        }
    }
    ...
    class HistoryDataHandler extends HistoryHandler {
        ...
        public void handleMessage(Message msg) {
            LogUtils.m1568d(HistoryInfoHelper.TAG, "handleMessage(" + msg + ")");
    ...
    private class Holder {
        ...
        public void addUniqueAll(List<Album> list) {
            if (ListUtils.isEmpty((List) list)) {
                LogUtils.m1577w(HistoryInfoHelper.TAG, "addUniqueAll() empty list!");
                return;
            }
            for (Album album : list) {
                ...
                if (contains(album.qpId) || TextUtils.isEmpty(album.qpId) || TextUtils.isEmpty(album.tvQid)) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1568d(HistoryInfoHelper.TAG, "addUniqueAll() remove " + DataUtils.albumInfoToString(album));
                    }
    ...
    private class LoadAnonymityListener extends Holder implements IVrsCallback<ApiResultHistoryList> {
        ...
        public void onSuccess(ApiResultHistoryList list) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(HistoryInfoHelper.TAG, "AnonyousListener.onSuccess(" + list + ")");
            }
        ...
        public void onException(ApiException exception) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(HistoryInfoHelper.TAG, "AnonyousListener.onException() " + exception.getCode() + ", " + exception.getHttpCode());
            }
        }
    ...
    private class LoadUserListener extends Holder implements IVrsCallback<ApiResultHistoryListForUser> {
        ...
        public void onSuccess(ApiResultHistoryListForUser list) {
            ...
            if (!(list == null || list.data == null)) {
                ...
                LogUtils.m1568d(HistoryInfoHelper.TAG, "UserListener.onSuccess total size(" + this.mTotal + ")");
            }
            LogUtils.m1568d(HistoryInfoHelper.TAG, "UserListener.onSuccess(" + list + ")");
            if (list != null) {
                addUniqueAll(list.getAlbumList());
            }
        ...
        public void onException(ApiException exception) {
            if (exception != null && LogUtils.mIsDebug) {
                LogUtils.m1568d(HistoryInfoHelper.TAG, "UserListener.onException() " + exception.getCode() + ", " + exception.getHttpCode());
            }
    ...
    private class SaveTvHistoryListener implements IVrsCallback<ApiResultCode> {
        ...
        public void onException(ApiException exception) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(HistoryInfoHelper.TAG, "SaveTvHistoryListener.onException() " + exception.getCode() + ", " + exception.getHttpCode());
            }
        ...
        public void onSuccess(ApiResultCode result) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(HistoryInfoHelper.TAG, "SaveTvHistoryListener.onSuccess(" + result + ")");
            }
    ...
    public List<HistoryInfo> getHistoryLongVideoMsg(int count) {
        ...
        for (HistoryInfo info22 : temp) {
            LogUtils.m1568d(HistoryInfoHelper.TAG, "getHistoryLongVideoMsg info = " + info22.toString());
        }
        if (temp.size() > count) {
    ...
    public List<HistoryInfo> getHistoryVideoMsg(int count) {
        ...
        List<HistoryInfo> temp = HistoryInfoHelper.sort(list);
        for (HistoryInfo info22 : temp) {
            LogUtils.m1568d(HistoryInfoHelper.TAG, "getHistoryVideoMsg info = " + info22.toString());
        }
        if (temp.size() > count) {
            return temp.subList(0, count);
        }
    ...
    private void handleLoadHistoryList(HistoryCloudRequest request, IHistoryResultCallBack callback) {
        if (callback != null) {
            ...
            LogUtils.m1568d(HistoryInfoHelper.TAG, "total album cache size: " + list.size());
    ...
    private void handleLoadHistoryListFromCloud() {
        ...
        LogUtils.m1568d(HistoryInfoHelper.TAG, "reload cloud play record is login? " + hasLogIn);
        ...
        if (hasLogIn) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(HistoryInfoHelper.TAG, "reload login user cloud end. " + list.size() + "request count=" + 2);
            }
            if (list.size() > 0) {
                for (HistoryInfo info : list) {
                    this.mLoginMemoryCache.put(getAlbumKey(info.getQpId()), info);
    ...
    private HistoryResults getHistoryList(HistoryCloudRequest param, List<HistoryInfo> totalRecords) {
        ...
        for (HistoryInfo item : totalRecords) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(HistoryInfoHelper.TAG, "getHistoryList return (" + item + ")");
            }
        }
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(HistoryInfoHelper.TAG, "play history request parameters(" + param.toString() + ")");
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(HistoryInfoHelper.TAG, "getHistoryList size(" + albums.size() + ")");
        }
    ...
    private void handleMergeDeviceAndCloudHistoryMsg() {
        ...
        for (HistoryInfo item : this.mHistoryDbCache.reload(AppRuntimeEnv.get().getDefaultUserId())) {
            LogUtils.m1568d(HistoryInfoHelper.TAG, "cookie = " + AppClientUtils.getCookie(AppRuntimeEnv.get().getApplicationContext()));
            item.changeToUserCookie(AppClientUtils.getCookie(AppRuntimeEnv.get().getApplicationContext()));
            temp.add(item);
        }
        LogUtils.m1568d(HistoryInfoHelper.TAG, "local history size = " + temp.size());
        List<HistoryInfo> list = this.mHistoryDbCache.reload(AppClientUtils.getCookie(AppRuntimeEnv.get().getApplicationContext()));
        if (list != null) {
            LogUtils.m1568d(HistoryInfoHelper.TAG, "cloud history size = " + list.size());
        }
    ...
    private void HandleUploadHistory(HistoryInfo info) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1568d(HistoryInfoHelper.TAG, "save() tvId=" + tvId + ", time=" + playTime);
        }
