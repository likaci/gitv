...
public class SettingCard implements ISettingApp {
    ...
    private MyObserver mCheckUpgradeObserver = new C17932();
    private BroadcastReceiver mCommonMessageReceiver = new C17921();
    private IDataBus mDataBus;
    private boolean mIsHomeVersion = false;
    protected SettingAppCard mSettingAppCard;

    class C17921 extends BroadcastReceiver {
        C17921() {
        }

        public void onReceive(Context context, Intent intent) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(SettingCard.LOG_TAG, "SystemMessageReceiver");
            }
            if (SettingCard.ACTION_SYSTEM_MESSAGE_UNREAD.equals(intent.getAction())) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.m1568d(SettingCard.LOG_TAG, "SystemMessage unRead Message Num=" + mMsgNum);
                }
            }
        }
    }

    class C17932 implements MyObserver {
        C17932() {
        }

        public void update(String event) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(SettingCard.LOG_TAG, "receive upgrade event");
            }
    ...
    public SettingCard(SettingAppCard settingAppCard) {
        ...
        if (this.mIsHomeVersion) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(LOG_TAG, "Register MessageReceiver");
            }
