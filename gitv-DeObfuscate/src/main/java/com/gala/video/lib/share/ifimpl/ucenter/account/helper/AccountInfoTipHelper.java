...
public class AccountInfoTipHelper {
    ...
    public static void checkUserInfo(final Activity context) {
        if (GetInterfaceTools.getIGalaAccountManager().isLogin(AppRuntimeEnv.get().getApplicationContext())) {
            ThreadUtils.execute(new Runnable() {
                ...
                class C17201 implements IVrsCallback<ApiResultKeepaliveInterval> {
                    C17201() {
                    }

                    public void onSuccess(ApiResultKeepaliveInterval result) {
                        LogUtils.m1568d(AccountInfoTipHelper.TAG, ">>>>>UserHelper.checkVipAccount.callSync---onSuccess");
                        AccountAdsHelper.handelCheckVipAccount(result);
                    }

                    public void onException(ApiException exception) {
                        LogUtils.m1573e(AccountInfoTipHelper.TAG, ">>>>>UserHelper.checkVipAccount.callSync---onException, code:", exception.getCode());
                ...
                public void run() {
                    ...
                    if (respBean != null) {
                        if (respBean.getRespResult()) {
                            LogUtils.m1568d(AccountInfoTipHelper.TAG, "check user info is success");
                            UserHelper.checkVipAccount.callSync(new C17201(), respBean.getCookie());
    ...
    private static void handleLogOut(ApiException exception, Activity activity) {
        if (exception != null && !"-50".equals(exception.getHttpCode())) {
            ...
            LogUtils.m1568d(TAG, "PassportTVHelper.userInfo.call exception code is : " + errorCode);
            if (!"-100".equals(errorCode) && !StringUtils.isEmpty(errorCode)) {
                ...
                if (StringUtils.isEmpty(warning) || activity == null) {
                    LogUtils.m1571e(TAG, ">>>>> warning tip is empty");
                } else {
    ...
    public static String getAccountWarningStr(String errorCode) {
        ...
        if (LoginConstant.ACCOUNT_ECODE_A00001.equals(errorCode)) {
            return AppRuntimeEnv.get().getApplicationContext().getResources().getString(C1632R.string.account_error_a00001);
        }
        if ("A00005".equals(errorCode)) {
            return AppRuntimeEnv.get().getApplicationContext().getResources().getString(C1632R.string.account_error_a00005);
        }
        if (LoginConstant.ACCOUNT_ECODE_A00055.equals(errorCode)) {
            return AppRuntimeEnv.get().getApplicationContext().getResources().getString(C1632R.string.account_error_a00055);
        }
        if (LoginConstant.ACCOUNT_ECODE_A00056.equals(errorCode)) {
            return AppRuntimeEnv.get().getApplicationContext().getResources().getString(C1632R.string.account_error_a00056);
        }
        return warning;
    }
                ...
                if (!CreateInterfaceTools.createUpdateManager().isShowingDialog()) {
                    ...
                    OnClickListener okListener = new OnClickListener() {
                        public void onClick(View view) {
                            LogUtils.m1568d(AccountInfoTipHelper.TAG, ">>>>>showAccountExceptionDialog --- OnClickListener -- ok");
                            ...
                            LogUtils.m1568d(AccountInfoTipHelper.TAG, ">>>>>showAccountExceptionDialog --- OnClickListener -- goto login page");
                            GetInterfaceTools.getLoginProvider().startLoginActivity(activity, "", 2);
                        }
                    };
                    OnClickListener cancelListener = new OnClickListener() {
                        public void onClick(View view) {
                            LogUtils.m1568d(AccountInfoTipHelper.TAG, ">>>>>showAccountExceptionDialog --- OnClickListener -- cancel");
                            confirmDialog.dismiss();
                        }
                    };
                    confirmDialog.setOnDismissListener(new OnDismissListener() {
                        public void onDismiss(DialogInterface dialog) {
                            LogUtils.m1568d(AccountInfoTipHelper.TAG, ">>>>>showAccountExceptionDialog --- OnDismissListener()");
                            if (!StringUtils.isEmpty(GetInterfaceTools.getIGalaAccountManager().getAuthCookie())) {
                                GetInterfaceTools.getIGalaAccountManager().logOut(context, "", LoginConstant.LGTTYPE_EXCEPTION);
                            }
                        }
                    });
                    confirmDialog.setParams(tip, activity.getResources().getString(C1632R.string.arefresh_login_ok), okListener, activity.getResources().getString(C1632R.string.arefresh_login_cancel), cancelListener);
                    confirmDialog.show();
                }
            }
        });
    ...
    public static void showAccountConflictDialog(final Activity activity, boolean isShowUI) {
        if (isShowUI) {
            mHandler.post(new Runnable() {
                public void run() {
                    final GlobalDialog confirmDialog = new GlobalDialog(activity);
                    confirmDialog.setParams(activity.getResources().getString(C1632R.string.account_conflict_mess), activity.getResources().getString(C1632R.string.account_conflict_ok), new OnClickListener() {
                        public void onClick(View view) {
                            LogUtils.m1568d(AccountInfoTipHelper.TAG, ">>>>>showAccountConflictDialog --- OnClickListener -- ok");
                            confirmDialog.dismiss();
                            GetInterfaceTools.getLoginProvider().startUcenterActivity(activity);
                        }
                    }, activity.getResources().getString(C1632R.string.arefresh_login_cancel), new OnClickListener() {
                        public void onClick(View view) {
                            LogUtils.m1568d(AccountInfoTipHelper.TAG, ">>>>>showAccountConflictDialog --- OnClickListener -- cancel");
                            confirmDialog.dismiss();
                        }
                    });
                    confirmDialog.show();
    ...
    private static void renewAuthCookie() {
        ...
        handler.post(new Runnable() {
            ...
            class C17291 implements IVrsCallback<ApiResultData> {
                C17291() {
                }
            ...
            public void run() {
                if (GetInterfaceTools.getIGalaAccountManager().isLogin(AppRuntimeEnv.get().getApplicationContext())) {
                    GetInterfaceTools.getIGalaAccountManager().renewCookie(new C17291());
                } else {
                    LogUtils.m1568d(AccountInfoTipHelper.TAG, "isLogout,stop renew");
                }
