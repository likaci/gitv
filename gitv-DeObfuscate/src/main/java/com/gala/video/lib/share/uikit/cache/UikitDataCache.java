...
public class UikitDataCache implements IUikitDataCache {
    ...
    public List<CardInfoModel> readPageInfoFromLocalCache(String id, int engineid, boolean isVip) {
        ...
        try {
            ...
            LogUtils.m1568d(TAG, "[start performance] read page data to cache start, id=" + id);
            ...
            try {
                ...
                LogUtils.m1568d(TAG, "read page data from cache finished,id=" + id + ",consumed = " + (System.currentTimeMillis() - startTime) + " ms");
                startTime = System.currentTimeMillis();
                cardInfoModelList = JSON.parseArray(new String(buffer), CardInfoModel.class);
                LogUtils.m1568d(TAG, "[start performance] read and parse page data from cache finished,id=" + id + ",consumed = " + (System.currentTimeMillis() - startTime) + " ms");
                if (fc != null) {
                    try {
                        fc.close();
                ...
                rFile = randomAccessFile;
                try {
                    LogUtils.m1571e(TAG, "read page info from local cache exception : " + e2);
                    if (fc != null) {
                        try {
                            fc.close();
                ...
                e1 = e4;
                rFile = randomAccessFile;
                LogUtils.m1571e(TAG, "read page info from local cache exception : " + e1);
                if (fc != null) {
                    try {
                        fc.close();
            ...
            e2222 = e5;
            LogUtils.m1571e(TAG, "read page info from local cache exception : " + e2222);
            if (fc != null) {
                fc.close();
            }
            if (rFile != null) {
    ...
    public ApiResultGroupDetail parse(byte[] raw) {
        ...
        LogUtils.m1568d(TAG, "parse bytes time = " + (System.currentTimeMillis() - System.currentTimeMillis()));
        return result;
    }
    ...
    public void writePageInfoToLocalCache(List<CardInfoModel> list, String raw, String id) {
        ...
        synchronized (id) {
            ...
            try {
                ...
                LogUtils.m1568d(TAG, "write page data to cache start,id=" + id + ", path = " + path);
                ...
                try {
                    ...
                    LogUtils.m1568d(TAG, "write page data to cache finished, id=" + id);
                    rFile = rFile2;
                } catch (IOException e3) {
                    e = e3;
                    rFile = rFile2;
    ...
    public boolean isPageCached(String sourceId) {
        File file = new File(UikitDataCacheConstants.PATH + "uikit_" + sourceId + ".dem");
        LogUtils.m1568d(TAG, "isPageCached path = " + file.getAbsolutePath());
        if (file == null || !file.exists()) {
            return false;
        }
        return true;
