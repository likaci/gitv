...
public class NetDianoseLogRecord {
    ...
    private INDDoneListener mCdnListener = new C16682();
    private INDDoneListener mCollectListener = new C16737();
    private Context mContext;
    NetDiagnoseController mController;
    private INDDoneListener mDnsListener = new C16715();
    private INDDoneListener mNetConnListener = new C16671();
    private INDDoneListener mNsLookListener = new C16726();
    private NetDiagnoseInfo mResult;
    private INDDoneListener mThirdSpeedListener = new C16693();
    private INDDoneListener mTraceRouteListener = new C16704();
    private INDUploadCallback mUploadcallback = new C16748();
    private List<INDWrapperOperate> mWrapperList;
    private String messagePushID;

    class C16671 implements INDDoneListener {
        C16671() {
        }

        public void onFinish(Map<String, Object> map) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDianoseLogRecord.TAG, "mNetConnListener");
            }
        }
    }

    class C16682 implements INDDoneListener {
        C16682() {
        }
    ...
    class C16693 implements INDDoneListener {
        C16693() {
        }

        public void onFinish(Map<String, Object> map) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDianoseLogRecord.TAG, "mThirdSpeedListener");
            }
        }
    }

    class C16704 implements INDDoneListener {
        C16704() {
        }

        public void onFinish(Map<String, Object> map) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDianoseLogRecord.TAG, "mTraceRouteListener");
            }
        }
    }

    class C16715 implements INDDoneListener {
        C16715() {
        }

        public void onFinish(Map<String, Object> resultMap) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDianoseLogRecord.TAG, "mDnsListener");
            }
            NetDianoseLogRecord.this.setReslut((NetDiagnoseInfo) resultMap.get("data"));
        }
    }

    class C16726 implements INDDoneListener {
        C16726() {
        }

        public void onFinish(Map<String, Object> resultMap) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDianoseLogRecord.TAG, "mNsLookListener");
            }
            NetDianoseLogRecord.this.setReslut((NetDiagnoseInfo) resultMap.get("data"));
        }
    }

    class C16737 implements INDDoneListener {
        C16737() {
        }

        public void onFinish(Map<String, Object> resultMap) {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDianoseLogRecord.TAG, "mCollectListener");
            }
            NetDianoseLogRecord.this.setReslut((NetDiagnoseInfo) resultMap.get("data"));
    ...
    class C16748 implements INDUploadCallback {
        C16748() {
        }

        public void uploadNetDiagnoseDone() {
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(NetDianoseLogRecord.TAG, "mUploadcallback uploadNetDiagnoseDone");
            }
        }
    }
    ...
    public void uploadAutoResult(String pushID) {
        if (this.mResult != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1568d(TAG, "uploadResult jsonResult size=" + (mCdnJsonResult != null ? Integer.valueOf(mCdnJsonResult.length()) : "NULL"));
            }
