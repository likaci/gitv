...
public class HeaderView extends LinearLayout implements IViewLifecycle<Presenter>, View {
    ...
    private void initTip(TextCanvas textCanvas) {
        if (textCanvas == null || TextUtils.isEmpty(textCanvas.getText())) {
            LogUtils.m1574i("HeaderView", Control.RETURN);
            if (getChildCount() > 0) {
                removeAllViews();
            }
        } else if (isLabelShow()) {
            ...
            if (getChildCount() <= 0) {
                ...
                this.mTipTextView.setTextSize(0, (float) getResources().getDimensionPixelSize(C1632R.dimen.dimen_16dp));
                this.mTipTextView.setMaxWidth(ResourceUtil.getDimen(C1632R.dimen.dimen_240dp));
                int paddingLeft = getResources().getDimensionPixelSize(C1632R.dimen.dimen_14dp);
                int paddingRight = getResources().getDimensionPixelSize(C1632R.dimen.dimen_9dp);
                int paddingTop = getResources().getDimensionPixelSize(C1632R.dimen.dimen_2dp);
                int paddingBottom = getResources().getDimensionPixelSize(C1632R.dimen.dimen_2dp);
