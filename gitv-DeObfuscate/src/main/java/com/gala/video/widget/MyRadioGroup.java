...
public class MyRadioGroup extends FrameLayout {
    ...
    private OnFocusChangeListener mChildFocusListener = new C18761();
    ...
    private final OnItemClickListener mDefItemClickListener = new C18772();
    ...
    class C18761 implements OnFocusChangeListener {
        C18761() {
        }

        public void onFocusChange(View v, boolean hasFocus) {
            if (DebugOptions.LOG) {
                LogUtils.m1598d(MyRadioGroup.TAG, "onFocusChange: " + v.getId() + ", " + hasFocus);
            }
    ...
    class C18772 implements OnItemClickListener {
        C18772() {
        }
    ...
    public void setDimens(int[] dimens) {
        if (this.mItemBgResId == Integer.MIN_VALUE) {
            ...
            LogUtils.m1598d(TAG, "setDimens: [0]=" + dimens[0] + ", [1]=" + dimens[1] + ", paddings=" + getBgDrawablePaddings() + ", calculated contentSpacing=" + this.mContentSpacing);
        }
    }
    ...
    public void setDataSource(List<String> itemList, int initialSelection) {
        if (this.mItemBgResId == Integer.MIN_VALUE || this.mItemWidthPadded <= 0 || this.mItemHeightPadded <= 0) {
            ...
            LogUtils.m1598d(TAG, "setDataSource: initial selection=" + this.mSelection);
            if (this.mItemBgResId == Integer.MIN_VALUE) {
    ...
    public void updateDataSource(List<String> list, int selection) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1598d(TAG, "updateDataSource, list=" + list);
        }
        if (list != null && list.size() != 0) {
            ...
            LogUtils.m1598d(TAG, "updateDataSource, invalid list.");
        }
    }
    ...
    private void checkFocusState() {
        ...
        if (DebugOptions.LOG) {
            LogUtils.m1604i(TAG, "checkFocusPos: " + this.mFocusState);
        }
    }
    ...
    private void showItemHint(int index) {
        if (index < 0 || index > this.mChildList.size()) {
            if (DebugOptions.LOG) {
                LogUtils.m1607w(TAG, "showItemHint(" + index + "): index out of range!");
            }
            ...
            if (hint != null) {
                ...
                LogUtils.m1607w(TAG, "no hint for current item (#" + index + ")");
            }
        } else if (DebugOptions.LOG) {
            LogUtils.m1607w(TAG, "hint window not created yet!");
        }
    }
    ...
    private void addChildViews(List<String> itemList, int initialSelection, boolean keepFocus) {
        ...
        while (i < listSize) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.m1598d(TAG, "addChildViews, realWidth=" + realWidth + ", realHeight=" + realHeight);
            }
            ...
            if (DebugOptions.LOG) {
                LogUtils.m1598d(TAG, "addChild: child[" + i + "]: id=" + txt.getId());
            }
    ...
    private int checkTextWidth(String str, int charSize) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1598d(TAG, ">> checkTextWidth, str=" + str + ", charSize=" + charSize);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1598d(TAG, "<< checkTextWidth, itemCount=" + itemCount + ", width=" + width);
        }
        return width;
    }
        ...
        if ((23 == event.getKeyCode() || 66 == event.getKeyCode()) && event.getAction() == 0 && event.getRepeatCount() == 0 && isShown() && hasFocus()) {
            ...
            if (focus != null) {
                ...
                if (this.mDisabledItems.contains(Integer.valueOf(clickedIndex))) {
                    if (DebugOptions.LOG) {
                        LogUtils.m1607w(TAG, "onClick: item is disabled");
                    }
                    return false;
                } else if (clickedIndex >= 0 && clickedIndex < this.mChildList.size()) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1598d(TAG, "dispatchKeyEvent, perform click event=" + event);
                    }
    ...
    private int getZoomInSpace(int w) {
        ...
        if (DebugOptions.LOG) {
            LogUtils.m1598d(TAG, "getZoomInSpace: deltaW=" + deltaW);
        }
        int result = negativeW + deltaW;
        if (DebugOptions.LOG) {
            LogUtils.m1598d(TAG, "getZoomInSpace: result=" + result);
        }
        return result;
    }
