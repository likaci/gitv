...
public class GridViewPagerAdapter<T> extends PagerAdapter {
    ...
    public void setDatas(ArrayList<T> dataList) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1598d(TAG, "setDatas: " + dataList);
        }
    ...
    public Object instantiateItem(ViewGroup container, int page) {
        if (LogUtils.mIsDebug) {
            LogUtils.m1598d(TAG, ">> instantiateItem, page=" + page);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.m1598d(TAG, "instantiateItem: subList=" + subList);
        }
        ...
        if (this.mViews != null && this.mDataList != null) {
            for (int i = 0; i < view_size; i++) {
                ...
                if (adapter != null) {
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.m1598d(TAG, "notify grid view data change, size=" + view_size + ", data_size=" + data_size + ", mNums=" + this.mNums);
                    }
