...
public class FinalBitmap {
    ...
    class C00271 implements ThreadFactory {
        C00271() {
        }
    ...
    class BitmapLoadTask extends AsyncTask<Object, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Object... objArr) {
            ...
            if (!this.mExitTasksEarly) {
                ...
                if (bitmap != null) {
                    FinalBitmap.this.mImageCache.addToMemoryCache(C0126b.m301a(this.imageRequest), new C0125a(bitmap));
                }
            }
            return bitmap;
        }
    ...
    private FinalBitmap init() {
        if (!this.mInit) {
            ...
            if (((double) this.mConfig.memCacheSizePercent) > 0.05d && ((double) this.mConfig.memCacheSizePercent) < 0.8d) {
                ...
                C0123G.m279a("FinalBitmap", ">>>>>afinal - set memory cache size: " + this.mConfig.memCacheSize);
                imageCacheParams.setMemCacheSize(this.mConfig.memCacheSize);
            } else {
                C0123G.m279a("FinalBitmap", ">>>>>afinal - set memory cache size [default]");
                imageCacheParams.setMemCacheSizePercent(this.mContext, 0.1f);
            }
            if (this.mConfig.diskCacheSize > 5242880) {
                C0123G.m279a("FinalBitmap", ">>>>>afinal - set disk cache size: " + this.mConfig.diskCacheSize);
                imageCacheParams.setDiskCacheSize(this.mConfig.diskCacheSize);
            } else {
                C0123G.m279a("FinalBitmap", ">>>>>afinal - set disk cache size [default 50M]");
            }
            if (this.mConfig.diskCacheCount > 50) {
                C0123G.m279a("FinalBitmap", ">>>>>afinal - set disk cache count: " + this.mConfig.diskCacheCount);
                imageCacheParams.setDiskCacheCount(this.mConfig.diskCacheCount);
            } else {
                C0123G.m279a("FinalBitmap", ">>>>>afinal - set disk cache count [default]");
            }
            ...
            C0123G.m279a("FinalBitmap", ">>>>>ThreadPool size=" + this.mConfig.poolSize);
            this.bitmapLoadAndDisplayExecutor = Executors.newFixedThreadPool(this.mConfig.poolSize, new C00271());
    ...
    public Bitmap getBitmapFromCache(ImageRequest imageRequest) {
        Bitmap bitmapFromMemoryCache = getBitmapFromMemoryCache(C0126b.m301a(imageRequest));
    ...
    public void loadBitmap(ImageRequest request, IImageCallback imageCallback) {
        ...
        Bitmap bitmap = null;
        if (this.mImageCache != null) {
            bitmap = this.mImageCache.getBitmapFromMemoryCache(C0126b.m301a(request));
        }
        if (bitmap != null) {
    ...
    public void stopAllTasks() {
        C0123G.m279a("FinalBitmap", ">>>>>afinal - stopAllTasks");
