...
public final class SysPropUtils {
    ...
    static {
        ...
        try {
            ...
            if (Log.DEBUG) {
                Log.m433e(TAG, "exception during reflection:", e);
            }
            METHOD_SYSPROP_GETINT = methodGetInt;
            METHOD_SYSPROP_GETBOOLEAN = methodGetBoolean;
            METHOD_SYSPROP_GET = methodGet;
    ...
    private static Object safeInvoke(Method method, String key, Object def) {
        if (Log.VERBOSE) {
            Log.m430d(TAG, ">> safeInvoke(method:" + method + ", key:" + key + ", def:" + def + ")");
        }
        if (method != null) {
            Object ret = def;
            try {
                ...
                if (Log.DEBUG) {
                    Log.m433e(TAG, "safeInvoke: exception happened: ", e);
                }
            }
            if (Log.DEBUG) {
                Log.m430d(TAG, "<< safeInvoke: return=" + ret);
            }
            return ret;
        } else if (!Log.DEBUG) {
            return def;
        } else {
            Log.m432e(TAG, "<< safeInvoke: reflection failed");
            return def;
        }
    }
}
