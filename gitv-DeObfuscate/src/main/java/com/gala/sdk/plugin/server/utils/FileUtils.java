...
public class FileUtils {
    ...
    public static int getFileCount(String folder, FileFilter filter) {
        ...
        if (Log.DEBUG) {
            Log.m434v(TAG, "getFileCount() folder=" + folder + ", return count=" + count);
        }
        return count;
    }
    ...
    public static List<String> deleteExportFile(String exportPath) {
        ...
        if (!Util.isEmpty(exportPath)) {
            ...
            if (parent != null) {
                if (Log.VERBOSE) {
                    Log.m434v(TAG, "deleteExportFile() export.getName()" + export.getName() + "parent= " + parent.getName());
                }
                String[] children = parent.list();
                if (children != null) {
                    for (int i = 0; i < children.length; i++) {
                        if (Log.VERBOSE) {
                            Log.m434v(TAG, "deleteExportFile() scanFile=" + children[i]);
                        }
                        if (!Util.equals(export.getName(), children[i])) {
                            if (Log.VERBOSE) {
                                Log.m434v(TAG, "deleteExportFile() deleteDir=" + children[i]);
                            }
                    ...
                    Log.m432e(TAG, "deleteExportFile() children is empty");
                }
            } else if (Log.VERBOSE) {
                Log.m432e(TAG, "deleteExportFile() getParentFile is empty");
            }
        } else if (Log.VERBOSE) {
            Log.m432e(TAG, "deleteExportFile() exportPath is empty");
        }
        return deleteList;
    }
    ...
    private static boolean deleteDir(File dir) {
            ...
            if (children != null) {
                ...
                while (i < children.length) {
                    if (deleteDir(new File(dir, children[i]))) {
                        ...
                        Log.m434v(TAG, "deleteDir(" + children[i] + ") return=" + false);
                        return false;
                    }
                }
            }
        ...
        if (Log.VERBOSE) {
            Log.m434v(TAG, "deleteDir() " + getFileName(dir.getAbsolutePath()) + "  return=" + success);
        }
        return success;
    }
    ...
    public static boolean writeFile(InputStream inputStream, String soPath, Context context) throws Throwable {
        ...
        try {
            ...
            try {
                ...
                if (Log.DEBUG) {
                    Log.m430d(TAG, "writeFile sleepSumtime " + sleepSumTime + "ms");
                }
                outputStream2.flush();
    ...
    public static boolean copyFromAssets(Context context, String assestsPath, String targetPath) throws Throwable {
        ...
        Throwable th;
        if (Log.DEBUG) {
            Log.m430d(TAG, "copyFromAssets(assetsPath=" + assestsPath + ", targetPath=" + targetPath + ")");
        }
            ...
            try {
                ...
                while (true) {
                    ...
                    if (sleepSize == 512) {
                        ...
                        sleepSumtime += sleeptime;
                        if (Log.DEBUG) {
                            Log.m430d(TAG, "copyFromAssets sleep " + sleeptime + "ms");
                        }
                        sleepSize = 0;
                    }
                }
                sDelaySumTime += (long) sleepSumtime;
                if (Log.DEBUG) {
                    Log.m430d(TAG, "copyFromAssets sleepSumtime " + sleepSumtime + "ms");
                }
                outputStream2.flush();
                closeStream(inputStream);
                closeStream(outputStream2);
                if (Log.DEBUG) {
                    Log.m430d(TAG, "copyFromAssets(return " + true + ")");
                }
                return true;
            } catch (Throwable th2) {
                th = th2;
    ...
    private static boolean createNewDirectory(File file) {
        Log.m430d(TAG, "createNewDirectory()");
        boolean success = file.mkdirs();
        if (file.exists()) {
            Log.m430d(TAG, "createNewDirectory(), path already exists!");
            return true;
        }
        Log.m430d(TAG, "createNewDirectory() mkdirs fail");
        return success;
    }

    private static boolean createNewFile(String filePath, File file) throws Throwable {
        Log.m430d(TAG, "createNewFile()");
        File parent = file.getParentFile();
        if (!(parent == null || parent.exists())) {
            Log.m430d(TAG, "createNewFile(), path parent is not exists!" + file.getParentFile().mkdirs());
        }
        if (file.createNewFile()) {
            Log.m430d(TAG, "createNewFile() success" + true);
            return true;
            ...
            Log.m430d(TAG, "createNewFile() exist? " + exist + " delete-success? " + deleted + " and create=" + success);
            return success;
        } else {
            Log.m430d(TAG, "createNewFile() file can not write ?/read=" + file.canRead() + "/write=" + file.canWrite() + "/execute=" + file.canExecute());
            file.setExecutable(true);
            file.setReadable(true);
            file.setWritable(true);
            Log.m430d(TAG, "createNewFile() file can write ?/read=" + file.canRead() + "/write=" + file.canWrite() + "/execute=" + file.canExecute());
            exist = file.exists();
            deleted = false;
            if (exist) {
                deleted = file.delete();
    ...
    public static boolean createNewFile(String filePath, Context context) throws Throwable {
        Log.m430d(TAG, "createNewFile(), path=" + filePath);
        boolean success = false;
        if (Util.isEmpty(filePath)) {
            Log.m430d(TAG, "empty filePath");
        } else {
            File file = new File(filePath);
            if (fileIsDirectory(filePath) == 3) {
                success = createNewDirectory(file);
                Log.m430d(TAG, "createNewDirectory()->" + success);
            } else if (fileIsDirectory(filePath) == 2) {
                success = createNewFile(filePath, file);
                Log.m430d(TAG, "createNewFile()->" + success);
            } else {
                Log.m430d(TAG, "createNewFile() what's is this Path is->" + filePath);
                Log.m430d(TAG, "createNewFile() what's is this File is->" + file);
                Log.m430d(TAG, "createNewFile() what's is this context is->" + context);
            }
        }
        Log.m430d(TAG, "createNewFile(), final return->" + success);
        return success;
    }
    ...
    public static String getURLFileName(String url) {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "getFileName<<(url=" + url + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.m434v(TAG, "getFileName>>() return " + fileName);
        }
        return fileName;
    }
    ...
    public static String toFilePath(String rootPath, String... folderName) {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "toFilePath<<(rootPath=" + rootPath + ", folderName=" + folderName + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.m434v(TAG, "toFilePath>>() return " + filePath);
        }
        return filePath;
    }
    ...
    public static boolean renameFile(String sourceFilePath, String targetFilePath) {
        if (!(Util.isEmpty(sourceFilePath) || Util.isEmpty(targetFilePath))) {
            ...
            if (Log.VERBOSE) {
                Log.m434v(TAG, "renameFile" + success);
            }
        }
        return true;
    }
    ...
    public static void closeStream(Closeable closeable) {
        if (closeable != null) {
            try {
                ...
                e.printStackTrace();
                Log.m430d(TAG, "closeStream exception!!");
            }
        }
    }
}
