...
public class StorageServer extends Service {
    ...
    public static class StorageBinder extends Stub {
        ...
        public StorageBinder(Context context, AppInfo info) {
            if (Log.VERBOSE) {
                Log.m434v(TAG, "StorageBinder<<(context=" + context + ", info=" + info + ")");
            }
            this.mStorageStub = new StorageStub(context, info);
            if (Log.VERBOSE) {
                Log.m434v(TAG, "StorageBinder>>()");
            }
        }
        ...
        public boolean copySoLibToHost(PluginInfo info, StorageException se) throws RemoteException {
            ...
            if (Log.VERBOSE) {
                Log.m434v(TAG, "copySoLibToHost>>() return " + success);
            }
            return success;
        }
    ...
    public IBinder onBind(Intent intent) {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "onBind<<(intent=" + intent + ")");
        }
        if (this.mBinder == null) {
            this.mBinder = new StorageBinder(this, DataUtils.getAppInfo(intent.getExtras()));
        }
        if (Log.VERBOSE) {
            Log.m434v(TAG, "onBind>>() return binder=" + this.mBinder);
        }
        return this.mBinder;
    }
    ...
    public boolean onUnbind(Intent intent) {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "onUnbind(intent=" + intent + ")");
        }
        this.mBinder = null;
