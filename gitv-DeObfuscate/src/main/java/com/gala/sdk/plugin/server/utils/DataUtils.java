...
public class DataUtils {
    ...
    public static void putAppInfo(Bundle bundle, AppInfo info) {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "putAppInfo(bundle=" + bundle + ", info=" + info + ")");
        }
    ...
    public static AppInfo getAppInfo(Bundle bundle) {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "getAppInfo<<(bundle=" + bundle + ")");
        }
        AppInfo info = (AppInfo) bundle.getParcelable("appinfo");
        if (Log.VERBOSE) {
            Log.m434v(TAG, "getAppInfo>>() return " + info);
        }
        return info;
    }
    ...
    public static DownloadInfo covertToDownloadInfo(UpgradeInfo upgradeInfo, String savePath) {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "covertToDownloadInfo<<(upgradeInfo=" + upgradeInfo + ", savePath=" + savePath + ")");
        }
        DownloadInfo downloadInfo = new DownloadInfo(upgradeInfo.getUrl(), savePath);
        downloadInfo.setMd5(upgradeInfo.getMd5());
        if (Log.VERBOSE) {
            Log.m434v(TAG, "covertToDownloadInfo>>() return " + downloadInfo);
        }
        return downloadInfo;
    }
}
