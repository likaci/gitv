...
public class UpgradeManager {
    ...
    private static class MyListener implements IApiCallback<ApiResultModuleUpdate> {
        ...
        public void onSuccess(ApiResultModuleUpdate resultModuleUpdate) {
            if (Log.DEBUG) {
                Log.m430d(UpgradeManager.TAG, "moduleUpdate() onSuccess!! upgateResult=" + resultModuleUpdate);
            }
        ...
        public void onException(ApiException exception) {
            if (Log.DEBUG) {
                Log.m430d(UpgradeManager.TAG, "moduleUpdate() onException!! exception=" + exception);
            }
    ...
    public UpgradeManager(Context context) {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "UpgradeManager(context=" + context + ")");
        }
    ...
    public UpgradeInfo checkUpgrade(String version, HostPluginInfo hPluginInfo) throws Exception {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "checkUpgrade<<(hPluginInfo=" + hPluginInfo + ", version=" + version + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.m434v(TAG, "checkUpgrade>>() return " + info);
        }
        return info;
    }
    ...
    public Map<String, UpgradeInfo> checkUpgrades(Map<String, String> requestInfos) throws ApiException {
        if (Log.VERBOSE) {
            Log.m434v(TAG, "checkUpgrades<<(requestInfos=" + requestInfos + ")");
        }
        Map<String, UpgradeInfo> result = new HashMap();
        ...
        if (Log.VERBOSE) {
            Log.m434v(TAG, "checkUpgrades>>() return " + result);
        }
        return result;
    }
