...
public class ContextProxy extends ContextWrapper {
    ...
    public ClassLoader getClassLoader() {
        Log.m432e(TAG, "getClassLoader(" + this.mClassLoaderProxy);
    ...
    private AssetManager createAssetManager(String dexPath) {
        try {
            ...
            Log.m433e(TAG, "createAssetManager(" + dexPath + ") fail!", throwable);
            return null;
        }
    }
    ...
    public Resources getResources() {
        if (Log.DEBUG) {
            Log.m430d(TAG, "getResources() mResources=" + this.mResources);
        }
    ...
    public AssetManager getAssets() {
        if (Log.DEBUG) {
            Log.m430d(TAG, "getAssets() mAssetManager=" + this.mAssetManager);
        }
        if (this.mAssetManager != null) {
    ...
    public Theme getTheme() {
        if (Log.DEBUG) {
            Log.m430d(TAG, "getTheme() mTheme=" + this.mTheme);
        }
        if (this.mTheme != null) {
            return this.mTheme;
        }
