...
public class StorageStub {
    ...
    public StorageStub(Context context, AppInfo info) {
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "StorageStub<<(context=" + context + ", info=" + info + ")");
        }
        this.mContext = context;
        this.mDownloadManager = new DownloadManager(1);
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "StorageStub>>()");
        }
    }
    ...
    public synchronized boolean download(DownloadInfo info, StorageException se) throws RemoteException {
        boolean success;
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "downloadApk<<(info=" + info + ")");
        }
        try {
        ...
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "downloadApk>>() return " + success);
        }
        return success;
    }
    ...
    public synchronized void removePluginFiles(HostPluginInfo hPluginInfo, PluginInfo info) {
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "removePluginFiles<<(info=" + info + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "removePluginFiles>>()" + hPluginInfo.getPluginId());
        }
    }

    public synchronized void savePluginInfos(HostPluginInfo hPluginInfo, List<PluginInfo> list) {
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "savePluginInfos<<(hPluginInfo=" + hPluginInfo + ", list=" + list + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "savePluginInfos>>()");
        }
    }
    ...
    public synchronized boolean copyPluginFromAssets(String assetsPath, String targetPath, StorageException se) {
        boolean success;
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "copyPluginFromAssets<<(assetsPath=" + assetsPath + ", targetPath=" + targetPath + ")");
        }
        try {
        ...
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "copyPluginFromAssets>>() return " + success);
        }
        return success;
    }
    ...
    public synchronized List<PluginInfo> loadPluginInfos(HostPluginInfo hPluginInfo, boolean needRemoveDumy) {
        ...
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "loadPluginInfos<<(hPluginInfo=" + hPluginInfo + ", needRemoveDumy=" + needRemoveDumy + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "loadPluginInfos>>() return " + historyInfo);
        }
        return historyList;
    }
    ...
    private void checkAndRemoveDummy(HostPluginInfo hPluginInfo, List<PluginInfo> list) {
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "checkAndRemoveDummy<<(list=" + list + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "checkAndRemoveDummy>>()");
        }
    }
    ...
    public synchronized boolean copySoLibToHost(PluginInfo info, StorageException se) {
        boolean success;
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "copySoLibToHost<<(info=" + info + ")");
        }
        try {
        ...
        if (Log.VERBOSE) {
            Log.m434v(this.TAG, "copySoLibToHost>>() return " + success);
        }
        return success;
    }
}
