...
public class CpuLoadBalance {
    ...
    private static final int[] mSleepTime = new int[]{0, 0, 0, 0, 100, 100, 200, 400, IMediaPlayer.AD_INFO_OVERLAY_LOGIN_SUCCESS, 1600, 3200};
    ...
    static class C01811 implements FileFilter {
        C01811() {
        }
    ...
    public int getSleepTime() {
        if (!FileUtils.sOpenBalance || this.mTotalDelayTime >= 5000) {
            return 0;
        }
    ...
    private static int getCpuCoreNums() {
        if (cpuCores == 0) {
            try {
                cpuCores = new File("/sys/devices/system/cpu/").listFiles(new C01811()).length;
            } catch (Exception e) {
                cpuCores = 1;
            }
        }
