...
final class Utils {
    ...
    static boolean parseResponseSourceHeader(String header) {
        ...
        try {
            if ("CONDITIONAL_CACHE".equals(parts[0]) && Integer.parseInt(parts[1]) == TipType.CONCRETE_TYPE_REPLAY_PLAYNEXT) {
                return true;
            }
            return false;
    ...
    static long calculateDiskCacheSize(File dir) {
        ...
        return Math.max(Math.min(size, IGalaDownloadParameter.FILE_SIZE_LIMIT), 5242880);
    }

    static int calculateMemoryCacheSize(Context context) {
        ActivityManager am = (ActivityManager) getService(context, "activity");
        boolean largeHeap = (context.getApplicationInfo().flags & SendFlag.FLAG_KEY_PINGBACK_SP) != 0;
        int memoryClass = am.getMemoryClass();
        if (largeHeap && VERSION.SDK_INT >= 11) {
            memoryClass = ActivityManagerHoneycomb.getLargeMemoryClass(am);
        }
        return (SendFlag.FLAG_KEY_PINGBACK_SP * memoryClass) / 7;
    }
