    ...
    public static int getResourceIdByReflection(Context context, String className, String name) {
        ...
        try {
            if (AppStoreDetailActivity.getMixFlag()) {
                ...
                LogUtils.m1738e(TAG, "getResourceId: getMixFlag required class = " + requiredClass.getName());
            } else {
                for (Class clazz : Class.forName(resourcePkgName + ".R").getClasses()) {
                    if (clazz.getSimpleName().equals(className)) {
                        LogUtils.m1738e(TAG, "getResourceId: clazz.getSimpleName().equals(className)");
                        requiredClass = clazz;
                        break;
                    }
                    LogUtils.m1738e(TAG, "getResourceId: clazz.getSimpleName() does not equals(className)");
                }
            }
            ...
            LogUtils.m1738e(TAG, "getResourceId: name = " + name + " requiredclass = " + requiredClass.getName() + " id = " + id);
            return id;
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
