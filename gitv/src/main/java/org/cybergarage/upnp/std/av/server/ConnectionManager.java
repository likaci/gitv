    ...
    public boolean actionControlReceived(Action action) {
        ...
        if (actionName.equals("GetProtocolInfo")) {
            ...
            for (int n = 0; n < mimeTypeCnt; n++) {
                if (n > 0) {
                    sourceValue = new StringBuilder(String.valueOf(sourceValue)).append(",").toString();
                }
                sourceValue = new StringBuilder(String.valueOf(sourceValue)).append("http-get:*:").append(getContentDirectory().getFormat(n).getMimeType()).append(":*").toString();
            }
            action.getArgument("Source").setValue(sourceValue);
            action.getArgument("Sink").setValue("");
            return true;
        } else if (actionName.equals("PrepareForConnection")) {
            action.getArgument("ConnectionID").setValue(-1);
            action.getArgument("AVTransportID").setValue(-1);
            action.getArgument("RcsID").setValue(-1);
            return true;
        } else if (actionName.equals("ConnectionComplete")) {
            return true;
        } else {
