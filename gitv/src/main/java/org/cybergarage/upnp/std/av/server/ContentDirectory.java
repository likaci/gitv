...
public class ContentDirectory extends ThreadCore implements ActionListener, QueryListener {
    ...
    private ContentDirectoryUpdateListener mDirectoryUpdateListener;
    ...
    public interface ContentDirectoryUpdateListener {
    ...
    public boolean removeDirectory(String name) {
        ...
        this.rootNode.removeNode((Node) dirNode);
        updateSystemUpdateID();
        return true;
    }
    ...
    private void initRoot() {
        addDirectory(new FileDirectory("rootFile", new StringBuilder(String.valueOf(Environment.getExternalStorageDirectory().getAbsolutePath())).append("/http").toString()));
    }
    ...
    public ContentNodeList sortContentNodeList(ContentNodeList contentNodeList, String sortCriteria) {
    ...
    private SearchCriteriaList getSearchCriteriaList(String searchStr) {
        ...
        if (searchStr != null && searchStr.compareTo(NetDiagnoseCheckTools.NO_CHECK_FLAG) != 0) {
    ...
    public int contentServiceRequestReceived(String resId) {
        ItemNode itemNode = (ItemNode) findContentNodeByID(resId);
        ...
        int connectionId = conMan.getNextConnectionID();
        ConnectionInfo conInfo = new ConnectionInfo(connectionId);
    ...
    public void contentServiceRequestFinished(int connectionId) {
        getMediaServer().getConnectionManager().removeConnectionInfo(connectionId);
    }
    ...
    public int getWebServerPort() {
        return getMediaServer().getServerPort();
    }

    public String getContentExportURL(String path, String id) {
        return new StringBuilder(WebConstants.WEB_SITE_BASE_HTTP).append(getInterfaceAddress()).append(SOAP.DELIM).append(getWebServerPort()).append(path).append("?").append("id").append(SearchCriteria.EQ).append(id).toString();
    }

    public String getContentExportURL(String id) {
        return new StringBuilder(WebConstants.WEB_SITE_BASE_HTTP).append(getInterfaceAddress()).append(SOAP.DELIM).append(getWebServerPort()).append("?").append("id").append(SearchCriteria.EQ).append(id).toString();
    }
    ...
    public void run() {
        ...
        if (this.mDirectoryUpdateListener != null) {
            this.mDirectoryUpdateListener.onDirectoryUpdate();
        }
        while (isRunnable()) {
            try {
    ...
    public ContentDirectoryUpdateListener getDirectoryUpdateListener() {
        return this.mDirectoryUpdateListener;
    }

    public void setDirectoryUpdateListener(ContentDirectoryUpdateListener mDirectoryUpdateListener) {
        this.mDirectoryUpdateListener = mDirectoryUpdateListener;
    }
}
