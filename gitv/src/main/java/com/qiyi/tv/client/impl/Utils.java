...
public class Utils {
    ...
    public static int parseErrorCode(Exception exception) {
        Log.d("Utils", "parseErrorCode(" + exception + ")", exception);
    ...
    public static boolean isServerDied(Exception exception) {
        ...
        Log.d("Utils", "isServerDied(" + exception + ") return " + z);
        return z;
    }
    ...
    public static void dumpBundle(String tag, Bundle bundle) {
        if (bundle != null) {
            Log.d("Utils", "dumpBundle() " + tag + ": bundle size=" + bundle.size());
            for (String str : bundle.keySet()) {
                Log.d("Utils", "dumpBundle() " + tag + ": key[" + str + "]=" + bundle.get(str));
            }
            return;
        }
        Log.d("Utils", "dumpBundle() " + tag + ": Null Bundle");
    }

    public static void dumpIntent(String tag, Intent intent) {
        if (intent != null) {
            Log.d("Utils", "dumpIntent() " + tag + ": action=" + intent.getAction());
            Log.d("Utils", "dumpIntent() " + tag + ": categories=" + intent.getCategories());
            Log.d("Utils", "dumpIntent() " + tag + ": flag=" + intent.getFlags() + "[" + Integer.toBinaryString(intent.getFlags()) + AlbumEnterFactory.SIGN_STR);
            dumpBundle(tag, intent.getExtras());
            return;
        }
        Log.d("Utils", "dumpIntent() " + tag + ": Null Intent");
    }
    ...
    public static void assertTrue(boolean value, String message) {
        if (!value) {
            Log.w("Utils", "assertTrue() fail! message is: " + message);
