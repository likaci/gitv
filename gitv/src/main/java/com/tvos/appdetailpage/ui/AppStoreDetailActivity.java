...
public class AppStoreDetailActivity extends BaseActivity implements Observer {
    ...
    private Callback<AppDetailResponse> mAppDetailCallback = new Callback<AppDetailResponse>() {
    ...
    private Callback<StatusResponse> mCallback = new Callback<StatusResponse>() {
        ...
        public void failure(RetrofitError e) {
            LogUtils.d(AppStoreDetailActivity.TAG, "AppStoreDetailActivity---mStatusResponsePingBack failure() Pingback failure! status = " + e.getCause() + " url=" + e.getUrl());
        }
    };
    ...
    private com.squareup.picasso.Callback mImgCallback = new com.squareup.picasso.Callback() {
        public void onError() {
    ...
    private Callback<RecommendResponse> mRecommAppCallback = new Callback<RecommendResponse>() {
        public void success(RecommendResponse resp, Response ignore) {
            if (AppStoreDetailActivity.this.mAppInfo != null) {
                ...
                if (resp.data != null) {
                    LogUtils.d(AppStoreDetailActivity.TAG, "AppStoreDetailActivity---mRecommAppCallback() ->mAppStoreClient.getAssociatedAppsAndDetail success()---" + resp.data);
                    if (AppStoreDetailActivity.this.mHandler != null) {
    ...
    private Thread mThread = new Thread(new Runnable() {
        public void run() {
            Looper.prepare();
            AppStoreDetailActivity.this.mHandler = new Handler() {
    ...
    private Callback<AppCategoriesResponse> mUpdateCategiriesCallback = new Callback<AppCategoriesResponse>() {
        public void success(AppCategoriesResponse resp, Response ignore) {
            if (resp.data != null) {
                LogUtils.d(AppStoreDetailActivity.TAG, "---mAppCategiriesCallback() ->mAppStoreClient." + resp.data);
    ...
    private void onDownloadProcess(DownloadState state) {
        ...
        try {
            ...
            LogUtils.i(TAG, "onProgress():" + progress + e.getMessage());
        }
    }
