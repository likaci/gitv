...
public class StorageManager {
    ...
    private static class EmptyStorageServer implements IStorageServer {
        ...
        public List<PluginInfo> loadPluginInfos(HostPluginInfo hPluginInfo, boolean needRemoveDumy) throws RemoteException {
            if (Log.VERBOSE) {
                Log.v(TAG, "loadPluginInfos<<(hPluginInfo=" + hPluginInfo + ", needRemoveDumy=" + needRemoveDumy + ")");
            }
            List<PluginInfo> list = new ArrayList();
            if (Log.VERBOSE) {
                Log.v(TAG, "loadPluginInfos>>() return " + list);
            }
            return list;
        }
        ...
        public void savePluginInfos(HostPluginInfo hPluginInfo, List<PluginInfo> infos) throws RemoteException {
            if (Log.VERBOSE) {
                Log.v(TAG, "savePluginInfos<<(hPluginInfo=" + hPluginInfo + ", infos=" + infos + ")");
            }
            if (Log.VERBOSE) {
        ...
        public boolean download(DownloadInfo info, StorageException e) throws RemoteException {
            if (Log.VERBOSE) {
                Log.v(TAG, "downloadAsync<<(info=" + info + ")");
            }
            if (Log.VERBOSE) {
        ...
        public boolean copyPluginFromAssets(String assetsPath, String targetPath, StorageException e) throws RemoteException {
            if (Log.VERBOSE) {
                Log.v(TAG, "copyPluginFromAssets<<(assetsPath=" + assetsPath + ", targetPath=" + targetPath + ")");
            }
            if (Log.VERBOSE) {
        ...
        public void removePluginFiles(HostPluginInfo hPluginInfo, PluginInfo info) throws RemoteException {
            if (Log.VERBOSE) {
                Log.v(TAG, "removePluginFiles<<(info=" + info + ")");
            }
            if (Log.VERBOSE) {
    ...
    private StorageManager(Context context, AppInfo info, boolean isMultiProcess) {
        if (Log.VERBOSE) {
            Log.v(TAG, "StorageManager<<(context=" + context + ", info=" + info + ", isMultiProcess" + isMultiProcess + ")");
        }
    ...
    private boolean isMultiProcessPlugin(String pluginId) {
        if (Log.VERBOSE) {
            Log.v(TAG, "isMultiProcessPlugin<<()  " + pluginId);
        }
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "isMultiProcessPlugin>>() return " + b);
        }
        return b;
    }
    ...
    private void connect() {
        ...
        boolean success = this.mHostApplicationContext.bindService(intent, new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                if (Log.VERBOSE) {
                    Log.v(StorageManager.TAG, "connect...() bindService onServiceConnected!! service=1" + service);
                }
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "connect...() bindService return " + success);
        }
        if (success) {
            lock.block();
        } else {
    ...
    public List<PluginInfo> loadPluginInfos(HostPluginInfo hPluginInfo, boolean needRemoveDumy) {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "loadPluginInfos>>() return list=" + list);
        }
        return list;
    }
    ...
    public void savePluginInfos(HostPluginInfo hPluginInfo, List<PluginInfo> list) {
        if (Log.VERBOSE) {
            Log.v(TAG, "savePluginInfos<<(hPluginInfo=" + hPluginInfo + ", list=" + list + ")");
        }
        if (isMultiProcessPlugin(hPluginInfo.getPluginId())) {
            try {
    ...
    private PluginInfo findPluginInfo(List<PluginInfo> list, String version) {
        if (Log.VERBOSE) {
            Log.v(TAG, "findPluginInfo<<(list=" + list + ", version=" + version + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "findPluginInfo>>() return " + find);
        }
        return find;
    }
    ...
    public PluginInfo loadPluginInfo(HostPluginInfo hPluginInfo, String versionName) {
        if (Log.VERBOSE) {
            Log.v(TAG, "loadPluginInfo<<(hPluginInfo=" + hPluginInfo + ", versionName=" + versionName + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "loadPluginInfo>>() return " + info);
        }
        return info;
    }
    ...
    public void savePluginInfo(PluginInfo info, HostPluginInfo hInfo) {
        if (Log.VERBOSE) {
            Log.v(TAG, "savePluginInfo<<(info=" + info + "HostPluginInfo" + hInfo + ")");
        }
    ...
    public PluginInfo parsePluginInfo(HostPluginInfo hPluginInfo, String version, String fileName) {
        if (Log.VERBOSE) {
            Log.v(TAG, "parsePluginInfo<<(hPluginInfo=" + hPluginInfo + ", version=" + version + ", fileName=" + fileName + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "parsePluginInfo>>() return " + info);
        }
        return info;
    }
    ...
    public boolean startDownload(DownloadInfo info, String pluginId) throws Throwable {
        boolean success;
        if (Log.VERBOSE) {
            Log.v(TAG, "startDownload<<(info=" + info + ")");
        }
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "startDownload>>()" + success);
        }
        return success;
    }
    ...
    public PluginInfo loadPluginInfoFromAssets(HostPluginInfo hPluginInfo, boolean useSd) throws Throwable {
        if (Log.VERBOSE) {
            Log.v(TAG, "loadPluginInfoFromAssets<<(hPluginInfo=" + hPluginInfo + ")");
        }
        PluginInfo find = null;
        ...
        if (find != null) {
            ...
            if (success) {
                ...
                if (soSuccess) {
                    ...
                    if (Log.VERBOSE) {
                        Log.v(TAG, "loadPluginInfoFromAssets>>() return++ " + info);
                    }
                    return info;
    ...
    private PluginInfo parsePluginInfoFromAssets(HostPluginInfo hPluginInfo, String assetsFileName) throws Exception {
        if (Log.VERBOSE) {
            Log.v(TAG, "parsePluginInfoFromAssets<<(assetsFileName=" + assetsFileName + ")" + "hPluginInfo " + hPluginInfo);
        }
        PluginInfo info = null;
        ...
        if (strings.length >= 3) {
            ...
            if (Log.VERBOSE) {
                Log.v(TAG, "parsePluginInfoFromAssets>>() return " + info);
            }
            return info;
        }
