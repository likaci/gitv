...
public class PluginInfo implements Parcelable {
    public static Comparator<PluginInfo> COMPARATOR = new Comparator<PluginInfo>() {
    ...
    public PluginInfo(String id, String versionName, String className, String path) {
        ...
        this.mSoFileFilter = new FileFilter() {
    ...
    public void setPluginPathSd(String sdpath) {
        if (Log.VERBOSE) {
            Log.v(TAG, "setPluginPathSd=" + sdpath + ")");
        }
        this.mPathSd = sdpath;
    }
    ...
    private PluginInfo(JSONObject json) {
        this.mShouldTrySd = false;
        this.mSoFileFilter = /* anonymous class already generated */;
        if (Log.VERBOSE) {
            Log.v(TAG, "PluginInfo<<(json=" + json.toString() + ")");
        }
    ...
    public boolean checkFileComplete() {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "checkFileComplete>>() return " + isFileComplete);
        }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "writeToParcel>>() return " + dest + flags);
        }
    }
    ...
    public static List<PluginInfo> getListFromJson(String json) {
        if (Log.VERBOSE) {
            Log.v(TAG, "getListFromJson<<(json=" + json + ")");
        }
        ...
        if (!Util.isEmpty(json)) {
            ...
            if (Log.VERBOSE) {
                Log.v(TAG, "getListFromJson>>() return " + list);
            }
        }
        return list;
    }
    ...
    public static String getJsonFromList(List<PluginInfo> list) {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "getJsonFromList>>() return " + json);
        }
        return json;
    }
}
