...
public class PluginDebugUtils {
    ...
    public static boolean needThrowable(String key) {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "needThrowable<<(key=" + key + ")");
        }
        if (allowDebug()) {
            needThrowable = SysPropUtils.getBoolean(key, false);
        }
        if (Log.VERBOSE) {
            Log.v(TAG, "needThrowable() returns " + needThrowable);
        }
    ...
    public static boolean needDebug(String key) {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "needDebug<<(key=" + key + ")");
        }
        if (allowDebug()) {
            needDebug = SysPropUtils.getBoolean(key, false);
        }
        if (Log.VERBOSE) {
            Log.v(TAG, "needDebug() returns " + needDebug);
        }
        return needDebug;
    }
    ...
    public static int getInt(String key) {
        int value = -1;
        if (Log.VERBOSE) {
            Log.v(TAG, "getInt<<(key=" + key + ")");
        }
        if (allowDebug()) {
            value = SysPropUtils.getInt(key, -1);
        }
        if (Log.VERBOSE) {
            Log.v(TAG, "getInt() returns " + value);
        }
        return value;
    }
}
