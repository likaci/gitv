...
class SoLibHelper {
    ...
    public static boolean copySoLibToHost(Context context, PluginInfo info) throws Throwable {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "copySoLibToHost(context=" + context + ", info=" + info + ")");
        }
        if (info == null) {
            return 1;
        }
            ...
            try {
                ...
                while (true) {
                    try {
                        ...
                        if (Log.VERBOSE) {
                            Log.v(TAG, "copySoLibToHost scan(entry=" + entry.getName() + ", isDirectory=" + entry.isDirectory() + ")");
                        }
                        if (!entry.isDirectory()) {
                            ...
                            if (Log.VERBOSE) {
                                Log.v(TAG, "copySoLibToHost scan(soName=" + soName + ")");
                            }
                            if (isSoFile(soName)) {
                                containSo = true;
                                soFileCount++;
                                ...
                                if (Log.VERBOSE) {
                                    Log.v(TAG, "copySoLibToHost copy(" + soName + ") " + success);
                                }
                            }
                        }
                    } catch (Throwable th2) {
                ...
                if (Log.VERBOSE) {
                    Log.v(TAG, "copySoLibToHost (containSo=" + containSo + ")");
                }
                info.setContainSoFile(containSo);
                info.setSoFileCount(soFileCount);
                if (Log.VERBOSE) {
                    Log.v(TAG, "unzipLibs() return " + success);
                }
                return success;
            } catch (Throwable th3) {
                exception = th3;
                try {
                    Log.w(TAG, "unzipLibs() fail!", exception);
                    throw exception;
                } catch (Throwable th4) {
                    th = th4;
