...
public final class PluginHelper {
    ...
    private final OnExceptionListener mOnExceptionListener = new OnExceptionListener() {
        public void onException(AbsPluginProvider provider, Throwable e) {
            if (Log.DEBUG) {
                Log.d(PluginHelper.TAG, "onException() provider=" + provider + ", e=" + e);
            }
    ...
    private class TimerHandler extends Handler {
        ...
        public void startTimer(HostPluginInfo hPluginInfo) {
            if (Log.VERBOSE) {
                Log.v(PluginHelper.TAG, "startTimer=" + hPluginInfo + ")");
            }
        ...
        public void handleMessage(Message msg) {
            if (Log.VERBOSE) {
                Log.v(PluginHelper.TAG, "handleMessage<<(msg=" + msg + ")");
            }
            ...
            if (hPluginInfo != null) {
                ...
                Log.e(PluginHelper.TAG, "handleMessage error (msg=" + msg + ")");
            }
            if (Log.VERBOSE) {
        ...
        private void checkUpgrade(HostPluginInfo hPluginInfo) {
            if (Log.VERBOSE) {
                Log.v(PluginHelper.TAG, "checkUpgrade<<(hPluginInfo=" + hPluginInfo + ")");
            }
            ...
            if (Log.VERBOSE) {
                Log.v(PluginHelper.TAG, "checkUpgrade>>() return " + info);
            }
        }
    }
    ...
    private void addProviders(AbsPluginProvider provider) {
        if (Log.VERBOSE) {
            Log.v(TAG, "addProviders<<(provider=" + provider + ")");
        }
        if (provider != null) {
            synchronized (this.mProviders) {
    ...
    public List<AbsPluginProvider> getProviders() {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "getProviders>>() return " + providers);
        }
        return providers;
    }
    ...
    private Map<String, String> getProvidersVersionInfo() {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "getProvidersVersionInfo>>() return " + versionInfos);
        }
        return versionInfos;
    }
    ...
    public Result<AbsPluginProvider> loadProvider(HostPluginInfo hPlugininfo) {
        int code = 1;
        if (Log.VERBOSE) {
            Log.v(TAG, "loadProvider<<(hPlugininfo=" + hPlugininfo + ")");
        }
    ...
    private AbsPluginProvider loadLocalProvider(HostPluginInfo hPluginInfo) throws Throwable {
        if (Log.VERBOSE) {
            Log.v(TAG, "loadLocalProvider<<(hPluginInfo=" + hPluginInfo + ")");
        }
                    ...
                    if (ListUtils.getCount(justOneList) <= 0) {
                        ...
                        if (ListUtils.getCount(justOneList) <= 0) {
                            if (provider == null) {
                                ...
                                if (Log.VERBOSE) {
                                    Log.v(TAG, "localHistory why error?, clean file");
                                }
                                this.mStorageManager.removePluginFiles(hPluginInfo, (PluginInfo) justOneList.get(0));
                            }
                        } else if (Log.VERBOSE) {
                            Log.v(TAG, "load local histroy error");
                        }
                        if (Log.VERBOSE) {
                            Log.v(TAG, "loadLocalProvider>>() return " + provider);
                        }
                        return provider;
                    }
                    if (Log.VERBOSE) {
    ...
    private AbsPluginProvider loadAssetsProvider(HostPluginInfo hPluginInfo, boolean useSd) throws Throwable {
        if (Log.VERBOSE) {
            Log.v(TAG, "loadAssetsProvider<<(hPluginInfo=" + hPluginInfo + ")");
        }
        ...
        if (info != null) {
            ...
            if (Log.VERBOSE) {
                Log.v(TAG, "loadAssetsProvider>>() return " + provider);
            }
            return provider;
        }
    ...
    private AbsPluginProvider loadDownloadProvider(HostPluginInfo hPluginInfo) throws Throwable {
        if (Log.VERBOSE) {
            Log.v(TAG, "loadDownloadProvider<<(hPluginInfo=" + hPluginInfo + ")");
        }
        AbsPluginProvider provider = null;
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "loadDownloadProvider>>() return " + provider);
        }
        return provider;
    }
    ...
    private AbsPluginProvider loadProvider(HostPluginInfo hPluginInfo, PluginInfo info) throws Throwable {
        if (Log.VERBOSE) {
            Log.v(TAG, "loadProvider<<(info=" + info + ")");
        }
        AbsPluginProvider provider = null;
        if (info != null) {
        ...
        if (Log.VERBOSE) {
            Log.v(TAG, "loadProvider>>() return " + provider);
        }
        return provider;
    }
    ...
    private PluginInfo downloadPlugin(HostPluginInfo hPluginInfo, UpgradeInfo upgradeInfo) throws Throwable {
        String oldVersion;
        if (Log.VERBOSE) {
            Log.v(TAG, "downloadPlugin>>() hPluginInfo" + hPluginInfo + "upgradeInfo" + upgradeInfo);
        }
        ...
        if (find == null) {
            ...
            if (Log.VERBOSE) {
                Log.v(TAG, "downloadPlugin>>() FileName" + FileName);
            }
    ...
    private void deleteOldFile(HostPluginInfo hPluginInfo) {
        ...
        String td;
        if (Log.VERBOSE) {
            Log.v(TAG, "deleteOldFile<<()" + hPluginInfo);
        }
        ...
        if (!ListUtils.isEmpty((List) deleteName)) {
            ...
            if (this.mDeleteListener != null) {
                ...
                if (Log.VERBOSE) {
                    Log.v(TAG, "mDeleteListener send");
                }
            }
        }
        if ("mounted".equals(Environment.getExternalStorageState())) {
