    ...
    public Object invoke(Object... args) {
        if (!this.mTried) {
            ...
            try {
                ...
                Log.w(TAG, "invoke(" + args + ") error! " + this.mClazz, e);
            }
        }
        if (!(this.mCached && this.mCacheValue)) {
            ...
            if (this.mMethod != null) {
                try {
                    ...
                    Log.w(TAG, "invoke(" + args + ") error!", e2);
                } catch (IllegalAccessException e3) {
                    Log.w(TAG, "invoke(" + args + ") error!", e3);
                } catch (InvocationTargetException e4) {
                    Log.w(TAG, "invoke(" + args + ") error!", e4);
                } catch (NullPointerException e5) {
                    Log.w(TAG, "invoke(" + args + ") error!", e5);
                }
            }
        }
        if (sLoggable) {
