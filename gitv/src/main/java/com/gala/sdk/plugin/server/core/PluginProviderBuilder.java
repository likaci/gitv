    ...
    private AbsPluginProvider createPluginProviderInstance(PluginInfo info, Map<String, String> params) throws Throwable {
        ...
        try {
            ...
            if (loadClassFailedDebug) {
                ...
                if (Log.DEBUG) {
                    Log.d(TAG, "createPluginProviderInstance() return " + pluginProvider);
                }
                return pluginProvider;
            }
        } catch (Exception e) {
            Log.w(TAG, "createPluginProviderInstance() fail!", e);
            throw e;
        } catch (Error error) {
            Log.w(TAG, "createPluginProviderInstance() error!", error);
            throw error;
        }
    }
    ...
    private Context createPluginContext(PluginInfo info) throws Exception {
        ...
        if (FileUtils.exists(path)) {
            ...
            if (pluginContext != null && Log.DEBUG) {
                Log.d(TAG, "createPluginContext() return " + pluginContext.getPackageCodePath());
            }
