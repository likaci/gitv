...
public abstract class AbsPluginProvider {
    ...
    public final void initialize(Context hostContext, Context context, Map<String, String> params, AppInfo info) {
        String str;
        if (Log.DEBUG) {
            Log.d(TAG, "initialize<<(hostContext=" + hostContext + ", context=" + context + ", params=" + params + ", info=" + info + ")");
        }
    ...
    public final IFeature getFeature(int featureType) {
        if (Log.DEBUG) {
            Log.d(TAG, "getFeature<<(featureType=" + featureType + ")");
        }
        IFeature find = null;
        ...
        if (Log.DEBUG) {
            Log.d(TAG, "getFeature>>() return " + find);
        }
        return find;
    }
    ...
    public final void notifyException(Throwable e) {
        if (Log.DEBUG) {
            Log.d(TAG, "notifyException<<(e=" + e + ", listener=" + this.mListener + ")");
        }
