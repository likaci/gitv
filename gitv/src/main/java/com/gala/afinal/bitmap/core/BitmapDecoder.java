...
public class BitmapDecoder {
    ...
    public static Bitmap decodeSampledBitmapFromByteArray(byte[] data, int offset, int length, int reqWidth, int reqHeight, ImageRequest imageRequest) {
        ...
        if (options.inSampleSize != 1) {
            G.a(LOG_TAG, ">>>>>【afinal】, inSampleSize is " + options.inSampleSize + ", [width,height]-" + reqWidth + "," + reqHeight + ", [oriWidth, oriHeight]-" + options.outWidth + ", " + options.outHeight + ", [ScaleType]-" + imageRequest.getScaleType() + ", [url]-" + imageRequest.getUrl());
        }
