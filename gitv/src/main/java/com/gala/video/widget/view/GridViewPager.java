...
public class GridViewPager<T> extends ViewPager {
    ...
    private OnFocusChangeListener mChildFocusChangeListener = new OnFocusChangeListener() {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "dispatchKeyEvent: " + event);
        }
        ...
        LogUtils.d(this.TAG, "dispatchKeyEvent: current page=" + curPage);
            ...
            switch (event.getKeyCode()) {
                case 21:
                    if (curPage == 0) {
                        ...
                        LogUtils.e(this.TAG, "selected item pos=" + selectedItemPos);
                        if (selectedItemPos % this.mNumColumn == 0 || selectedItemPos == 0) {
                            LogUtils.d(this.TAG, "dispatch: LEFT edge reached");
                            if (this.mEdgeListener != null) {
                                this.mEdgeListener.onEdgeReached(Edge.LEFT);
                            }
                        }
                        if (selectedItemPos == 0) {
                            LogUtils.e(this.TAG, "the 1st child has focus!!!");
                            return true;
                        }
                    }
                    break;
                case 22:
                    if (curPage == getPageCount() - 1) {
                        ...
                        if (selectedItemPos % this.mNumColumn == this.mNumColumn - 1 || selectedItemPos == gridView.getChildCount() - 1) {
                            LogUtils.d(this.TAG, "dispatch: RIGHT edge reached");
                            if (this.mEdgeListener != null) {
                                this.mEdgeListener.onEdgeReached(Edge.RIGHT);
                            }
                        }
                        if (selectedItemPos == gridView.getChildCount() - 1) {
                            LogUtils.e(this.TAG, "the last child has focus!!!");
                            return true;
                        }
                    }
                    break;
    ...
    private void addLayoutChildrenListener(final DrawingOrderGridView curGridView) {
        LogUtils.d(this.TAG, "addLayoutChildrenListener: cur grid view=" + curGridView);
        if (curGridView != null) {
            curGridView.setLayoutChildrenListener(new LayoutChildrenListener() {
                public void onLayoutBegin() {
                    LogUtils.d(GridViewPager.this.TAG, "addLayoutChildrenListener: onLayoutBegin");
                ...
                public void onLayoutEnd() {
                    LogUtils.d(GridViewPager.this.TAG, "initAlbumPager: onLayoutEnd");
                    GridViewPager.this.post(new Runnable() {
                        public void run() {
    ...
    public void setDataSource(ArrayList<T> datas) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setDataSource: " + datas);
        }
    ...
    private Rect getBgDrawablePaddings(int resId) {
        ...
        LogUtils.d(this.TAG, "getBgDrawablePaddings: " + bgDrawablePaddings);
    ...
    private float calculateZoomRatio(int itemContentWidth, int itemContentSpacing) {
        ...
        LogUtils.d(this.TAG, "calculateZoomRatio: " + ratio);
        return ratio;
    }
    ...
    public void setItemDimens(int[] basicDimens, int itemBgResId, float arbitraryZoomRatio, boolean innerBackground) {
        ...
        LogUtils.d(this.TAG, "setItemDimens: itemContent w/h=" + itemContentWidth + "/" + itemContentHeight + ", contentSpacing=" + itemContentSpacing + ", itemBgResId=" + itemBgResId);
        Rect itemBgPadding = getBgDrawablePaddings(itemBgResId);
        LogUtils.d(this.TAG, "setItemDimens: itemBgPadding=" + itemBgPadding);
        ...
        LogUtils.d(this.TAG, "setItemDimens: actualVPadding=" + actualVPadding);
        int itemPaddedWidth = itemContentWidth + itemHPadding;
        int itemPaddedHeight = itemContentHeight + actualVPadding;
        LogUtils.d(this.TAG, "setItemDimens: item padded w/h=" + itemPaddedWidth + "/" + itemPaddedHeight);
        ...
        LogUtils.d(this.TAG, "setItemDimens: zoomRatio=" + zoomRatio + ", delta w/h=" + deltaWidth + "/" + deltaHeight);
        setItemStretchMode(0);
        int columnWidth = (itemContentWidth + itemHPadding) + deltaWidth;
        LogUtils.d(this.TAG, "setItemDimens: columnWidth=" + columnWidth);
        setColumnWidth(columnWidth);
        int horizontalSpacing = (itemContentSpacing - itemHPadding) - deltaWidth;
        setHorizontalSpacing(horizontalSpacing);
        LogUtils.d(this.TAG, "setItemDimens: column width=" + columnWidth + ", H spacing=" + horizontalSpacing);
        ...
        LogUtils.d(this.TAG, "setItemDimens: pager total w/h (w/o extra anim space)=" + totalItemWidth + "/" + totalItemHeight);
        ...
        LogUtils.d(this.TAG, "setItemDimens: content padding=" + this.mContentPadding);
        ...
        LogUtils.d(this.TAG, "setItemDimens: pager total w/h (w/ extra anim space)=" + params.width + "/" + params.height + ", deltaMargin=" + this.mDeltaMarginLeft);
        Iterator it = this.mViews.iterator();
        while (it.hasNext()) {
    ...
    private void resetItemDimensIfNeeded() {
        ...
        if (!(this.mDeltaMarginLeft == 0 || params == null)) {
            params.leftMargin -= this.mDeltaMarginLeft;
            LogUtils.d(this.TAG, "resetItemDimensIfNeeded: left margin reseted to " + params.leftMargin);
        }
        if (this.mDeltaMarginTop != 0 && params != null) {
            params.topMargin -= this.mDeltaMarginTop;
            LogUtils.d(this.TAG, "resetItemDimensIfNeeded: top margin reseted to " + params.topMargin);
        }
    }
