...
public class CachedConfigReader implements IConfigReader {
    ...
    public void readConfigAsync(OnConfigReadListener listener) {
        MyLogUtils.d(this.TAG, "readConfigAsync(OnConfigReadListener:" + listener + ")");
    ...
    private String parseCachedFileSync(String filePath) throws Throwable {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "parseCachedFileSync(" + filePath + ")");
        }
        String content = "";
        try {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "read file:" + filePath + ", length = " + length);
            }
            ...
            LogUtils.d(this.TAG, "read file:" + filePath + ", exception occurs!", e);
            return content;
        }
    }
    ...
    private boolean cachedConfigAvailable(String path, String subChannelJsPath) {
        MyLogUtils.d(this.TAG, "cachedConfigAvailable(path: " + path + "), return " + equalsWithRemotePath(path, subChannelJsPath));
        return false;
    }
    ...
    private boolean equalsWithRemotePath(String remotePath, String subChannelJsPath) {
        ...
        MyLogUtils.d(this.TAG, "equalsWithRemotePath(remotePath: " + remotePath + " ,subChannelJsPath:" + subChannelJsPath + "), cachedJsPath: " + cachedJsPath + ", return " + equals);
        return equals;
    }
}
