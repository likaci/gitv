...
public class DailyNewsItemView extends UIKitCloudItemView implements IViewLifecycle<Presenter> {
    ...
    private IGifLoadCallback mGifLoadCallback = new IGifLoadCallback() {
    ...
    private IImageLoadCallback mImageCallback = new IImageLoadCallback() {
        ...
        public void onFailed(String url) {
            LogUtils.d(DailyNewsItemView.this.TAG, "mImageCallback > onFailed,url=" + url);
            if (DailyNewsItemView.this.mGifDrawable != null) {
                DailyNewsItemView.this.mGifDrawable.recycle();
                DailyNewsItemView.this.mGifDrawable = null;
            }
    ...
    private IImageCallback mLoadFocusIconCallback = new IImageCallback() {
    ...
    private IImageCallback mLoadNormalIconCallback = new IImageCallback() {
        public void onSuccess(ImageRequest imageRequest, Bitmap bitmap) {
    ...
    IHomeDataObserver observer = new IHomeDataObserver() {
    ...
    private void fetchIcons() {
        ...
        if (ulrs != null && ulrs.length >= 2) {
            ...
            LogUtils.i(this.TAG, "normalIconUrl = ", normalIconUrl, " focusIconUrl = ", ulrs[1]);
    ...
    private void loadStaticImage() {
        ...
        LogUtils.d(this.TAG, "loadImage url = ", url, " shouldSpliceUrl = ", Boolean.valueOf(this.mShouldSpliceUrl));
        if (this.mShouldSpliceUrl) {
            url = PicSizeUtils.getUrlWithSize(PhotoSize._480_270, url);
            LogUtils.d(this.TAG, "loadImage---PhotoSize._480_270", "newUrl = ", url);
        }
    ...
    private void onLoadImageSuccess(Drawable drawable) {
        CuteImage coreImageView = getCoreImageView();
        LogUtils.e(this.TAG, "onLoadImageSuccess --- getCoreImageView() = ", coreImageView, "drawable = ", drawable);
        if (coreImageView != null) {
    ...
    private void onLoadImageFailed(String url) {
        LogUtils.e(this.TAG, "onLoadImageFailed --- getCoreImageView() = ", getCoreImageView(), "url=", url);
        setDefaultImage();
    }
    ...
    private synchronized Album getDailyNewAlbum(int position) {
        ...
        if (ListUtils.isLegal(this.mDailyNewsData, position)) {
            ...
            LogUtils.e(this.TAG, "mDailyNewsData.size() = ", Integer.valueOf(this.mDailyNewsData.size()), "position = ", Integer.valueOf(position));
        }
        return album;
    }
    ...
    private void update() {
        ...
        if (currentAlbum != null) {
            ...
            LogUtils.d(this.TAG, "dayliy news title is ", name, " gif url = ", gifUrl, " image url = ", this.mCurrentImageUrl);
            CuteText titleView = getTitleView();
            if (titleView != null) {
