...
class ScreenSaverOperator extends Wrapper implements IScreenSaverOperate {
    ...
    ScreenSaverOperator() {
                ...
                switch (msg.what) {
                    ...
                    case 102:
                        ...
                        if (screenSaverImageSize <= 0 || !isScreenSaverAdPrepared) {
                            LogUtils.w(ScreenSaverOperator.TAG, "handleMessage, message = SHOW_SCREEN_SAVER, is screensaver ad prepared :" + isScreenSaverAdPrepared + " screensaver image size " + screenSaverImageSize);
        ...
        this.mAdEndCallback = new AdCallback() {
            public void onAdEnd() {
            ...
            public void onAdPlay(int currentIndex, ScreenSaverAdModel data, boolean isShowQr) {
                LogUtils.d(ScreenSaverOperator.TAG, "mAdEndCallback, onAdPlay, started ad id = " + data.getAdId());
                ScreenSaverOperator.this.mScreenSaverAdModel = data;
                AdsClientUtils.getInstance().onAdStarted(data.getAdId());
            }
        };
        this.mScreenSaverCallback = new ScreenSaverPreEndCallback() {
        ...
        this.mScreenSaverAdOnKeyListener = new OnKeyListener() {
            public boolean onKeyEvent(KeyEvent event) {
                boolean result = false;
                if (event.getAction() == 1) {
                    LogUtils.d(ScreenSaverOperator.TAG, "screensaver ad click, onKeyEvent(): event.getAction - " + event.getAction());
        ...
        this.mScreenSaverOnKeyListener = new OnKeyListener() {
            public boolean onKeyEvent(KeyEvent event) {
                LogUtils.i(ScreenSaverOperator.TAG, "onKeyEvent(), screen saver image, event.getAction - " + event.getAction());
                boolean result = false;
                if (event.getAction() == 1) {
    ...
    private void showScreenSaverAd(List<ScreenSaverAdModel> adModelList) {
        LogUtils.d(TAG, "showScreenSaverAd, Ad info size : " + ListUtils.getCount((List) adModelList));
        if (this.mActivity == null || this.mActivity.isFinishing()) {
            ...
            LogUtils.w(str, append.append(str2).toString());
            return;
        }
    ...
    public void setScreenSaverEnable(boolean enable) {
        ...
        if (!enable) {
            LogUtils.d(TAG, "setScreenSaverEnable, remove messages in message queue, is screensaver enable : " + enable);
            this.mHandler.removeMessages(100);
            this.mHandler.removeMessages(102);
        }
    }
