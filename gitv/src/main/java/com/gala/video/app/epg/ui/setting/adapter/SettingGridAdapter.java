...
public class SettingGridAdapter extends Adapter<SettingHolder> {
    ...
    private void setViewParams(SettingHolder holder, int position) {
        ...
        if (!item.isItemFocusable()) {
            ...
            holder.itemView.setBackgroundDrawable(this.mContext.getResources().getDrawable(R.drawable.epg_setting_item_bg));
            if (StringUtils.isEmpty(item.getItemAction())) {
                ...
                if (item.isSelected()) {
                    ...
                    holder.itemNameText.setTextColor(ResourceUtil.getColor(R.color.item_option));
                    return;
                }
                holder.itemNameText.setTextColor(ResourceUtil.getColor(R.color.item_name));
                return;
            }
            holder.itemTitleView.setVisibility(8);
            if (ListUtils.isEmpty(item.getItemOptions())) {
    ...
    public SettingHolder onCreateViewHolder(ViewGroup parent, int type) {
        View view = LayoutInflater.from(this.mContext).inflate(R.layout.epg_setting_item_view, parent, false);
    ...
    public String switchOptions(View convertView, int pos, KeyEvent event) {
        ...
        if (!ListUtils.isEmpty(options)) {
            View itemOptionView = convertView.findViewById(R.id.epg_setting_item_options);
            TextView itemOptionText = (TextView) convertView.findViewById(R.id.epg_option_text);
            TextView itemOptionMaxText = (TextView) convertView.findViewById(R.id.epg_option_text_max);
            String curOption = "";
    ...
    public void changeTextColor(View convertView, boolean hasFocus, int pos) {
        int nameColor;
        TextView itemNameText = (TextView) convertView.findViewById(R.id.epg_setting_item_name);
        TextView itemDescText = (TextView) convertView.findViewById(R.id.epg_setting_item_desc);
        TextView itemOptionText = (TextView) convertView.findViewById(R.id.epg_option_text);
        TextView itemOptionMaxText = (TextView) convertView.findViewById(R.id.epg_option_text_max);
        TextView itemLastText = (TextView) convertView.findViewById(R.id.epg_laststate_text);
        int nameNormal = R.color.item_name;
        if (this.mSelectedPos != -1 && this.mSelectedPos == pos) {
            nameNormal = R.color.item_option;
        }
        if (hasFocus) {
            nameColor = R.color.item_name_focus;
        } else {
            nameColor = nameNormal;
        }
        int descColor = hasFocus ? R.color.item_desc_focus : R.color.item_desc;
        int optionColor = hasFocus ? R.color.item_name_focus : R.color.item_option;
        int lastStateColor = hasFocus ? R.color.item_name_focus : R.color.albumview_yellow_color;
    ...
    public void refreshOptionsText(View convertView, String value) {
        TextView itemOptionMaxText = (TextView) convertView.findViewById(R.id.epg_option_text_max);
        ((TextView) convertView.findViewById(R.id.epg_option_text)).setText(value);
        itemOptionMaxText.setText(value);
    }

    public void refreshLastState(View convertView, String lastState) {
        LogUtils.d(LOG_TAG, ">>>>> lastState: ", lastState);
        TextView itemLastText = (TextView) convertView.findViewById(R.id.epg_laststate_text);
        itemLastText.setTextColor(ResourceUtil.getColor(R.color.item_name_focus));
