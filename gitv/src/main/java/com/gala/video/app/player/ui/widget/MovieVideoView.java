...
public class MovieVideoView extends VideoSurfaceView {
    ...
    private void init() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "init() format=" + format);
        }
    }
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        MyLogUtils.d(TAG, "onVisibilityChanged(" + changedView + ", " + visibility + ")");
        super.onVisibilityChanged(changedView, visibility);
    }

    public void setVisibility(int visibility) {
        MyLogUtils.d(TAG, "setVisibility(" + visibility + ")");
        super.setVisibility(visibility);
    }

    protected void onWindowVisibilityChanged(int visibility) {
        MyLogUtils.d(TAG, "onWindowVisibilityChanged(" + visibility + ") " + this.mIgnoreWindowChange);
    ...
    protected void onAttachedToWindow() {
        MyLogUtils.d(TAG, "onAttachedToWindow() ");
        if (VERSION.SDK_INT >= 19) {
            MyLogUtils.d(TAG, "isAttachedToWindow() " + isAttachedToWindow());
        }
        super.onAttachedToWindow();
    }

    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        MyLogUtils.d(TAG, "onMeasure() ");
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    }

    public void draw(Canvas canvas) {
        MyLogUtils.d(TAG, "draw() ");
        super.draw(canvas);
    }

    public Display getDisplay() {
        MyLogUtils.d(TAG, "getDisplay() ");
        if (VERSION.SDK_INT >= 19) {
            MyLogUtils.d(TAG, "isAttachedToWindow() " + isAttachedToWindow());
        }
    ...
    public void requestLayout() {
        MyLogUtils.d(TAG, "requestLayout() ");
        super.requestLayout();
    }

    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        MyLogUtils.d(TAG, "onLayout() ");
