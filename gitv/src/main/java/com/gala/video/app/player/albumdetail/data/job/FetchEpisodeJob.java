...
public class FetchEpisodeJob extends AlbumJob {
    ...
    public void onRun(final JobController controller) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onRun() albumId=" + ((AlbumInfo) getData()).getAlbumId() + "getPlayOrder()" + ((AlbumInfo) getData()).getPlayOrder());
        }
    ...
    private void modifyVideoData(List<Episode> episodes, int total) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "modifyVideoData(), episodes size=" + episodes.size() + "maxOrder=" + maxOrder);
        }
