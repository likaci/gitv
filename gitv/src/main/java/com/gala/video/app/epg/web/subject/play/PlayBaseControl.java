...
public abstract class PlayBaseControl {
    ...
    private OnPlayerStateChangedListener mVideoStateListener = new OnPlayerStateChangedListener() {
        public void onVideoSwitched(IVideo video, int type) {
            ...
            if (video != null && video.getAlbum() != null) {
                ...
                LogUtils.d(PlayBaseControl.TAG, "onVideoPluginSwitched" + albumJson);
            }
        }
        ...
        public void onScreenModeSwitched(ScreenMode newMode) {
            LogUtils.d(PlayBaseControl.TAG, "onScreenModeSwitched newMode = " + newMode);
        ...
        public boolean onError(IVideo video, ISdkError error) {
            LogUtils.e(PlayBaseControl.TAG, "onError: error=" + error + ", video=" + video + ", mCurScreenMode=" + PlayBaseControl.this.mCurScreenMode);
    ...
    public int findPlayIndex(String tvQid) {
        ...
        LogUtils.d(TAG, "findPlayIndex result=" + result + ",tvQid=" + tvQid);
        return result;
    }
        ...
        if (!this.mActivityPaused) {
            try {
                ...
                if (playParams != null) {
                    ...
                    LogUtils.e(TAG, "initVideoPlayer() ->  w:" + width + ",h:" + height + "x:" + x + ",y" + y);
                    initPlay(playParams);
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void startWindowPlay(String playInfo) {
        if (!this.mIsErrorState || this.mGalaVideoPlayer == null) {
            LogUtils.d(TAG, "startWindowPlay playInfo = " + playInfo);
    ...
    public void switchScreenMode(String mode) {
        LogUtils.d(TAG, "switchScreenMode mode = " + mode);
    ...
    public void checkLiveInfo(String albumInfo) {
        new VipLiveApi().query(this.mAlbum, new IStatusCallback() {
    ...
    public void switchPlay(String playInfo) {
        LogUtils.d(TAG, "switchPlay playInfo = " + playInfo);
    ...
    public void onPause(boolean isFnishing) {
        if (isFnishing) {
            ...
            LogUtils.d(TAG, ">> onPause mGalaVideoPlayer ,sleep()");
        }
    ...
    public List<AbsVoiceAction> getSupportedVoices(List<AbsVoiceAction> actions) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onGetSceneAction List<AbsVoiceAction>:" + actions);
        }
        if (this.mMultiEventHelper != null) {
