...
public class PerformanceMonitor extends AbsPerformanceMonitor {
    ...
    private ITrackerConfig mConfig = new AbsTrackerConfig() {
    ...
    private View makeFloatingView() {
        return LayoutInflater.from(getContext()).inflate(R.layout.player_floating_view, null);
    }
    ...
    private void recordLog(String logMsg) {
        switch (this.mConfig.getLogType()) {
            case LOGCAT:
                if (LogUtils.mIsDebug) {
                    LogUtils.d(this.mPerfLogTag, logMsg);
                    return;
                }
                return;
            case BOTH:
    ...
    private void updatePerformanceTitle(String title) {
        setText(this.mFloatingWindow != null ? this.mFloatingWindow.getContentView() : null, R.id.txt_currentrunning, title);
    }
    ...
    private void updatePerformanceContent(CharSequence content) {
        ...
        switch (this.mConfig.getTrackType()) {
            case FLOATWINDOW_ONLY:
                appendText(contentView, R.id.txt_datarecord, content);
                return;
            case LOG_ONLY:
