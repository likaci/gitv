...
public abstract class AbsErrorStrategy implements IErrorStrategy {
    ...
    private OnClickListener mFeedBackBtnListener = new OnClickListener() {
        public void onClick(View v) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(AbsErrorStrategy.this.TAG, "c1=" + c1);
            }
            PingBackParams params = new PingBackParams();
            params.add(Keys.T, "20").add("rpage", "player").add("block", MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR).add("rseat", "fb").add("c1", c1).add("now_c1", c1).add("now_qpid", now_qpid).add("r", "");
    ...
    private OnFeedBackPrepareListener mFeedbackPrepareListener = new OnFeedBackPrepareListener() {
    ...
    private OnClickListener mReplayBtnListener = new OnClickListener() {
        public void onClick(View view) {
            ...
            PingBackParams params = new PingBackParams();
            params.add(Keys.T, "20").add("rpage", "player").add("block", MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR).add("rseat", "retry").add("c1", c1).add("now_c1", c1).add("now_qpid", now_qpid).add("r", "");
            PingBack.getInstance().postPingBackToLongYuan(params.build());
    ...
    public void handleLiveProgramFinished(IVideo iBasicVideo) {
        ...
        PlayerToastHelper.showToast(this.mContext, R.string.live_program_finished, (int) QToast.LENGTH_LONG);
    }

    public void showErrorWithServerMsg(String content, String errLog, String errCode, String qrMessage) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "82 showErrorWithServerMsg errCode:" + errCode + ",content:" + content);
        }
    ...
    private void startPurchasePage(int pageType, int enterType, Album albumInfo, int requestCode) {
        if (this.mContext instanceof Activity) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "goToBuy:pageType=" + pageType + ", enterType=" + enterType + ", buySource=" + buySource + ", album=" + DataUtils.albumInfoToString(albumInfo) + ", requestCode=" + 2 + ", state=" + state);
            }
    ...
    private void showQrDialog(boolean needLogcat, String content, String errLog, String errCode, String qrMessage) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "showQrDialog content=" + content + ",qrMessage=" + qrMessage + ",errCode=" + errCode + ", mError=" + this.mError);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "c1=" + c1);
        }
        PingBackParams params = new PingBackParams();
        params.add(Keys.T, "21").add("qtcurl", "player").add("block", MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR).add("c1", c1).add("now_c1", c1).add("now_qpid", now_qpid).add("qpid", "");
        PingBack.getInstance().postPingBackToLongYuan(params.build());
        this.mController.showQRDialog(model, this.mOnDismissListener, this.mContext.getString(R.string.popup_dialog_feedback_btn_text), this.mFeedBackBtnListener, this.mContext.getString(R.string.error_dialog_retry), this.mReplayBtnListener);
    }
    ...
    public void handleVideoOfflineError(String insideMsg, String log, String qRMsg) {
        showQrDialog(!StringUtils.isEmpty((CharSequence) insideMsg) ? insideMsg : this.mContext.getString(R.string.video_offline_error), log, null, qRMsg);
    }

    public void handleForeignIpError(String insideMsg, String log, String qRMsg) {
        showQrDialog(!StringUtils.isEmpty((CharSequence) insideMsg) ? insideMsg : this.mContext.getString(R.string.foreign_ip_error), log, null, qRMsg);
    }

    public void handleCopyrightRestrictionError(boolean needlobacat, String insideMsg, String log, String qRMsg) {
        showQrDialog(needlobacat, !StringUtils.isEmpty((CharSequence) insideMsg) ? insideMsg : this.mContext.getString(R.string.copy_restriction_error), log, null, qRMsg);
    }

    public void handleLiveCopyrightRestrictionError(String insideMsg, String log, String qRMsg) {
        showQrDialog(!StringUtils.isEmpty((CharSequence) insideMsg) ? insideMsg : this.mContext.getString(R.string.copy_restriction_live_error), log, null, qRMsg);
    }

    public void handleLiveCommonError(String errCode, String insideMsg, String log, String qRMsg) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "handleLiveCommonError: errCode={" + errCode + "}");
        }
        showQrDialog(!StringUtils.isEmpty((CharSequence) insideMsg) ? insideMsg : this.mContext.getString(R.string.common_live_error), log, null, qRMsg);
    }

    public void handleNativePlayerBlockError(String log, String qRMsg) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "handleNativePlayerBlockError: qrMsg={" + qRMsg + "}");
        }
        handleBlockError(log, qRMsg);
    }

    private void handleBlockError(String log, String qRMsg) {
        String rightText = this.mContext.getString(R.string.restart);
        String content = this.mContext.getString(R.string.native_player_block);
        PlayerFeedbackModel model = new PlayerFeedbackModel();
        model.setRecord(this.mRecord);
        model.setErrorMsg(content);
        ...
        this.mController.showQRDialog(model, new OnDismissListener() {
        ...
        }, null, null, rightText, new OnClickListener() {
            public void onClick(View v) {
                if (LogUtils.mIsDebug) {
    ...
    public void handleNativePlayer4016Error(String log, String qRMsg) {
        showQrDialog(false, this.mContext.getString(R.string.nativeerror_4016), log, null, qRMsg);
    }

    public void handleNativePlayer4011And4012Error(String log, String qRMsg) {
        showQrDialog(false, this.mContext.getString(R.string.nativeerror_4011_4012), log, null, qRMsg);
    }

    public void handleSystemPlayerOfflinePlaybackError(String log, String qRMsg) {
        showQrDialog(this.mContext.getString(R.string.offline_player_error), log, null, qRMsg);
    }

    public void handleSystemPlayerCommonError(String errorCode, String log, String qRMsg) {
        showQrDialog(this.mContext.getString(R.string.system_player_error, new Object[]{errorCode}), log, null, qRMsg);
    }

    public void handleNativePlayerCommonError(String errCode, String log, String qRMsg, int playerType) {
        String content = this.mContext.getString(R.string.native_player_error, new Object[]{errCode});
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "playerType = " + playerType);
        }
    ...
    public void skipError() {
        PlayerToastHelper.showToast(this.mContext, R.string.jump_error_video, (int) QToast.LENGTH_LONG);
    }
    ...
    public void handleMediaPlayerError(int errorCode) {
        ...
        switch (errorCode) {
            case -1010:
                errMsg = this.mContext.getString(R.string.unsupport_format);
                break;
            case -1004:
                errMsg = this.mContext.getString(R.string.connection_error);
                break;
            default:
                errMsg = this.mContext.getString(R.string.unknown_error);
                break;
        }
    ...
    public void handleCarouselNativePlayerBlockError(String log, String qRMsg) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "handelCarouselNativePlayerBlockError: qrMsg={" + qRMsg + "}");
        }
        this.mOverlay.showError(ErrorType.COMMON, "");
        handleBlockError(log, qRMsg);
    }
    ...
    public boolean handleWithServerMsg(IVideo video, ISdkError error, String log) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "handleWithServerMsg: error=" + error + ", video=" + video);
        }
        if (error == null) {
            return false;
        }
        ...
        if (error.getModule() == 101) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "handleWithServerMsg: error type is NATIVE_PLAYER_ERROR, server_check_code is: " + finalCode);
            }
        ...
        if (errorModel == null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "handleWithServerMsg: error model is null, retry check with first code: " + errorCode);
            }
            errorModel = GetInterfaceTools.getErrorCodeProvider().getErrorCodeModel(errorCode);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "handleWithServerMsg: errorModel=" + errorModel + ", error=" + error + ", video=" + video);
        }
        if (errorModel == null) {
            return false;
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "handleWithServerMsg: error=" + error + ", content=" + errorModel.getContent());
        }
    ...
    public void setMedia(IMedia media) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setMedia " + media + " , " + this);
        }
    ...
    public void setErrorInfo(ISdkError error) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setErrorInfo error=" + error + " , " + this);
        }
        this.mError = error;
    }
    ...
    public void setTrackerRecord(TrackerRecord record) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setFeedBackRecord() record=" + record);
        }
        this.mRecord = record;
    }
    ...
    public void handleDRMCommonError(String errorCode, String log, String qRMsg) {
        ...
        String content = this.mContext.getString(R.string.intertrust_drm_error, new Object[]{errorCode});
        if (StringUtils.equals(errorCode, String.valueOf(ISdkError.CODE_INTERTRUST_DRM_DEVICE_NOT_SUPPORT))) {
            content = this.mContext.getString(R.string.common_player_error, new Object[]{errorCode});
        }
        showQrDialog(true, content, log, null, qRMsg);
    }
    ...
    public void setOnUserReplayListener(OnUserReplayListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "OnUserReplayListener " + this + "  " + listener);
        }
    ...
    private void startDiagnose() {
        ...
        if (this.mDiagnoseListener != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "startDiagnose() + provider=" + provider);
            }
            if (provider != null) {
