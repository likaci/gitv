...
public class HomePingbackSender {
    ...
    private void registerScreenSaverStatusChangeListener() {
        GetInterfaceTools.getIScreenSaver().getStatusDispatcher().register(new IStatusListener() {
            public void onStart() {
    ...
    private void registerScreenSaverAdAnimListener() {
        GetInterfaceTools.getIScreenSaver().getAdRegister().register(new IAd() {
            public void beforeFadeIn(ScreenSaverAdModel model) {
                ...
                switch (model.getAdClickType()) {
                    ...
                    case CAROUSEL:
                        ...
                        return;
                    case NONE:
                        HomePingbackFactory.instance().createPingback(CommonPingback.SCREEN_SAVER_PAGE_SHOW_PINGBACK).addItem("ct", "161212_outside").addItem("a", "21").addItem("qtcurl", "screensaver").addItem("block", ScreenSaverPingBack.BLOCK_SCREENSAVER_AD_NONE).addItem("rfr", rfr).addItem(Keys.T, "11").addItem("e", HomePingbackSender.getInstance().getScreenSaverE()).setOthersNull().post();
                        return;
                    default:
                        return;
                }
    ...
    private void registerScreenSaverImageAnimListener() {
        GetInterfaceTools.getIScreenSaver().getImgRegister().register(new IImage() {
