...
public class PlayerFeatureProvider {
    ...
    private final IPluginDeleteListener mDeleteListener = new IPluginDeleteListener() {
        public void deletedOldFile(PluginPingbackParams pluginPingbackParams) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(PlayerFeatureProvider.TAG, "deletedOldFile");
            }
    ...
    private final IPluginDownloadListener mDownloadlistnener = new IPluginDownloadListener() {
        public void downloaded(PluginPingbackParams pluginPingbackParams) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(PlayerFeatureProvider.TAG, TrackingConstants.TRACKING_EVENT_DOWNLOADED);
            }
            Map<String, String> pingbackInfos = new HashMap();
            pingbackInfos.put("11", pluginPingbackParams.get("11"));
    ...
    private class WorkHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(PlayerFeatureProvider.TAG, ">> handleMessage() playerFeature=" + PlayerFeatureProvider.this.mPlayerFeature + ", listener=" + listener);
            }
    ...
    public void loadPlayerPluginAsync(Context activity, OnStateChangedListener listener, boolean showLoading) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "loadPlayerPluginAsync() playerFeature=" + this.mPlayerFeature + ", listener=" + listener);
        }
        if (isPlayerAlready()) {
            listener.onSuccess();
            return;
        ...
        synchronized (this.mWorkHandler) {
            if (!(activity instanceof Activity)) {
                if (LogUtils.mIsDebug) {
                    LogUtils.e(TAG, "what is this" + activity);
                }
            }
    ...
    private void loadPlayerPluginAsync(int what, PluginStateChangedListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "loadPlayerPluginAsync() what=" + what + ", listener=" + listener);
        }
    ...
    private void loadPlayerFeature(PluginStateChangedListener listener) {
        synchronized (this.mLock) {
            if (this.mPlayerFeature == null) {
                ...
                LogUtils.d(TAG, "loadPluginPlayerFeature timeCost=" + (SystemClock.elapsedRealtime() - startTime));
            } else if (LogUtils.mIsDebug) {
    ...
    private void sendLoadPluginPingback(Result<AbsPluginProvider> result, String pluginId, long td, String eventId) {
        ...
        if (!st.equals("0") || this.mIsFirstLoad.get()) {
            ...
            pingbackInfos.put(Keys.T, "11");
            pingbackInfos.put("istodayfirst", isTodayFirst(st) ? "1" : "0");
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendLoadPluginPingback: info=[" + pingbackInfos + AlbumEnterFactory.SIGN_STR);
            }
            Map<String, String> map = new PingBackParams().build();
            map.putAll(pingbackInfos);
            PingBack.getInstance().postPingBackToLongYuan(map);
    ...
    private boolean isTodayFirst(String st) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "isTodayFirst now is ->" + day + " last is->" + lastLoadDay);
        }
    ...
    private void initializePlayerJava(Context context, IPlayerFeature feature) {
        ...
        if (feature != null) {
            ...
            LogUtils.d(TAG, "initializePlayerJava timeCost=" + (SystemClock.elapsedRealtime() - startTime));
        }
    }

    private void initializePlayer(Context context, IPlayerFeature feature) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, ">> initializePlayer: " + feature);
        }
        if (feature != null) {
    ...
    private int getDelayTimeSec() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "getDelayTimeS: return " + testDelaySeconds);
        }
