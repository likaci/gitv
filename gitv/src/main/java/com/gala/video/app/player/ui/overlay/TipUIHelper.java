...
public class TipUIHelper {
    ...
    public TipWrapper decorateTip(TipWrapper origintip) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, ">> decorateTip origintip=" + origintip);
        }
        TipWrapper tip = null;
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "<< decorateTip origintip=" + tip);
        }
        return tip;
    }

    private TipWrapper decorateSkipAd(TipWrapper origintip) {
        return origintip.setContent(PlayerUIHelper.getVIPHighLightString(this.mContext.getResources().getString(R.string.already_jump) + new String[]{"60", "75", "90", "105", "120"}[new Random().nextInt(5)] + this.mContext.getResources().getString(R.string.second_ad)));
    }
    ...
    private TipWrapper decorateAdStart(TipWrapper tip) {
        String tipMsg = this.mContext.getResources().getString(R.string.ad_tip);
    ...
    private TipWrapper decorateBitStreamChange(TipWrapper tip) {
        ...
        SpannableString sp = PlayerUIHelper.getHighLightString(this.mContext.getResources().getString(R.string.has_change_stream) + PlayerDataUtils.getBitStreamString(this.mContext, toDefinition), PlayerDataUtils.getBitStreamString(this.mContext, toDefinition));
    ...
    private TipWrapper decorateBitStreamHDRToSDRChange(TipWrapper tip) {
        BitStream toDefinition = tip.getTipExtra().getBitStream();
        return tip.setContent(PlayerUIHelper.getHighLightString(String.format(this.mContext.getResources().getString(R.string.hdr_to_sdr_stream1), new Object[]{PlayerDataUtils.getBitStreamString(this.mContext, toDefinition)}), PlayerDataUtils.getBitStreamString(this.mContext, toDefinition)));
    }

    private TipWrapper decorateHDRBitStreamFailToSDR(TipWrapper tip) {
        return tip.setContent(this.mContext.getResources().getString(R.string.hdr_change_error));
    }

    private TipWrapper decorate4KFailToSDR(TipWrapper tip) {
        BitStream toDefinition = tip.getTipExtra().getBitStream();
        return tip.setContent(PlayerUIHelper.getHighLightString(String.format(this.mContext.getResources().getString(R.string.definition4k_to_sdr_stream), new Object[]{PlayerDataUtils.getBitStreamString(this.mContext, toDefinition)}), PlayerDataUtils.getBitStreamString(this.mContext, toDefinition)));
    }

    private TipWrapper decorateBitStreamBecomeHDR(TipWrapper tip) {
        BitStream toDefinition = tip.getTipExtra().getBitStream();
        return tip.setContent(PlayerUIHelper.getVIPHighLightString(String.format(this.mContext.getResources().getString(R.string.will_become_hdr_stream), new Object[]{PlayerDataUtils.getBitStreamString(this.mContext, toDefinition)})));
    }

    private TipWrapper decorateVIPBitStreamChange(TipWrapper tip) {
        return tip.setContent(PlayerUIHelper.getVIPHighLightString(this.mContext.getResources().getString(R.string.has_change_vip_stream) + PlayerDataUtils.getBitStreamString(this.mContext, tip.getTipExtra().getBitStream())));
    }

    private TipWrapper decorateCarouselAdReady(TipWrapper tip) {
        String name = tip.getTipExtra().getProgramNmae();
        return tip.setContent(PlayerUIHelper.getHighLightString(this.mContext.getResources().getString(R.string.carousel_playing_next) + name, name));
    }
    ...
    private TipWrapper decorate3D(TipWrapper tip) {
        if (!PlayerAppConfig.isNeedShowFullScreenHint()) {
            return tip.setContent(this.mContext.getString(R.string.toast_3d_tv));
        }
        if (new AppPreference(this.mContext, PlayerExitHelper.SHARED_PREF_NAME).getBoolean("player_3d_hint_shown", false)) {
            return tip.setContent(this.mContext.getString(R.string.toast_3d));
        }
        return null;
    }

    private TipWrapper decorateMiddleAdReady(TipWrapper tip) {
        return tip.setContent(this.mContext.getString(R.string.middle_ad_tip));
    }

    private TipWrapper decorateSelection(TipWrapper tip) {
        return tip.setContent(PlayerUIHelper.getHighLightString(this.mContext.getString(R.string.selection_panel_tip).toString(), this.mContext.getResources().getString(R.string.selection_highlight_tip)));
    }
    ...
    private TipWrapper decorateCheckTailOrder(TipWrapper tip) {
        ...
        return tip.setContent(PlayerUIHelper.getHighLightString(this.mContext.getString(R.string.continue_play_next, new Object[]{episodeString}).toString(), this.mContext.getString(R.string.play_order, new Object[]{Integer.valueOf(video.getPlayOrder())})));
    }
    ...
    private TipWrapper decorateCheckTailName(TipWrapper tip) {
        ...
        return tip.setContent(PlayerUIHelper.getHighLightString(this.mContext.getString(R.string.continue_play_next, new Object[]{name}).toString(), name));
    }

    private TipWrapper decorateReplay(TipWrapper tip) {
        return tip.setContent(this.mContext.getString(R.string.video_replay));
    }

    private TipWrapper decorateJumHead(TipWrapper tip) {
        int resId;
        if (NetworkUtils.isNetworkAvaliable()) {
            resId = R.string.has_jump_header;
        } else {
            resId = R.string.has_jump_header_no_menu;
        }
    ...
    private TipWrapper decorateHistory(TipWrapper tip) {
        ...
        if (!isTvSeries || playOrder <= 0) {
            if (videoPlayTime < 60000) {
                history = this.mContext.getString(R.string.video_history_continue_play_in_1_minute);
            } else {
                history = PlayerUIHelper.getHighLightString(this.mContext.getString(R.string.episode_history_continue_play2, new Object[]{timeContent}).toString(), this.mContext.getString(R.string.play_time, new Object[]{Integer.valueOf(videoPlayTime / 60000), Integer.valueOf(videoPlayTime / 60000), Integer.valueOf(videoPlayTime % 60000)}));
            }
        } else if (videoPlayTime < 60000) {
            history = this.mContext.getString(R.string.video_history_continue_play_in_1_minute);
        } else {
            String orderContent = this.mContext.getString(R.string.play_order, new Object[]{Integer.valueOf(playOrder)});
            history = PlayerUIHelper.getHighLightString(this.mContext.getString(R.string.episode_history_continue_play2, new Object[]{content}).toString(), orderContent + this.mContext.getString(R.string.play_time, new Object[]{Integer.valueOf(videoPlayTime / 60000)}));
        }
    ...
    private TipWrapper decorateVipBitstream(TipWrapper tip) {
        ...
        int height = 0;
        BitStream toDefinition = tip.getTipExtra().getBitStream();
        SpannableString sp = PlayerUIHelper.getHighLightString(String.format(this.mContext.getString(R.string.tryplay_vip_bitstream_tip), new Object[]{PlayerDataUtils.getBitStreamString(this.mContext, toDefinition)}), PlayerDataUtils.getBitStreamString(this.mContext, toDefinition));
        ...
        if (object == null || object[0] == null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendVipBitStreamTip bitmap width/height=" + object[1] + "/" + object[2]);
            }
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendVipBitStreamTip bitmap" + object.length + object[0]);
            }
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendVipBitStreamTip bitmap is null");
            }
        } else {
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendVipBitStreamTip bitmap width/height=" + object[1] + "/" + object[2]);
    ...
    private TipWrapper decoratePreview(TipWrapper itip) {
        ...
        if (this.mVideo.getProvider().getSourceType() == SourceType.PUSH) {
            msg = this.mContext.getString(R.string.tryplay_tip_vip_push);
            ...
            if (this.mVideo.getAlbum().getPayMarkType() == PayMarkType.COUPONS_ON_DEMAND_MARK) {
                msg = this.mContext.getResources().getString(R.string.tryplay_tip_need_coupon);
            } else if (this.mVideo.getAlbum().getPayMarkType() == PayMarkType.PAY_ON_DEMAND_MARK) {
                msg = this.mContext.getResources().getString(R.string.tryplay_tip_need_single_buy);
            } else if (this.mVideo.getAlbum().getPayMarkType() == PayMarkType.VIP_MARK) {
                if (StringUtils.isEmpty(tryTip) || object == null || object[0] == null) {
                    msg = this.mContext.getResources().getString(R.string.tryplay_tip_need_buy);
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendPreviewTip bitmap width/height=" + object[1] + "/" + object[2]);
            }
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendPreviewTip bitmap" + object.length + object[0]);
            }
            if (((Integer) object[1]).intValue() > 0 && ((Integer) object[2]).intValue() > 0) {
                width = ((Integer) object[1]).intValue();
                height = ((Integer) object[2]).intValue();
            ...
            if (StringUtils.isEmpty(tryTip) || object == null || object[0] == null) {
                msg = this.mContext.getString(R.string.live_tryplay_tip_need_buy);
                object = PlayerUIHelper.getLiveDefaultBitmap(this.mContext);
            } else {
                msg = tryTip;
            }
            ...
            msg = this.mContext.getString(R.string.tryplay_tip_only_see);
    ...
    private TipWrapper decorateHDROpen(TipWrapper itip) {
        ...
        if (this.mVideo.getProvider().getSourceType() == SourceType.PUSH) {
            ...
            msg = this.mContext.getString(R.string.hdr_tip_need_buy);
            Object[] object = PlayerUIHelper.getHDRDefaultBitmap(this.mContext, this.mVideo.is3d());
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendHDRTip bitmap width/height=" + object[1] + "/" + object[2]);
            }
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendHDRTip bitmap" + object.length + object[0]);
            }
            if (((Integer) object[1]).intValue() > 0 && ((Integer) object[2]).intValue() > 0) {
                width = ((Integer) object[1]).intValue();
                height = ((Integer) object[2]).intValue();
    ...
    private TipWrapper decorateQuestionnaireStart(TipWrapper itip) {
        return itip.setContent(this.mContext.getResources().getString(R.string.questionnaire_start));
    }
    ...
    private TipWrapper decorateLogin(TipWrapper itip) {
        ...
        int width = 0;
        int height = 0;
        String str = this.mContext.getResources().getString(R.string.player_login_tip);
