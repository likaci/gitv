...
public class MediaControllerContainer extends LinearLayout implements IMediaController, IScreenUISwitcher {
    ...
    public void updateView(boolean is3d, SourceType sourceType) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "updateView(" + is3d + ", " + sourceType + ") " + this.mControllerCount + ", config show 3d = " + Project.getInstance().getConfig().shouldDuplicateUIForStereo3D());
        }
        this.m3dMode = is3d;
    ...
    public void setThreeDimensional(boolean enable) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setThreeDimensional(" + enable + ") m3dMode=" + this.m3dMode);
        }
        this.m3dMode = enable;
        if (this.mVideo != null) {
    ...
    public void showAdPlaying(int countDownTime) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "showAdPlaying(" + countDownTime + ")");
        }
        showSelf();
    ...
    public void setBufferPercent(int percent) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setBufferPercent(" + percent + ") size=" + this.mControllers.size());
        }
        for (AbsMediaController ctrl : this.mControllers) {
    ...
    public void setNetSpeed(long bytePerSecond) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setNetSpeed(" + bytePerSecond + ") size=" + this.mControllers.size());
        }
        for (AbsMediaController ctrl : this.mControllers) {
    ...
    public void showTip(TipWrapper tip) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "showTip(" + tip.getContent() + ") size=" + this.mControllers.size());
        }
        showSelf();
        for (AbsMediaController ctrl : this.mControllers) {
            ctrl.showTip(tip);
    ...
    public void setSubtitle(String subtitle) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setSubtitle(" + subtitle + ") size=" + this.mControllers.size());
        }
        for (AbsMediaController ctrl : this.mControllers) {
    ...
    public void setSecondaryProgress(int secondaryProgress) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setSecondaryProgress(" + secondaryProgress + ")" + this.mControllers.size());
        }
        for (int i = 0; i < this.mControllers.size(); i++) {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        for (AbsMediaController ctrl : this.mControllers) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, ">> dispatchKeyEvent(" + event + ");" + isDispatched + " ctrl = " + ctrl);
            }
    ...
    public void switchScreen(boolean isFullScreen, float zoomRatio) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setWindowMode(" + isFullScreen + ")");
        }
    ...
    public void showPanel(int duration) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "showPanel(" + duration + ")");
        }
        showSelf();
        for (AbsMediaController ctrl : this.mControllers) {
    ...
    public void updateBitStreamDefinition(String bitStream) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "updateBitStreamDefinition(" + bitStream + ") size=" + this.mControllers.size());
        }
        for (AbsMediaController ctrl : this.mControllers) {
    ...
    public void showBottomAndTop(int duration) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "showBottomAndTop: size=" + this.mControllers.size() + ", duration=" + duration);
        }
        for (AbsMediaController ctrl : this.mControllers) {
    ...
    public boolean showSeekBar() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "showSeekBar(" + isShowSeekBar + ")");
        }
