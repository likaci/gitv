...
public class MyTabWidget extends TabWidget {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        LogUtils.d(TAG, "onMeasure: old width=" + width + ", mode=" + widthMode);
        int newWidth = ((int) getContext().getResources().getDimension(R.dimen.dimen_210dp)) * getVisibleTabCount();
        LogUtils.d(TAG, "onMeasure: new width=" + newWidth);
    ...
    public void addView(View child) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "1111 child id=" + child.getId() + ", hasFocus=" + child.hasFocus() + ", isFocusable=" + child.isFocusable() + ", isFocused=" + child.isFocused());
        }
        super.addView(child);
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "2222 child id=" + child.getId() + ", hasFocus=" + child.hasFocus() + ", isFocusable=" + child.isFocusable() + ", isFocused=" + child.isFocused());
        }
        if (getOrientation() == 1) {
    ...
    public void focusCurrentTab(int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, ">> focusCurrentTab(), index=" + index);
        }
