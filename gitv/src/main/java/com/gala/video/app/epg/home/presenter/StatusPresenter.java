...
public class StatusPresenter {
    ...
    public StatusPresenter(View root, Context context) {
        this.mRootView = root;
        this.homeStatueBar = root.findViewById(R.id.epg_home_state_bar);
        this.mTimeSate = new TimeState((TextView) root.findViewById(R.id.epg_time));
        ImageView mNetStateView = (ImageView) root.findViewById(R.id.epg_net_state);
        ImageView mPhoneConnectedView = (ImageView) root.findViewById(R.id.epg_phone_connect);
        StorageStatePresenter storageStatePresenter = new StorageStatePresenter(AppRuntimeEnv.get().getApplicationContext(), (ImageView) root.findViewById(R.id.epg_usb_storage_state));
    ...
    public void onDlnaActionNotifyEvent(RequestKind kind) {
        if (kind != RequestKind.PULLVIDEO) {
            ...
            LogUtils.e(TAG, "unhandled dlna notify event kind(" + kind + ")");
        }
    }
