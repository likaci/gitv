...
public class DetailListView extends ListView {
    ...
    private OnFocusChangeListener mFocusChangeListener = new OnFocusChangeListener() {
        public void onFocusChange(View v, boolean hasFocus) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(DetailListView.this.TAG, "onFocusChange hasFocus = {" + hasFocus + "},mLastFocusPos = {" + DetailListView.this.mLastFocusPos + "} mChangedFocusPos = {" + DetailListView.this.mChangedFocusPos + "}");
            }
            if (!hasFocus) {
    ...
    private OnItemSelectedListener mItemSelectedListener = new OnItemSelectedListener() {
        public void onItemSelected(AdapterView<?> adapterView, View view, int position, long id) {
            LogUtils.d(DetailListView.this.TAG, ">>onItemSelected position ==" + position);
    ...
    private OnScrollListener mScrollListener = new OnScrollListener() {
        public void onScrollStateChanged(AbsListView view, int scrollState) {
            ...
            if (scrollState == 0 && DetailListView.this.mNeedAjust) {
                DetailListView.this.mHandler.postDelayed(new Runnable() {
                    public void run() {
    ...
    private View getChildView(int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "getChildView index=" + index);
        }
        View view = null;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        LogUtils.d(this.TAG, "onKeyDown curItemPos= " + curItemPos + " keyCode " + keyCode);
        switch (keyCode) {
            case 19:
                LogUtils.d(this.TAG, "curItemPos= " + curItemPos);
    ...
    private void adjustPositionOffset(int needSelectIndex) {
        ...
        LogUtils.d(this.TAG, ">>adjustPositionOffset needSelectIndex =" + needSelectIndex + ", position = " + position);
    ...
    public void setPlayingIndex(final int index) {
        LogUtils.d(this.TAG, ">>setPlayingIndex index = " + index);
        this.mPlayIndex = index;
        post(new Runnable() {
            public void run() {
                int currentIndex = DetailListView.this.getSelectedItemPosition();
                LogUtils.d(DetailListView.this.TAG, "currentIndex != index currentIndex=" + currentIndex);
                if (DetailListView.this.hasFocus()) {
                    int time = (int) ((SystemClock.elapsedRealtime() - DetailListView.this.mLastFocusChangeTime) / 1000);
                    LogUtils.e(DetailListView.this.TAG, "focus change time == " + time + ", mPlayIndex = " + DetailListView.this.mPlayIndex);
                    ...
                    LogUtils.d(DetailListView.this.TAG, "adjustPositionOffset index = " + index);
                    if (index >= 0) {
                        DetailListView.this.setSelectionFromTop(index, DetailListView.this.mSelectionTopOffset);
    ...
    public void setDetailListViewAdapter(BaseDetailListAdapter adapter) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setAdapter is null ? =" + (adapter == null));
        }
        if (adapter != null) {
    ...
    public int getFocusIndex() {
        ...
        LogUtils.d(this.TAG, "getFocusIndex return" + selectIndex);
        return selectIndex;
    }
