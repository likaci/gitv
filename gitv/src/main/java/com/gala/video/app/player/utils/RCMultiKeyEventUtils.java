...
public class RCMultiKeyEventUtils {
        ...
        public synchronized int getNum() {
            ...
            for (int i = 0; i < size; i++) {
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, "getNum: keycode[" + i + "]=" + ((KeyEvent) this.mMultiKeys.get(i)).getKeyCode());
                }
    ...
    public void onKeyDown(KeyEvent event, int currentOrder) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onKeyDown() " + event);
        }
        if (event.getAction() == 0 && NUMKEYS.contains(Integer.valueOf(event.getKeyCode()))) {
            ...
            TimerTask combinationKeyTask = new TimerTask() {
                public void run() {
                    ...
                    if (context == null) {
                        ...
                        String nthEpisodeStr = context.getResources().getString(R.string.vc_nth_episode, new Object[]{Integer.valueOf(num)});
                        if (LogUtils.mIsDebug) {
                            LogUtils.d(RCMultiKeyEventUtils.TAG, "run: episode string=" + nthEpisodeStr);
                        }
                        ...
                        if (LogUtils.mIsDebug) {
                            LogUtils.d(RCMultiKeyEventUtils.TAG, "run: runnable=" + r);
                        }
                        String noEpisodeStr = "";
                        boolean showToast = false;
                        if (r == null) {
                            noEpisodeStr = context.getResources().getString(R.string.vc_missing_nth_episode, new Object[]{Integer.valueOf(num)});
                            showToast = true;
                        } else if (num == RCMultiKeyEventUtils.this.mCurrentOrder) {
                            showToast = true;
                            noEpisodeStr = context.getResources().getString(R.string.vc_already_nth_episode, new Object[]{Integer.valueOf(num)});
                        } else {
