...
public class CheckInDialog extends GuideBaseDialog {
    ...
    public void onCreate() {
        ...
        this.mCheckInView.setOnClickListener(new OnCheckClickListener() {
            public void onClick(View v) {
                LogUtils.i(CheckInDialog.TAG, "onClick: v" + v);
    ...
    private void initializeParams() {
        ...
        layoutParams.x = ResourceUtil.getDimen(R.dimen.dimen_279dp);
        layoutParams.y = ResourceUtil.getDimen(R.dimen.dimen_2dp);
    ...
    private void sendShowPingback() {
        ...
        HomePingbackFactory.instance().createPingback(ShowPingback.SCENE_INSIDE_GUIDE_CHECK_IN_IMAGE_SHOW_PINGBACK).addItem(Keys.T, "21").addItem("bstp", "1").addItem("qtcurl", "appstartsign").addItem("block", this.mStartCount > 0 ? ISearchConstant.TVSRCHSOURCE_OTHER : "first").addItem("c1", "").addItem("qpid", "").post();
    }

    private void sendClickPingback(String rseat) {
        LogUtils.i(TAG, "sendClickPingback rseat: " + rseat);
        HomePingbackFactory.instance().createPingback(ClickPingback.SCENE_INSIDE_GUIDE_CHECK_IN_IMAGE_CLICK_PINGBACK).addItem(Keys.T, "20").addItem("rpage", "appstartsign").addItem("block", this.mStartCount > 0 ? ISearchConstant.TVSRCHSOURCE_OTHER : "first").addItem("rseat", rseat).addItem("c1", "").addItem("r", "").addItem("td", String.valueOf(System.currentTimeMillis() - this.mFirstShowTime)).addItem("rt", "i").addItem("tabid", "").post();
    }
}
