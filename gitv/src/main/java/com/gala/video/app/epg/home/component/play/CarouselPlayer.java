...
public class CarouselPlayer {
    ...
    private OnPlayerStateChangedListener mPlayerStateListener = new OnPlayerStateChangedListener() {
        public void onVideoSwitched(IVideo video, int type) {
            LogUtils.d(CarouselPlayer.TAG, "onVideoSwitched video = " + video);
        }

        public void onPlaybackFinished() {
            LogUtils.d(CarouselPlayer.TAG, "on carousel video play finished");
        }

        public void onScreenModeSwitched(ScreenMode newMode) {
            int i = 1;
            LogUtils.d(CarouselPlayer.TAG, "Home/CarouselPlayer---onScreenModeSwitched() newMode=" + newMode);
            if (LogUtils.mIsDebug) {
                LogUtils.d(CarouselPlayer.TAG, "onScreenModeSwitched newMode=" + newMode);
            }
        ...
        public boolean onError(IVideo video, ISdkError error) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(CarouselPlayer.TAG, "onError: error=" + error + ", video = " + video);
            }
            LogUtils.e(CarouselPlayer.TAG, "current screen mode is " + CarouselPlayer.this.mCurScreenMode);
            CarouselPlayer.this.mPlayerStatus = PlayStatus.ERROR;
            return false;
        }

        public void onVideoStarted(IVideo video) {
            LogUtils.d(CarouselPlayer.TAG, "onVideoStarted current screen mode : " + CarouselPlayer.this.mCurScreenMode);
        }

        public void onAdStarted() {
            LogUtils.d(CarouselPlayer.TAG, "onAdStarted");
        }
    ...
    public boolean onKeyEvent(KeyEvent event) {
        LogUtils.d(TAG, "onKeyEvent() keycode=" + event.getKeyCode());
        ...
        LogUtils.d(TAG, "video player handle key event result " + result);
        return result;
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        LogUtils.d(TAG, "onKeyDown() -> keyCode" + keyCode + ", event" + event);
        if (this.mPlayer == null || !this.mPlayer.handleKeyEvent(event)) {
            return false;
        }
        LogUtils.d(TAG, "key event is cosumed by player keycode=" + keyCode);
        return true;
    }
