...
public class MultiSubjectActivity extends QMultiScreenActivity {
    ...
    private class ImageCallback implements IImageLoadCallback {
        ...
        public void onFailed(String url) {
            LogUtils.w(MultiSubjectActivity.TAG, "mImageLoadCallback onFailed. url = " + url);
    ...
    private class NetworkListener implements INetworkStateListener {
        ...
        public void onConnected(boolean isChanged) {
            if (LogUtils.mIsDebug) {
                LogUtils.e(MultiSubjectActivity.TAG, "onConnected() isChanged：" + isChanged);
            }
    ...
    public void onUikitEvent(UikitEvent event) {
        if (this.mEngine != null) {
            LogUtils.d(TAG, this + "event.uikitEngineId = " + event.uikitEngineId + "  ;mEngine.getId() = " + this.mEngine.getId());
            if (event.uikitEngineId == this.mEngine.getId()) {
                LogUtils.d(TAG, "receive loader event: " + event);
                switch (event.eventType) {
                    case 32:
                        LogUtils.d(TAG, "onUikitEvent LOADER_SET_CARDS-" + event.sourceId);
                        ...
                        if (event.cardList == null || event.cardList.isEmpty()) {
                            ...
                            if (event.background == null || event.background.isEmpty()) {
                                LogUtils.d(TAG, "onUikitEvent LOADER_SET_CARDS- event.background = " + event.background);
                        ...
                        return;
                    case 33:
                        LogUtils.d(TAG, "onUikitEvent LOADER_ADD_CARDS-" + event.sourceId + "-pageNo-" + event.pageNo);
    ...
    public void init(Context context) {
        this.mBlocksView = (BlocksView) findViewById(R.id.epg_multi_subject_gridview_id);
        this.mBgView = (MultiSubjectBgView) findViewById(R.id.epg_multi_subject_bg_view_id);
