...
public class LiveMediaControllerOverlay extends AbsMediaController {
    ...
    private onCountDowntimeListener mCountDowntimeListener = new onCountDowntimeListener() {
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(LiveMediaControllerOverlay.TAG, "handleMessage(" + msg + ")");
            }
            switch (msg.what) {
                case 4:
    ...
    private void initView() {
        ...
        this.VIEWID_COUNTDOWN = R.id.live_countdown_time;
        this.VIEWID_LIVETAG = R.id.image_live_tag;
        this.VIEWID_INFO_PANEL = R.id.video_info_panel;
        this.VIEWID_HINT = R.id.live_hint_tag;
        this.VIEWID_NAME = R.id.video_name;
    ...
    protected void sendLiveInteractionPingback(int clickCount) {
        if (this.mVideo != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "sendLiveInteractionPingback id=" + this.mVideo.getLiveChannelId());
            }
            PingbackFactory.instance().createPingback(21).addItem(PingbackStore.R.R_TYPE(resouceId)).addItem(BLOCK.BLOCK_TYPE(BLOCK_LIVE_INTERACTION)).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE(String.valueOf(clickCount))).addItem(RPAGETYPE.PLAYER).addItem(C1.C1_TYPE(c1)).addItem(C2.C2_TYPE(c2)).addItem(NOW_C1.NOW_C1_TYPE(c1)).addItem(NOW_QPID.NOW_QPID_TYPE(resouceId)).addItem(NOW_C2.NOW_C2_TYPE(c2)).addItem(STATE.STATE_TYPE(state)).post();
        }
    }
    ...
    private void changeShowStyle(LiveViewMode mode) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "changeShowStyle mode=" + mode);
        }
        switch (mode) {
    ...
    private void updateVideoInfo() {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "updateVideoInfo()");
        }
        if (this.mVideo != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "setVideoInfo mIsNeedVip=" + this.mIsNeedVip);
            }
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "setVideoInfo mLiveStartTime=" + this.mLiveStartTime);
            }
    ...
    private void setVideoName(String name) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setVideoName name=" + name + ", mLiveStartTime=" + this.mLiveStartTime);
        }
        long currentTime = DeviceUtils.getServerTimeMillis();
        if (this.mLiveStartTime > 0) {
            if (currentTime >= this.mLiveStartTime) {
                this.mTxtHint.setText(this.mContext.getString(R.string.current_play));
            } else {
                this.mTxtHint.setText(this.mContext.getString(R.string.upcoming_live));
            }
        }
    ...
    public void showTip(TipWrapper tip) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "showTip(" + tip.getContent() + ")" + dumpState());
        }
        if (!this.mInitized) {
            return;
        }
    ...
    private void ensureShowStyle() {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "ensureShowStyle mLiveStartTime=" + this.mLiveStartTime + ", mIsFullScreen=" + this.mIsFullScreen + ", mIsVip=" + this.mIsNeedVip);
        }
        long currentTime = DeviceUtils.getServerTimeMillis();
    ...
    public void setNetSpeed(long netSpeed) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setNetSpeed(" + netSpeed + ")");
        }
        if (this.mInitized) {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        switch (key) {
            ...
            case 164:
                ...
                LogUtils.e(TAG, "volume is invalid");
                return false;
            default:
                return false;
        }
    ...
    public void switchScreen(boolean isFullScreen, float zoomRatio) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "switchScreen(isFullScreen=" + isFullScreen + ", zoomratio=" + zoomRatio);
        }
    ...
    private void updateLiveTagSize(boolean isFullScreen, float zoomRatio) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "updateLiveTagSize(isFullScreen=" + isFullScreen + ", ratio=" + ratio + ")");
        }
        int height = (int) (((float) getResources().getDimensionPixelSize(R.dimen.live_tag_height)) * ratio);
        int marginR = (int) (((float) getResources().getDimensionPixelSize(R.dimen.live_tag_margin_right)) * ratio);
        int marginT = (int) (((float) getResources().getDimensionPixelSize(R.dimen.live_tag_margin_top)) * ratio);
        float textSize = ((float) getResources().getDimensionPixelSize(R.dimen.live_tag_text_size)) * ratio;
        LayoutParams layoutParams = (LayoutParams) this.mLiveTag.getLayoutParams();
        layoutParams.width = (int) (((float) getResources().getDimensionPixelSize(R.dimen.live_tag_width)) * ratio);
    ...
    private void updateLiveCountDownView(boolean isFullScreen, float zoomRatio) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "updateLiveCountDownView(isFullScreen=" + isFullScreen + ", ratio=" + ratio + ")");
        }
        int marginTop = (int) (((float) getResources().getDimensionPixelSize(R.dimen.live_tag_margin_top)) * ratio);
        int marginRight = (int) (((float) getResources().getDimensionPixelSize(R.dimen.live_tag_margin_right)) * ratio);
        LayoutParams layoutParams = (LayoutParams) this.mCountDownView.getLayoutParams();
        layoutParams.width = (int) (((float) getResources().getDimensionPixelSize(R.dimen.live_countdown_view_width)) * ratio);
    ...
    private void showView(int... viewIds) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "showView()" + dumpViewId(viewIds));
        }
        if (this.mRoot != null) {
