...
public class ExitOperateImageRequestTask extends BaseRequestTask {
    ...
    private void fetchExitOperateImageData() {
        ...
        if (dynamicResult != null) {
            LogUtils.w(TAG, "fetchExitOperateImageData, exit operation image resource id : " + dynamicResult.getSpecifiedOperateImageResId(OperationImageType.EXIT));
            VrsHelper.channelLabelsSize.call(new IVrsCallback<ApiResultChannelLabels>() {
                public void onSuccess(ApiResultChannelLabels result) {
                    if (result == null) {
                        LogUtils.w(ExitOperateImageRequestTask.TAG, "fetchExitOperateImageData, onSuccess, ApiResultChannelLabels is null");
                    ...
                    if (labels == null) {
                        LogUtils.w(ExitOperateImageRequestTask.TAG, "fetchExitOperateImageData, onSuccess, ChannelLabels is null");
                        ExitOperateImageRequestTask.this.clearLimitBehavior();
                        ExitOperateImageProvider.getInstance().onEmptyExitOperateData();
                        return;
                    }
                    ...
                    if (ListUtils.isEmpty((List) labelList)) {
                        LogUtils.w(ExitOperateImageRequestTask.TAG, "fetchExitOperateImageData, onSuccess, ChannelLabel List is empty");
                        ExitOperateImageRequestTask.this.clearLimitBehavior();
                        ExitOperateImageProvider.getInstance().onEmptyExitOperateData();
                        return;
                    }
                    LogUtils.w(ExitOperateImageRequestTask.TAG, "fetchExitOperateImageData, onSuccess, ChannelLabel size :" + ListUtils.getCount((List) labelList));
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (ResourceType.LIVE == label.getType() && !label.checkLive()) {
                            iterator.remove();
                            LogUtils.d(ExitOperateImageRequestTask.TAG, "fetchExitOperateImageData, remove out of date live data, id : " + label.itemId);
                        }
                    }
                ...
                public void onException(ApiException e) {
                    LogUtils.w(ExitOperateImageRequestTask.TAG, "fetchExitOperateImageData, onException, ApiException e ", e);
                    ExitOperateImageRequestTask.this.clearLimitBehavior();
                }
            }, id, "0", "5.0", "60");
            return;
        }
        LogUtils.w(TAG, "fetchExitOperateImageData, get dynamic data, dynamicResult is null");
    }
}
