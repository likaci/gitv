...
public class EpisodeListContent implements IContent<List<IVideo>, IVideo> {
    ...
    private void handleSelectionRefreshed(IVideo video) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> handleSelectionRefreshed, new video=" + video);
        }
        if (video != null) {
    ...
    private LayoutParams getContentViewFLP() {
        ...
        if (this.mUiStyle.isDetail()) {
            return new LayoutParams(-1, ResourceUtil.getDimen(R.dimen.dimen_165dp));
        }
        if (this.mIsHDR) {
            lp = new LayoutParams(-1, ResourceUtil.getDimen(R.dimen.dimen_215dp));
        } else {
            lp = new LayoutParams(-1, ResourceUtil.getDimen(R.dimen.dimen_195dp));
        }
        lp.gravity = 16;
        return lp;
    }
    ...
    private void setArrow() {
        ...
        if (this.mUiStyle.isDetail()) {
            ...
            lefttop = ResourceUtil.getDimen(R.dimen.dimen_103dp);
            righttop = ResourceUtil.getDimen(R.dimen.dimen_103dp);
        } else {
            lefttop = this.mUiStyle.getArrowMarginLeftTop();
            righttop = this.mUiStyle.getArrowMarginRightTop();
        }
    ...
    private void adjustEpisodelistView() {
        ...
        if (this.mCurVideo == null) {
            ...
            if (playOrder > 0) {
                ...
                LogUtils.d(this.TAG, "adjustEpisodelistView, invalid play order, video=" + video);
            }
        } else if (LogUtils.mIsDebug) {
    ...
    public void setSelection(IVideo item) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> setSelection, newVideo=" + item);
        }
        if (Looper.getMainLooper() == Looper.myLooper()) {
        ...
        if (data != null) {
            ...
            if (this.mContentView != null && this.mAdView == null) {
                ...
                if (this.mAdView != null) {
                    ...
                    params.rightMargin = this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_34dp);
                    params.gravity = 5;
    ...
    public void notifyEpisodelistContentHDR(boolean isHDR) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "notifyEpisodelistContentHDR:" + isHDR);
        }
        this.mIsHDR = isHDR;
        if (this.mContentView != null) {
            LayoutParams lp = (LayoutParams) this.mContentView.getLayoutParams();
            if (isHDR) {
                lp.height = ResourceUtil.getDimen(R.dimen.dimen_215dp);
            } else {
                lp.height = ResourceUtil.getDimen(R.dimen.dimen_195dp);
            }
