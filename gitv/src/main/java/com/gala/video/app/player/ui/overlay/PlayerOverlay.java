...
public class PlayerOverlay extends AbsPlayerOverlay {
    ...
    public PlayerOverlay(GalaPlayerView playerView, boolean initByWindowMode, float zoomRatio) {
        LogUtils.d(this.TAG, " PlayerOverlay.<init>: initByWindowMode=" + initByWindowMode);
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        LogUtils.d(this.TAG, ">> dispatchKeyEvent: " + keyEventToString(event) + "mSourceType=" + this.mSourceType);
    ...
    public void setAlbumId(String albumId) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setAlbumId(" + albumId + ")");
        }
    ...
    public void showLoading(String message) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "showLoading(" + message + ")");
        }
    ...
    public void showLoading(String message, long delayMs) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "showLoading(" + message + ", " + delayMs + ")");
        }
        if (delayMs > 0) {
    ...
    public void showAdPlaying(int countDownTime) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "showAdPlaying(" + countDownTime + ")");
        }
        this.mState = 1004;
        this.mHandler.removeCallbacks(this.mPostLoadingRunnable);
        this.mMenuPanel.hide();
    ...
    protected String getBlock(boolean isUgcThrow) {
        ...
        if (this.mState == 1002 || this.mState == 1004) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "getBlock():" + block + ", mState=" + this.mState);
            }
            return block;
        }
    }
