...
public class BannerAdProvider extends Wrapper {
    ...
    public List<BannerImageAdModel> fetchBannerAdData() {
        ...
        try {
            ...
            LogUtils.e("BannerAdProvider", "fetchBannerAdData, Exception :" + e);
            HomePingbackFactory.instance().createPingback(CommonPingback.BANNER_AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_banner_tab").addItem("td", String.valueOf(SystemClock.elapsedRealtime() - startRequestTime)).addItem("st", MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR).addItem(Keys.T, "11").addItem("ct", "150619_request").addItem("qtcurl", "tab首页").setOthersNull().post();
        }
        ...
        LogUtils.d("BannerAdProvider", "fetchBannerAdData, banner ad count = " + (!ListUtils.isEmpty(resultList) ? resultList.size() : 0));
        HomePingbackFactory.instance().createPingback(CommonPingback.BANNER_AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_banner_tab").addItem("td", String.valueOf(SystemClock.elapsedRealtime() - startRequestTime)).addItem("st", !ListUtils.isEmpty(resultList) ? "1" : "0").addItem(Keys.T, "11").addItem("ct", "150619_request").addItem("qtcurl", "tab首页").setOthersNull().post();
        return resultList;
    }
    ...
    public BannerAdResultModel parseAd(AdsClient adclient, String adJson) {
        ...
        try {
            ...
            if (slots == null || slots.size() <= 0) {
                LogUtils.d("BannerAdProvider", "no slot with the type of SLOT_TYPE_PAGE, asjson=" + adJson);
                        ...
                        if (ads == null || ads.isEmpty()) {
                            ...
                            if (ad == null) {
                                ...
                                if (imgUrlObj != null) {
                                    ...
                                    models.add(model);
                                    LogUtils.d("BannerAdProvider", "get banner {" + model + "}");
                                } else {
                                    LogUtils.d("BannerAdProvider", "banner url is null.");
                                }
                            } else {
                                LogUtils.e("BannerAdProvider", "creative type: " + ad.getCreativeType());
                            }
                        }
                    }
                }
            }
            LogUtils.d("BannerAdProvider", "banner ad size = " + models.size());
        } catch (JSONException e1) {
            LogUtils.e("BannerAdProvider", "parseAd exception", e1);
        } catch (Exception e) {
    ...
    public String getAdNetworkInfo() {
        ...
        LogUtils.d("BannerAdProvider", "get getAdNw value = : " + result);
        return result;
    }
}
