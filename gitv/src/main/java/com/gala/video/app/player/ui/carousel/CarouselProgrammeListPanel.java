...
public class CarouselProgrammeListPanel implements OnItemRecycledListener, OnItemClickListener, OnItemFocusChangedListener {
    ...
    private void notifyChannelChange(Album album) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "notifyChannelChange() live=" + album.isLive + ", startTime=" + startTime + ", endTime=" + endTime + ", currentTime=" + currentTime);
        }
    ...
    private void notifyVideoChange(IVideo video) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "notifyVwwideoChange(" + video + ") videoChangeListener: " + this.mVideoChangeListener);
        }
    ...
    private void changeMode(ProgrammeViewMode mode) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "changeMode mode=" + mode);
        }
        switch (mode) {
    ...
    private int getPlayIndex() {
        ...
        LogUtils.d(this.TAG, "<<== getPlayIndex=" + currentId + ", index=" + index);
        return index;
    }
    ...
    public void setVideo(IVideo video) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setVideo()=" + video);
        }
    ...
    private void sendClickPingback(List<IVideo> list, int position) {
        if (this.mCurrentVideo == null) {
            ...
            if (clickedAlbum != null) {
                ...
                PingbackFactory.instance().createPingback(22).addItem(PingbackStore.R.R_TYPE("")).addItem(BLOCK.BLOCK_TYPE(BLOCK_CAROUSELLIST)).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE(String.valueOf(position - curPlayIndex))).addItem(RPAGE.RPAGE_ID(BLOCK_CAROUSELLIST)).addItem(C1.C1_TYPE(String.valueOf(clickedAlbum.chnId))).addItem(C2.C2_TYPE(c2)).addItem(NOW_C1.NOW_C1_TYPE(now_c1)).addItem(NOW_C2.NOW_C2_TYPE(now_c2)).post();
            }
        } else if (LogUtils.mIsDebug) {
    ...
    public void onItemFocusChanged(ViewGroup arg0, ViewHolder holder, boolean isSelected) {
        if (holder != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "onItemFocusChanged position = " + position + "/" + isSelected);
            }
        ...
        if (holder != null) {
            ...
            if (!ListUtils.isEmpty(this.mList)) {
                ...
                if (clickedVideo != null) {
                    ...
                    LogUtils.d(this.TAG, "onItemClick(port): pos=" + position);
                }
            }
        } else if (LogUtils.mIsDebug) {
