...
public class HomeActivity extends QMultiScreenActivity implements IPingbackContext {
    ...
    private onPreviewCompleteListener mOnPreviewCompletedListener = new onPreviewCompleteListener() {
        public void onStartCompleted(boolean nextActivity) {
            boolean z = true;
            LogUtils.d(HomeActivity.TAG, "preview show completed,next activity = " + nextActivity);
    ...
    private MyObserver mStartUpErrorObserver = new MyObserver() {
    ...
    private MyObserver mUpdateActionBarObserver = new MyObserver() {
        public void update(String event) {
            HomeActivity.this.runOnUiThread(new Runnable() {
                public void run() {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LayoutParams lp = getRootView().findViewById(R.id.epg_pager).getLayoutParams();
        ...
        LogUtils.d(TAG, "home activity create cost " + (SystemClock.elapsedRealtime() - start) + " ms");
        sendStartupPingback();
    }
    ...
    private void showExitDialog() {
        if (!this.mShowExitDialog) {
            ...
            this.mExitDialog.setOnBtnClickListener(new OnClickListener() {
    ...
    private void showExitUpdateDialog() {
        ...
        UpdateManager.getInstance().showExitUpdateDialog(this, new UpdateOperation() {
            public void exitApp() {
    ...
    public String onActionNotifyEvent(RequestKind kind, String message) {
        LogUtils.d(TAG, "dlna event,onActionNotifyEvent kind(" + kind + ")");
        this.mHomeController.onDlnaActionNotifyEvent(kind);
        return null;
    }

    public void onActionFlingEvent(KeyKind kind) {
        LogUtils.d(TAG, "dlna event,onActionFlingEvent,KeyKind(" + kind + ")");
    ...
    public boolean onKeyChanged(int keycode) {
        LogUtils.d(TAG, "dlna event,onKeyChanged,keycode(" + keycode + ")");
        return false;
    }
    ...
    private void onMenuKeyPressedForMenuFloatLayer(KeyEvent keyEvent) {
        if (keyEvent.getAction() == 0) {
            ...
            if (this.menuFloatLayerWindow == null) {
                ...
                this.menuFloatLayerWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
