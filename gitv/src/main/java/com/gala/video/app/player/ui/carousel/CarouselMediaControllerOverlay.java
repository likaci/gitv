...
public class CarouselMediaControllerOverlay extends RelativeLayout implements ICarouselMediaController, IScreenUISwitcher {
    ...
    private AllChannelCallback mAllChannelCallback = new AllChannelCallback() {
    ...
    private AllChannelDetailCallback mAllChannelDetailCallback = new AllChannelDetailCallback() {
        public void onCacheReady(TVChannelCarouselTag tag, List<CarouselChannelDetail> allDetailInfo) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(CarouselMediaControllerOverlay.TAG, "AllChannelDetailCallback onCacheReady = " + allDetailInfo);
            }
            if (CarouselMediaControllerOverlay.this.mCarouselChannelListPanel != null) {
        ...
        public void onDataReady(TVChannelCarouselTag tag, List<CarouselChannelDetail> allDetailInfo) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(CarouselMediaControllerOverlay.TAG, "AllChannelDetailCallback onDataReady = " + allDetailInfo);
            }
            if (CarouselMediaControllerOverlay.this.mCarouselChannelListPanel != null) {
                CarouselMediaControllerOverlay.this.mCarouselChannelListPanel.setChannelProgramName(allDetailInfo, tag);
            }
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(CarouselMediaControllerOverlay.TAG, "handleMessage(" + msg + ")");
            }
            switch (msg.what) {
    ...
    private Runnable mNotifyChannelChangeRunnable = new Runnable() {
        public void run() {
            if (LogUtils.mIsDebug) {
    ...
    PlayerListListener mPlayerlistListener = new PlayerListListener() {
        ...
        public void onItemFocusChanged(ViewHolder holder, boolean isSelected, TVChannelCarouselTag label, int tag) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(CarouselMediaControllerOverlay.TAG, "onItemFocusChanged() tag=" + tag);
            }
        ...
        public void onItemClick(ViewHolder holder, int tag) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(CarouselMediaControllerOverlay.TAG, "onItemClick() tag=" + tag);
            }
    ...
    private ProgramListCallback mProgramListCallback = new ProgramListCallback() {
    ...
    public void switchScreen(boolean isFullScreen, float zoomRatio) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "switchScreen() isFullScreen = " + isFullScreen);
        }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "dispatchKeyEvent(" + event + "), fullscreen=" + this.mIsFullScreen);
        }
    ...
    public void updateChannelInfo(TVChannelCarousel channelCarousel, boolean needRequest) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "updateChannelInfo() channelCarousel=" + channelCarousel + ", mAllDetailInfo=" + this.mAllDetailInfo);
        }
    ...
    public void setNetSpeed(long speed) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setNetSpeed(" + speed + ")");
        }
    ...
    private void handleChannelChangeEvent(boolean isIncrease) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "handleChannelChangeEvent(isIncrease:" + isIncrease + ")");
        }
