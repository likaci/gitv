...
public class RemoteConfigReader implements IConfigReader {
    ...
    public void readConfigAsync(OnConfigReadListener listener) {
        ...
        MyLogUtils.d(this.TAG, "readConfigAsync(path:" + path + ",subChannelPath：" + subChannelPath + " OnConfigReadListener:" + listener + ")");
    ...
    public String readConfigSync() {
        ...
        MyLogUtils.d(this.TAG, "readConfigAsync(path:" + path + " ,subChannelPath:" + subChannelPath + ",OnConfigReadListener:");
        if (StringUtils.isEmpty(path)) {
            return null;
        }
        try {
    ...
    private String mergeJsonResult(String genernalConfig, String subChannelConfig) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "mergeJsonResult(genernalConfig: " + genernalConfig + ", subChannelConfig: " + subChannelConfig + ")");
        }
    ...
    private void parseRemoteFileAsync(final String url, final String subChannelUrl, final OnConfigReadListener listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "parseRemoteFileAsync(" + url + "," + subChannelUrl + ")");
        }
    ...
    private String parseRemoteFileSync(String url) throws Throwable {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "parseRemoteFileSync(" + url + ")");
        }
    ...
    private byte[] readRemoteFile(String url) throws ClientProtocolException, IOException, AssertionError {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "httpRequest: url=" + url);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "httpRequest: response code=" + response.getStatusLine().getStatusCode() + ", request time consumed=" + (SystemClock.uptimeMillis() - start));
        }
        byte[] bytes = null;
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "<< httpRequest(), total time consumed=" + (SystemClock.uptimeMillis() - start));
        }
        return bytes;
    }
    ...
    private byte[] readStream(HttpResponse response) throws IOException {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "<< readStream(), total time consumed=" + (SystemClock.uptimeMillis() - readStart));
        }
        return bytesBuffer;
    }
}
