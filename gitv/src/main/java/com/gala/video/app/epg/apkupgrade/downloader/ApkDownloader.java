...
public class ApkDownloader {
    ...
    IGalaDownloadListener mDownloadListener = new IGalaDownloadListener() {
        public void onStart() {
        ...
        public void onProgress(long download, long fileSize) {
            ...
            if (ApkDownloader.this.mLastProgress != progress) {
                LogUtils.d("ApkDownloader", "onProgress()---" + progress);
            }
        ...
        public void onError(GalaDownloadException e) {
            LogUtils.d("ApkDownloader", "onError()---errorCode=" + e.getErrorCode());
        ...
        if (!StringUtils.isEmpty((CharSequence) strDir) && !StringUtils.isEmpty((CharSequence) apkName)) {
            ...
            if (fileDir.isDirectory() && fileDir.canWrite()) {
                ...
                for (int i = 0; i < files.length; i++) {
                    if (files[i].exists()) {
                        LogUtils.d("ApkDownloader", "deleteOldApks()---" + files[i].getAbsolutePath());
                        if (!files[i].delete()) {
                            LogUtils.e("ApkDownloader", "deleteOldApks()---fail---" + files[i].getAbsolutePath());
                        }
                    }
                }
            }
    ...
    private boolean checkMd5(String filePath, String md5) {
        ...
        LogUtils.d("ApkDownloader", "checkMd5()---md5 sum cost :" + (System.currentTimeMillis() - befMd5Sum));
    ...
    public void checkApkDownloadedAndDeleteInvalidFile(final AppVersion version) {
        ThreadUtils.execute(new Runnable() {
            public void run() {
                ...
                LogUtils.d("ApkDownloader", "delete existed files cost time-" + (System.currentTimeMillis() - start));
            }
        });
    }
