    ...
    private void handleTimeWord(List<IMsgContent> list) {
        ...
        for (IMsgContent content : list) {
            ...
            if (offset < 0) {
                ...
                if (minOffset < 1) {
                    str = ResourceUtil.getStr(R.string.msg_showtime_1);
                } else if (minOffset < 60) {
                    str = ResourceUtil.getStr(R.string.msg_showtime_2, Long.valueOf(minOffset));
                } else if (minOffset < 1440) {
                    str = ResourceUtil.getStr(R.string.msg_showtime_3, Long.valueOf(minOffset / 60));
                } else if (minOffset < 43200) {
                    str = ResourceUtil.getStr(R.string.msg_showtime_4, Long.valueOf(minOffset / 1440));
                } else {
                    Date date = new Date(content.localTime);
                    int year = date.getYear() + SSDP.PORT;
                    int month = date.getMonth() + 1;
                    int day = date.getDate();
                    str = ResourceUtil.getStr(R.string.msg_showtime_5, Integer.valueOf(year), Integer.valueOf(month), Integer.valueOf(day));
                }
