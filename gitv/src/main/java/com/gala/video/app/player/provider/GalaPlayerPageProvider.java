...
public class GalaPlayerPageProvider extends Wrapper {
    ...
    private void startPlayForCarousel(Context context, ChannelCarousel oldChannel, String from, String tabSource) {
        LogUtils.d(TAG, ">> stratPlayForCarousel Channel" + oldChannel + "from=" + from + ", tabSource=" + tabSource);
        Intent intent = new Intent();
    ...
    private void startPlayForLive(Context context, Album album, String from, ArrayList<Album> flowerList, String buySource, String tabSource) {
        LogUtils.d(TAG, ">> startPlayForLive: album=" + DataUtils.albumInfoToString(album) + ", from=" + from + ", flowerList=" + flowerList + ", buySource=" + buySource + ", tabSource=" + tabSource);
        Intent intent = new Intent();
        if (!(context instanceof Activity)) {
        ...
        if (ListUtils.isEmpty((List) flowerList)) {
            ...
            LogUtils.d(TAG, "flower for live, size=" + flowerList.size());
            Iterator it = flowerList.iterator();
            while (it.hasNext()) {
                Album flower = (Album) it.next();
                LogUtils.d(TAG, "name = " + flower.name + "albumId=" + flower.qpId + ",tvId=" + flower.tvQid);
            }
        }
        LogUtils.d(TAG, "startPlayForLive(), album[" + album + " isLive=" + album.isLive + " isFlower=" + album.isFlower + " liveChannelId=" + album.live_channelId + "], currentServerTime = " + new Date(DeviceUtils.getServerTimeMillis()) + ", starttime = " + new Date(StringUtils.parse(album.sliveTime, -1)));
        GetInterfaceTools.getPlayerFeatureProxy().loadPlayerFeatureAsync(context, new MyPluginStateListener(context, intent), true);
    }
    ...
    private void startDetailForLoop(Context context, String from, String channelName, NewsParams params, String buySource, String tabSource) {
        if (context != null) {
            LogUtils.d(TAG, ">> startDetailForLoop: context=" + context + ", from=" + from + ", channelName=" + channelName + ", params" + params + ", buySource=" + buySource + ", tabSource=" + tabSource);
            Intent intent = new Intent();
            if (!(context instanceof Activity)) {
                intent.setFlags(VoiceUtils.INTENT_FLAG_DEFAULT);
            }
    ...
    public void startBasePlayerPage(Context context, BasePlayParamBuilder builder) {
        if (builder.mPlayParams == null) {
            LogUtils.e(TAG, ">> startBasePlayerPage: mPlayParams" + builder.mPlayParams);
        ...
        if (!ListUtils.isEmpty(builder.mPlayParams.continuePlayList)) {
            ...
            LogUtils.d(TAG, "size=" + size + ", index=" + builder.mPlayParams.playIndex);
    ...
    public void startPushPlayerPage(Context context, PushPlayParamBuilder builder) {
        ...
        LogUtils.d(TAG, ">> startVideoPlayForPushVideo: command=" + builder.mMultiScreenParams + ", from=" + builder.mFrom + ", se=" + builder.mSe + ", buySource=" + builder.mBuySource + ", tabSource=" + builder.mTabSource);
