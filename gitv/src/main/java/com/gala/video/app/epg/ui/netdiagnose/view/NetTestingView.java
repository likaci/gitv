...
public class NetTestingView extends RelativeLayout {
    ...
    private void init(Context context) {
        ...
        View child = ((LayoutInflater) context.getSystemService("layout_inflater")).inflate(R.layout.epg_net_diagnose_testing_view, null);
        LayoutParams layoutParams = new LayoutParams(-2, -2);
        layoutParams.addRule(13);
        addView(child, layoutParams);
        this.mRouter = (ImageView) child.findViewById(R.id.epg_icon_router);
        this.mInternet = (ImageView) child.findViewById(R.id.epg_icon_internet);
        this.mCDN = (ImageView) child.findViewById(R.id.icon_cdn);
        this.mStatusRouter = (ImageView) child.findViewById(R.id.epg_status_router);
        this.mStatusInternet = (ImageView) child.findViewById(R.id.epg_status_internet);
        this.mStatusCND = (ImageView) child.findViewById(R.id.epg_status_cdn);
        this.mRouterCross = (ImageView) child.findViewById(R.id.epg_status_router_cross);
        this.mInternetCross = (ImageView) child.findViewById(R.id.epg_status_internet_cross);
        this.mCDNCross = (ImageView) child.findViewById(R.id.epg_status_cdn_cross);
        this.mRouterOval = (NetDiagnoseOvalView) child.findViewById(R.id.epg_icon_router_oval);
        this.mInternetOval = (NetDiagnoseOvalView) child.findViewById(R.id.epg_icon_internet_oval);
        this.mCDNOval = (NetDiagnoseOvalView) child.findViewById(R.id.epg_icon_cdn_oval);
        this.mLoadingView = child.findViewById(R.id.progress_loading);
        this.mResult = (TextView) child.findViewById(R.id.txt_result);
    }

    private void showResult(String msg) {
        LogUtils.d(this.TAG, "showResult=" + msg);
    ...
    public void setAverageSpeed(int averageSpeed) {
        LogUtils.d(this.TAG, "setAverageSpeed=" + averageSpeed);
        this.mAvgNetSpeed = averageSpeed;
    }

    public void setNetStatus(DiagnoseStatus status) {
        LogUtils.d(this.TAG, "setNetStatus = " + status);
        switch (status) {
            case TESTING_BEGIN:
    ...
    private SpannableStringBuilder getSpeedTestResultString() {
        ...
        if (this.mAvgNetSpeed < 512) {
            ...
            String speedLowResult = this.mContext.getString(R.string.net_speed_low, new Object[]{speedDescription});
            ...
            String speedResult = this.mContext.getString(R.string.net_speed, new Object[]{result, recommemdDefinition});
            sb = new SpannableStringBuilder(speedResult);
            ForegroundColorSpan coloSpanSpeed = new ForegroundColorSpan(getResources().getColor(R.color.skin_net_yellow_tip_txt));
        ...
        sb.append(this.mContext.getString(R.string.feedback_guide));
        return sb;
    }
    ...
    private String getRecommendDefinition(int kb) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "getRecommendDefinition, kB=" + kb);
        }
        String ret = "NULL";
        if (kb < 512) {
            if (!LogUtils.mIsDebug) {
            ...
            return this.mContext.getString(R.string.definition_standard);
        } else {
            if (kb >= 1024 && kb < 3072) {
                return this.mContext.getString(R.string.definition_high);
            }
            if (kb >= 3072 && kb < 5120) {
                return this.mContext.getString(R.string.definition_720P);
            }
            if (kb >= 5120) {
                return this.mContext.getString(R.string.definition_1080P);
            }
            return ret;
        }
    }
