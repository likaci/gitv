...
public class JobPerfMonitor {
    ...
    private IPerformanceDataReadyListener mDataListener = new IPerformanceDataReadyListener() {
        public void onPerformanceDataReady(IPerformanceDataProvider provider) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(JobPerfMonitor.TAG, "onPerformanceDataReady: data={name:" + requestName + ", id:" + requestId + ", total:" + totalTimeSpan + ", indiv:" + individualTimeSpans + ", json:" + jsonParseTimeSpan + ", success:" + isSuccess + ", main:" + isMainRoutine + ", errCode:" + errorCode);
            }
    ...
    private String getValueFromContext(String key) {
        ...
        if (context == null) {
            if (LogUtils.mIsDebug) {
                LogUtils.w(TAG, "getValueFromContext: no available context!");
            }
            return "";
        }
        try {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.w(TAG, "getValueFromContext: exception happened", e);
            }
            return "";
        }
    }
    ...
    public void registerDataProvider(IPerformanceDataProvider... providers) {
        if (providers != null) {
            for (IPerformanceDataProvider p : providers) {
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, "registerDataProvider: " + p);
                }
    ...
    private void sendRequestPingback(String requestName, String requestId, long time, String st, String pfec, List<Long> requestTimes, long jsonParseTime, String playerType, boolean isMainRoute) {
        ...
        params.add(Keys.T, "11").add("ct", "150619_request").add(Keys.RI, requestName).add("r", requestId).add("e", getValueFromContext("e")).add("td", String.valueOf(time)).add(Keys.TM1, tm1).add("tm2", tm2).add("tm3", tm3).add("st", st).add("ec", ec).add("pfec", pfec).add("localtime", getFormatTime()).add("player", playerType).add(Keys.IS_PLAY_PATH, isMainRoute ? "1" : "0").add("hcdn", getValueFromContext("hcdn"));
    ...
    private static String getFormatTime() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, ">>getFormatTime:" + result);
        }
        return result;
    }
}
