...
public class AppDownloadManager extends Wrapper {
    ...
    private OnClickListener mBackgroundButtonClickListener = new OnClickListener() {
    ...
    private AppDownloaderListener mDownloadListener = new AppDownloaderListener() {
        public void onStart() {
    ...
    private Runnable mRetryDownloadRunnable = new Runnable() {
        public void run() {
            AppDownloadManager.this.startDownloadApp();
        }
    };
    ...
    private GlobalDialog createRetryDialog(String text) {
        GlobalDialog dialog = Project.getInstance().getControl().getGlobalDialog(this.mContext);
        dialog.setParams(text, ResourceUtil.getStr(R.string.download_app_btn_retry), new OnClickListener() {
            public void onClick(View v) {
    ...
    private void sendChinaPokerDownloadFinishedClickPingback(int status) {
        if (this.mPromotionAppInfo != null && 2 == this.mPromotionAppInfo.getAppType()) {
            HomePingbackFactory.instance().createPingback(CommonPingback.CHINA_POKER_LOAD_FINISHED_PINGBACK).addItem(Keys.LDTYPE, "斗地主").addItem("st", status == 8 ? "1" : "0").addItem("ct", "160602_load").addItem(Keys.T, "11").post();
        }
    }
