...
public class ScrollViewPager extends ViewPager {
    ...
    public static class ViewPageAdapter extends PagerAdapter {
            ...
            for (int index = 0; index < newPages.size(); index++) {
                if (index < oldPages.size()) {
                    if (HomeDebug.DEBUG_LOG) {
                        LogUtils.d(TAG, "new page = " + newPages.get(index) + ", old page = " + oldPages.get(index));
                    }
        ...
        public int getItemPosition(Object object) {
            ...
            if (HomeDebug.DEBUG_LOG) {
                LogUtils.d(TAG, "getItemPosition object = " + object + " ret = " + -2);
            }
            return -2;
        }
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            if (child != null) {
                ...
                LogUtils.d(TAG, "instantiate page position = " + position + "child = " + child);
                container.addView(child);
            } else {
                LogUtils.e(TAG, "instantiate page ,child is not builded,position = " + position);
            }
            return child;
        }

        public void destroyItem(ViewGroup container, int position, Object object) {
            LogUtils.d(TAG, "destroyItem position = " + position + ", object = " + object + ",container = " + container);
    ...
    public void clearChildFocus(View child) {
        ...
        LogUtils.d(TAG, "clearChildFocus child@" + child);
        if (this.mTabbar != null) {
            this.mTabbar.post(new Runnable() {
                public void run() {
    ...
    public void addPageViews(List<ViewGroup> pageViews, int count, Context context) {
        if (HomeDebug.DEBUG_LOG) {
            for (int index = 0; index < pageViews.size(); index++) {
                LogUtils.d(TAG, " index = " + index + ", page view = " + pageViews.get(index));
            }
        }
