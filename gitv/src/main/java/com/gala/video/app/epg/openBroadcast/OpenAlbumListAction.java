...
public class OpenAlbumListAction extends BaseAction {
    ...
    public void process(Context context, Intent intent, LoadingCallback loadingCallback) {
        try {
            ...
            LogUtils.e("OpenAlbumListAction", "[UNKNOWN-EXCEPTION] [reason:exception occurred when OpenAlbumListAction process.][Exception:" + e.getMessage() + AlbumEnterFactory.SIGN_STR);
            e.printStackTrace();
            if (loadingCallback != null) {
                loadingCallback.onFail();
    ...
    private void dealAlbumList(Context context, JSONObject dict) {
        ...
        if (LIST_TYPE.vip.name().equalsIgnoreCase(listType)) {
            ...
            LogUtils.e("OpenAlbumListAction", "invalid listType =  " + listType);
        }
    }
    ...
    public boolean checkParamsValidity(JSONObject playDict) {
        if (super.checkParamsValidity(playDict)) {
            ...
            LogUtils.d("OpenAlbumListAction", "listType = " + listType);
            if (TextUtils.isEmpty(listType)) {
                LogUtils.e("OpenAlbumListAction", "[INVALID-PARAMTER] [action:ACTION_ALBUMLIST][reason:missing field--listType][playInfo:" + playDict.toString() + AlbumEnterFactory.SIGN_STR);
            } else if (!LIST_TYPE.channel.name().equals(listType)) {
                return true;
            } else {
                String chnId = playDict.optString("chnId");
                String chnName = playDict.optString("chnName");
                LogUtils.d("OpenAlbumListAction", "chnId = " + chnId);
                LogUtils.d("OpenAlbumListAction", "chnName = " + chnName);
                if (!TextUtils.isEmpty(chnId) && !TextUtils.isEmpty(chnName)) {
                    return true;
                }
                LogUtils.e("OpenAlbumListAction", "[INVALID-PARAMTER] [action:ACTION_ALBUMLIST][reason:missing field--chnId|chnName][playInfo:" + playDict.toString() + AlbumEnterFactory.SIGN_STR);
                return false;
            }
        }
        return false;
