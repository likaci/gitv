    ...
    public void callLoginRequest() {
        ...
        if ((this.mLoginView.isVerifyVisible() && checkVerifycodeAvailability()) || !this.mLoginView.isVerifyVisible()) {
            ...
            ThreadUtils.execute(new Runnable() {
                public void run() {
                    GetInterfaceTools.getIGalaAccountManager().loginByKeyInput(account, password, code, new ILoginCallback() {
    ...
    public void loadVerifyCode() {
        if (this.mVerifClickable && this.mLoginView.isVerifyVisible()) {
            ...
            ThreadUtils.execute(new Runnable() {
                public void run() {
                    try {
                        final Bitmap bitmap = TVApiBase.getTVApiImageTool().downloadImage(PassportTVHelper.getRegisterEMailVCode((int) CommLoginPresenter.this.mContext.getResources().getDimension(R.dimen.dimen_60dp), (int) CommLoginPresenter.this.mContext.getResources().getDimension(R.dimen.dimen_129dp), DeviceUtils.getMacAddr()));
