...
public class SettingMainActivity extends QMultiScreenActivity implements ISettingEvent {
    ...
    private void switchFragment(SettingBaseFragment fragment, Bundle bundle) {
        ...
        if (stackSize <= 0) {
            transaction.replace(R.id.epg_setting_main_frame, fragment);
        } else {
            transaction.add(R.id.epg_setting_main_frame, (Fragment) fragment);
            transaction.hide(this.mCurFragment);
        }
        transaction.addToBackStack(fragment.getClass().getName());
        transaction.commit();
        LogUtils.e("EPG/setting/SettingMainActivity", "switchFragment," + (stackSize <= 0 ? "replace " : "add ") + fragment);
    }

    private void onBackPressed(SettingItem settingItem) {
        int size = getSupportFragmentManager().getBackStackEntryCount();
        LogUtils.i("EPG/setting/SettingMainActivity", "TEST - " + size);
        if (size > 1) {
