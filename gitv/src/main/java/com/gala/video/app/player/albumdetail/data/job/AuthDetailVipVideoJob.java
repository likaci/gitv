        ...
        if (albumInfo != null) {
            ...
            if (albumInfo.isAlbumVip() || albumInfo.isAlbumSinglePay() || albumInfo.isAlbumCoupon()) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, "fetchVipStatusData: defaultUserId=" + defaultUserId + ", cookie=" + cookie);
                }
                ...
                BOSSHelper.authVipVideo.call(new IVrsCallback<ApiResultAuthVipVideo>() {
                    public void onSuccess(ApiResultAuthVipVideo apiResult) {
                        if (apiResult != null) {
                            if (LogUtils.mIsDebug) {
                                LogUtils.d(AuthDetailVipVideoJob.TAG, "onSuccess: canPreview = " + apiResult.canPreview() + ", previewEpisodes = " + Arrays.toString(apiResult.getPreviewEpisodes()));
                            }
                    ...
                    public void onException(ApiException e) {
                        if (LogUtils.mIsDebug) {
                            LogUtils.d(AuthDetailVipVideoJob.TAG, "onException: code = " + e.getCode() + ", msg = " + e.getMessage());
                        }
                        albumInfo.setVipAuthorized(false);
                        AuthDetailVipVideoJob.this.notifyJobSuccess(controller);
                    }
