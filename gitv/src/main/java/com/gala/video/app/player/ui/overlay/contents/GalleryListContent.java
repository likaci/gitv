...
public class GalleryListContent implements IContent<List<IVideo>, IVideo> {
    ...
    public GalleryListContent(Context context, IGalleryUIStyle uiStyle, String title, boolean showPlaying, boolean autoFocus, boolean showExclusive) {
        ...
        this.mMainHandler = new Handler(Looper.getMainLooper()) {
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    ...
                    default:
                        if (LogUtils.mIsDebug) {
                            LogUtils.d(GalleryListContent.this.TAG, "unhandled msg, what=" + msg.what);
                            return;
                        }
                        return;
                }
        ...
        this.mItemSelectedListener = new OnItemClickListener() {
            public void onItemClick(ViewGroup viewGroup, ViewHolder viewHolder) {
                if (LogUtils.mIsDebug) {
                    LogUtils.d(GalleryListContent.this.TAG, "onItemClick ");
                }
                int index = viewHolder.getLayoutPosition();
                if (LogUtils.mIsDebug) {
                    LogUtils.d(GalleryListContent.this.TAG, "onItemClick, clicked position=" + index);
                }
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(GalleryListContent.this.TAG, "onItemClick clickVideo " + clickedVideo);
                }
                if (clickedVideo == null) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.e(GalleryListContent.this.TAG, "onItemClick: pos=" + index + ", null video!! backtrace=", new Throwable().fillInStackTrace());
                    }
        ...
        this.mItemFocusChangedListener = new OnItemFocusChangedListener() {
            public void onItemFocusChanged(ViewGroup viewGroup, ViewHolder viewHolder, boolean hasFocus) {
                View v = viewHolder.itemView;
                if (LogUtils.mIsDebug) {
                    LogUtils.d(GalleryListContent.this.TAG, "onItemFocusChanged, hasFocus=" + hasFocus);
                }
                if (ListUtils.isEmpty(GalleryListContent.this.mDataList)) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.d(GalleryListContent.this.TAG, "onItemFocusChanged, mDataList is empty.");
                    }
                    ...
                    if (index >= 0 && index <= GalleryListContent.this.mDataList.size() - 1) {
                        ...
                        if (LogUtils.mIsDebug) {
                            LogUtils.d(GalleryListContent.this.TAG, "onItemFocusChanged, index=" + index + ", focusedVideo=" + focusedVideo);
                        }
                    } else if (LogUtils.mIsDebug) {
                        LogUtils.d(GalleryListContent.this.TAG, "onItemFocusChanged, invalid index, index=" + index + ", mDataList.size()=" + GalleryListContent.this.mDataList.size());
                    }
                } else {
                    AnimationUtil.zoomOutAnimation(v, 1.0f);
                }
        ...
        this.mItemRecycledListener = new OnItemRecycledListener() {
            public void onItemRecycled(ViewGroup viewGroup, ViewHolder viewHolder) {
                if (LogUtils.mIsDebug) {
                    LogUtils.d(GalleryListContent.this.TAG, ">> onItemRecycled");
                }
                View v = viewHolder.itemView;
                int index = viewHolder.getLayoutPosition();
                if (LogUtils.mIsDebug) {
                    LogUtils.d(GalleryListContent.this.TAG, "onItemRecycled, index=" + index + ", v=" + v);
                }
        ...
        this.mScrollListener = new OnScrollListener() {
            public void onScrollStart() {
                if (LogUtils.mIsDebug) {
                    LogUtils.d(GalleryListContent.this.TAG, ">> onScrollStart");
                }
            ...
            public void onScrollStop() {
                if (LogUtils.mIsDebug) {
                    LogUtils.d(GalleryListContent.this.TAG, ">> onScrollStop");
                }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "GalleryListContent, title=" + title + ", mTitle=" + this.mTitle + ", mNeedPlayingIcon=" + this.mNeedPlayingIcon + "mIsShowExclusive=" + this.mIsShowExclusive + "mIsPort=" + this.mIsPort + ", mIsDetail=" + this.mIsDetail + ", mIsAutoFocus=" + this.mIsAutoFocus);
        }
    }
    ...
    private void initContentView() {
        this.mContentView = LayoutInflater.from(this.mContext).inflate(R.layout.player_gallery_content_common, null);
        if (LogUtils.mIsDebug) {
    ...
    private void checkVisibleItems(int first, int last) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> checkVisibleItems, first=" + first + ", last=" + last);
        }
        if (first >= 0 && last >= 0) {
            int i;
            ...
            for (i = 0; i < size; i++) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(this.TAG, "checkVisibleItems, index=" + index + ", x=" + location[0] + ", y=" + location[1] + ", view.measuredWidth=" + view.getMeasuredWidth() + ", view=" + view);
                }
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "checkVisibleItems, list=" + list);
            }
    ...
    private List<View> resetVisibleViewWithCache(int first, int last) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> resetVisibleViewWithCache, first=" + first + ", last=" + last);
        }
        this.mCurVisibleViews.clear();
        if (first < 0 || last < 0) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, "resetVisibleViewWithCache, invalid index, first=" + first + ", last=" + last);
            }
    ...
    public void setItemListener(IItemListener<IVideo> listener) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> setItemListener[@" + listener + AlbumEnterFactory.SIGN_STR);
        }
    ...
    private int updatePlayingSelection(IVideo item) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> updatePlayingSelection, video=" + item);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "updatePlayingSelection, shouldShowPlaying=" + shouldShowPlaying + ", position=" + position);
        }
    ...
    private boolean shouldShowPlayingIcon() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "shouldShowPlayingIcon, ret=" + ret + ", mIsDetail=" + this.mIsDetail + ", mNeedPlayingIcon=" + this.mNeedPlayingIcon + ", mEnableTvWindow=" + this.mEnableTvWindow);
        }
        return ret;
    }
    ...
    private void updateSelection(int position) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> updateSelection, position=" + position);
        }
        if (this.mHorizontalGridView != null) {
            if (LogUtils.mIsDebug) {
    ...
    private int findPosition(List<VideoData> list, IVideo cur) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "findPosition() find=" + find);
        }
        return find;
    }
    ...
    private void addAd(BaseAdData data) {
        ...
        if (data != null) {
            ...
            if (this.mContentView != null && this.mAdView == null) {
                ...
                if (this.mAdView != null) {
                    LayoutParams params = new LayoutParams(this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_256dp), this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_36dp));
                    params.topMargin = this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_8dp);
