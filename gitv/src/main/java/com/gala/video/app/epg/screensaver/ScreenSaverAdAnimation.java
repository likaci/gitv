...
class ScreenSaverAdAnimation extends FadeAnim implements IRegisterAd {
    ...
    protected boolean getNext(int index) {
        ...
        LogUtils.d(TAG, "getNext, currentIndex = " + index);
    ...
    protected void beforeFadeIn(int currentIndex) {
        ...
        LogUtils.d(TAG, "show screen saver animation info = " + model);
        if (!model.shouldShowQr()) {
            ...
            if (ScreenSaverPingBack.SEAT_KEY_LEFT.equals(model.getQrPosition())) {
                ((LayoutParams) this.mQrLayout.getLayoutParams()).rightMargin = ResourceUtil.getDimensionPixelSize(R.dimen.dimen_1110dp);
            } else {
                ((LayoutParams) this.mQrLayout.getLayoutParams()).rightMargin = ResourceUtil.getDimensionPixelSize(R.dimen.dimen_30dp);
            }
