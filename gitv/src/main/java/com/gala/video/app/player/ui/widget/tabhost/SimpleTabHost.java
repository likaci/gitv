...
public class SimpleTabHost extends LinearLayout implements ISimpleTabHost {
    ...
    public class IndicatorView extends FrameLayout {
        Rect bgRect = UiHelper.getBgDrawablePaddings(ResourceUtil.getDrawable(R.drawable.player_episode_item_bg));
        int layoutHeight = ResourceUtil.getDimen(R.dimen.dimen_53dp);
        int layoutWidth;
        int mIndex;
        TextView mText;
        String mTitle;
        ...
        private void initViews() {
            ...
            int textSize = ResourceUtil.getDimen(R.dimen.dimen_20dp);
        ...
        private void setup() {
            ...
            this.mText.setTextSize(0, (float) ResourceUtil.getDimensionPixelSize(R.dimen.dimen_20dp));
        ...
        public boolean dispatchKeyEvent(KeyEvent event) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(SimpleTabHost.this.TAG, "IndicatorView dispatchKeyEvent =" + event);
            }
    ...
    public SimpleTabHost(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mDataChangedListener = new OnDataChangedListener() {
    ...
    private void initContainers() {
        ...
        this.mIndicatorContainer.setOnFocusChangeListener(new OnFocusChangeListener() {
            public void onFocusChange(View v, boolean hasFocus) {
                if (hasFocus) {
    ...
    public void setCurrentTab(int index) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> setCurrentTab, index=" + index);
        }
        if (index < 0 || index >= this.mTabsCount) {
            if (LogUtils.mIsDebug) {
                LogUtils.e(this.TAG, ">> setCurrentTab, invalid index=" + index);
            }
            ...
            LogUtils.d(this.TAG, "set the same current tab, index=" + index);
        }
    }
    ...
    public void setAdapter(SimpleTabHostAdapter adapter) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> setAdapter, adapter=" + adapter);
        }
        if (adapter != null) {
    ...
    private void setIndicatorStyle(int index, int style) {
        ...
        switch (style) {
            ...
            case 4:
                ...
                indicator_text.setBackgroundResource(R.drawable.share_btn_focus);
                return;
            default:
                if (LogUtils.mIsDebug) {
                    LogUtils.d(this.TAG, ">>setIndicatorStyle, unhandled style, index=" + index + ", style=" + style);
                    return;
                }
                return;
        }
