...
public class UcenterActivity extends QMultiScreenActivity implements IUCenterView {
    ...
    private GlobalVipCloudViewCallBack mGlobalVipCloudViewCallBack = new GlobalVipCloudViewCallBack() {
    ...
    private MyObserver mHistoryChangedObserver = new MyObserver() {
        public void update(String event) {
            UcenterActivity.this.mHandler.post(new Runnable() {
                public void run() {
                ...
                if (ucenterActivity != null) {
                    ...
                    if (model.getItemType() == UIKitConfig.ITEM_TYPE_HISTORY_ALL_ENTRY) {
                        ...
                        if (ListUtils.isEmpty((List) historyInfosList)) {
                            LogUtils.e(UcenterActivity.LOG_TAG, ">>>>> GetInterfaceTools.getIHistoryCacheManager().getLatestVideoHistory(10), historyInfosList is empty");
                        } else {
                            LogUtils.d(UcenterActivity.LOG_TAG, ">>>>> GetInterfaceTools.getIHistoryCacheManager().getLatestVideoHistory(10), size = ", Integer.valueOf(historyInfosList.size()));
                ...
                for (CardInfoModel newModel : moreModels) {
                    ...
                    for (CardInfoModel oldModel : ucenterActivity.mCardInfoModels) {
                        ...
                        if (!StringUtils.isEmpty(newType) && !StringUtils.isEmpty(oldType) && newType.equals(oldType)) {
                            LogUtils.d(UcenterActivity.LOG_TAG, ">>>>>filter same cardType=", newType);
                            filterFlag = true;
                            break;
                        }
                    }
    ...
    private CardInfoModel createHeadModel() {
        ...
        model.setBodyHeight((short) ResourceUtil.getDimen(R.dimen.dimen_330dp));
        return model;
    }
    ...
    private void initUserInfoCard() {
        ...
        if (ListUtils.isEmpty(this.mCardInfoModels)) {
            LogUtils.d(LOG_TAG, ">>>>> init userinfo ui ---- mCardInfoModels is null");
            this.mCardInfoModels = new ArrayList();
            this.mCardInfoModels.add(this.mHeadModel);
        } else {
            LogUtils.d(LOG_TAG, ">>>>> init userinfo ui ---- mCardInfoModels is not null");
    ...
    private void updateHistoryCard(boolean updateFocus) {
        if (this.mHistoryModel != null) {
            ...
            if (ListUtils.isEmpty((List) historyInfosList)) {
                LogUtils.e(LOG_TAG, ">>>>> GetInterfaceTools.getIHistoryCacheManager().getLatestVideoHistory(10), historyInfosList is empty");
            } else {
                LogUtils.d(LOG_TAG, ">>>>> GetInterfaceTools.getIHistoryCacheManager().getLatestVideoHistory(10), size = ", Integer.valueOf(historyInfosList.size()));
                for (HistoryInfo historyInfo : historyInfosList) {
                    albums.add(historyInfo.getAlbum());
                }
            }
    ...
    public void showSecurityUI() {
        ...
        this.mChangePassView.setOnClickListener(new OnClickListener() {
    ...
    public void showLogoutUI() {
        ...
        this.mConfirmDialog.setOnDismissListener(new OnDismissListener() {
        ...
        if (ListUtils.isEmpty(bitmapAlbums)) {
            this.mConfirmDialog.setParams(ResourceUtil.getStr(R.string.confirm_logout_vip), ResourceUtil.getStr(R.string.logout_ok), okListener, ResourceUtil.getStr(R.string.logout_cancel), cancelListener, true, null, null, null);
        } else if (bitmapAlbums.size() < 3) {
            this.mConfirmDialog.setParams(ResourceUtil.getStr(R.string.confirm_logout_vip), ResourceUtil.getStr(R.string.logout_ok), okListener, ResourceUtil.getStr(R.string.logout_cancel), cancelListener, true, null, null, null);
        } else {
            this.mConfirmDialog.setParams(ResourceUtil.getStr(R.string.confirm_logout_vip), ResourceUtil.getStr(R.string.logout_ok), okListener, ResourceUtil.getStr(R.string.logout_cancel), cancelListener, true, (BitmapAlbum) bitmapAlbums.get(0), (BitmapAlbum) bitmapAlbums.get(1), (BitmapAlbum) bitmapAlbums.get(2));
        }
        boolean isVipStatus = GetInterfaceTools.getIGalaAccountManager().isVip();
        LogUtils.d(LOG_TAG, "isVipStatus = " + isVipStatus);
    ...
    public void showExceptionUI(String tipValue) {
        ...
        globalDialog.setOnDismissListener(new OnDismissListener() {
            public void onDismiss(DialogInterface dialog) {
        ...
        globalDialog.setParams(tipValue, ResourceUtil.getStr(R.string.arefresh_login_ok), okListener);
        globalDialog.show();
    }
    ...
    public void refreshUserInfoUI() {
        LogUtils.d(LOG_TAG, ">>>>> checkUserInfo - refresh userinfo ui - after check userinfo");
        initUserInfoCard();
    }
