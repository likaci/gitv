...
public class MultiSubjectPingbackUtils {
    ...
    public static void sendPageShowPingback(PageShowPingbackModel model) {
        if (model != null) {
            ...
            if (HomeDebug.DEBUG_LOG) {
                LogUtils.i(LOG_TAG, "sendPageShowPingback --- tabsrc = ", tabsrc, " qtcurl = ", qtcurl, " block = ", block, " model = ", model);
            }
            PingBackParams params = new PingBackParams();
            params.add("bstp", bstp).add("e", model.e).add("s1", model.s1).add("s2", model.s2).add("plid", model.plid).add("tabsrc", tabsrc).add("qtcurl", qtcurl).add("block", block).add(Keys.T, "21");
    ...
    public static void sendCardShowPingback(CardShowPingbackModel model) {
        if (model != null) {
            ...
            String qtcurl = MultiSubjectEnterUtils.PLAY_TYPE_FROM_MULTI;
            if (HomeDebug.DEBUG_LOG) {
                LogUtils.i(LOG_TAG, "sendCardShowPingback --- model = ", model);
            }
            PingBackParams params = new PingBackParams();
            params.add("bstp", bstp).add("e", model.e).add("s1", model.s1).add("plid", model.plid).add("s2", model.s2).add("tabsrc", tabsrc).add("qtcurl", qtcurl).add("block", model.block).add("qpid", model.qpid).add("allitem", model.allitem).add("dftitem", model.dftitem).add("sawitem", model.sawitem).add("line", model.line).add(Keys.T, "21");
            PingBack.getInstance().postPingBackToLongYuan(params.build());
        }
    }
    ...
    public static void sendPageClickPingback(PageClickPingbackModel model) {
        ...
        if (HomeDebug.DEBUG_LOG) {
            LogUtils.i(LOG_TAG, "sendPageClickPingback --- model = ", model);
        }
        PingBackParams params = new PingBackParams();
        params.add("allitem", model.allitem).add("block", model.block).add("dftitem", model.dftitem).add("e", model.e).add("line", model.line).add("plid", model.plid).add("r", model.r).add("rseat", model.rseat).add("s2", model.s2).add("rt", rt).add("rpage", rpage);
        PingBack.getInstance().postPingBackToLongYuan(params.build());
    }
}
