...
public class BitStreamContent extends AbsTabContent<List<BitStream>, BitStream> {
    ...
    private OnCheckedChangeListener mNormalBitStreamCheckedListener = new OnCheckedChangeListener() {
        public void onCheckedChanged(int index) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(BitStreamContent.this.TAG, "onCheckedChanged(def): index=" + index + ", saved def=" + savedDef + ", new def=" + newDef);
            }
            ...
            if (index >= 0 && index < size) {
                if (BitStreamContent.this.hasBitStreamHDRType()) {
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.d(BitStreamContent.this.TAG, "onCheckedChanged is HDR ?? " + isHDR);
                    }
                    if (!isHDR) {
                ...
                if (((BitStream) BitStreamContent.this.mBitStreams.get(index)).getDefinition() == 10 && ((BitStream) BitStreamContent.this.mBitStreams.get(index)).isVip()) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.d(BitStreamContent.this.TAG, "click 4k: index=" + index);
                    }
        ...
        public void onItemChecked(int index) {
            boolean is1080p;
            boolean is4K;
            LogUtils.d(BitStreamContent.this.TAG, ">> onItemChecked getDefinition , mBitStreams.get(index)" + ((BitStream) BitStreamContent.this.mBitStreams.get(index)).getDefinition() + ", getPlayType " + ((BitStream) BitStreamContent.this.mBitStreams.get(index)).getBenefitType());
            boolean isCurrent4K;
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(BitStreamContent.this.TAG, "onItemChecked is1080p : " + is1080p + ", is4K " + is4K);
            }
            if (is1080p) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(BitStreamContent.this.TAG, "onItemChecked(def): " + index);
            }
            int size = BitStreamContent.this.mBitStreams.size();
            if (index >= 0 && index < size) {
    ...
    private OnHDRToggleListener mOn4KToggleListener = new OnHDRToggleListener() {
    ...
    private OnHDRUserPlayListener mOn4KUserPlayListener = new OnHDRUserPlayListener() {
    ...
    private OnHDRToggleListener mOnHDRToggleListener = new OnHDRToggleListener() {
        public void onToggle(boolean isOpen) {
            if (isOpen) {
    ...
    private OnHDRUserPlayListener mOnHDRUserPlayListener = new OnHDRUserPlayListener() {
        public void onPlay(String type) {
            int i = 0;
            PlayerAppConfig.setIsOpenHDR(false);
    ...
    private OnHDRToggleListener mOnSDR1080PToggleListener = new OnHDRToggleListener() {
        public void onToggle(boolean isOpen) {
            if (isOpen) {
                for (int i = 0; i < BitStreamContent.this.mBitStreams.size(); i++) {
    ...
    private OnHDRToggleListener mOnSDR4KToggleListener = new OnHDRToggleListener() {
        public void onToggle(boolean isOpen) {
            if (isOpen) {
                for (int i = 0; i < BitStreamContent.this.mBitStreams.size(); i++) {
                    if (((BitStream) BitStreamContent.this.mBitStreams.get(i)).getDefinition() == 10) {
    ...
    static {
        HDR_TOGGLE_LIST.add(new Pair(Integer.valueOf(1), Integer.valueOf(R.string.hdr_open)));
        HDR_TOGGLE_LIST.add(new Pair(Integer.valueOf(4), Integer.valueOf(R.string.hdr_close)));
    }
    ...
    private void initViews() {
        ...
        this.mContentView = LayoutInflater.from(this.mContext).inflate(R.layout.player_tabpanel_hdr, null);
        if (LogUtils.mIsDebug) {
    ...
    private void initNormalBitStreamWidget(View contentView) {
        HorizontalScrollView scrollView = (HorizontalScrollView) contentView.findViewById(R.id.rg);
        this.mBitStreamTitle = (TextView) contentView.findViewById(R.id.definition_txt);
        this.mBitStreamGroupView = (MyRadioGroup) contentView.findViewById(R.id.rg_definition);
        this.mDefinitionPanel = (RelativeLayout) contentView.findViewById(R.id.ll_definition);
    ...
    private void initHDRBitStreamWidget(View contentView) {
        if (hasBitStreamHDRType()) {
            ...
            lp.topMargin = (int) this.mContext.getResources().getDimension(R.dimen.player_radiogroup_margin_top);
            lp.leftMargin = (int) this.mContext.getResources().getDimension(R.dimen.player_radiogroup_margin_left);
            this.mDefinitionPanel.setLayoutParams(lp);
            this.mHDRToggleGroupView = (MyRadioGroup) contentView.findViewById(R.id.rg_definition2);
            this.mHDRLayout = (LinearLayout) contentView.findViewById(R.id.ll_hdr_layout);
            ...
            this.mHDRToggleGroupView.setOnCheckedChangedListener(new OnCheckedChangeListener() {
                public void onItemChecked(int index) {
                    if (index == 0) {
                ...
                public void onCheckedChanged(int index) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.d(BitStreamContent.this.TAG, "onCheckedChanged index " + index);
                    }
                    BitStreamContent.this.mItemListener.onItemClicked(null, index);
                }
            });
            ...
            for (int i = 0; i < this.mHDRBitStreamList.size(); i++) {
                for (int j = 0; j < this.mSDRBitStreamList.size(); j++) {
                    if (((BitStream) this.mHDRBitStreamList.get(i)).getDefinition() == ((BitStream) this.mSDRBitStreamList.get(j)).getDefinition()) {
                        if (LogUtils.mIsDebug) {
                            LogUtils.d(this.TAG, ">> find " + j + ", will remove");
                        }
    ...
    private boolean hasBitStreamHDRType() {
        ...
        while (i < this.mAllBitStreamList.size()) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(this.TAG, ">> hasBitStreamHDRType getDRType " + ((BitStream) this.mAllBitStreamList.get(i)).getDynamicRangeType());
            }
    ...
    public void setData(List<BitStream> data) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> setData, data=" + data);
        }
    ...
    public void setVipData(List<BitStream> data) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> setVipData, data=" + data);
        }
    ...
    public void setSelection(BitStream item) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> setSelection, item=" + item);
        }
        if (item != null) {
    ...
    private List<Integer> findVipIndex(List<BitStream> list, List<BitStream> viplist) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, ">> findVipIndex, list=" + list + ", viplist=" + viplist);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "<< findVipIndex, vipList=" + vipList);
        }
        return vipList;
    }
    ...
    private int findSelectIndex(List<BitStream> list, BitStream bitStream) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "<< findSelectIndex, ret=" + ret);
        }
        return ret;
    }
    ...
    private List<String> convertBitstreamToDefinition() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "<< convertBitstreamToDefinition, defs=" + defs);
        }
        return defs;
    }
    ...
    public void setAdData(BaseAdData data) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setAdData:" + data);
        }
        this.mAdData = data;
    }
    ...
    private void showAd(BaseAdData data) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "-->>showAd:" + data + "/" + this.mOnAdStateListener);
        }
        ...
        if (this.mAdView == null) {
            ...
            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_180dp), this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_60dp));
            params.rightMargin = this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_20dp);
            params.addRule(11);
            params.addRule(15);
    ...
    private void sendGuidePingBackClick(String blockType, String rseatType) {
        ...
        PingbackFactory.instance().createPingback(46).addItem(RPAGE.RPAGE_ID(rpage)).addItem(BLOCK.BLOCK_TYPE(blockType)).addItem(RTTYPE.RT_I).addItem(RSEAT.RSEAT_TYPE(rseatType)).addItem(C1.C1_TYPE("")).addItem(PingbackStore.R.R_TYPE("")).post();
    }
