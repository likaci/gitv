...
public class MsgDialogHelper {
    ...
    public IMsgOnMsgListener getOnMsgListener() {
        if (this.mMsgListener == null) {
            this.mMsgListener = new IMsgOnMsgListener() {
                public void onMessage(final Context context, final IMsgContent content) {
                    if (MsgDialogHelper.this.isOutApp && context != null && !IMsgUtils.isAppLive(context)) {
                        LogUtils.d(MsgDialogHelper.TAG, "onReceivermsg, content = " + content.toString());
        ...
        mExec.execute(new Runnable() {
            public void run() {
                while (System.currentTimeMillis() - MsgDialogHelper.this.mLastShowTime < 1000) {
                    LogUtils.d(MsgDialogHelper.TAG, "wait to show, System.currentTimeMillis() - mLastShowTime=" + (System.currentTimeMillis() - MsgDialogHelper.this.mLastShowTime));
                    try {
                        Thread.sleep(1000);
                    } catch (Exception e) {
                    }
                ...
                if (MsgDialogHelper.cacheManager.getCount(isOutApp) == 0) {
                    LogUtils.d(MsgDialogHelper.TAG, "needShowList.size==0");
                } else if (context == null) {
                    LogUtils.d(MsgDialogHelper.TAG, "context == null");
                } else if (MsgDialogHelper.this.isShow) {
                    LogUtils.d(MsgDialogHelper.TAG, "dialog block by: isShow = " + MsgDialogHelper.this.isShow);
                    ...
                    if (!isOutApp && IMsgUtils.isAppLive(context)) {
                        if (!MsgDialogHelper.this.sIsHomeActivity || UpdateManager.getInstance().isShowingDialog()) {
                            LogUtils.d(MsgDialogHelper.TAG, "dialog block by: sIsHomeActivity = " + MsgDialogHelper.this.sIsHomeActivity + ", UpdateManager.getInstance().isShowingDialog()=" + UpdateManager.getInstance().isShowingDialog());
                            ...
                            LogUtils.d(MsgDialogHelper.TAG, "dialog block by: isShowScreenSaver = " + MsgDialogHelper.this.isShowScreenSaver());
                            return;
                        }
                    }
                    ...
                    LogUtils.d(MsgDialogHelper.TAG, "dialog block by: IMsgUtils.isShowDialog = " + IMsgUtils.isShowDialog());
                }
            }
        });
    }
