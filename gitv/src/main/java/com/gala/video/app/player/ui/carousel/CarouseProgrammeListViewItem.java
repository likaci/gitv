...
public class CarouseProgrammeListViewItem extends RelativeLayout {
    ...
    private void initViews() {
        ...
        initUI();
        LayoutInflater.from(this.mContext).inflate(R.layout.player_carousel_programme_listview, this);
        this.mProgrammeName = (MarqueeTextView) findViewById(R.id.channel_programme_name);
        this.mProgrammeTime = (TextView) findViewById(R.id.channel_programme_time);
        setBackgroundResource(this.mItemNormalBgResId);
    }

    public void setProgrammeName(String name) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setProgrammeName = " + name);
        }
        this.mProgrammeName.setViewBound(this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_237dp), this.mContext.getResources().getDimensionPixelSize(R.dimen.dimen_28dp));
    ...
    public void setProgrammeTime(String time) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setProgrammeTime = " + time);
        }
    ...
    public void setIsLive(int live) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setIsLive(" + live + ")");
        }
        if (live == 1) {
            this.mIsLive = true;
        } else {
    ...
    public void setEndTime(String end) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "setEndTime(" + end + ")");
        }
