...
public class UICreator extends Wrapper {
    ...
    public Bitmap maketNoResultView(Context context, GlobalQRFeedbackPanel noResultPanel, ErrorKind kind, ApiException e) {
        LogUtils.d(TAG, "EPG/album4/UICreator---kind=" + kind + "---e=" + e);
    ...
    private static Bitmap createText(Context context, GlobalQRFeedbackPanel noResultPanel, ErrorKind kind) {
        ...
        switch (kind) {
            ...
            case NET_ERROR:
                txtResId = R.string.hisense_album_net_timeout;
                break;
            case NO_RESULT_AND_NO_MENU:
                txtResId = R.string.search_result_nothing;
                break;
            case NO_PLAYHISTORY:
                txtResId = R.string.temporary_no_play_history;
                break;
            case NO_PLAYHISTORY1:
                txtResId = R.string.temporary_no_play_history_long;
                break;
            case ACCOUNT_ERROR:
                txtResId = R.string.account_verify_fail;
                break;
            case NO_RESULT:
                txtResId = R.string.album_no_content;
                break;
            default:
                txtResId = R.string.album_no_content;
                break;
    ...
    private static Bitmap createQR(Context context, GlobalQRFeedbackPanel noResultPanel, ErrorKind kind, ApiException e) {
        try {
            ...
            return createNoResultText(context, noResultPanel, context.getResources().getString(R.string.album_no_content), kind);
        }
    }
    ...
    private static Bitmap createNoResultImage(Context context, GlobalQRFeedbackPanel noResultPanel, ErrorKind kind) {
        switch (kind) {
            case NO_FAVOURITE_RESULT:
                int noResultImageId = R.drawable.epg_no_favorite_record_alter_image;
                ...
                param.width = ResourceUtil.getDimen(R.dimen.dimen_907dp);
                param.height = ResourceUtil.getDimen(R.dimen.dimen_384dp);
                param.topMargin = ResourceUtil.getDimen(R.dimen.dimen_100dp);
    ...
    private static Bitmap createNoResultText(Context context, GlobalQRFeedbackPanel panel, String warnText, ErrorKind kind) {
        ...
        params2.leftMargin = ResourceUtil.getDimen(R.dimen.dimen_3dp);
        params1.gravity = 16;
        ImageView img = new ImageView(context);
        ...
        tv.setTextColor(ResourceUtil.getColor(R.color.albumview_yellow_color));
        tv.setTextSize(0, (float) ResourceUtil.getDimen(R.dimen.dimen_32dp));
        img.setImageResource(R.drawable.epg_no_album_text_warn);
