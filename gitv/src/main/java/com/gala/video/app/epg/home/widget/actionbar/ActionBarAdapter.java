...
public class ActionBarAdapter implements ActionBarStateListener, IActionBarClickListener {
    ...
    private final LoginCallbackRecorderListener mLoginListener = new LoginCallbackRecorderListener() {
        public void onLogout(String uid) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(ActionBarAdapter.TAG, "Logout =" + uid);
            }
            ActionBarAdapter.this.mHandler.post(new Runnable() {
                public void run() {
        ...
        public void onLogin(String uid) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(ActionBarAdapter.TAG, "Login =" + uid);
            }
            ActionBarAdapter.this.mHandler.post(new Runnable() {
                public void run() {
                    ActionBarAdapter.this.updateMyAndLoginView();
    ...
    public void onChildFocusChanged(View v, boolean hasFocus) {
        ...
        if (type == ActionBarType.SEARCH) {
            if (hasFocus) {
                itemView.setTextColor(this.mContext.getResources().getColor(R.color.action_bar_text_focus));
                itemView.setIconDrawable(this.mContext.getResources().getDrawable(R.drawable.epg_action_bar_search_focused));
            } else {
        ...
        if (type == ActionBarType.MY) {
            if (hasFocus) {
                itemView.setTextColor(this.mContext.getResources().getColor(R.color.action_bar_text_focus));
                itemView.setIconDrawable(this.mContext.getResources().getDrawable(R.drawable.epg_action_bar_my_focused));
            } else {
                itemView.setTextColor(this.mContext.getResources().getColor(R.color.action_bar_text_normal));
        ...
        if (type == ActionBarType.CHECKIN) {
            if (hasFocus) {
                itemView.setTextColor(ResourceUtil.getColor(R.color.action_bar_text_focus));
        ...
        if (type == ActionBarType.VIP) {
            ...
            if (hasFocus) {
                ...
                itemView.setTextColor(this.mContext.getResources().getColor(R.color.action_bar_vip_text_focus));
                itemView.setIconDrawable(this.mContext.getResources().getDrawable(R.drawable.epg_action_bar_vip_focused));
            } else {
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        if (actionBarItemInfo.isVip()) {
            ...
            this.mVipChildView.setChildFocusBackgroundResource(R.drawable.epg_action_bar_vip_bg_focused);
        }
    ...
    public void updateVipViewText() {
        ...
        if (this.mVipChildView != null) {
            ...
            if (GetInterfaceTools.getIGalaVipManager().needShowActivationPage()) {
                ...
                LogUtils.d(TAG, "updateVipViewText() --- cookie = " + cookie);
                ...
                LogUtils.d(TAG, "updateVipViewText() --- isAccountExpire = " + isAccountExpire);
                if (UserUtil.isOTTVip()) {
                    LogUtils.d(TAG, "updateVipViewText() --- text = 续费VIP");
                    this.mVipChildView.setText(ActionBarDataFactory.TOP_BAR_TIME_NAME_RENEW_VIP);
                    return;
                }
                LogUtils.d(TAG, "updateVipViewText() --- text = 开通VIP");
                this.mVipChildView.setText(ActionBarDataFactory.TOP_BAR_TIME_NAME_OPEN_VIP);
            }
        }
    }
        ...
        if (actionBarTip != null) {
            ...
            if (GetInterfaceTools.getIGalaAccountManager().isLogin(this.mContext)) {
                ...
                if (userType == null) {
                    ...
                    LogUtils.d(TAG, "updateVipTips, user expire time = " + expireTime);
                    if (expireTime <= 0) {
                        actionBarTip.setVisibility(8);
                        return;
                    }
    ...
    public void updateMessageCountOnReceive(IMsgContent content) {
        if (this.mMyChildView != null && content != null) {
            ...
            LogUtils.d(TAG, "updateMessageCountOnReceive, unread message count = " + count);
        ...
        if (this.mMyChildView != null) {
            new Thread8K(new Runnable() {
                public void run() {
                    ...
                    LogUtils.d(ActionBarAdapter.TAG, "updateMessageCountOnStart, unread message count = " + count);
    ...
    public void lastYAnimation(View view) {
        if (AnimationUtils.currentAnimationTimeMillis() - this.mLastAnimationY > 500) {
            view.startAnimation(AnimationUtils.loadAnimation(this.mContext, R.anim.epg_shake_y));
    ...
    public int getAnimationLength() {
        ...
        if (this.mVipTipsView != null && this.mVipTipsView.getVisibility() == 0) {
            end = this.mVipTipsView.getRight() - ResourceUtil.getDimensionPixelSize(R.dimen.dimen_65dp);
        }
        LogUtils.d(TAG, "vip left=" + start + ",vip right=" + this.mVipChildView.getRight() + ",tip right=" + this.mVipTipsView.getRight());
        return end - start;
    }
