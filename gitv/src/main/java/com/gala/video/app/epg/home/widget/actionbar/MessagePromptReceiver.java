...
public class MessagePromptReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        LogUtils.d(TAG, "message type = " + msgType + " message content = " + msgContent);
        if (msgType == 39) {
            IMsgContent content = null;
            try {
                content = (IMsgContent) JSON.parseObject(msgContent, IMsgContent.class);
            } catch (Exception e) {
                LogUtils.w(TAG, "parse message content occur exception :", e);
            }
            if (content != null) {
                MsgDialogHelper.get().onMessage(content);
                int msgLevel = content.msg_level;
                int msgTemplateId = content.msg_template_id;
                LogUtils.d(TAG, "message level = " + msgLevel + " msg template id = " + msgTemplateId + " page jumping = " + content.page_jumping);
