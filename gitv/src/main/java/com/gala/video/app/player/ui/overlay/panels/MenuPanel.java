...
public class MenuPanel extends AbsMenuPanel {
    ...
    private OnTabChangedListener mTabChangedListener = new OnTabChangedListener() {
        public void onTabChanged(int position) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(MenuPanel.TAG, ">> onTabChanged position:" + position);
            }
            ...
            if (DetailConstants.CONTENT_TAG_RECOMMEND.equals(holder.getTag()) && MenuPanel.this.mIsRecommendDataFilled) {
                ...
                if (!ListUtils.isEmpty(list)) {
                    if (LogUtils.mIsDebug) {
                        LogUtils.d(MenuPanel.TAG, ">> onTabChanged list:" + list.toString());
                    }
    ...
    private OnTabFocusChangedListener mTabFocusChangedListener = new OnTabFocusChangedListener() {
        public void onTabFocusChanged(View view, boolean hasFocus) {
            view.setNextFocusUpId(view.getId());
        }
    };
    private OnVideoDataChangedListener mVideoDataChangedListener = new OnVideoDataChangedListener() {
        public void onVideoDataChanged(int dataFlag) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(MenuPanel.TAG, ">> mVideoDataChangedListener.onVideoDataChanged, dataFlag=" + Integer.toHexString(dataFlag));
            }
            if (MenuPanel.this.mAssociativeContentHolder != null) {
                ...
                if (DetailConstants.CONTENT_TAG_EPISODE.equals(tag) && 1 == dataType) {
                    ...
                    LogUtils.d(MenuPanel.TAG, "mVideoDataChangedListener.onVideoDataChanged, unhandled callback, tag=" + tag + ", dataType=" + dataType);
                }
            } else if (LogUtils.mIsDebug) {
        ...
        public void onException(int dataFlag) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(MenuPanel.TAG, ">> mVideoDataChangedListener.onException, dataFlag=" + Integer.toHexString(dataFlag));
            }
        }
    };

    static {
        DISP_MODE_LIST.add(new Pair(Integer.valueOf(1), Integer.valueOf(R.string.screen_original)));
        DISP_MODE_LIST.add(new Pair(Integer.valueOf(4), Integer.valueOf(R.string.screen_fullscreen)));
    }
    ...
    public void setVideo(IVideo video) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, ">> setVideo, video=" + video);
        }
        if (video != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "setVideo: oldVideo=" + oldVideo + ", new video=" + this.mCurrentVideo);
            }
    ...
    private boolean isLiveChannelChanged(IVideo lastVideo) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, ">> isLiveChannelChanged, sourceType=" + sourceType + ", curLiveChannelID=" + curLiveChannelID + ", lastLiveChannelID=" + lastLiveChannelID);
        }
        if ((SourceType.CAROUSEL == sourceType || SourceType.LIVE == sourceType) && !curLiveChannelID.equals(lastLiveChannelID)) {
            ret = true;
        }
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "<< isLiveChannelChanged, ret=" + ret);
        }
        return ret;
    }
    ...
    private void fillBitStreamData(IContent<?, ?> content) {
        ...
        if (this.mCurrentVideo != null && this.mCurrentVideo.getAllBitStreams() != null) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "fillBitStreamData, SourceType=" + this.mCurrentVideo.getSourceType() + "listAll=" + listAll + ", listVIP=" + listVIP);
            }
    ...
    public void notifyHDRToggle(boolean isOpen) {
        if (!ListUtils.isEmpty(this.mContentHolderList)) {
            ...
            if (holder != null && DetailConstants.CONTENT_TAG_BITSTREAM.equals(holder.getTag())) {
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, ">> notifyHDRToggle bitStramContent!!! isOpen " + isOpen);
                }
                if (isOpen) {
    ...
    protected void doShow() {
        ...
        if (!this.mIsShown) {
            ...
            if (!PlayerAppConfig.isSelectionPanelShown() && this.mKey == 20) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.e(TAG, "getSelectionPanelShownCount：" + countstr);
                }
    ...
    private void sendBitStreamContentShowPingback(String block) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.e(TAG, "sendBitStreamContentShowPingback allList=" + allList + ", vipList=" + vipList);
        }
    ...
    private void setDefaultFocusOnShow() {
        ...
        if (this.mKey == 82) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "setDefaultFocusOnShow contentType=" + contentType);
            }
            switch (contentType) {
                case 1:
                ...
                case 2:
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.d(TAG, "setDefaultFocusOnShow content=" + content);
                        break;
                    }
                    break;
                case 3:
            ...
            if (content instanceof EpisodeListContent) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, "setDefaultFocusOnShow empty=" + empty);
                }
                if (empty) {
                    this.mTabHost.getIndicatorView().requestFocus();
                } else {
    ...
    protected void handleContentRequestFocus() {
        ...
        if (this.mKey == 20 && this.mAssociativeContentHolder != null) {
            ...
            int contentType = this.mAssociativeContentHolder.getType();
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "handleContentRequestFocus contentType=" + contentType);
            }
            switch (contentType) {
                case 1:
                    content = findContentByTag(DetailConstants.CONTENT_TAG_EPISODE);
            ...
            if (content instanceof EpisodeListContent) {
                ...
                boolean empty = ListUtils.isEmpty((List) content.getContentData());
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, "handleContentRequestFocus empty=" + empty);
                }
                content.getFocusableView().requestFocus();
            }
        }
    ...
    private void showDefinitionTab() {
        ...
        if (content != null) {
            MyRadioGroup rg = (MyRadioGroup) content.getView().findViewById(R.id.rg_definition);
            int selection = findCurBitstreamIndex();
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "setDefaultFocusOnShow, list= " + this.mCurrentVideo.getAllBitStreams() + ", current bitstream=" + this.mCurrentVideo.getCurrentBitStream() + ", selection=" + selection);
            }
            if (selection >= 0) {
    ...
    private IContent<?, ?> findContentByTag(String tag) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, ">> findContentByTitle, tag=" + tag);
        }
        for (ContentHolder each : this.mContentHolderList) {
    ...
    private int findCurBitstreamIndex() {
        ...
        if (ListUtils.isEmpty(list)) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "findCurBitstreamIndex, list=" + list + "current bitstream=" + cur);
            }
            int size = list.size();
            for (int i = 0; i < size; i++) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "<< findCurBitstreamIndex, ret=" + ret);
            }
            return ret;
        }
    }
    ...
    private void preAdjustHeight() {
        ...
        int panelHeight = UiUtils.getDimensionInPx(this.mContext, R.dimen.dimen_248dp);
        ...
        if (this.mAssociativeContentHolder != null && (this.mAssociativeContentHolder.getWrappedContent() instanceof GalleryListContent)) {
            panelHeight = UiUtils.getDimensionInPx(this.mContext, R.dimen.dimen_315dp);
        }
        if (LogUtils.mIsDebug) {
    ...
    public void updateSkipHeadAndTail(boolean isSkipTail) {
        if (LogUtils.mIsDebug) {
            LogUtils.i(TAG, "updateSkipHeadAndTail = " + isSkipTail);
        }
    ...
    public boolean onDlnaKeyEvent(DlnaKeyEvent event, KeyKind key) {
        LogUtils.i(TAG, "onDlnaKeyEvent = " + event + ", key " + key);
        if (isShown() && event == DlnaKeyEvent.FLING) {
            switch (key) {
                ...
                default:
                    if (LogUtils.mIsDebug) {
                        LogUtils.w(TAG, "invalid key for fling event: " + key);
                        break;
                    }
                    break;
            }
