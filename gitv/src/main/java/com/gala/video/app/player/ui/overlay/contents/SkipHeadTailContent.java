...
public class SkipHeadTailContent extends AbsTabContent<List<Boolean>, Boolean> {
    ...
    private void initViews() {
        ...
        this.mContentView = LayoutInflater.from(this.mContext).inflate(R.layout.player_tabpanel_skipheadtail, null);
        if (LogUtils.mIsDebug) {
    ...
    private void initDefinitionWidget(View contentView) {
        this.mHScrollViewSkipHeadTail = (HorizontalScrollView) contentView.findViewById(R.id.rg);
        this.mRGSkipHeadTail = (MyRadioGroup) contentView.findViewById(R.id.rg_skipheadtail);
        this.mSkipHeadTailPanel = (LinearLayout) contentView.findViewById(R.id.ll_skipheadtail);
    ...
    public void show() {
        ...
        List<String> list = Arrays.asList(new String[]{resources.getString(R.string.open_jump_header), resources.getString(R.string.close_jump_header)});
        boolean skipHeaderTail = PlayerAppConfig.shouldSkipVideoHeaderAndTail();
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "initSkipHeaderTailChoices: " + skipHeaderTail);
        }
        ...
        this.mRGSkipHeadTail.setOnCheckedChangedListener(new OnCheckedChangeListener() {
            public void onCheckedChanged(int index) {
                boolean z;
                boolean z2 = true;
                if (LogUtils.mIsDebug) {
                    LogUtils.d(SkipHeadTailContent.this.TAG, "onCheckedChanged(skip): index=" + index);
                }
                if (index == 0) {
                    z = true;
                } else {
            ...
            public void onItemChecked(int index) {
                if (LogUtils.mIsDebug) {
                    LogUtils.d(SkipHeadTailContent.this.TAG, "onItemChecked(skip): " + index);
                }
    ...
    private void updateSkipHeadAndTail(boolean isSkipTail) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(this.TAG, "updateSkipHeadAndTail( " + isSkipTail + " )");
        }
