    ...
    private PlayerListListener mPlayerListListener = new PlayerListListener() {
        public void onItemFocusChanged(ViewHolder holder, boolean isSelected, TVChannelCarouselTag label, int tag) {
            if (holder != null) {
                ...
                int itemFocusedBgResId = R.drawable.player_carousel_label_focus;
                int itemNormalBgResId = R.drawable.player_carousel_btn_transparent;
                int itemSpreadBgResId = R.drawable.player_carousel_label_spread;
                int normalColor = CarouselLabelPanel.this.mContext.getResources().getColor(R.color.player_ui_text_color_default);
                int spreadColor = CarouselLabelPanel.this.mContext.getResources().getColor(R.color.player_ui_carousel_item_channel_selected);
                int focusColor = CarouselLabelPanel.this.mContext.getResources().getColor(R.color.player_ui_text_color_focused);
                if (isSelected) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(CarouselLabelPanel.this.TAG, "onItemFocusChanged isSelected=" + isSelected + ", position=" + position + ", mSelectedPosition=" + CarouselLabelPanel.this.mSelectedPosition);
                }
            }
        }
        ...
        public void onItemClick(ViewHolder holder, int tag) {
            if (holder != null) {
                int position = holder.getLayoutPosition();
                if (LogUtils.mIsDebug) {
                    LogUtils.d(CarouselLabelPanel.this.TAG, "onItemClick position = " + position);
                }
    ...
    public void requestFocus() {
        ...
        if (this.mContent != null) {
            if (this.mSpreadPosition >= 0) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(this.TAG, "requestFocus() index=" + index);
                }
            }
