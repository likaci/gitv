...
public class StartScreenAdHandler {
    ...
    private AdStatusCallBack mCallBack = new AdStatusCallBack() {
    ...
    public void setRequestCallback(AdStatusCallBack callback) {
        LogUtils.d(TAG, "set ad callback = " + callback);
    ...
    public void start() {
        ...
        LogUtils.d(TAG, "getScreenAd adsClientVersion:" + adVer);
    ...
    private boolean parseAd(String adJson) {
        ...
        try {
            if (StringUtils.isEmpty((CharSequence) adJson)) {
                LogUtils.d(TAG, "[StartScreenVideoAd-Performance]parseAd timeCost=" + (SystemClock.elapsedRealtime() - startTime));
                return false;
            }
            ...
            if (slots == null || slots.size() <= 0) {
                LogUtils.d(TAG, "no slot with the type of SLOT_TYPE_PAGE, asjson=" + adJson);
                ...
                if (ads == null || ads.isEmpty()) {
                    ...
                    if (ad == null) {
                        ...
                        if (obj != null) {
                            ...
                            LogUtils.d(TAG, "screen ad render type is " + type);
                        ...
                        LogUtils.e(TAG, "creative type: " + ad.getCreativeType());
                    }
                }
            }
            LogUtils.d(TAG, "[StartScreenVideoAd-Performance]parseAd timeCost=" + (SystemClock.elapsedRealtime() - startTime));
            ...
            LogUtils.e(TAG, "parseAd exception", e1);
        } catch (Exception e) {
    ...
    private void handleAdResult(AdResult result, Exception e) {
        if (e != null) {
            ...
            this.mAdsClient.sendAdPingBacks();
            HomePingbackFactory.instance().createPingback(CommonPingback.AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_startapk").addItem("st", MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR).addItem("td", String.valueOf(SystemClock.elapsedRealtime() - this.mStartTime)).addItem(Keys.T, "11").addItem("ct", "150619_request").setOthersNull().post();
            LogUtils.e(TAG, "AdCallback, onGetAdDone, Exception", e);
        } else if (result != null) {
            long elapse = SystemClock.elapsedRealtime() - this.mStartTime;
            LogUtils.d(TAG, "[StartScreenVideoAd-Performance]fetch advertisement timeCost=" + elapse);
            if (StringUtils.isEmpty(result.ad)) {
                this.mAdsClient.onRequestMobileServerFailed();
                this.mAdsClient.sendAdPingBacks();
                LogUtils.d(TAG, "no ad");
                HomePingbackFactory.instance().createPingback(CommonPingback.AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_startapk").addItem("st", "0").addItem("td", String.valueOf(elapse)).addItem(Keys.T, "11").addItem("ct", "150619_request").setOthersNull().post();
