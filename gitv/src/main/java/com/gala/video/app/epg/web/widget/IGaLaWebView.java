...
public class IGaLaWebView extends WebView implements IWebViewLoad, IWebDialog, ISkipCallback {
    ...
    private IScreenCallback mScreenCallback = new IScreenCallback() {
    ...
    private Runnable mToastRunnable = new Runnable() {
        public void run() {
            QToast.makeTextAndShow(AppRuntimeEnv.get().getApplicationContext(), R.string.web_timeout_error, (int) QToast.LENGTH_4000);
        }
    };
    ...
    public void init(WebViewDataImpl data, String url) {
        ...
        init(url);
        setILoadingState(new ILoadingState() {
            public void messageState(int state) {
                LogUtils.i(IGaLaWebView.this.TAG, "ILoadingState state:" + state);
                if (state == 0) {
