...
public class LoadingViewAnimManager {
    ...
    private List<Bitmap> getLoadingImage(TreeMap<String, String> map) {
        ...
        if (this.mNewHeight > 0 && this.mNewWidth > 0) {
            ...
            for (Object obj : map.keySet()) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, "getLoadingImage cachePath=" + cachePath);
                }
                if (image != null) {
                    list.add(image);
                }
    ...
    private void checkPictureSize(String path) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "checkPictureSize width=" + width + ", height=" + height);
        }
        double ratio = 0.0d;
        double xratio;
        double yratio;
    ...
    private Bitmap createTargetBitmap(String path, int targetWidth, int targetHeight, ScaleType type, Config config) {
        ...
        try {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "createTargetBitmap: original w/h=" + nativeWidth + "/" + nativeHeight);
            }
            ...
            if (nativeWidth > targetWidth || nativeHeight > targetHeight) {
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, "createTargetBitmap: scale=" + scale);
                }
                ...
                if (LogUtils.mIsDebug) {
                    LogUtils.d(TAG, "createTargetBitmap: inSampleSize=" + options.inSampleSize);
                }
            } else {
            ...
            LogUtils.d(TAG, "<< createTargetBitmap: result bitmap=" + bitmap);
            return bitmap;
    ...
    public void setBackgroundPath(String bgPath) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "setBackgroundPath newPath=" + bgPath + ", oldPath=" + sBackgroundPath);
        }
        if (bgPath == null) {
