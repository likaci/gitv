...
public class StorageStatePresenter {
    ...
    class StorageStateJavaUI implements StorageStateUIListener {
        ...
        public void onUsbConnected() {
            this.mUsbStorageImage.setImageResource(R.drawable.epg_main_page_image_usb);
            this.mUsbStorageImage.setVisibility(0);
        }
    }
    ...
    private void initExternalStorageWatcherCallback() {
        ExternalStorageWatcher.setup(this.mContext, new ExternalStorageWatcherCallback() {
            public void onUsbDisconnected() {
    ...
    public static int getExternalStorageTag() {
        ...
        if (hasSdCard && hasUsb) {
            LogUtils.d(TAG, " getExternalStorageTag() ---  TAG_EXIST_USB_SDCARD");
            return 3;
        } else if (hasSdCard) {
            LogUtils.d(TAG, " getExternalStorageTag() ---  TAG_EXIST_SDCARD");
            return 1;
        } else if (hasUsb) {
            LogUtils.d(TAG, " getExternalStorageTag() ---  TAG_EXIST_USB");
            return 2;
        } else {
            LogUtils.d(TAG, " getExternalStorageTag() ---  TAG_NONE");
            return 0;
        }
    }
}
