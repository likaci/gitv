    ...
    private synchronized void requestAndParseAdInfo() {
        if (this.mAdsClient != null) {
            ...
            try {
                ...
                HomePingbackFactory.instance().createPingback(CommonPingback.FOCUS_AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_focus").addItem("td", String.valueOf(SystemClock.elapsedRealtime() - startTime)).addItem("st", this.mAdCount > 0 ? "1" : "0").addItem(Keys.T, "11").addItem("ct", "150619_request").setOthersNull().post();
            } catch (Exception e) {
                HomePingbackFactory.instance().createPingback(CommonPingback.FOCUS_AD_DATA_REQUEST_PINGBACK).addItem(Keys.RI, "ad_focus").addItem("td", String.valueOf(SystemClock.elapsedRealtime() - startTime)).addItem("st", MultiScreenParams.DLNA_PHONE_CONTROLL_ERROR).addItem(Keys.T, "11").addItem("ct", "150619_request").setOthersNull().post();
            ...
            if (!StringUtils.isEmpty((CharSequence) adJson) && adsClient != null) {
                ...
                for (CupidAdSlot slot : slotList) {
                    ...
                    if (ListUtils.isEmpty((List) cupidAdList)) {
                        LogUtils.w(TAG, "parseAdJson, the list of CupidAd gotten by SloId : " + mSlotId + " is empty.");
                    } else {
                                    ...
                                    if (clickThroughType == null) {
                                        ...
                                        switch (clickThroughType) {
                                            ...
                                            case VIDEO:
                                                ...
                                                if (!isFindPlid) {
                                                    if (!isFindVideo) {
                                                        LogUtils.w(TAG, "clickThroughType is " + ClickThroughType.VIDEO + "(play a video in the APP)" + "but the info of jumping is illegal, " + "info : " + clickInfo);
                                            ...
                                            case CAROUSEL_STATION:
                                                ...
                                                if (!carouselMatcher.find()) {
                                                    LogUtils.w(TAG, "clickThroughType is " + ClickThroughType.CAROUSEL_STATION + " but the info of jumping is illegal, info : " + clickInfo);
                ...
                this.mDownloader.loadFile(imageRequest, new IFileCallback() {
                    public void onSuccess(FileRequest imageRequest, String s) {
                        ...
                        if (HomeFocusImageAdRequestTask.access$706(HomeFocusImageAdRequestTask.this) == 0) {
                            ...
                            HomePingbackFactory.instance().createPingback(CommonPingback.LOAD_FINISHED_PINGBACK).addItem(Keys.LDTYPE, "ad_focus").addItem("td", String.valueOf(SystemClock.elapsedRealtime() - HomeFocusImageAdRequestTask.this.mStartTime)).addItem(Keys.T, "11").addItem("ct", "160602_load").setOthersNull().post();
                    ...
                    public void onFailure(FileRequest imageRequest, Exception e) {
                        LogUtils.w(HomeFocusImageAdRequestTask.TAG, "downloadAdFocusItemImage, IFileCallback--- onFailure, imageUrl = " + imageRequest.getUrl(), e);
                        if (HomeFocusImageAdRequestTask.access$706(HomeFocusImageAdRequestTask.this) == 0) {
                            LogUtils.d(HomeFocusImageAdRequestTask.TAG, "download focus ad item images finished ");
                            HomePingbackFactory.instance().createPingback(CommonPingback.LOAD_FINISHED_PINGBACK).addItem(Keys.LDTYPE, "ad_focus").addItem("td", String.valueOf(SystemClock.elapsedRealtime() - HomeFocusImageAdRequestTask.this.mStartTime)).addItem(Keys.T, "11").addItem("ct", "160602_load").setOthersNull().post();
                            HomeFocusImageAdProvider.getInstance().notifyAdData(HomeFocusImageAdRequestTask.this.mAdResultList);
