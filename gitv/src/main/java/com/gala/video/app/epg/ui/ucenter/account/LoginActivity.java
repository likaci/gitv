...
public class LoginActivity extends QMultiScreenActivity implements ILoginEvent {
    ...
    public Drawable getContainerDrawable() {
        return findViewById(R.id.epg_login_container).getBackground();
    }
    ...
    public void onSwitchFragment(BaseLoginFragment fragment, Bundle bundle) {
        ...
        if (stackSize <= 0) {
            transaction.add(R.id.epg_login_main_frame, (Fragment) fragment);
        } else {
            transaction.replace(R.id.epg_login_main_frame, fragment);
        }
    ...
    public boolean handleKeyEvent(KeyEvent event) {
        ...
        LogUtils.i(LOG_TAG, "TEST - " + size);
        if (size > 1) {
            LogUtils.i(LOG_TAG, "TEST ---- mCurFragment.updateItem --- onBackPressed");
