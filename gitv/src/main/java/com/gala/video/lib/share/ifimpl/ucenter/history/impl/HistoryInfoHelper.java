...
public class HistoryInfoHelper {
    private static Comparator<HistoryInfo> ASCEND_COMPARATOR = new Comparator<HistoryInfo>() {
    ...
    private static Comparator<HistoryInfo> DESCEND_COMPARATOR = new Comparator<HistoryInfo>() {
        public int compare(HistoryInfo r1, HistoryInfo r2) {
    ...
    public static List<HistoryInfo> sort(List<HistoryInfo> list) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "getLastest(" + list.size() + ") return " + temp.size());
        }
        return temp;
    }
