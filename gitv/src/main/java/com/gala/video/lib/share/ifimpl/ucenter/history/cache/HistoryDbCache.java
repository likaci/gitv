...
public class HistoryDbCache {
    ...
    private class HistoryOpenHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            LogUtils.e(HistoryDbCache.TAG, "history database upgrade!" + oldVersion + " to " + newVersion);
            if (oldVersion == 4 && newVersion == 5) {
                try {
                    db.execSQL("ALTER TABLE history ADD COLUMN hdr TEXT");
                    return;
                } catch (SQLException e) {
                    LogUtils.e(HistoryDbCache.TAG, "on upgrade database exception " + oldVersion + " to " + newVersion, e);
                    return;
                }
            }
            try {
    ...
    public void put(HistoryInfo info) {
        ...
        try {
            ...
            LogUtils.e(TAG, "put(" + info + ") error! ", e);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "put(" + info + ") add " + count);
        }
    }

    public HistoryInfo getTv(String tvId) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "getFromTv(" + tvId + ")");
        }
        ...
        try {
            ...
            LogUtils.e(TAG, "getFromTv(" + tvId + ") error!", e);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "getFromTv(" + tvId + ") return " + info);
        }
        return info;
    }
    ...
    public HistoryInfo getAlbum(String albumId) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "getFromAlbum(" + albumId + ")");
        }
        HistoryInfo info = null;
        try {
            ...
            LogUtils.e(TAG, "getFromAlbum(" + albumId + ") error!", e);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "getFromAlbum(" + albumId + ") return " + info);
        }
        return info;
    }
    ...
    public void removeAlbum(String albumId) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "removeAlbum(" + albumId + ")");
        }
        ...
        try {
            ...
            LogUtils.e(TAG, "removeAlbum(" + albumId + ") error!", e);
        }
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "removeAlbum(" + albumId + ") " + count);
        }
    }

    public List<HistoryInfo> reload(String cookie) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "reload()" + cookie);
        }
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "reload() return " + list.size());
        }
        return list;
    }
    ...
    private HistoryInfo createInfoFromCursor(Cursor cursor) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "createInfoFromCursor() return " + info);
        }
        return info;
    }
        ...
        if (list != null && !list.isEmpty()) {
            ...
            for (HistoryInfo info : list) {
                try {
                    ...
                    if (LogUtils.mIsDebug) {
                        LogUtils.d(TAG, "update() insert count " + count);
                    }
                } catch (Exception e2) {
                    LogUtils.e(TAG, "update() error! " + info, e2);
                }
            }
            try {
                db.setTransactionSuccessful();
