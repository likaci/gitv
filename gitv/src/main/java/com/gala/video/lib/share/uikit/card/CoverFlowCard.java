...
public class CoverFlowCard extends Card {
    ...
    private MyObserver mFocusImageAdDataObserver = new MyObserver() {
        public void update(String event) {
            CoverFlowCard.this.mHandler.post(new Runnable() {
                public void run() {
    ...
    private IStatusListener mScreenSaverListener = new IStatusListener() {
        public void onStart() {
    ...
    private class CoverFlowActionPolicy extends CardActionPolicy {
        ...
        public void onFocusPositionChanged(ViewGroup parent, int position, boolean hasFocus) {
            ...
            LogUtils.d(CoverFlowCard.TAG, "CoverFlowCard@" + hashCode() + ", onFocusPositionChanged, position :" + position + ", hasFocus : " + hasFocus);
            if (hasFocus) {
                CoverFlowCard.this.sendAdPingback();
            }
        }
    ...
    public void parserItems(CardInfoModel cardInfoModel) {
        ...
        LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ",CoverFlowItem=" + this.mItem.hashCode() + ", parserItems" + ", card pl: " + pl + ", card width : " + width);
    ...
    private void registerObserver() {
        if (!this.hasRegister) {
            ...
            LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ", start" + ", current CoverFlowCard is located in default tab :" + isDefaultPage + ", is located in first line : " + isFirstLine + ", is cache data : " + isCatchData);
    ...
    public void sendAdPingback() {
        for (Integer intValue : this.mTempAdIdPoolList) {
            ...
            LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ", start, switch to current CoverFlowCard" + ", focus image fully show, send ad show pingback");
    ...
    private void removeObserver() {
        ...
        LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ", stop" + ", current CoverFlowCard is located in default tab :" + isDefaultPage + ", is located in first line : " + isFirstLine);
    ...
    private void updateFocusImageAd() {
        ...
        if (count != 0) {
            ...
            for (HomeFocusImageAdModel adModel : adModelList) {
                ...
                LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ", ad raw width :" + rawAdWidth + ", ad raw height : " + rawAdHeight);
                short height = this.mCardInfoModel.getItemInfoModels()[0][0].getHeight();
                LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ", CoverFlowCard item height, :" + height);
                ...
                LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ", ad image url: " + adModel.getImageUrl());
    ...
    private boolean sendFocusImageAdShowPingback(int focusImageAdId) {
        ...
        if (!childVisible || isShowingScreenSaver || !isStartPreViewFinished) {
            LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ", sendFocusImageAdShowPingback, is showing screensaver : " + isShowingScreenSaver + "," + "childVisible=" + childVisible + ",isStartPreViewFinished=" + isStartPreViewFinished);
            return false;
            ...
            LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + ", sendFocusImageAdShowPingback, send focus image ad show pingback ,onAdStarted : " + focusImageAdId);
            adShowCount++;
            return true;
        }
    }
    ...
    public void checkCoverFlowMsg() {
        ...
        if (this.mItem != null && this.mItem.getView() != null) {
            ...
            LogUtils.d(str, append.append(z).append(",isStartPreViewFinished=").append(isStartPreViewFinished).append(",item=").append(this.mItem).append(",item.getView()=").append(this.mItem.getView()).toString());
            if (childVisible && !isShowingScreenSaver && isStartPreViewFinished) {
    ...
    private void stopCoverFlowMsg(CoverFlowItem item) {
        if (item != null && item.getView() != null) {
            LogUtils.d(TAG, "CoverFlowCard@" + hashCode() + "stopCoverFlowMsg,item=" + item + ",item.getView()=" + item.getView());
