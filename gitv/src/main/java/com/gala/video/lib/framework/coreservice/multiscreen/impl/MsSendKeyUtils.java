...
class MsSendKeyUtils {
    ...
    public boolean sendSysKey(Context context, KeyKind kind) {
        ...
        if (msHelper.isKeySysEnable()) {
            LogUtils.d(TAG, "sendSysKey(" + context + ", " + kind + ") send system key!");
            msHelper.sendKeyCode(kind);
            handled = true;
        } else {
            LogUtils.d(TAG, "sendSysKey(" + context + ", " + kind + ") send app key!");
            handled = sendSysKeyWithApp(context, kind);
        }
        LogUtils.d(TAG, "sendSysKey(" + kind + ") return " + handled);
        return handled;
    }

    public boolean sendSysKeyWithApp(Context context, KeyKind kind) {
        LogUtils.d(TAG, "sendSysKeyWithApp(" + context + ", " + kind + ")");
        ...
        LogUtils.d(TAG, "sendSysKeyWithApp(" + kind + ") return " + handled);
        return handled;
    }
    ...
    private boolean sendSimulateKeyCode(final int keyCode) {
        ...
        if (DeviceUtils.isAppForeground(AppRuntimeEnv.get().getApplicationContext(), packageName)) {
            LogUtils.d(TAG, "sendSimulateKeyCode(" + keyCode + ") forgound!");
            ThreadUtils.execute(new Runnable() {
                public void run() {
                    try {
                        LogUtils.d(MsSendKeyUtils.TAG, "sendSimulateKeyCode(" + keyCode + ") excute!");
            ...
            LogUtils.d(TAG, "sendSimulateKeyCode(" + keyCode + ") not forgound!");
        }
        LogUtils.d(TAG, "sendSimulateKeyCode(" + keyCode + ") return " + handled);
        return handled;
    }
}
