...
public abstract class Card extends ComponentGroup implements Presenter {
    ...
    public class CardActionPolicy extends ActionPolicy {
        ...
        public void onItemFocusChanged(ViewGroup parent, ViewHolder holder, boolean hasFocus) {
            if (hasFocus) {
                LOG.d("holder.itemView = " + holder.itemView + " position = " + holder.getLayoutPosition());
            ...
            try {
                ...
                if (item != null) {
                    ...
                    if (scale > 0.0f) {
                        holder.itemView.setTag(R.id.focus_end_scale, Float.valueOf(scale));
                        AnimationUtil.zoomAnimation(holder.itemView, hasFocus, scale, 200, true);
                    } else {
                        float cardScale = this.mCard.getModel().getScale();
                        holder.itemView.setTag(R.id.focus_end_scale, Float.valueOf(cardScale));
                ...
                holder.itemView.setTag(R.id.focus_end_scale, Float.valueOf(1.1f));
        ...
        protected void performClick(ViewGroup parent, int realViewPos, int itemPingbackPos, Item item) {
            if (item.getModel() == null) {
                LogUtils.d("onItemClick", "pos=" + realViewPos + ",itemPingbackPos=" + itemPingbackPos + ",actionModel=null");
                return;
            }
            BaseActionModel actionModel = item.getModel().getActionModel();
            LogUtils.d("onItemClick", "pos=" + realViewPos + ",itemPingbackPos=" + itemPingbackPos + ",actionModel=" + actionModel);
    ...
    public void parserItems(CardInfoModel cardInfoModel) {
        ...
        LOG.d("item size = " + this.mItems.size());
    }
