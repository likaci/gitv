...
public class ProgressBarItem extends FrameLayout {
    ...
    private int getLayoutId() {
        return R.layout.share_progressbar_item;
    }
    ...
    private void init(Context context) {
        ...
        this.mLoadingBg = (RelativeLayout) progressbarView.findViewById(R.id.share_loading_bg);
        this.itemTextView = (TextView) progressbarView.findViewById(R.id.share_progressbar_tagtext_id);
        this.progessBarImage = (ImageView) progressbarView.findViewById(R.id.share_progressbar_item_image_id);
        if (Project.getInstance().getBuild().isLitchi() || Project.getInstance().getConfig().setAnimationInXml()) {
            this.progessBarImage.setImageDrawable(ResourceUtil.getDrawable(R.drawable.share_new_anim_load_center));
