        ...
        if (ListUtils.getCount((List) list) != 0) {
            GetInterfaceTools.getISubscribeProvider().getSubscribeState(new IVrsCallback<ApiResult>() {
                public void onSuccess(ApiResult apiResult) {
                    LogUtils.d(SubscribeCard.TAG, SubscribeCard.this.hashCode() + "@success,list.size:" + list.size());
                }

                public void onException(ApiException e) {
                    LogUtils.e(SubscribeCard.TAG, SubscribeCard.this.hashCode() + "@exception " + e + ",list.size:" + list.size());
                }
            }, list);
        }
    }
