...
public class AccountLoginHelper {
    ...
    private static IVrsCallback<ApiResultCode> iVrsFavouriteCallback = new IVrsCallback<ApiResultCode>() {
    ...
    private static IVrsCallback<ApiResultCode> iVrsHistoryCallback = new IVrsCallback<ApiResultCode>() {
        public void onSuccess(ApiResultCode arg0) {
    ...
    public static String getCommErrorTip(ApiException exception) {
        String tip = AppRuntimeEnv.get().getApplicationContext().getResources().getString(R.string.error_comm_tip);
    ...
    static void mergeHistoryAndCollect(String authCookie) {
        ...
        LogUtils.d(LOG_TAG, "saveUserInfo 【authCookie = ", authCookie, "】");
    }
    ...
    static String getLimitCodeByCustomerName() {
        ...
        for (int i = 0; i < CUSTOMER_LIMITCODE_SET.length; i++) {
            if (CUSTOMER_LIMITCODE_SET[i][0].equals(mCustomerName)) {
                LogUtils.d(LOG_TAG, "mCustomName=" + mCustomerName);
