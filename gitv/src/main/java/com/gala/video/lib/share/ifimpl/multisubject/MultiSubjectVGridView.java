...
public class MultiSubjectVGridView extends VerticalGridView implements IVerticalCardView {
    ...
    private OnItemRecycledListener mOnItemRecycledListener = new OnItemRecycledListener() {
        ...
        private void recycle(ViewHolder holder) {
            if (ShareDebug.DEBUG_LOG) {
                LogUtils.e(MultiSubjectVGridView.this.LOG_TAG, "recycle,holder=" + holder);
            }
    ...
    private OnScrollListener mOnScrollListener = new OnScrollListener() {
        ...
        public void onScroll(ViewParent parent, int firstAttachedItem, int lastAttachedItem, int totalItemCount) {
            ...
            if (ShareDebug.DEBUG_LOG) {
                LogUtils.e(MultiSubjectVGridView.this.LOG_TAG, "onScroll --- firstAttachedItem = ", Integer.valueOf(firstAttachedItem), " lastAttachedItem = ", Integer.valueOf(lastAttachedItem));
            }
        }
        ...
        public void onScrollBefore(int position) {
            ...
            if (ShareDebug.DEBUG_LOG) {
                LogUtils.e(MultiSubjectVGridView.this.LOG_TAG, "onScrollBefore --- position = ", Integer.valueOf(position));
            }
            boolean need = false;
            if (MultiSubjectVGridView.this.mVAdapter.getActionListener() != null) {
    ...
    private TextCanvas getTip(String tips) {
        ...
        tc.setText(tips);
        tc.setTextSize(ResourceUtil.getDimensionPixelSize(R.dimen.dimen_20dp));
        tc.setHeight(ResourceUtil.getDimen(R.dimen.dimen_23dp));
        tc.setTextColor(ResourceUtil.getColor(R.color.detail_text_color_default));
        tc.setBackground(R.drawable.transparent_drawable);
        tc.setPadding(ResourceUtil.getDimen(R.dimen.dimen_7dp), ResourceUtil.getDimen(R.dimen.dimen_3dp), ResourceUtil.getDimen(R.dimen.dimen_7dp), 0);
        return tc;
    }
    ...
    public boolean isViewVisible(int pos, boolean isJudgeHoleItemVisible, int gridViewTopShade, int cardBottomOffset) {
        ...
        if (ShareDebug.DEBUG_LOG) {
            LogUtils.e(this.LOG_TAG, "isViewVisible --- topY = " + topY + "v.getTop() = " + v.getTop() + " getScrollY() = " + getScrollY());
            LogUtils.e(this.LOG_TAG, "isViewVisible --- bottomY = " + topY + "v.getBottom() = " + v.getBottom() + " getScrollY() = " + getScrollY());
        }
    ...
    private void onCardScrollForPingback() {
        ...
        for (int i = 0; i <= last; i++) {
            ...
            if (view != null) {
                ...
                if (ShareDebug.DEBUG_LOG) {
                    LogUtils.e(this.LOG_TAG, "onCardScrollForPingback --- isVisible = ", Boolean.valueOf(isVisible), " isTimeKeeping = ", Boolean.valueOf(isTimeKeeping), " i = ", Integer.valueOf(i));
                }
    ...
    public void onPauseForPingback() {
        ...
        for (int i = first; i <= last; i++) {
            ...
            if (view != null) {
                ...
                boolean isTimeKeeping = view.isTimeKeeping();
                if (ShareDebug.DEBUG_LOG) {
                    LogUtils.i(this.LOG_TAG, "onPauseForPingback --- i = ", Integer.valueOf(i), "isVisible = ", Boolean.valueOf(isVisible), " isTimeKeeping = ", Boolean.valueOf(isTimeKeeping));
