...
public class BroadcastConfigPlayer {
    private static BroadcastConfigPlayer mInstance = null;
    private final String TAG = getClass().getSimpleName();
    private Context mContext;
    private List<String> mFeatures = null;

    private BroadcastConfigPlayer(Context context, List<String> features) {
        this.mContext = context;
        this.mFeatures = features;
    }

    public static synchronized BroadcastConfigPlayer initialize(Context context, List<String> features) {
        BroadcastConfigPlayer broadcastConfigPlayer;
        synchronized (BroadcastConfigPlayer.class) {
            if (mInstance == null) {
                mInstance = new BroadcastConfigPlayer(context, features);
            }
            broadcastConfigPlayer = mInstance;
        }
        return broadcastConfigPlayer;
    }

    public void initBroadcastFeatures() {
        Log.d(this.TAG, "iniBroadcastFeatures");
        try {
            addAction(CreateInterfaceTools.createPlayerBroadcastHolder().getActionHolder());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
