...
public class AccountAdsHelper {
    ...
    public static void handelCheckVipAccountException(ApiException e) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "handelCheckVipAccountException(), exception code:" + e.getCode());
        }
    ...
    public static void handelCheckVipAccount(ApiResultKeepaliveInterval result) {
        ...
        if (result != null) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "handelCheckVipAccount(), checksign=" + result.checkSign());
            }
            if (result.checkSign()) {
            ...
            LogUtils.e(TAG, "handelCheckVipAccount(), result=" + result);
        }
    }
    ...
    public static boolean isShouldSkipAd(Context ctx) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "isShouldSkipAd, return " + isShouldSkipAd);
        }
    ...
    public static void updateSkipAdState(Context context, boolean isSkipAd) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "updateSkipAdState: " + isSkipAd);
        }
