...
public class RecordItemView extends LinearLayout implements IViewLifecycle<Presenter> {
    ...
    private MyObserver mHistoryChangedObserver = new MyObserver() {
        public void update(String event) {
            RecordItemView.this.post(new Runnable() {
                public void run() {
                    LogUtils.d(RecordItemView.this.LOG_TAG, "on receive history change event");
    ...
    private OnFocusChangeListener mItemFocusChangeListener = new OnFocusChangeListener() {
        public void onFocusChange(View view, boolean hasFocus) {
            ...
            LogUtils.i(RecordItemView.this.LOG_TAG, "mItemFocusChangeListener >view.getTag() =  " + view.getTag(), "hasFocus = " + hasFocus);
    ...
    public void setViewType(int viewType, ItemInfoModel itemInfoModel) {
        com.gala.video.lib.framework.core.utils.LogUtils.i(this.LOG_TAG, "setViewType mViewType = ", Integer.valueOf(this.mViewType), " viewType = ", Integer.valueOf(viewType));
    ...
    private void setStylesAndTags(ItemInfoModel itemInfoModel) {
        if (!this.mIsAlreadySetStyle) {
            ...
            this.mFirstLongRecordView.setTag(R.id.focus_res_ends_with, skinEnd);
            this.mSecondLongRecordView.setTag(R.id.focus_res_ends_with, skinEnd);
            this.mSearchHistoryView.setTag(R.id.focus_res_ends_with, skinEnd);
        }
    }
    ...
    private void requestFocusAgainIfNeeded() {
        LogUtils.i(this.LOG_TAG, "requestFocusAgainIfNeeded --mFoucTag = " + this.mFoucTag + " ;mViewType = " + this.mViewType);
        if (this.mFoucTag != -1) {
            switch (this.mViewType) {
                case 1:
