...
public class NetworkStateLogic {
    ...
    private OnNetStateChangedListener mNetStateListener = new OnNetStateChangedListener() {
    ...
    public void dealCode(ErrorEvent startupEvent) {
        LogUtils.d(TAG, " start up event : " + startupEvent);
    ...
    public boolean checkStateIllegal() {
        LogUtils.d(TAG, "checkStateIllegal()");
    ...
    private void showExceptionTip(String message, boolean showBtn) {
        ...
        if (this.mContext != null) {
            ...
            if (showBtn) {
                this.mDialog.setParams(message, this.mContext.getString(R.string.show_native_information), this.mOkBtnListener);
            } else {
    ...
    protected void showNetErrorDialog(int resId) {
        LogUtils.d(TAG, "showNetErrorDialog()---resId = " + resId);
        dismissDialog();
        if (this.mContext != null) {
    ...
    private void showFirstErrorDialog(boolean hasFeedBckBtn, boolean isNetNone) {
        LogUtils.d(TAG, "showFirstErrorDialog()---hasFeedBckBtn = " + hasFeedBckBtn + ",isNetNone = " + isNetNone + ",mContext = " + this.mContext);
        dismissDialog();
        if (this.mContext == null) {
            return;
        }
    ...
    protected void checkNetworkStateFirst() {
        this.mNetworManager.checkNetWork(new StateCallback() {
    ...
    private void showNetErrorByNetState(int state) {
        LogUtils.d(TAG, "showNetErrorByNetState()---state = " + state);
        if (state == 0) {
    ...
    protected void onNetworkAvailable() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.e(TAG, "onNetworkAvailable() start data request, has internet");
        }
        LogUtils.e(TAG, "force execute start up service error : " + this.mDeviceCheckProvider.getErrorEvent());
