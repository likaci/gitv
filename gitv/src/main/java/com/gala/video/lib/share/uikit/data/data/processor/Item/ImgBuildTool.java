...
public class ImgBuildTool {
    ...
    private static boolean isUseOperationimg(Item item) {
        for (short[] size : SPECIAL_SEZE) {
            if (item.w == ResourceUtil.getPxShort(size[0]) && item.h == ResourceUtil.getPxShort(size[1])) {
                return false;
            }
        }
        return true;
    ...
    private static String buildGif(ChannelLabel label, Item item) {
        if (item.style.startsWith("circle") || !GetInterfaceTools.getIDynamicQDataProvider().getDynamicQDataModel().getIsSupportGif() || MemoryLevelInfo.isLowConfigDevice() || item.w > (short) 552 || label == null || label.itemKvs == null) {
            return null;
        }
