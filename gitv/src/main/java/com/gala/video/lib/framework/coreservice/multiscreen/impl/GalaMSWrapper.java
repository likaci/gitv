...
class GalaMSWrapper extends GalaMSWrapperBase implements IGalaMSExpand, IMSGalaCustomOperate {
    ...
    public void onKeyEvent(KeyKind kind) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onKeyEvent(" + kind + ")");
        }
    ...
    public Notify onPhoneSync() {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onPhoneSync() return=" + (notify == null ? null : notify.toString()));
        }
        return notify;
    }
    ...
    public boolean onActionChanged(Action action) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onActionChanged() return=" + ret);
        }
        return ret;
    }
    ...
    public boolean onKeyChanged(int keycode) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onKeyChanged() return=" + ret);
        }
    ...
    public boolean onResolutionChanged(String newResolution) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onResolutionChanged() return=" + ret);
        }
        return ret;
    }
    ...
    public boolean onSeekChanged(long newPosition) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onSeekChanged() newPosition=" + newPosition);
        }
        boolean ret = false;
        if (isMSEnable()) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onSeekChanged() return=" + ret);
        }
        return ret;
    }
    ...
    private boolean doPushoPlayList() {
        ...
        if (this.mPlayList != null && this.isAdStart) {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "onPushPlayList() return=" + ret);
            }
            this.mPlayList = null;
    ...
    public void onNotifyEvent(RequestKind requestKind, String message) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "onNotifyEvent(" + requestKind + ", " + message + ")" + " mMSListener=" + this.mMSListener);
        }
