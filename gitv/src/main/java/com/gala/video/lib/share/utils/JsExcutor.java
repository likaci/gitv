...
public class JsExcutor {
    ...
    public static String runScript(String js, String functionName, Object[] functionParams, Context context) {
        ...
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "runScript(functionName:" + functionName + ",functionParams:" + params.toString() + ")");
        }
        DexClassLoader clzLoader = getDexClassLoader(context);
        if (clzLoader == null) {
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "runScript(functionName:" + functionName + "), clzLoader is null, return null");
            }
            return null;
        }
        ...
        try {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "runScript(functionName:" + functionName + ",functionParams:" + functionParams + "), exception occurs!", e);
            }
            try {
        ...
        LogUtils.d(TAG, "runScript(functionName:" + functionName + "), return " + jsonResult);
        return jsonResult;
    }
    ...
    private static String getJsExcutorJarPath(Context context, String pathInAssets, String currentFileVersion) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "getJsExcutorJarPath(pathInAssets:" + pathInAssets + ",currentFileVersion:" + currentFileVersion + ")");
        }
        ...
        if (!isCheckPass) {
            ...
            LogUtils.d(TAG, "getJsExcutorJarPath, return " + targetFilePath);
            return targetFilePath;
        }
    }
    ...
    private static boolean copyFileFromAssets(Context context, String fileName, String targetFilePath) {
        if (LogUtils.mIsDebug) {
            LogUtils.d(TAG, "copyFileFromAssets(fileInAssets: " + fileName + ", targetFilePath:" + targetFilePath + ")");
        }
        try {
            ...
            if (LogUtils.mIsDebug) {
                LogUtils.d(TAG, "copyFileFromAssets(fileInAssets: " + fileName + ", targetFilePath:" + targetFilePath + "), exception occurs!", e);
            }
            return false;
        }
    }
